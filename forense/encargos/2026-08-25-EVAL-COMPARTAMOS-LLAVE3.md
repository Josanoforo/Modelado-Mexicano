...
5. **Tarea 5 — cumplida.** Nota con párrafo a mesa (§8), tres filas de tablero (`FP-123` firmada, `FP-131`/`FP-132` nuevas), `ADR-161` candidateado y renumerado dos veces por concurrencia, recifrado de `canon/estado-programa-v1_10.md` (ADR `160`→`161`; llaves `3 de 3`→`3 de 4`; WARN `147`→`148`), suite `--baseline` **19 FAIL · 148 WARN · LÍNEA BASE VERDE** con tail, y este encargo `CONSUMIDO`.
...
**Concurrencia — la regla del encargo se aplicó, y dos veces.** `origin/main` se movió dos veces mientras este acto estaba abierto: `21ab042` → `e8ce5ef` (`PR #327`, que trajo `ADR-159`) y luego → `e70b424` (`PR #328`, `ACTO R34-CONDA-V2`, que se llevó `ADR-160` **y** las filas `FP-129`/`FP-130`). Las dos se absorbieron rebasando antes de cerrar; este acto quedó en `ADR-161`, `FP-131` y `FP-132`, *renumera quien fusiona segundo*. Los tres archivos en colisión se resolvieron **por unión**, sin perder ni sobrescribir ninguna entrada ajena — verificado contra `origin/main`, con la corrección de una pasada de renumeración que tocó por error tres referencias ajenas dentro de líneas compartidas (detalle en §9 de la nota).
