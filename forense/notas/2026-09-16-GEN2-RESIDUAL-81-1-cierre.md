...
- `python3 tests/check.py --baseline`: **3 FAIL · 4 349 WARN**, **LÍNEA BASE VERDE**. Los 3 `FAIL` son los heredados del corpus documental (`T06` ×2, `T08`). La primera corrida sí dio `ROJO` por **`T16`**, que es autorreferente: `ADR-520` declaraba `3 FAIL · 4348 WARN` y la corrida real no coincidía. Se corrigió **declarando la cifra medida en el `ADR` nuevo**, no tocando el test. De los `+3`: **2** son las filas `NC-0245`/`NC-0246` que este acto abre (`T34`, como `A.12` manda) y **la tercera no es de este perímetro** — `ADR-520` midió sobre `1fac27a` y este acto cierra sobre `10f8617`, con cuatro PR ajenos fusionados en medio; no se atribuye a este acto un WARN que no se midió como suyo.
