...
**Firmas que abre este acto:** `FP-264` (recibo del lote) y `FP-265` (el desvío de perímetro de `relaciones.tsv`, elevado a mesa; ver §P1). **ADR:** `ADR-314`. Numeración **renumerada al cerrar**: las candidatas eran `ADR-312`/`FP-261`/`FP-264`, y `PR #502` (`ACTO MAESTRA36-L13`) fusionó segundo llevándose `ADR-312` y `FP-261` — la regla «renumera quien fusione segundo», aplicada a este acto. Re-derivado contra `origin/main = 18fd2bd3`: máximo `ADR-312` sin duplicados, máximo `FP-261`. `L12` sigue abierto.
...
(`forense/firmas-pendientes.tsv`, `FP-265`) en vez de resolverse por inferencia.
