#!/usr/bin/env python3
"""Medidor de CALC-AGG-marco-M-sorteado-v1_3-ola -- dos vistas, cero colapso.

`ACTO GEN2-T9 · EL MOTOR ES LA MATRIZ`, P3(d)/(e).
...
def medir(inputs: dict, contrato: dict) -> dict:
    ...
    for fila in elegibles:
        r = por_regla.setdefault(fila["regla"], {"celdas": 0, "modulan": 0})
        r["celdas"] += 1
        marca = str(res_ola.get(f"RESULT-MOLA-{fila['id']}-MODELA-OLA", "NO"))
        if marca.startswith("SERIE-LOO"):
            r["modulan"] += 1
    vista_regla = ";".join(
    ...
    return {
        ...
        "RESULT-AGGOLA-N-MODULAN-OLA": sum(
            ...
            if str(res_ola.get(f"RESULT-MOLA-{f['id']}-MODELA-OLA", "")).startswith("SERIE-LOO")),
