...
3. **`tests/test_descarga_manifiesto.py` (CI, job `adicionales`) congelaba conteos literales**: exactamente 9 ids `banxico_sie_*` y exactamente 6 entradas con `estado_reserva`. Este acto añadió 2 series SIE y 70 entradas reservadas, que E.6 obliga a registrar, y el job cayó con 2 FAIL. Corregido (D-21, defecto adyacente que impedía cerrar): las pruebas 5 y 6 exigen ahora que los 9 y los 6 originales sigan presentes y que la propiedad valga para todo el universo. Todo `banxico_sie_*` sale NO-ACCESIBLE, y la guardia E.6 de `_descarga_un_id` se niega a las 76 reservadas sin tocar la red. Resultado: 28 PASS · 0 FAIL. Control positivo: al renombrar un id original, la prueba 5 falla.
