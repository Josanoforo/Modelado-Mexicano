...
def _tabla_futura(nombre, objeto):
    """Propuesta de serializador futuro: dos tablas del CALC propio."""
    nombres = {"d-k-ahorro-formal.json", "d-k-horizonte-ahorro.json"}
    if nombre not in nombres:
        raise PermissionError("tabla futura fuera de lista blanca")
    calc = Path(__file__).resolve().parent
    if calc.name != "CALC-FAMILIA-2027-ENIF-EVALUACION-0001":
        raise PermissionError("serializador requiere CALC futuro propio autorizado")
    carpeta = calc / "tablas"
    if carpeta.is_symlink():
        raise PermissionError("tabla futura no puede ser enlace")
    carpeta.mkdir(exist_ok=True)
    destino = carpeta/nombre
    if destino.is_symlink() or destino.resolve().parent != carpeta.resolve():
        raise PermissionError("tabla futura fuera del CALC")
    raw = (json.dumps(objeto, ensure_ascii=False, allow_nan=False, sort_keys=True,
                      separators=(",", ":"))+"\n").encode("utf-8")
    if destino.exists():
        if destino.read_bytes() != raw:
            raise PermissionError("tabla futura existente discordante: no reescribir")
    else:
        destino.write_bytes(raw)
    return "REF:data/corrida0/"+calc.name+"/tablas/"+nombre+"#sha256:"+hashlib.sha256(raw).hexdigest()
...
def medir_futuro(inputs, contrato):
    ...
    for fam, valores in r["familias"].items():
        ...
        out[prefijo+"D-K"] = _tabla_futura("d-k-"+fam.lower()+".json", valores["d_k"])
