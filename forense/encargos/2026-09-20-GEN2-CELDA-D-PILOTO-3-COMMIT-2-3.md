...
|---|---|---|---|
| «P1 · COMMIT-2 · emisiones. python3 tools/corrida0.py run CALC-GOB-DIGITAL-EXE-EMISIONES-0002 … Sella. Reporta el conteo de código 97 en 2025 y si dispara RESERVA-S2 … Registro en la vista y asiento de replay … Commit y push.» | PARO-PREMISA | El código congelado no corre bajo `corrida0 run`: `preflight` BLOQUEADO (`spec_md` relativo al CALC → ausente; 4 inputs repo sin `sha256`; `dependencias_materiales` ausente) y `run` devuelve `NO-EJECUTADO`. No se parcha (PARO a/b). `N_corridas_selladas` +0; `encig25_base_datos_csv` no se abrió; conteo 97 de 2025 sin hacer. `NC-0447` | COMMIT-1 v1.2 de otra sesión (`FP-407`) y su COMMIT-2 |
| «P2 · COMMIT-3 · la realidad y el veredicto. python3 tools/corrida0.py run CALC-GOB-DIGITAL-EXE-ADJUDICACION-0001 …» | DIFERIDO-A:COMMIT-1 v1.2 + su COMMIT-2 | Mismo bloqueo. Sin R, adjudicación, ΔMAE ni soporte de 2025; el par sigue sin consumir. `NC-0448` | Sesión que ejecute el COMMIT-2 del v1.2 |
| «P3 · El veredicto, con las palabras de la spec …» | DIFERIDO-A:COMMIT-3 del v1.2 | Sin corrida no hay veredicto ni sección EXPLORATORIO. `NC-0449` | Acto del COMMIT-3 v1.2 |
| «P4 · Registro. Celda-D GOB… con veredicto, margen_material, champion_actual … Marcador re-derivado por comando … celdas_validadas antes/después.» | DIFERIDO-A:COMMIT-3 del v1.2 | Celda-D sigue `SPEC-CONGELADA` (`unidad_objetivo: persona`; `FP-393` sigue ABIERTA en `04a2edeb`); marcador no re-derivado; el par sigue `RESERVADA`. `NC-0450` | Acto del COMMIT-3 v1.2, tras `FP-393` |
| «Propuesta — tu lanzamiento con este archivo es el sello: Cuentan (cuenta_gen2 = SI) … sea cual sea el veredicto» | DECISIÓN-DE-MESA-PENDIENTE | La firma quedó dada sin corrida a la que aplicarse; mesa decide si se hereda al v1.2 y a sus COMMIT-2/3. `NC-0451` | `FP-407` |
