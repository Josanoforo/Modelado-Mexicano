...
- **NC abiertas por razón (token A.14, prefijo exacto).** abiertas `179` · por token: `DECISIÓN-DE-MESA-PENDIENTE` 16 · `DIFERIDO-A` 23 · `FUERA-DE-PERÍMETRO` 43 · `NO-VERIFICABLE-AQUÍ` 5 · `PARO-ENTORNO` 3 · `PARO-PREMISA` 18 · `SUSTITUIDO-POR` 1 · prosa (sin token reconocible) `70`.
- **GEN2 (derivado de `corrida0 status`).** corridas selladas `119` / requeridas `86` · resultados sellados `13019` / activos `210` · pendientes `210` · dependencias numéricas legacy activas `149` · validación independiente `215` · diferencias materiales `0` · NC- abiertas `179` · replays LEGACY-GEN1 sellados `5` (no cuentan). El `0 / N` es la lectura correcta: el aparato se construyó antes que las corridas.
