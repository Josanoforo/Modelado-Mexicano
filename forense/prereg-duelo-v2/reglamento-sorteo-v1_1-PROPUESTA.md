# `REGLAMENTO-SORTEO-MARCO-M` · regla 3 con implementación exacta — PROPUESTA, 1/sep/2026

**Estado: `PENDIENTE-DE-MESA`.** Este documento NO está sellado. Ningún
sorteo corre ni se re-corre por este documento — es redacción, igual que
`sorteo-act-pil-3-v2-PROPUESTA.md` lo fue hasta el `Sello de mesa` de
`ADR-178`/`FP-150`. Escrito por `ACTO MAESTRA33-S1 · SORTEO-v3-Y-PROPAGA`,
P2, a partir del hallazgo de `FP-213`/`ADR-248`
(`forense/prereg-duelo-v2/sorteo-marco-M-resultados-v1_1.md`, sección
«Hallazgo») y de la implementación de `sorteo_v3.py` (P1 del mismo acto).
