...
**Nota 2026-09-04 (recibo, `ACTO MAESTRA38-N7 · PRE-REGISTRO-CIVICO-LAPOP`, NUBE, `FP-299` — recibo, no requiere firma; encargo citaba `ADR-339`/`FP-300`, off-by-one contra el máximo real re-derivado, mismo patrón D-13 que `N5` ya declaró).** Dos specs de pre-registro selladas, cero medición, ampliando el conteo de `forense/prereg-caja/` de 3 a 5: `S4-L4-spec-v1_0.md` (`civico.voto.clientelar_si_observable`, objeto de `N5 §2.6`, LAPOP 2019 `clien1n`/`clien1na` × `vb3n`) y `S5-L5-spec-v1_0.md` (`civico.protesta.agravio_urbano`, objeto de `N5 §2.8`, multi-ola 2004/2006/2019, `TAMANO` como estrato), cada una con su `.sha256`. **Hallazgo central de esta pieza, no citado por el encargo ni por `N5`:** `MAESTRA35-L9`/`L11` (2/sep/2026, dos días antes de `N5`) **ya corrieron falsaciones reales** de estos mismos dos `id` de canon — `civico.voto.clientelar_si_observable` vía el brazo de observabilidad percibida (LAPOP 2023 + ENCUCI 2020, veredicto `CONTRARIA` replicado dos veces) y `civico.protesta.agravio_urbano` vía agravio×entorno (LAPOP 2019 + ENCUCI 2020, veredicto `CORROBORADA-PARCIAL` replicado), las dos `PENDIENTE-DE-MESA` en `FP-298` sin sellar. `S4`/`S5` no repiten esas corridas: `S4` ataca el otro brazo de la disyunción (proximidad/focalización del reparto, único medible en la ola 2019 que trae la batería `clien*`) y declara por qué la celda de tres factores que pedía el encargo no es construible en una sola ola; `S5` completa el diseño de cuatro antecedentes que `L9` dejó en dos, con los dos reactivos (`AOJ12` falla estatal, `CP6`/`CP9` red previa) que el inventario `v1_1` —nacido un día después de `L9`— sí trae. `S5` corrige además la caracterización de `N5` sobre `LAPOP-E8` (ítem de aprobación normativa, no de asistencia propia). Perímetro explícito del encargo: `NO toca: canon` — no se abre ningún ADR (`ADR-338` sigue libre, no se abre aquí; propagación a canon diferida al sucesor que mesa dispare, mismo criterio que `N5`). Este snapshot del tablero sigue sin regenerarse (mismo `adr_max`/`fp_max` obsoletos de las notas de arriba).
