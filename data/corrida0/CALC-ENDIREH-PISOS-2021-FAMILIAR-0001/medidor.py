"""ENDIREH 2021, violencia familiar, todos los cuestionarios.
...
MEMBER = "TB_SEC_XI.csv"
DEM = "TSDem.csv"
ACTS = tuple(range(1, 21))
...
def measure_rows(rows, replicas=200, seed=20260923):
    axes = {"nacional": ["MX"], "edad": ["15-29", "30-44", "45-59", "60+"],
            ...
            "localidad": ["U", "C", "R"],
            "pareja": ["A1", "A2", "B1", "B2", "C1", "C2"],
            "entidad": [f"{i:02d}" for i in range(1, 33)]}
    out = []
    for axis, cats in axes.items():
        for cat in cats:
            x = _aggregate(rows, "desde_octubre_2020", axis, cat,
                           seed + sum(map(ord, axis + cat)), replicas)
            out.append({"ventana": "desde_octubre_2020", "eje": axis,
                        "categoria": cat, **x})
    return out
...
def medir(inputs, contrato):
        ...
        with _member(archive, MEMBER) as stream:
            for r in csv.DictReader(stream):
                ...
                if not r.get("EST_DIS") or not r.get("UPM_DIS"):
                    continue
                rows.append({"desde_octubre_2020": classify([r.get(f"P11_1_{i}", "") for i in ACTS]),
    ...
    return {"RESULT-ENDIREH2021-FAM-TABLA": json.dumps(table, ensure_ascii=False, sort_keys=True),
            "RESULT-ENDIREH2021-FAM-N-ELEGIBLES": len(rows)}
