...
**16/sep/2026 · CAJA · `ADR-535` (candidato) · `PR` pendiente al redactar.**
...
evidencia cruda. La **`FP-379 · ENMIENDA`** ratificó el match y **venció `D2(a)` por premisa falsa**. `NC-0293`, CERRADA.
...
## 5 · Un defecto de proceso propio (`NC-0297`)
...
* **`L+corpus`: DIFERIDO** (`NC-0300`) — por **costo y tiempo**, *no* por falta de mandato. Sólo se vuelve necesario
  si `L-solo` venciera a algún piso, y no venció a ninguno.
* **El intervalo al 80 % NO se extrae** (`NC-0296`): la regla congelada devuelve un punto, y escribir una segunda
...
* **Las filas `NC` colisionaron** (`NC-0301`): la spec `v1.2`, congelada, cita `NC-0283`…`NC-0287` porque al
  redactarse el máximo era `NC-0282`; `GEN2-TRAMITE-4` ya ocupaba hasta `NC-0292`. **La spec sellada no se edita**
  (`E.3`); el mapa vive en `NC-0301`: `0283→0293 · 0284→0294 · 0285→0295 · 0286→0296 · 0287→0297`.

---

## 10 · Dos hallazgos abiertos sobre una guardia ajena

* **`NC-0298`** — el catálogo se declara *«append-only por construcción»* (`milpa/src/momentos.py::sellar_catalogo`),
...
* **`NC-0299`** — `test_c` del mismo archivo **ya fallaba antes** y no lo causó este acto: el catálogo y
...
3. **La guardia `test_a2`** (`NC-0298`), con perímetro abierto sobre `tests/test_motor_holdout.py`.
4. **Regla de extracción de intervalos de elicitación**, pre-registrada antes de mirar capturas (`NC-0296`).
5. **Consumo por el marcador por segmento**, *gated* a `NC-0275` resuelta.
6. **El hueco de vocabulario** «persistencia» en el enum `estrategia` (`NC-0294`).
