"""ENDIREH 2021, pareja no física A/B/C y ayuda B/C.
...
GROUPS = {
    ...
    "digital": (30, 31),
    "economica_patrimonial": tuple(range(32, 39)),
    "no_fisica_alguna": tuple(range(10, 39)),
}


def acts_for(group, instrument):
    numbers = GROUPS[group]
    if instrument == "C1":
        numbers = tuple(i for i in numbers if i not in SPECIAL)
    return [f"P14_1_{i}{'AB' if i in SPECIAL else ''}" for i in numbers]
...
def measure_rows(rows, replicas=200, seed=20260923):
    axes = {"nacional": ["MX"], "edad": ["15-29", "30-44", "45-59", "60+"],
            ...
            "localidad": ["U", "C", "R"], "pareja": ["A1", "A2", "B1", "B2", "C1"],
            "entidad": [f"{i:02d}" for i in range(1, 33)]}
    out = []
    for group in GROUPS:
        for window in ("vida_relacion", "desde_octubre_2020"):
            outcome = f"{group}__{window}"
            for axis, categories in axes.items():
                for category in categories:
                    ...
                    out.append({"resultado": group, "ventana": window,
                                "eje": axis, "categoria": category, **x})
    for outcome in ("ayuda_bc", "denuncia_bc"):
        for axis, categories in axes.items():
            for category in categories:
                x = _aggregate(rows, outcome, axis, category,
                               seed + sum(map(ord, outcome + axis + category)), replicas)
                out.append({"resultado": outcome, "ventana": "desde_inicio_relacion",
                            "eje": axis, "categoria": category, **x})
    for prefix, count in (("institucion_bc", 10), ("razon_bc", 15)):
        for i in range(1, count + 1):
            outcome = f"{prefix}_{i:02d}"
            x = _aggregate(rows, outcome, "nacional", "MX",
                           seed + sum(map(ord, outcome)), replicas)
            out.append({"resultado": outcome, "ventana": "desde_inicio_relacion",
                        "eje": "nacional", "categoria": "MX", **x})
    return out
        ...
        with _member(archive, MEMBER) as stream:
            for r in csv.DictReader(stream):
                ...
                if not r.get("EST_DIS") or not r.get("UPM_DIS"):
                    continue
                row = {"weight": weight, "stratum": r["EST_DIS"], "psu": r["UPM_DIS"],
                       "edad": age_group, "escolaridad": _school_group(level),
                       "localidad": r.get("DOMINIO"), "pareja": instrument,
                       "entidad": r.get("CVE_ENT")}
                for group in GROUPS:
                    acts = acts_for(group, instrument)
                    life = classify([r.get(a, "") for a in acts])
                    recent = []
                    for a in acts:
                        ever = r.get(a, "").strip()
                        suffix = a[len("P14_1_"):]
                        recent.append("4" if ever == "4" else r.get("P14_3_" + suffix, ""))
                    row[group + "__vida_relacion"] = life
                    row[group + "__desde_octubre_2020"] = (
                        classify(recent) if life is not None else None)
                if instrument in {"B1", "B2", "C1"}:
                    all_acts = [f"P14_1_{i}{'AB' if i in SPECIAL else ''}"
                                for i in range(1, 39) if instrument != "C1" or i not in SPECIAL]
                    violence = classify([r.get(a, "") for a in all_acts])
                    help_ = _yesno(r.get("P14_7_1")) if violence == 1 else None
                    report = _yesno(r.get("P14_7_2")) if violence == 1 else None
                else:
                    help_ = report = None
                row["ayuda_bc"] = help_
                row["denuncia_bc"] = report
                for i in range(1, 11):
                    row[f"institucion_bc_{i:02d}"] = (
                        _yesno(r.get(f"P14_8_{i}")) if help_ == 1 else None)
                vals = reasons.get(r.get("ID_PER"), [])
                for i in range(1, 16):
                    row[f"razon_bc_{i:02d}"] = (
                        _reason(vals[i - 1]) if help_ == 0 and report == 0 and len(vals) == 15
                        else None)
                rows.append(row)
    if not rows:
        raise ValueError("sin mujeres con relación elegible")
    table = measure_rows(rows, int(contrato["parametros"]["bootstrap_replicas"]),
                         int(contrato["seed"]["valor"]))
    return {"RESULT-ENDIREH2021-NF-BC-TABLA": json.dumps(table, ensure_ascii=False, sort_keys=True),
            "RESULT-ENDIREH2021-NF-BC-N-ELEGIBLES": len(rows)}
