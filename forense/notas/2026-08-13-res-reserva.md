...
- **Primera corrida tras editar todos los archivos: `LÍNEA BASE: ROJO`, 5 entradas nuevas.** Dos defectos propios de este acto, ninguno del terreno:
  ...
  2. `T03` (referencias colgantes): la enmienda in situ sobre `gobernanza:866` citó, en un punto, el nombre de archivo sin el prefijo de fecha (basename solo, sin `2026-08-13-` por delante) — T03 solo resuelve nombres de archivo sin ruta (`` `([A-Za-z0-9_\-áéíóúñÁÉÍÓÚÑ.]+\.(?:md|yaml))` ``, `tests/check.py:218`), y ese basename no existe (el archivo real es `2026-08-13-enasic-split-verificacion.md`). Corregido a la cita con ruta completa, mismo patrón que las otras tres citas de la misma nota en el mismo párrafo.
