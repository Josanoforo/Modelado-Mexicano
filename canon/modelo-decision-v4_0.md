...
| **Sin reactivo verificado — búsqueda abierta** | `exposicion_violencia` — su reactivo reportado (ENVIPE `BP1_20/23/28`) fue **retirado**: mide conducta de denuncia condicionada a victimización, no exposición a violencia (`PR #57`, `forense/hitoE-campana-medicion-v2_0.md` §15); dos actos de búsqueda ya en curso (posición 4 rehecha sobre `TPer_Vic1`; barrido de alcanzabilidad ENDIREH/ENSU); un tercero se nombra cuando esos dos cierren · `sens_estatus` — sale de "sin reactivo o no determinable": ese veredicto descansaba en una limitación del inventario, no en una fuente examinada y descartada (`hitoE §17`); ningún acto de búsqueda nombrado todavía | **2** |
| **Sin reactivo — búsqueda cerrada (límite de régimen)** | `aversion_riesgo` — único con candidato examinado y descartado con argumento: ENIF `P5_23`/`P5_24` mide conocimiento de protección de depósitos IPAB, el moderador que `dinero.ahorro.seguro_deposito_atenua_aversion` pone en el `SI`, no una medida de aversión (`hitoE §17`) | **1** |
...
- **Condicionales medidas sobre atributos: ~~6~~ 8 de 14** (§1.1.F · §6.1, corregido 4/ago/2026 — `forense/notas/2026-08-03-cal-conf-faseb-pos5-6-radio-familismo.md` §5). De las 14: **8 MEDIDO·PARCIAL** (`confianza_institucional[salud/educación/financiera/seguridad-FFAA/justicia-policía/electoral-partidos]`, 6 de 6, más `radio_confianza` y `familismo_apoyo`), 2 sin reactivo verificado — búsqueda abierta (`exposicion_violencia`, `sens_estatus`; clase sellada 4/ago/2026, `gobernanza` §4, ADR-52 A, ver §1.1.F Paso 5), 1 sin reactivo — búsqueda cerrada, límite de régimen (`aversion_riesgo`), 3 solo proxy.
