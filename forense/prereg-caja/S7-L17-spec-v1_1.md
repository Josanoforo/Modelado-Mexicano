...
### `prereg-caja-S7-L17` · **v1.1** · 7 de septiembre de 2026 (corrige `§2`, mapeo del bloque `a0927`; `v1.0` intacta)

> | | |
> |---|---|
> | **ARCHIVO** | `forense/prereg-caja/S7-L17-spec-v1_1.md` |
...
**Acto (v1.0):** `ACTO MAESTRA38-N11 · PRE-REGISTRO-OLA6-MEDIBLES-Y-FICHAS`, 5/sep/2026, entorno **NUBE**, sobre `origin/main = b17d19bd1d566220ac81ebbac47c1c80ae14d66e` (SHA de redacción del encargo).

**Acto (v1.1, corrección de §2):** `ACTO MAESTRA38-N18`, 7/sep/2026, entorno **NUBE**, sobre `origin/main = b1be143` — registra el mapeo real (`ADR-357`/`FP-326`); no re-corre la medición de `L17`.

---

## 0 · Ficha bajo prueba y corrección de premisa (A.8/D-13)

**v1.1 registra el mapeo real; la medición de L17 (`ADR-357`) ya lo usó; no se re-corre.**
...
**Corrección de mapeo (v1.1, `ADR-357`/`FP-326`).** `v1.0` asumía letra=vacuna/dígito=razón; el codebook, confirmado contra `adultos_ensanut2024_w.dta` en `MAESTRA38-LOTE-ENSANUT`/`L17`, lo invierte: **letra = razón, dígito = vacuna**.

| letra (razón) | motivo verbatim |
|---|---|
| `a` | «…**no había vacunas**…» — disponibilidad, antecedente negado directamente observado |
| `b` | «…por **no ser derechohabiente**…» — barrera institucional/logística |
| `c` | «…porque **no estaba la persona que aplica**…» — barrera de personal, logística |
| `d` | «…porque **estaba enfermo**…» — contraindicación médica, no logística ni actitudinal |
| `e` | «…por **otra razón**…» — residual, sin discriminar |

| dígito (vacuna) | vacuna |
|---|---|
| `1` | Influenza |
| `2` | Neumococo |
| `3` | Tétanos |
...
*(sufijos exactos `a0927a1`…`a0927e4`, `data/inventario-reactivos-descargas-mx-v1_1.tsv:27885-27904` — mapeo letra=razón/dígito=vacuna verificado contra el codebook, no el asumido por `v1.0`)*
...
**Rama B** (falsador de una sola muestra, no de cruce; mapeo corregido `v1.1`, letra=razón/dígito=vacuna): `RAZON_LOGISTICA` = 1 si la razón marcada es letra `a` (no había vacunas) o letra `c` (no estaba quien aplica), sobre cualquier dígito de vacuna (`1`-`4`); `RAZON_NO_LOGISTICA` = 1 si es letra `b` (no derechohabiente — institucional, se reporta aparte, ni logística de suministro ni actitudinal), letra `d` (enfermedad) o letra `e` (otra). Proporción de `RAZON_LOGISTICA` sobre el total de razones declaradas, por vacuna (dígito) y agregada.
