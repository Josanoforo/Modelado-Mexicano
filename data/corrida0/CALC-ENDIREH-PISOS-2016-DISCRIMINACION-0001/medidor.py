"""ENDIREH 2016, discriminación laboral 7.3; agregados de diseño únicamente."""
    ...
    for outcome in OUTCOMES:
        for axis, categories in axes.items():
            for category in categories:
                ...
                out.append({"resultado": outcome, "ventana": "octubre_2011_a_entrevista",
    ...
    with zipfile.ZipFile(inputs[PAYLOAD]["ruta_absoluta"]) as archive:
        with _member(archive, "TSDem.csv") as stream:
            dem = {r["ID_MUJ"]: (r["EDAD"], r["NIV"]) for r in csv.DictReader(stream)
                   if r.get("ID_MUJ")}
        rows = []
        with _member(archive, "TB_SEC_VII.csv") as stream:
            for r in csv.DictReader(stream):
                ...
                if not r.get("EST_DIS") or not r.get("UPM_DIS"):
                    continue
                t1 = classify_binary([r.get("P7_3_1_1")])
                t2 = classify_binary([r.get("P7_3_1_2")])
                pregnancy = [r.get(f"P7_3_2_{i}") for i in range(1, 4)]
                row = {"prueba_ingreso": t1, "prueba_continuidad": t2,
                       "prueba_alguna": classify_binary([r.get("P7_3_1_1"), r.get("P7_3_1_2")]),
                ...
                rows.append(row)
    if not rows:
        raise ValueError("sin trabajadoras elegibles 2011-2016")
    table = measure_rows(rows, int(contrato["parametros"]["bootstrap_replicas"]),
                         int(contrato["seed"]["valor"]))
    return {"RESULT-ENDIREH2016-DIS-TABLA": json.dumps(table, ensure_ascii=False, sort_keys=True),
            "RESULT-ENDIREH2016-DIS-N-ELEGIBLES": len(rows)}
