...
def _tabla(nombre, objeto):
    """Serialización nativa REF; exclusivamente tablas del CALC propietario."""
    if nombre not in {"replicas.json", "potencia.json"}:
        raise PermissionError("tabla fuera de lista blanca")
    calc = Path(__file__).resolve().parent
    if calc.name != "CALC-FAMILIA-2027-ENIF-ORO-0002":
        raise PermissionError("medidor sucesor fuera de CALC propietario")
    carpeta = calc / "tablas"
    ...
    if destino.is_symlink() or destino.resolve().parent != carpeta.resolve():
        raise PermissionError("tabla fuera del CALC")
...
def medir(inputs, contrato):
    ...
    out["RESULT-FAMILIA-ENIF-ORO-REPLICAS"] = _tabla("replicas.json", resultado["replicas"])
    ...
    out["RESULT-FAMILIA-ENIF-ORO-POTENCIA"] = _tabla("potencia.json", escenarios(resultado["p"], resultado["replicas"]))
    return out
