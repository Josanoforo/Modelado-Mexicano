...
**PR #77, atendido al arrancar y no al cerrar.** El worktree nuevo no traía `data/raw` ni `data/raices.local.yaml` (ambos gitignorados). Antes de bajar el primer byte se creó el symlink `data/raw -> /home/pc0/mm-corpus/raw` y se copió `data/raices.local.yaml` (con ruta, no a secas: T03 vigila nombres de archivo desnudos entre acentos graves y este está gitignorado, así que nunca existe en un checkout limpio). Comprobación de cierre: `ls -la data/raw` → symlink al corpus compartido; los 89 payloads viven en `/home/pc0/mm-corpus/raw/ADQ15_*`, **no** dentro del worktree. `git status` no lista ninguno.
...
**Cifra de la suite sobre el árbol fusionado**, que es la única que existe (lección de la colisión `#264`/`#265`, 18/ago): **21 FAIL · 119 WARN · `LÍNEA BASE: VERDE`** contra el `HEAD` congelado `e24d033`, nada nuevo. No hay cascada a `estado-programa` desde este acto: la cifra la recifró `PR #276` y este acto no la mueve.

**Y esa lección se cobró aquí mismo, en CI.** La primera corrida de la Verificación del corpus sobre `PR #277` salió **`ROJO` con tres entradas nuevas**, y sólo una era mía:

- **`T03` — mía, real.** La nota citaba el archivo de raíces **a secas** — el nombre solo, entre acentos graves, sin el `data/` delante. `T03` vigila nombres de archivo desnudos y ese archivo está gitignorado: **nunca existe en un checkout limpio**, así que la cita es colgante por construcción. Es el mismo trampolín que `MAP-2` ya había pisado el 6/ago con este archivo exacto, y la solución es la misma que aquella nota dejó escrita: **citarlo con ruta** (`data/raices.local.yaml`), porque el patrón de `T03` no admite `/`. Corregido. **Y el `VERDE` local no podía atraparlo**, que es la mitad interesante: `T03` construye su conjunto de archivos existentes con un `glob` sobre el árbol de trabajo, y en mi disco `data/raices.local.yaml` **sí está** — lo creé yo al arrancar. En el checkout limpio de CI no está. Para reproducir CI hay que **esconder los gitignorados** (`data/raices.local.yaml` y el symlink `data/raw`) y volver a correr la suite; hecho, y ahí sí aparece el `T03`. Cualquier acto que cite un archivo gitignorado tiene este punto ciego.
- **`T16` ×2 — no eran mías, eran de la cifra.** CI evalúa la **fusión** de la rama con `main` en ese momento. Mi rama traía `estado-programa` de la fusión con `6650047`, que declaraba `118 WARN`; `main` ya había avanzado a `f11a363` (`PR #276`), que la recifró a `21 FAIL · 119 WARN`. La fusión producía `119` y el documento seguía diciendo `118`. Se resolvió **fusionando `main` de nuevo**, no editando la cifra: `estado-programa` es de `main`, y este acto no tiene por qué tocarlo.

*Lección, que es la de `#264`/`#265` otra vez y por eso vale registrarla: un `VERDE` local sólo vale contra el `main` de ese instante. Entre la fusión y el CI, `main` se movió, y el contador que otro acto recifró correctamente convirtió mi rama en roja sin que yo hubiera tocado ese archivo. La respuesta correcta no es recifrar —sería pisar la cifra de `#276`— sino volver a fusionar.*
