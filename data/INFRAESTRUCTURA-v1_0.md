...
| `data/corrida0/resultados.tsv` | ídem | cabecera `# DERIVADO — NO EDITAR`, luego 18 columnas: `resultado_id · origen · corrida_id · spec_id · valor · tipo · unidad · estado · generacion · cuenta_gen2 · tolerancia · validacion_independiente · valor_legacy · delta_legacy · sello · depende_de · n_usos · sucesor` | ídem | **La unicidad de un `resultado_id` es POR CORRIDA, no global**: un replay declarado (`repite_de`) reproduce los mismos ids a propósito — es lo que significa replicar. El mismo id en dos corridas **sin** esa cadena sí PARA. Los `RESULT-SMOKE-*` entran con `cuenta_gen2 = NO` y no suman a ningún contador GEN2. |
