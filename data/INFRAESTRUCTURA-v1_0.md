...
## `forense/prereg-caja/` — pre-registros congelados antes de que caja corra (`ACTO MAESTRA38-N3 · PRE-REGISTRO-DE-CAJA`, 4/sep/2026; ampliado por `ACTO MAESTRA38-N7 · PRE-REGISTRO-CIVICO-LAPOP`, mismo día; y por `ACTO MAESTRA38-N11 · PRE-REGISTRO-OLA6-MEDIBLES-Y-FICHAS`, 5/sep/2026)
...
| `forense/prereg-caja/S6-L6-spec-v1_0.md` + `.sha256` | dirección, a mano, en NUBE, sin corpus montado — mismo patrón COMMIT-1-por-spec que `S1-S5`, objeto `MEDIBLE-COMO-ESTÁ` de `forense/notas/2026-09-05-MAESTRA38-N10-cobertura-ola6.md §2.2` | mismo contrato de `.sha256` que las filas de arriba | el futuro acto `MAESTRA38-L6` (`salud.atencion.grave`, `R4.4`, ENNViH-3 `es09`×`cen10*`) — compara su primer resultado contra la spec congelada aquí; nombre de acto nuevo, asignado por `S6`, sin colisión verificada | verifica y declara que `R4.4` **no** tiene veredicto archivado en Hito D (ausente de las 27 reglas del perímetro, `canon/modelo-decision-v4_0.md §7`) — primer intento de falsación pre-registrado sobre este `id` |
| `forense/prereg-caja/S7-L7-spec-v1_0.md` + `.sha256` | dirección, a mano, en NUBE, sin corpus montado — mismo patrón COMMIT-1-por-spec que `S1-S6`, objeto `MEDIBLE-COMO-ESTÁ` de `forense/notas/2026-09-05-MAESTRA38-N10-cobertura-ola6.md §2.5` | mismo contrato de `.sha256` que las filas de arriba | el futuro acto `MAESTRA38-L7` (`salud.vacunacion.disponible`, `R9.2`, ENNViH costo×recepción de vacuna) — compara su primer resultado contra la spec congelada aquí; ola de ENNViH declarada pendiente de CAJA, no adivinada | **A.8/D-13:** declara que `canon/modelo-decision-v4_0.md §7` ya archiva veredicto `D` de Hito D para este `id` (`ADR-56`, 4/ago/2026) — por un diseño distinto (auditor institucional tercero de abasto/campaña) del que esta pieza pre-registra (autorreporte de hogar); no lo reabre |
| `forense/prereg-caja/S8-L8-spec-v1_0.md` + `.sha256` | dirección, a mano, en NUBE, sin corpus montado — mismo patrón COMMIT-1-por-spec que `S1-S7`, hallazgo nuevo de `forense/notas/2026-09-05-MAESTRA38-N10-cobertura-ola6.md §2.6` | mismo contrato de `.sha256` que las filas de arriba | el futuro acto `MAESTRA38-L8` (`comunicacion.inseguridad.ver_oir_callar`, `R10.3`, módulo `AOJ` de LAPOP, multi-ola) — compara su primer resultado contra la spec congelada aquí; lista de olas no cerrada (a diferencia de `S5`), declarado pendiente de CAJA | verifica y declara que `R10.3` **no** tiene veredicto archivado en Hito D; continuidad declarada, no repetida, con las corridas `EXISTE-NO-SATISFACE` de `N5` (ENDIREH) y `N6` (CNGMD) sobre el mismo `id` |
