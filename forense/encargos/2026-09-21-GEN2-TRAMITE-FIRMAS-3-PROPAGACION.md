...
Ejecutado por `PR #940` (`https://github.com/Josanoforo/Modelado-Mexicano/pull/940`), rama `acto/gen2-tramite-firmas-3-propagacion`. `ADR-574`, `canon/estado-programa-v1_14.md` L0 recifrado, `canon/registro-rotulos.tsv` (rótulo `GEN2-TRAMITE-FIRMAS-3-PROPAGACION`). `python3 tests/check.py --baseline --parallel` → LÍNEA BASE ROJO por defecto ajeno pre-existente (2 `T22` de `GEN2-SENAL-1`, verificado no causado por este acto, ver `## NO-CORRIDO / RESERVAS`). **NO FUSIONAR** sin que mesa revise ese punto y `NC-0260`/`corridas.tsv` (mismo NO-CORRIDO).
