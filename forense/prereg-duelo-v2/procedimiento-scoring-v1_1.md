...
verificado al sellar (`sha256sum procedimiento-scoring-v1_1-PROPUESTA.md`):
`7c7cfc2c8d273ed50048438b9a21e1e7d9612ce183558d8461140541b5a8fd39` — ver
`procedimiento-scoring-v1_1.sha256` adjunto (hash del propio archivo
sellado, mismo patrón que `L-spec-v1_1.sha256`). La PROPUESTA **no se
borra ni se edita**: sigue íntegra en su ruta original, registro de lo
redactado antes de la firma (A.10).

**Firma de mesa (verbatim, 1/sep/2026):** «[sello scoring v1_1 — cinco
decisiones]» — sella las cinco decisiones enumeradas en
`forense/prereg-duelo-v2/mesa-pendientes.md` §5 (unidades `z = dif/EE(R)`;
agregado por corredor con bootstrap `seed=42`/`IC=0.95`; comparación
principal `L_SOLO_vs_M` pareada, solo PASO 2; baseline `B` NO-APLICA a
marco-M; F-DD fuera del agregado y del pareado) — ninguna decisión nueva,
ninguna elegida por este acto de sello: activa lo que `ACTO MAESTRA33-E10`
ya había reconciliado contra texto sellado (cabecera "P2" original, abajo).
Firmar esto **no ejecuta nada**: `tools/score_marco_m.py` y
`scoring-adv1-m3.py` siguen sin tocarse; `delta=0.5` sigue sin entrar a
ninguna entrada real de scoring hasta que un acto sucesor invoque el
cargador con intención de puntuar (este acto solo verificó que corre, sin
puntuar nada: `forense/notas/2026-09-01-maestra33-e12-sella-1-cierre.md`,
P2).

**SHA de redacción (heredado de la PROPUESTA, sin cambio):** `b3c6a1d`
(`main` tras fusionar `PR #427`, `ACTO MAESTRA33-E9 · L-SPEC-v1_1` — la spec
`L` congelada que esta propuesta asume, sin abrirla: ninguna cifra de
`L-spec-v1_1.json` se lee ni se cita aquí, solo su existencia).
