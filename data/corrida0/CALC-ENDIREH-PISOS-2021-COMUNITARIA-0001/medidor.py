"""ENDIREH 2021, violencia comunitaria, todos los cuestionarios.
...
MEMBER = "TB_SEC_IX.csv"
DEM = "TSDem.csv"
ACTS = tuple(range(1, 17))
...
def medir(inputs, contrato):
        ...
        with _member(archive, MEMBER) as stream:
            for r in csv.DictReader(stream):
                ...
                if not r.get("EST_DIS") or not r.get("UPM_DIS"):
                    continue
                life = classify_binary_union([r.get(f"P9_1_{i}", "") for i in ACTS])
                recent_values = []
                for i in ACTS:
                    ever = r.get(f"P9_1_{i}", "").strip()
                    recent_values.append("4" if ever == "2" else r.get(f"P9_3_{i}", ""))
    ...
    return {"RESULT-ENDIREH2021-COM-TABLA": json.dumps(table, ensure_ascii=False, sort_keys=True),
            "RESULT-ENDIREH2021-COM-N-ELEGIBLES": len(rows)}
