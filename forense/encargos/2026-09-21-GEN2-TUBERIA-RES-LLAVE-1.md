...
rama `claude/lucid-hypatia-j9m0xj`, 21/sep/2026. `ADR-587` (renumerado 583 → 586 → 587: dos tandas de actos fusionaron antes; renumera quien fusiona segundo).

**PROPUESTO, no fusionado: el merge es de mesa** (firma D-r4 y §5 del encargo).

**ENMIENDA FECHADA (21/sep/2026) — no se edita lo de arriba, se enmienda (A.10).** La línea anterior describe el estado en que el ejecutor dejó el PR. Después de ella, **mesa autorizó explícitamente al ejecutor a fusionar este PR**, tras confirmar las dos lecturas medidas como decisión registrada y no como bloqueo: el `149 → 150` de `dependencias_numericas_legacy_activas` es la deriva en vivo ya declarada y ajena a este PR (`NC-260921-GEN2-TUBERIA-RES-LLAVE-1-5573-01`), y las dos filas de más (`marco-M::CIV-M-01::R`, `marco-M::DIN-M-01::R`) son cosméticas — sólo `canales_observados`, con veredicto, `calc_candidato` y pin idénticos (`…-5573-02`). **Fusionado por el ejecutor bajo esa autorización: merge commit `e24a7a53c936c210bab7dee94ddc34c135b37cb1`.** La rama `claude/lucid-hypatia-j9m0xj` **no se borra**, por excepción declarada a la política de cero ramas (A.14): es la rama de trabajo designada de la sesión, no una rama de acto residual; GitHub la auto-borró al fusionar y se restauró en el acto.
