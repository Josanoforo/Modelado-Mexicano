# `CALC-AGG-marco-M-sorteado-v1_3-ola-v2` — dos vistas, cero colapso

> **Sucesora de `CALC-AGG-marco-M-sorteado-v1_3-ola`** (`repite_de`), que
> queda `SUPERADA` con sus bytes intactos. Consume los `RESULT` de
> `CALC-M-marco-M-sorteado-v1_3-ola-v2`, cuyo rótulo de modulación es
> `SERIE-PREVIA` — la predecesora emitía `SERIE-LOO`, la regla que la ADENDA
> DE MESA del 8/sep/2026 derogó por fuga temporal. Añade dos conteos:
> `N-SIN-PREVIA` (celdas con serie pero sin ola anterior) y
> `N-VERIFICACION-NO-PUNTUA` (celdas cuya ola usada es `ORIGEN-ARBITRO`).
