...
## §4 · Post-cierre — fusión con `main` (`PR #269`, mismo día)

`origin/main` avanzó mientras este acto corría: `PR #264` (`ESTADO-SPLIT`), `PR #265` (`CONF-07-CIERRE`) y `PR #266` (`LANE-A-E0-E5`) fusionaron por delante, cada uno tocando la misma línea de cascada de WARN de `estado-programa`. `git merge origin/main --no-edit` sobre esta rama dio **un** conflicto real, en `canon/estado-programa-v1_10.md` (dos bloques, la cita de §4·S3 y la cita de cierre de §7) — `firmas-pendientes.tsv` y `hallazgos.md` fusionaron limpios (ambos son de anexo, sin colisión de línea). Resuelto conservando el lado de `main` (ya traía el recifrado correcto `T03=47`, corregido por `LANE-A-E0-E5` al fusionar `CONF-07-CIERRE`) y re-derivando la cifra final sobre el árbol ya fusionado en vez de sumar a mano: `python3 tests/check.py --baseline` da **`19 FAIL · 127 WARN`, VERDE** (no `128`, la cifra de `main` antes de traer este acto) — la diferencia es exactamente el mismo WARN de `T22` que este acto ya documentó arriba (`FP-18` deja de estar `FIRMADA`-sin-ejecutar), aplicado ahora sobre el árbol fusionado en vez de sobre el árbol viejo de esta rama. Las dos citas de cascada (`estado-programa`, bloque §4·S3 y bloque de cierre de §7) se corrigieron a `127` con parentético fechado propio, sin editar la prosa que ya traía `main`. `tests/baseline.json` sin tocar, cero `--freeze`.
