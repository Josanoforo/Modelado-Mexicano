...
**`tests/check.py --baseline`: VERDE** — `19 FAIL · 118 WARN`, nada nuevo frente a `tests/baseline.json`. Se midió también sobre el árbol prístino y da **exactamente lo mismo**: este acto no mueve `--baseline` ni un WARN. El `diff` completo de la salida del vigía, a los dos lados, es **una sola línea** — la muestra de filas `ABIERTA` avanza de `FP-47` a `FP-57` porque `FP-47` se cerró. **Cifra recifrada a media ejecución, y la corrección se deja escrita:** medida primero sobre `e563e5d` dio `124 WARN`; al fusionar `origin/main = e6864ed` (`PR #267`, `ACTO MESA-19AGO`, seis firmas propagadas) el árbol pasó a `118`. **Las seis no son de este acto.** Es literalmente la lección que `ACTO ESTADO-SPLIT` dejó escrita el 18/ago — *un contador derivado no es propiedad del acto que lo deriva, sino del árbol* —, y la única forma de tener la cifra es volver a correr la suite después de fusionar. Se volvió a correr. Se deja escrito porque `FP-47` afirmaba *"esta fila ES la entrada que pone `--baseline` en ROJO"* — **eso ya no es cierto** al ejecutar, y no lo dejó de ser por este acto; la cascada de `B2-SEMANTICO` lo había resuelto antes. Sin `--freeze`.
