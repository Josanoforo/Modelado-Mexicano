"""ENDIREH 2021, decisiones y dinero, cuestionario A.
...
def _binary(value, positive, negative):
...
def measure_rows(rows, replicas=500, seed=20260923):
    ...
    out = []
    for outcome in ("dinero_libre", "decide_sobre_dinero_ella", "decide_gasto_ella"):
        for axis, cats in axes.items():
            for cat in cats:
                result = _aggregate(rows, outcome, axis, cat,
                                    seed + sum(map(ord, outcome + axis + cat)), replicas)
                out.append({"resultado": outcome, "eje": axis, "categoria": cat, **result})
    return out
...
def medir(inputs, contrato):
    with zipfile.ZipFile(inputs[PAYLOAD]["ruta_absoluta"]) as archive:
        ...
        with _member(archive, "TB_SEC_IV.csv") as stream:
            money = {r["ID_PER"]: r.get("P4_11", "") for r in csv.DictReader(stream)
                     if r.get("ID_PER")}
        rows = []
        with _member(archive, "TB_SEC_XV.csv") as stream:
            for r in csv.DictReader(stream):
                ...
                if not r.get("EST_DIS") or not r.get("UPM_DIS"):
                    continue
                rows.append({"dinero_libre": _binary(money.get(r["ID_PER"]), {"1"}, {"2"}),
                             "decide_sobre_dinero_ella": _binary(r.get("P15_1AB_3"),
                                                                 {"1", "4", "5"}, {"2", "3"}),
                             "decide_gasto_ella": _binary(r.get("P15_1AB_7"),
                                                            {"1", "4", "5"}, {"2", "3"}),
    ...
    return {"RESULT-ENDIREH2021-DEC-TABLA": json.dumps(table, ensure_ascii=False, sort_keys=True),
            "RESULT-ENDIREH2021-DEC-N-ELEGIBLES": len(rows)}
