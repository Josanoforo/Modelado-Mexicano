...
**20 de septiembre de 2026 · `ADR-558` · rama `claude/eager-goodall-2lh66n` · NUBE `cloud_default`, Opus 5**
...
estaba**. `NC-0380`, con sucesor. Se aplicó la regla del encargo: «un slot que
...
|---|---|---|
| `dependencias_numericas_legacy_activas` | **182** | **171** (−11) |
...
está en el perímetro sólo como lectura. `NC-0381`.
...
- `FP-391` y `FP-392` → **`FIRMADA`**, con `ADR-558` y el PR de este acto (A.12).
- `data/corrida0/decisiones.tsv`: `adopcion:relevo-tanda-2` y `estimando:RES-0028-personas`.
- **`cuenta_gen2` de `CALC-ENVIPE-RES0028-U4-DERIVADO-0001` NO se decide aquí: sigue `PENDIENTE-DE-MESA`** (`NC-0382`).
