"""ENDIREH 2016, violencia física de pareja actual, cuestionario A.
...
def measure_rows(rows, replicas=500, seed=20260923):
    ...
    for window in ("vida", "desde_octubre_2015"):
...
def medir(inputs, contrato):
        ...
        with _member(archive, MEMBER) as stream:
            for r in csv.DictReader(stream):
                ...
                if not r.get("EST_DIS") or not r.get("UPM_DIS"):
                    continue
                life = classify([r.get(f"P13_1_{i}", "") for i in PHYSICAL])
                recent_values = []
                for i in PHYSICAL:
                    ever = r.get(f"P13_1_{i}", "").strip()
                    recent_values.append("4" if ever == "4" else r.get(f"P13_3_{i}", ""))
                recent = classify(recent_values) if life is not None else None
                rows.append({"vida": life, "desde_octubre_2015": recent,
    ...
    return {"RESULT-ENDIREH2016-PF-TABLA": json.dumps(table, ensure_ascii=False, sort_keys=True),
            "RESULT-ENDIREH2016-PF-N-ELEGIBLES": len(rows)}
