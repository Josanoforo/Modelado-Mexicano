...
- **Procedencia.** SHA `4f125e70` · fecha del commit `2026-09-26` · ¿árbol == origin/main? `True`.
...
- **Gobernanza operativa.** ADR máximo del espacio numérico CERRADO `593` · FP máximo del mismo espacio `409` · ids con raíz de acto (época vigente) `{'ADR': 128, 'FP': 153, 'NC': 422}` · FP abiertas: FP-260921-GEN2-CORPUS-INTEGRIDAD-Y-RESPALDO-1-3d56-01, FP-260923-GEN2-TRAMITE-FIRMAS-12-c3fa-05, FP-260923-GEN2-FRONT-1-4296-01, FP-260924-GEN2-ASTRA5-U5-ADQUISICION-1-43d6-01, FP-260926-GEN2-SEGURIDAD-ENSU-SERIE-1-5916-01 · encargos archivados `768` (consumidos `690`) · instrucciones vigentes `v2.16` · cola de encargos (solo estados != CONSUMIDO; consumidos `61`):
...
- **NC abiertas por razón (token A.14, prefijo exacto).** abiertas `411` · por token: `DECISIÓN-DE-MESA-PENDIENTE` 54 · `DIFERIDO-A` 113 · `FUERA-DE-PERÍMETRO` 59 · `NO-VERIFICABLE-AQUÍ` 27 · `PARO-ENTORNO` 5 · `PARO-PREMISA` 37 · `SUSTITUIDO-POR` 5 · prosa (sin token reconocible) `111`.
- **GEN2 (derivado de `corrida0 status`, valores EN ÁRBOL; vistas del árbol distintas de HEAD: `ninguna`).** adoptados activos `81` · pendientes de adopción `10` · corridas selladas `287` / requeridas `87` · resultados sellados `198802` / activos `211` · pendientes `211` · dependencias numéricas legacy activas `67` · validación independiente `215` · diferencias materiales `0` · NC- abiertas `411` · replays LEGACY-GEN1 sellados `5` (no cuentan). El `0 / N` es la lectura correcta: el aparato se construyó antes que las corridas.
...
- **GEN2 · medición vs. adopción (ACTO GEN2-PRE-E5 · P3).** sellados `175097` · pendientes de adopción (citados en la propuesta, ningún consumidor activo aún) `10` · vetados por decisión vigente (sellados, pero una firma prohíbe adoptarlos: no son cola) `4` · adoptados por un consumidor activo `81`. Sellar un RESULT no mueve `dependencias_numericas_legacy_activas` por sí solo: solo el consumidor activo que lo adopta la baja.
