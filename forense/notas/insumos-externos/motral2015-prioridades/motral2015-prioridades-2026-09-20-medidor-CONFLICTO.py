...
>>>>>>> 7a64858 (SPEC MOTRAL2015 prioridades prestaciones congelada)
    ...
    for r in rows:
        ...
        r["RANK_STATUS"], r["RANK_FIRST"] = status, first
    complete = lambda r: r["RANK_STATUS"] == "COMPLETO"
    segments = {"TOTAL": lambda r: True, "HOMBRE": lambda r: r["SEX"] == "1", "MUJER": lambda r: r["SEX"] == "2"}
    q = {status: _rounded(_estimate(rows, lambda r: True, lambda r, s=status: r["RANK_STATUS"] == s))
         for status in ("COMPLETO", "INCOMPLETO-CON-PRIMERO", "INCONSISTENTE", "INCOMPLETO-SIN-PRIMERO")}
    matrix, top, pairs, p17, sensitivity = {}, {}, {}, {}, {}
    for seg, predicate in segments.items():
        domain = lambda r, p=predicate: complete(r) and p(r)
        for i, benefit in enumerate(BENEFITS):
            for pos in range(1, 6):
                matrix[f"{seg}:{benefit}:{pos}"] = _rounded(_estimate(rows, domain, lambda r, i=i, pos=str(pos): r[RANK_FIELDS[i]] == pos))
            for k in (2, 3):
                top[f"{seg}:{benefit}:TOP{k}"] = _rounded(_estimate(rows, domain, lambda r, i=i, k=k: int(r[RANK_FIELDS[i]]) <= k))
        for i in range(5):
            for j in range(i + 1, 5):
                pairs[f"{seg}:{BENEFITS[i]}>{BENEFITS[j]}"] = _rounded(_estimate(rows, domain, lambda r, i=i, j=j: int(r[RANK_FIELDS[i]]) < int(r[RANK_FIELDS[j]])))
    for answer, code in (("SI", "1"), ("NO", "2")):
        domain = lambda r, code=code: complete(r) and r["P17"] == code
        for i, benefit in enumerate(BENEFITS):
            for pos in range(1, 6): p17[f"{answer}:{benefit}:{pos}"] = _rounded(_estimate(rows, domain, lambda r, i=i, pos=str(pos): r[RANK_FIELDS[i]] == pos))
            p17[f"{answer}:{benefit}:TOP2"] = _rounded(_estimate(rows, domain, lambda r, i=i: int(r[RANK_FIELDS[i]]) <= 2))
    for i, benefit in enumerate(BENEFITS):
        recovered = lambda r: sum(r[x] == "1" for x in RANK_FIELDS) == 1 and r["RANK_STATUS"] != "INCONSISTENTE"
        a = _rounded(_estimate(rows, recovered, lambda r, i=i: r[RANK_FIELDS[i]] == "1")); b = _rounded(_estimate(rows, complete, lambda r, i=i: r[RANK_FIELDS[i]] == "1"))
        sensitivity[benefit] = {"recuperado": a, "completo": b, "diferencia_puntos": round(a["p"] - b["p"], 12)}
    contrast = {b: round(p17[f"SI:{b}:TOP2"]["p"] - p17[f"NO:{b}:TOP2"]["p"], 12) for b in BENEFITS}
    result = {"meta_dbf": meta, "n_elegibles": len(rows), "calidad": q, "matriz": matrix, "top": top, "parejas": pairs, "p17": p17, "contrastes_p17_top2": contrast, "sensibilidad": sensitivity,
