...
CALC_ID = "CALC-ENUT2024-DISTRIBUCION-HORAS-0001"
...
def medir(inputs: dict, contrato: dict) -> dict:
    ...
    return {
        "RESULT-ENUTDH-G-INPUT-BD-SHA256": _sha(raw),
        "RESULT-ENUTDH-G-INPUT-FD-SHA256": _sha(fd),
        "RESULT-ENUTDH-G-N-MARCO": int(len(frame)),
        "RESULT-ENUTDH-G-N-VALIDO-COMUN": int(len(valid)),
        "RESULT-ENUTDH-G-MASA-VALIDA-COMUN": float(valid["FAC_PER"].sum()),
        "RESULT-ENUTDH-G-N-ESTRATOS": meta["n_estratos"],
        "RESULT-ENUTDH-G-N-UPM": meta["n_upm"],
        "RESULT-ENUTDH-G-N-ESTRATOS-UPM-UNICA": meta["n_estratos_upm_unica"],
        "RESULT-ENUTDH-G-PARES-DISENO-SHA256": meta["pares_sha256"],
        "RESULT-ENUTDH-G-DISTRIBUCION-N-FILAS": len(estimates),
        "RESULT-ENUTDH-G-CONTRASTES-N-FILAS": len(contrasts),
        "RESULT-ENUTDH-G-DISTRIBUCION-SHA256": dist_sha,
        "RESULT-ENUTDH-G-CONTRASTES-SHA256": contrast_sha,
        "RESULT-ENUTDH-G-INCIDENCIAS-SHA256": incidence_sha,
        "RESULT-ENUTDH-G-INCIDENCIAS": json.dumps(incidence, ensure_ascii=False, sort_keys=True),
        "RESULT-ENUTDH-A-CON-TODAS-P50": float(idx[("CON_CP", "total", "todas_validas", "p50")]["estimacion"]),
        "RESULT-ENUTDH-A-CON-TODAS-P90": float(idx[("CON_CP", "total", "todas_validas", "p90")]["estimacion"]),
        "RESULT-ENUTDH-A-CON-TODAS-CONCENTRACION": float(idx[("CON_CP", "total", "todas_validas", "concentracion_decil_superior")]["estimacion"]),
        "RESULT-ENUTDH-A-CON-PARTICIPANTES-P50": float(idx[("CON_CP", "total", "participantes", "p50")]["estimacion"]),
        "RESULT-ENUTDH-A-CON-PARTICIPANTES-P90": float(idx[("CON_CP", "total", "participantes", "p90")]["estimacion"]),
        "RESULT-ENUTDH-A-CON-PARTICIPANTES-CONCENTRACION": float(idx[("CON_CP", "total", "participantes", "concentracion_decil_superior")]["estimacion"]),
        "RESULT-ENUTDH-A-CON-BRECHA-TODAS-P50": float(cidx[("mujer_menos_hombre", "mujer-hombre", "todas_validas", "p50")]["estimacion"]),
        "RESULT-ENUTDH-A-CON-BRECHA-TODAS-P90": float(cidx[("mujer_menos_hombre", "mujer-hombre", "todas_validas", "p90")]["estimacion"]),
        "RESULT-ENUTDH-G-SALIDA-DISTRIBUCION": str(distribution_path.relative_to(root)),
        "RESULT-ENUTDH-G-SALIDA-CONTRASTES": str(contrasts_path.relative_to(root)),
        "RESULT-ENUTDH-G-SALIDA-INCIDENCIAS": str(incidence_path.relative_to(root)),
    }
