"""`CALC-R-<celda>` — el árbitro `R` de una celda ENVIPE del marco `M`, **olas DBF**.

Interfaz estable: `medir(inputs, contrato) -> {"RESULT-…": v}`.

ESCRITO Y CONGELADO EN EL COMMIT-1 DE `ACTO GEN2-R-SERIE-DBF`, ANTES DE
LEER UN SOLO REGISTRO DE MICRODATO. Spec sellada que lo gobierna:
`forense/prereg-caja/R-ENVIPE-SERIE-DBF-spec-v1_0.md`, sucesora por extension
de `forense/prereg-caja/R-ENVIPE-SERIE-spec-v1_0.md` (`prereg-caja-R-ENVIPE-SERIE`),
que NO se edita.

Este archivo es BYTE-IDENTICO en los tres `CALC-R-CIV-M-{01,02,04}`: todo lo
especifico de la ola (miembro del ZIP, id de payload, NOMBRES FISICOS DE
COLUMNA, codigos de BPCOD) entra por `contrato["parametros"]`. Que los tres
`script_blob_sha256` coincidan en los tres `sello.json` es la prueba de que las
tres olas corrieron el mismo codigo.

DIFERENCIA CON EL MEDIDOR DEL TRIO CSV, y por que existe este archivo:
  1. El insumo es DBF de ancho fijo, no CSV. El lector va abajo (`_lee_dbf`) y
     lee el DESCRIPTOR DEL PROPIO ARCHIVO (cabecera de 32 bytes + descriptores
     de campo), nunca el FD ni una plantilla de anchos.
  2. Los nombres fisicos de las columnas de DISENO cambian por ola: `EST`/`UPM`
     en 2012 y 2013, `EST_DIS`/`UPM_DIS` en 2015. El medidor NO los conoce: los
     recibe en `parametros.mapa_columnas` como vinculo ROL -> NOMBRE FISICO.
  3. `FAC_DEL` es Caracter(6) en 2012/2013 y Numerico(12) en 2015. Todo campo
     DBF se lee como TEXTO CRUDO y se convierte despues: asi el tipo declarado
     en la cabecera no cambia el resultado.
  4. El catalogo de `BPCOD` de 2012 tiene CATORCE codigos, no quince, y su
     particion hogar/personal esta CORRIDA UN LUGAR frente a 2013/2015. Los
     conjuntos entran por `parametros.codificacion` y este archivo no los
     deriva ni los adivina.
...
Este medidor no recibe el valor GEN1 por ninguna via.
"""
...
def medir(inputs, contrato):
    ...
    for s in ("N-FILAS-TABLA", "N-BP1-23-01-09", "N-BP1-23-99", "N-BP1-23-BLANCO",
        ...
        out[P + s] = 0
    for s in ("METODO-IC", "PERFIL-DISENO", "PERFIL-DBF", "PERFIL-BPCOD", "VEREDICTO-CV"):
    ...
    filas, faltan, perfil_dbf = _lee_dbf(zf.read(miembro), COLS)
    out[P + "PERFIL-DBF"] = perfil_dbf
