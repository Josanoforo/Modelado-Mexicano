...
| archivo | productor | contrato | quién lo lee | advertencia |
|---|---|---|---|---|
| `data/adq-config.yaml` | escrito por este acto (P6); antes de este acto las URLs, las claves física/lógica y los ids de referencia de manifiesto de la PDN, la zona horaria, la URL de la sonda de red, el runbook y el timeout de `claude -p` vivían cableados a mano dentro de `tools/adquiere_cron.sh` | YAML: `zona_horaria`, `sonda_red_url`, `claude_timeout_segundos`, `runbook`, `t_cron_gracia_minutos` (P5), `pdn.ventana_dia_inicio/fin` y `pdn.sistemas.<s1\|s2\|s3\|s6>.{url,sis_descarga,id_manifiesto}` | `tools/adquiere_cron.sh` (vía `tools/adq_config.py`, nunca YAML a mano desde bash) y `tools/adq_doctor.py` (P4, solo lectura) | No se auto-actualiza: cambiar un `id_manifiesto` cuando la PDN regenera un bulk sigue siendo una decisión de mesa, igual que cuando vivía cableado en el shell — solo cambió dónde vive, no quién decide. `tools/adq_config.py` no inventa defaults por clave ausente (`KeyError` explícito); el runner sí trae *sus propios* defaults de emergencia para los escalares no críticos (timeout, sonda, runbook) por si el archivo de config tiene un problema, para no perder la corrida del día por un YAML roto — los datos por-sistema de la PDN (URL/clave/id) no tienen ese colchón: si faltan, el paso aborta con `set -e` en vez de re-bajar con un id equivocado. |
...
## `data/fuentes-financieras-20/` — CNBV, CONDUSEF y ENCRIGE (`ACTO GEN2-CNBV-CONDUSEF-FUENTES-Y-SERIES`, 11/sep/2026)
...
| `tools/extrae_fuentes_financieras.py` · `tests/test_fuentes_financieras.py` | extractor determinista y pruebas de fixtures puras; `--verifica` reconstruye las seis salidas byte a byte en CAJA | resuelve raíces lógicas del manifiesto, verifica payloads, rechaza llaves duplicadas, conteos fraccionarios, porcentajes publicados discordantes y cambio de unidad IMOR | reproducción y CI (la prueba unitaria no necesita corpus ni red) | `xlrd` es dependencia explícita porque el original CNBV es XLS/OLE. Las pruebas no fijan totales del árbol; los contrastes de bytes reales ocurren en CAJA. |
