...
No crear un segundo motor divergente ni reescribir todas las reglas. Reutilizar el emisor y factorizar la selección/aptitud. Es importante distinguir `milpa/src/motor.py` (rebanada matricial de estados GEN2-E0) de `milpa/src/emisor.py` y `tools/emite_m.py` (emisión probabilística usada por el duelo).
