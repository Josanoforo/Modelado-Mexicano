...
`PENDIENTE-DE-MESA` al pie de `milpa/tramite-ola5-propuesta-v0.yaml`, `ADR-306`,
`FP-254`. **`ADR` renumerado dos veces, `304`→`305`→`306`, y `FP` de `253` a `254`**: `PR #492` (`ACTO MAESTRA35-N8 · SELLA-L7`) fusionó primero y se llevó el `304`; después `PR #490` (`ACTO MAESTRA35-L4 · R-v1_2-CIEGA`) fusionó y se llevó el `305` **y** el `FP-253` — la colisión que este acto había declarado por escrito antes de que ocurriera. Renumera quien fusiona segundo.
