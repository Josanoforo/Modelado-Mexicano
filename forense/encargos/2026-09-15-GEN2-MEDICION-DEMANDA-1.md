...
|---|---|---|---|
| «Correr en caja **todas** las corridas cuya spec el encargo 1 vaya congelando … tanda tras tanda» — las corridas de la **tanda 2** del encargo 1 (`CORR-0017`, `CORR-0016`, residuos de `CORR-0009` `RES-0031/0032/0065` y de `CORR-0007` `RES-0025/0026`) (`NC-0200`) | `DIFERIDO-A:ACTO GEN2-MEDICION-DEMANDA-2`. Sus contratos no existen: `ACTO GEN2-SPECS-DEMANDA-1` cerró con `PR #775` congelando sólo la tanda 1 (4 specs) y difirió la tanda 2 a una sesión futura (`NC-0193`). Este acto corrió **las 4 que existían** (`CORR-0011..0014`), que es lo que el lanzamiento pide de esta tanda | `N_corridas_selladas` se queda en **67** de 82 en vez de seguir subiendo; 4 `CORR` construibles sin bloqueo material siguen `SIN-CANDIDATO`. La firma de contador con OBJETO del lanzamiento cubre también esas corridas («todas las corridas cuya spec el encargo 1 vaya congelando») | `ACTO GEN2-MEDICION-DEMANDA-2` (CAJA), a despachar cuando la tanda 2 de `GEN2-SPECS-DEMANDA-1` esté fusionada |
| `CALC-ENUT-0001 · RESULT-ENUT-A-SHARE-POBLACIONAL-MUJERES-40MAS` — el ponderador del share lo eligió el medidor (`NC-0201`) | `DECISIÓN-DE-MESA-PENDIENTE`. Reserva, no omisión: la spec sellada (§2.2) dice «ponderada» sin nombrar el ponderador y declara `FAC_PER` «sólo para la guarda». El medidor congelado (`d13529e`) pondera el share con `FAC_PER` — ponderador de persona, el único que reproduce el 0.2657 de GEN1 (resultado 0.2657197) — y lo declara en su cabecera. `FAC_HOG` sigue siendo el único ponderador de la razón `A-R` | Ningún contador: `A-R`, su IC y la adopción no dependen del share. Si mesa quiere el share con `FAC_HOG`, la spec no se edita: sucesora v1.1 con archivo propio y `CALC-ENUT-0002` | MESA |
| «Firma de contador con OBJETO embebida desde el lanzamiento» — la vía de `decisiones.tsv` | **Hecho**, con reserva declarada: los cuatro `spec.yaml` del encargo 1 conservan `cuenta_gen2: PENDIENTE-DE-MESA` en su `COMMIT-1` (E.3, no se editan); la firma vive en las cuatro filas de `data/corrida0/decisiones.tsv`, que `_cuenta_gen2_resuelto` lee con precedencia 1. El merge de mesa la perfecciona | Ninguno: el registro ya cuenta las cuatro (`N_corridas_selladas` +4: 63 → 67 sobre la base de arranque, 66 → 70 sobre la base de cierre tras `PR #774`). Cierra `NC-0192` y `NC-0198` por producto | mesa (merge) |
| Adopción de los cuatro puntos `LISTADO-PARA-MESA*` a `milpa/` | `FUERA-DE-PERÍMETRO` (sellar no es adoptar, E.2; el encargo dice «no adopta ninguna cifra») | `N_resultados_gen2_adoptados_activos` 16 → 16; `usos.tsv` idéntico a `origin/main` | MESA (ventanilla de adopción) |

## CONSUMIDO

PR [#779](https://github.com/Josanoforo/Modelado-Mexicano/pull/779), abierto contra `main` el 15/sep/2026 y **no fusionado por el ejecutor** (el merge es de mesa). Cascada de cierre completa en `1e26c2d`: **ADR-511** en `canon/gobernanza-v1_15.md`, anotación L0 en `canon/estado-programa-v1_13.md`, los tres contadores mecánicos reconciliados por `tools/cierre_acto.py --aplica` (`gobernanza 510→511 · L0 510→511 · tabla estado 510→511`), rótulo `GEN2-MEDICION-DEMANDA-1` censado en `canon/registro-rotulos.tsv`, `NC-0192`/`NC-0198` CERRADAS y `NC-0200`/`NC-0201` nuevas en `forense/no-corrido.tsv`, nota de cierre en `forense/notas/2026-09-15-GEN2-MEDICION-DEMANDA-1-cierre.md`. Producto: `CALC-ENIGH-0001`, `CALC-ENFIH-0001`, `CALC-EDER-0003`, `CALC-ENUT-0001` sellados (`verify REPRODUCE · CONTEXTO=IDENTICO` 8/8), cuatro filas de firma en `data/corrida0/decisiones.tsv`, registro re-derivado sobre la base de cierre `62faadb` (`PR #774` fusionó a mitad del cierre: cuatro merges, conflicto resuelto en `decisiones.tsv`/`no-corrido.tsv` con la convención de la casa y re-derivación de los dos TSV derivados con `--lote` declarado para las 11 corridas ajenas blanqueadas a `NO-VERIFICADO`): `N_corridas_selladas` +4 (66 → 70), `N_resultados_gen2_sellados` +119 (3 085 → 3 204), cero adopciones. `python3 tests/check.py --baseline` **VERDE** (`3 FAIL · 3766 WARN`, nada nuevo frente a `tests/baseline.json`).

**Este encargo queda consumido para la tanda 1.** El lanzamiento cubre «todas las corridas cuya spec el encargo 1 vaya congelando»; la tanda 2 del encargo 1 no existe todavía (`NC-0193`) y sus corridas van a `ACTO GEN2-MEDICION-DEMANDA-2` (`NC-0200`), con la misma firma de contador con OBJETO.
