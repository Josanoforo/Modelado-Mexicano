...
- **Margen material propuesto:** `Δ_material = 0.5 · EE(R)` de la celda evaluada. Esta cifra NO se inventa: es la misma constante que `ADV1-M3` ya usa como una de las dos condiciones de `INDECIDIBLE` ("si |d_L−d_M| < 0.5·EE(R)"). Reutilizar la constante ya sellada en `ADV1-M3` evita introducir una segunda constante arbitraria sin relación con el resto del diseño — la banda TOST y la condición INDECIDIBLE de `ADV1-M3` miden, con la misma vara, "diferencia sin importancia práctica".
