...
P-f · Cierre. Cascada de /acto, check.py --baseline --parallel VERDE o PARO, ## NO-CORRIDO / RESERVAS

Ninguno.

Las seis piezas (P0, P-a, P-b, P-c, P-d, P-e) se ejecutaron y P-f es este cierre.
Dos cosas que el encargo previó y que **no** son filas de esta sección, porque
no son trabajo no corrido sino resultado declarado:
...
- **Los dos CALC del piloto** eran alcance declarado de `PR #944`, no deuda de
  este acto. `#944` fusionó durante este cierre y **el chequeo los verificó en
  el acto**: `FAIL=0`, con la cadena de `ADJUDICACION-0001` clasificada `WARN`
  —es lo que la lista de estados legítimos existe para hacer—. Es la primera
  validación del chequeo contra un arreglo real que nadie fabricó para él.

**Fila que existió y ya está CERRADA** (`NC-260921-GEN2-TUBERIA-PREFLIGHT-CI-1-9919-01`):
el criterio de «hecho» pedía `LÍNEA BASE VERDE` y durante buena parte del acto la
suite daba 2 FAIL de `T22` sobre archivos de `ACTO GEN2-SENAL-1`, verificados
**heredados, no causados** (la misma suite sobre `origin/main` `a61dd000` limpio,
en clon temporal, daba exactamente esos dos). Se asentaron con su dueño en vez de
tocar archivos ajenos para forzar el verde; `main` los cerró por su cuenta antes
de que este PR fusionara, y hoy la suite da **LÍNEA BASE VERDE** en la cabeza de
esta rama. La fila queda CERRADA **por el hecho**, no por trabajo de este acto.

## CONSUMIDO citando el número real de PR, verificado contra el HEAD remoto.
