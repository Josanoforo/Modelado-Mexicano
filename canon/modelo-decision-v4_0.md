...
| **49 reglas · 27 de perímetro · 18 de 27 corridas · 20 `[FUERTE]`**<!-- T20:HITO-D pob=reglas --> | 49/27/20 sin cambio respecto del v3.4, recuento mecánico sobre §3.B, reproducible con `python3 tests/validador_registro_ids.py`; corridas 2→3 por `R7.2` → `D`, archivado 4/ago/2026 (ADR-45); 3→4 por `R4.2` → `D`, archivado 4/ago/2026 (ADR-55, Encargo Y); 4→8 por `R4.1`/`R9.1`/`R4.3`(A,B)/`R9.2` → `D`, archivados 4/ago/2026 (ADR-56, Encargo Z); 8→11 por `R5.1`/`R5.2` → `A` y `R1.2` → `E`, archivados 4/ago/2026 (ADR-58); 11→12 por `R3.1` → `B`, archivado 4/ago/2026 (ADR-60) — tier `[FUERTE]` de `R3.1` sin tocar, 20 `[FUERTE]` sin cambio; 12→13 por `R1.3` → `E`, archivado 5/ago/2026 (ADR-63, Encargo CU); 13→14 por `R8.1` → `D`, archivado 20/ago/2026 (ADR-138); 14→15 por `R7.1` → `A`, archivado 24/ago/2026 (ADR-145); 15→16 por `R7.3` → `C`, archivado 24/ago/2026 (ADR-155, `ACTO SELLA-AGO24-C-v2`, firma de mesa `FP-112`, primera fila `C` del programa); 16→18 por `R7.4`/`R7.5` → `D`, archivados 24/ago/2026 (ADR-158, `ACTO ADQ-CORRE-R74R75`, falsador compartido inejecutable por diseño de instrumento en las tres fuentes reales adquiridas para probarlo) — tiers `[MEDIA-FUERTE]` de ambas sin tocar | **Heredado, con receta ejecutable — corridas actualizado 24/ago/2026 (`ACTO ADQ-CORRE-R74R75`)** |
