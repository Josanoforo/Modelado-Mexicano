...
- `requirements.txt` no declara `numpy`/`pandas`: `FUERA-DE-PERÍMETRO` (`tools/corrida0.py` es de otro dueño, §9). Fila `NC-260924-GEN2-FRONT-2-d095-01`. Impacto: `verify` sin esos paquetes da `NO-EJECUTABLE` por import antes de mostrar limpiamente la ausencia de corpus (ya corregido en `docs/verificar.md`). Sucesor: quien mantenga esos archivos.
