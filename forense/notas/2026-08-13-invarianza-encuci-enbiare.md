...
**Qué NO es.** No rediseña el acto (`ADR-76(d)` ya lo selló). No sella `ADR-80` en `canon/gobernanza-v1_15.md` (fuera de perímetro de este acto — el borrador vive en la conversación de dirección de esta sesión, listo para que un acto con `canon/` en perímetro lo incorpore). No toca `AP5_1_3` (declarado fuera de alcance, per Propuesta 2 punto 2). No dicotomiza para el test mismo (usa 0-10 ordinal completo, per Propuesta 2 punto 3 — la dicotomización ≥8/10 es solo para comparación externa, si la hubiera).

**Estándar de éxito, sellado en esta conversación (`ADR-80`, borrador — ver `forense/hallazgos.md` de este mismo acto):** argumento de vinculación declarado (anclas de diseño OCDE 2017 + invarianza parcial hasta donde el par de dos ítems identifique + juicio experto rotulado como tal) — no invarianza clásica completa, que `ADR-76(d)` ya calificó inalcanzable hoy (sin muestra puente, sin ítems ancla verificados estadísticamente, ENCUCI 2020/ENBIARE 2021 sin panel compartido).

**Nota de colisión de numeración, corregida antes de push (no reescribe nada ya compartido).** El borrador de esta sección citaba originalmente `ADR-79` para el estándar de éxito. Al fusionar `origin/main` tras Commit 2 (PR #221/`inv-descmx` había aterrizado mientras este acto corría), `ACTO SELLA-3` había sellado un `ADR-79` real y distinto — ocho de nueve decisiones de mesa no relacionadas (`D-A`..`D-H`, `forense/notas/2026-08-13-sella-3.md`), ninguna de las cuales fija el estándar de éxito de este acto (su inciso `D-B` solo autoriza correr el diseño ya sellado por `ADR-76(d)`, no adjudica la sub-pregunta del punto 4). Receta T15 re-derivada contra el `origin/main` fusionado: única y máxima 79, sin huecos → **`ADR-80`** es el número correcto, aplicado aquí antes de cualquier push. Ninguno de estos commits había sido compartido cuando se corrigió.
...
Bajo el estándar sellado en esta conversación (`ADR-80`, argumento de vinculación declarado), **NO RECHAZAR** significa:
...
`INVARIANZA PARCIAL` cae en `NO RECHAZAR` (COMMIT 1 §4), el desenlace declarado de antemano como más probable. Bajo `ADR-80` (argumento de vinculación declarado):
