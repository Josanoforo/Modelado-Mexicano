"""Regresión del fallo real 2022T2: BOM y T_LOC_TRI."""
import csv
import io
import tempfile
import unittest
...
from tools.dominios.enoe.pisos_v1_1 import medir_ola


class BomYLocalidad(unittest.TestCase):
    def test_encabezado_y_localidad(self):
        with tempfile.TemporaryDirectory() as d:
            path = Path(d) / "enoen.zip"
            buf = io.StringIO()
            cols = ["r_def", "c_res", "eda", "sex", "fac_tri", "est_d_tri",
                    "upm", "ent", "t_loc_tri", "niv_ins", "clase1", "clase2",
            ...
            filas, base, _ = medir_ola(path, "2022T2", "enoen", 40, 42)
