...
| **`gobernanza`** | `gobernanza-v1.15.md` | 55 ADR, protocolo de cambio |
...
**L5 · Validación — el estrato más problemático.** **Hito D** (reglas del motor; perímetro de 27, subconjunto de las 49 del motor): **4 de 27 corridas archivadas** — `R1.1` → veredicto `D`, `R3.2` → veredicto `B`, `R7.2` → veredicto `D`, `R5.1` → veredicto `A` (bloque append-only de `hitoD-preregistro`, ver §7). 49 refutaciones narrativas corridas *(población distinta, `milpa/refutations.yaml`, ref.A.*)*: 27 pasan, 3 fallan, 8 sin objeto, 11 requieren el ejecutable. **15 coeficientes, cero medidos.** Cuatro generadores sin falsar (**Hito C**, distinto de lo anterior — ver `hitoC-prueba-generadores`).
...
- **45 de 49 reglas del motor sin prueba de falsación pre-registrada corrida** (4 corridas archivadas: `R1.1` → `D`, `R3.2` → `B`, `R7.2` → `D`, `R5.1` → `A`). **Dentro del perímetro decidido del Hito D (27 reglas, subconjunto de las 49): 23 de 27 sin corrida.**
...
**Paso 2 — EN CURSO. 4 de 27 corridas.**
...
**`R5.1` → veredicto `A` (refutada).** `[FUERTE]`, `familia.seguro.volatilidad_ausencia_estado` — la **única** regla del motor que declara `PORQUE G5` (`modelo:467`). Adjudicada por mesa #19, 4/ago/2026 (ADR-55), sobre las 6 olas de ENIGH 2012-2022, hogares con adulto mayor, beneficiario vs. no beneficiario de la Pensión del Bienestar. **Refutada por INVERSIÓN DE SIGNO, no por reducción pequeña:** la fila `A` está redactada *"<10% de reducción"* y lo medido es **reducción ≤0 — o sea, aumento**. Ni la corresidencia intergeneracional ni la transferencia intrafamiliar hacia mayores bajan en ninguna de las 6 olas ni en ningún tercil de ingreso post-reforma; en la transferencia la brecha es positiva y **significativa** en 2016-2022, y en corresidencia es positiva y significativa en el tercil **alto** (+9.1 a +9.8pp). La familia no se retira cuando el Estado entra: si acaso, se mueve en el mismo sentido.
- **`B` se descarta primero por ausencia de dosis-respuesta**, y solo después por la magnitud del monto. El monto de la pensión pasa de **32.8%** del gasto per cápita (2018, mínimo) a **63.4%** (2020, máximo) y el desenlace no se mueve — sin reducción en ninguna de las dos olas, en ninguna de las dos medidas; ausente en todo el rango de las seis. *Limitación declarada:* es comparación entre olas, confundida con una década de otros cambios — es evidencia, no un test limpio de dosis. Pero es evidencia derivada del dato, y un umbral de "suficiencia" elegido después de ver el resultado no lo es (el pre-registro nunca define "monto suficiente"). El monto documentado, subordinado: 60-63% del gasto per cápita post-reforma.
- **`C` no se invoca:** describe un panel que ENIGH no tiene y que ninguna fuente del corpus tiene para este choque; el Umbral (`hitoD-preregistro:143`) pre-registró literalmente la comparación **transversal** que este acto corrió. **El precedente de ADR-49 (CAL-G3) no aplica:** ahí falló la identificación, aquí el instrumento respondió.
- **Cuatro reservas, no opcionales:** transversal repetido, no panel; comparabilidad controlada solo por tercil de ingreso per cápita (sin pareo ni matching); `P040` no distingue donante familiar de no familiar; cobertura del programa no es 100% en ninguna ola. Acotan la fuerza causal, no la dirección.
- **Lo que este veredicto NO hace:** el tier `[FUERTE]` **no se toca** (un tier entra por el mapa de evidencia de un report al glosario, ADR-02 — acto aparte); **G5 no se apaga** (`familismo_apoyo` 0.50 sigue ASIGNADO — la corrida refuta la sustitución, no el canal familiar); **ningún report del corpus se reescribe** en este acto. Detalle: `forense/notas/2026-08-04-hitoD-r5-1-pension-bienestar.md`, narrado en Nota 16 del pre-registro.

⚠️ **Lectura peligrosa que este veredicto NO autoriza:** *"la pensión no desplaza el apoyo familiar"* **no** dice que la transferencia sea redundante ni que "se pueda recortar porque la familia cubre". El dato no mide bienestar ni suficiencia, y deja abiertas dos explicaciones incompatibles —selección y complementariedad— que este acto no distingue.

**Siguiente en el orden, por valor informativo y no por facilidad:** ~~R5.1 (Pensión del Bienestar como choque exógeno)~~ — **corrida y archivada 4/ago/2026.** Siguiente: `R5.2` (ENUT) o `R4.2` (ENSANUT) — ambas separan un driver **(b)** de uno **(a)**.

**HITO F · MILPA Fase 1 — POSPUESTO por decisión.**

---

**Suite de verificación (`tests/check.py`) — última corrida verificada, 4/ago/2026.** La suite corre completa: **18 FAIL · 84 WARN** *(sin cambio neto tras sellar D-09/D-10 con ADR-47 y subir `gobernanza` a v1.15 (ADR-48) esta misma sesión: las citas viejas a `gobernanza-v1.14.md` y al conteo de ADR anterior en este mismo archivo se corrigieron en el mismo commit que sube la versión, así que T03/T15 no llegan a disparar. Trayectoria de WARN: 107 (auditoría del 29/jul, antes de `TRANSFER-maestra-7/8.md`) → 89 (`26e7ef4` normaliza T03) → 86 (esta sesión declara 3 nombres en `HISTORICOS`) → 83 (esta sesión corrige `gobernanza §2` para dejar de citar en backticks los nombres viejos que corrige) → 84 (sesión de la subida a v2.2: `gobernanza` se renombra a v1_10, una cita fechada a `gobernanza-v1_9.md` pasa a colgante) → 82 (Cambio 6 de esa misma sesión añade `renombr` al regex de disparo de T03) → 84 (sesión de cierre del gate T17, 30/jul: aterrizar `TRANSFER-maestra-9.md` sube T03 en 2 — cita verbatim, a modo de ejemplo, el mismo falso positivo `-v3.2.md`/`-v3_2.md` ya documentado arriba) → 87 (refreeze tras el merge del PR #1, 30/jul: `revision-publicacion-2026-07-30.md` cita 3 veces, como propuesta descartada por D-05, el artefacto LICENSE-CORPUS que no se creará — patrón I-01, bucket propio en el baseline) → 84 (30/jul, ADR-48 · R0: la marca `{cita-ilustrativa}` cierra I-01 y esas mismas 3 citas dejan de disparar T03 — 21 → 18); `censo-integridad-v1_0.md` C1-06)*. **FAIL subió de 18 a 19 (29/jul):** T17 se amplió para escanear también el cuerpo vigente del propio `hitoD-preregistro` (antes solo leía `estado`), y con eso capturó la autodeclaración falsa de `hitoD-preregistro:8` ("contiene 27 fichas", contra 25 reales) — deuda congelada por decisión de mesa en `tests/baseline.json`, pendiente de un ADR futuro que decida si la cabecera append-only de un pre-registro está cubierta por esa protección. **FAIL vuelve a 18 (4/ago/2026, Encargo O):** las fichas de `R3.1` y `R3.4` (Notas 14 y 15 de `hitoD-preregistro`) llevan el conteo real de encabezados a 27 — la misma cifra que la cabecera del pre-registro ya declaraba, así que la autodeclaración deja de ser falsa y T17 pasa. No es I-03/ADR pendiente resuelto por decreto: es la ficha que faltaba, escrita. Según la nota del 29/jul, el desglose por test se reprodujo de forma independiente en Windows 11/Python 3.14 contra la corrida original en contenedor Linux. `CONTRIBUTING.md` tenía un byte UTF-8 truncado (pos 2149) que rompía T03 con `UnicodeDecodeError` antes del final de la corrida; corregido en `09bfb05`.
