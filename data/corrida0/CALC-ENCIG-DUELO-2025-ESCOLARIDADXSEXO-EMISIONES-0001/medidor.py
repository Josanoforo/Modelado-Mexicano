...
_GUARDIAS_DE_ENTRADA = {"emisiones": {"_guardia_emisiones", "_guardia_firma"},
                        "adjudicacion": {"auditoria", "_guardia_sellos", "_guardia_payload"}}
...
def adjudicar(res, emis, marg25, estado_gana, resumen, par, diag):
    ...
    out[f"{PA}-G-MARCA-EMISIONES"] = "PROSPECTIVA -- las cinco emisiones se sellaron antes de R; el IC de C2 NACE-CON-R"
...
def adjudicacion(inputs, contrato):
    ...
    out = adjudicar(res, emis, marg25, estado_gana, resumen, par, diag)
    out[f"{PA}-G-AUDITORIA-CODIGO"] = "LIMPIA"
    out[f"{PA}-G-SELLOS-EMISIONES"] = "COINCIDEN -- " + ",".join(sorted(CRUCES))
...
def esquema_adjudicacion():
    ...
    for cruce in CRUCES:
        ...
        for ka, kb in celdas(cruce):
            b = f"{pc}-{rotulo(ka, kb)}"
            f += [_fila(f"{b}-{s}", t, u) for s, t, u in (
                ...
                ("C2-P", "proporcion", "proporción de trámites [0,1] (punto sellado adoptable)"),
