...
> | **VERIFICAS ASÍ** | ADR-36 tiene **adenda (c)** sobre series numeradas · §2 lista los tres `milpa-*` · §4 (registro del perímetro del Hito D) trae la corrección de RÓTULO fechada 29/jul — el perímetro sigue en **27** · §4 trae ADR-44 (publicación del repositorio, sin registro previo), ADR-45 (vocabulario de "prueba de falsación", D-05), ADR-46 (unidad de contaminación es la SESIÓN, no la máquina — corrige `cola.yaml` E-03) y ADR-47 (falsar una regla ≠ calibrar un coeficiente — cierra `cola.yaml` D-09/D-10) |
...
**ADR-47 · Falsar una regla no es calibrar un coeficiente — son dos tareas distintas, y el perímetro del Hito D las mezcla sin marca.** Decisión de mesa del autor, 30/jul/2026, sobre la evidencia registrada en `cola.yaml` D-09 y D-10 (chequeo `CAL-X` de `CAL-G3`, `hitoD-preregistro-v2_0.md` Nota 8, y la aritmética de poder sobre el panel completo — `tests/calx_g3.py`, extensión narrada en `forense/notas/2026-07-30-calx-g3-salida.txt`).
...
**Lo que este ADR NO hace.** No reclasifica ninguna de las otras 27 fichas del perímetro del Hito D como falsación o calibración — eso es trabajo aparte, abierto sin resolver en `cola.yaml` (ver la entrada nueva de esta misma sesión). No estima la elasticidad de `G3 → horizonte_temporal`: la Fase C sigue sin correrse en esta sesión. No toca `milpa/procedencia.yaml`: `-0.60` sigue `ASIGNADO` hasta que exista una estimación archivada.

**Requisito de salida.** `hitoD-preregistro-v2_0.md` recibe nota fechada (Nota 9) registrando ambas decisiones y este número de ADR, declarando la Fase C de `CAL-G3` **desbloqueada con alcance reducido** (olas 2-3, entregable descriptivo) y **no corrida en esta sesión**. `cola.yaml` D-09 y D-10 cierran citando este ADR, sin editar su texto original — mismo molde que `D-06`. Ninguna ficha futura del Hito D que combine pregunta de existencia con pregunta de magnitud puede pre-registrarse sin declarar primero cuál de las dos es, siguiendo el distingo de este ADR.
→ **Vigente. S2.** *(Aprobado 30/jul/2026, decisiones de mesa del autor sobre `cola.yaml` D-09/D-10 y la evidencia de `hitoD-preregistro-v2_0.md` Nota 8 y `forense/notas/2026-07-30-calx-g3-salida.txt`. No re-corre ningún cálculo: verifica los números citados contra archivo antes de escribir. Cierra `cola.yaml` D-09 y D-10 con nota de cierre. Abre en `cola.yaml`, sin resolver, la revisión de las 27 fichas del perímetro por tipo de tarea.)*
