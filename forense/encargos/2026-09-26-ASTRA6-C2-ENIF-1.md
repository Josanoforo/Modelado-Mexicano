# ENCARGO · ACTO ASTRA6-C2-ENIF-1 · dos familias ENIF con emisiones congeladas y prueba futura preparada
...
Entregar las familias ENIF-AHORRO-FORMAL, ENIF-HORIZONTE-AHORRO como paquetes ejecutables, con emisiones históricas congeladas y condiciones de activación verificables. Hoy se preparan dos familias para una ola objetivo; no dos aperturas independientes.

«Hecho»: `python3 tools/familias-2027/enif/cierre.py --verifica` (verificador propio a entregar) comprueba specs/código/hashes, pruebas de guardias y fronteras, emisión sin R futura, oro histórico y evidencia separada de reproducción, cobertura de las dos familias, calendario con estado y escenarios de potencia. Una familia inviable tiene dictamen fundado y propuesta concreta, no emisión inventada. La atestación externa se acredita solo con comprobante; su dependencia se declara si sigue pendiente.
...
[LEÍDO] `forense/prereg-caja/FAMILIA-2027-ENIF-HORIZONTE-AHORRO-spec-v1_2.md` L19: el primario usa diferencias respecto a un punto fijo, no una diferencia entre dos parámetros poblacionales con incertidumbre histórica propagada. Respetar el objeto y distinguirlo de un análisis de sensibilidad temporal.
...
ENIF-HORIZONTE-AHORRO presenta un problema material que hay que resolver antes de emitir: la spec v1.2 describe aritmética de marginales y reconoce que no equivale automáticamente al conteo individual futuro. Demostrar desde definiciones y universos si la identidad representa la intersección. Si los términos fueran «solo formal» y «solo informal», no asumir que la fórmula de unión aplica a conjuntos disjuntos. No corregir el piso sellado ni atribuirle otra interpretación. Si no hay equivalencia, dictaminar NO-COMPARABLE para esa propuesta y llevar una familia sucesora a firma; completar AHORRO-FORMAL. Cotejar también NS/NR, blancos y la lista completa de reactivos. No llamar «horizonte temporal» a una intersección de vías solo por el nombre histórico.

Para incertidumbre histórica autorizada obtener réplicas conjuntas desde la ola abierta en un CALC auxiliar nuevo, con spec previa; no inferir covarianza de intervalos marginales. El oro del acto reproduce lo autorizado sin sustituir el histórico.
...
Sellar emisiones por familia con ids propios `CALC-FAMILIA-2027-ENIF-*`, sin R futura. Verificar su reproducción y, por separado, la del oro histórico. En esta primera tanda la recomendación de Astra es **cero retadores externos**: el resultado buscado es una prueba de las emisiones base. Si se identifica una hipótesis estructural distinta con valor claro, documentar propuesta bajo el máximo de uno por familia; no abrir una selección retrospectiva de candidatos ni hacerla dependencia del piso.
...
Propio: versiones nuevas de las dos specs `FAMILIA-2027-ENIF-*`; `tools/familias-2027/enif/`; CALC y auxiliares nuevos del instrumento bajo `data/corrida0/`; análisis `forense/analisis/familias-2027/astra6-enif/`; pruebas propias, asientos reales de replay y cascada de cierre. No cambiar la hoja global de familias ni los archivos de otros instrumentos: entregar hoja local para integración posterior.
...
| Pieza | Razón | Impacto | Sucesor |
|---|---|---|---|
| P5 · Atestación externa | DIFERIDO-A:SIN-ASIGNAR: siguiente circuito de sellos de mesa; no hay comprobante de envío ni OTS. | Cero familias atestiguadas externamente; conservar estado SELLADO-INTERNAMENTE. | SIN-ASIGNAR; circuito de mesa manifiesto de sellos y OTS |
| Cierre · recibo técnico de Claude | DIFERIDO-A:SIN-ASIGNAR: revisión real GEN2-RECIBO-ASTRA-PRODUCTO-N solicitada en PR. | Entrega no aceptada ni fusionada hasta recibo real. | SIN-ASIGNAR; GEN2-RECIBO-ASTRA-PRODUCTO-N por mesa |
| P4/P5 · activación y fecha ENIF futura | DIFERIDO-A:SIN-ASIGNAR: fecha oficial no confirmada; adquisición futura y cotejo antes de apertura compartida. | Una ola condicional; ninguna evaluación futura ni fecha inventada. | SIN-ASIGNAR; adquisición/COMMIT-3 ENIF futuro por mesa |

## CONSUMIDO

PR #1174 · codex/astra6-c2-enif-1 · ejecución histórica/interna completa; dos familias para una ola, cero R futura y cero atestaciones externas. Recibo técnico de Claude pendiente. No fusionado. ADENDA-1 propia autoriza sólo sucesora de serialización; cuerpo y sello originales conservados.
