...
Sucede a `CALC-C0D-MARCADOR`, que corrio contra la spec `v1.0` y PARO por su
propia guardia (`NO-ADJUDICA-POR-CONTROL`). UNA diferencia y ninguna mas: el
punto `L` sale de `L-extraido-v1_2.tsv` y no de `valor_extraido`, que es
`null` en las 224 capturas por diseno. Los bytes de la corrida anterior
quedan INTACTOS. Spec sellada: `C0D-MARCADOR-spec-v1_1.md`.
...
`forense/prereg-caja/C0D-MARCADOR-spec-v1_1.md` (`prereg-caja-C0D-MARCADOR`).
