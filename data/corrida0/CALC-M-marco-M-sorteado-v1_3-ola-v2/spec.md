# `CALC-M-marco-M-sorteado-v1_3-ola-v2` — modulación por ola, corregida

> **Sucesora de `CALC-M-marco-M-sorteado-v1_3-ola`** (`repite_de`), que queda
> `SUPERADA` y **no se reescribe**: una corrida sellada es evidencia
> histórica de lo que se corrió, incluida la regla que se corrió. La
> predecesora aplicó «ola más cercana distinta a la del árbitro»; la
> ADENDA DE MESA del 8/sep/2026 la derogó por fuga temporal. Que las dos
> convivan es el punto — así se ve qué regla produjo qué número.
>
> **Los seis puntos `p` no cambian.** Sobre las 6 celdas que hoy modulan, las
> dos reglas coinciden: las seis ya tenían una ola anterior. Lo que cambia es
> el rótulo (`SERIE-LOO` → `SERIE-PREVIA`), el guard `SIN-PREVIA` y el
> rótulo `ORIGEN-ARBITRO`. La regla se instala por lo que impide mañana.
