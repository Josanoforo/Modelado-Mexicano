...
## 12 · Firma de mesa — `L8`/`FP-132`, 25/ago/2026 (`ACTO SELLA-AGO25-F`)

**Firma, verbatim:** *"¿Qué necesidad/θ reclama a Compartamos? (el acto dejó el mapeo candidato) → el que el acto propuso."* El único candidato que `ACTO EVAL-COMPARTAMOS-LLAVE3` dejó nombrado —sin afirmarlo— es la observación de su §8 (reproducida en §10 de este registro, alta de `EXP-COMPARTAMOS-1`): los reactivos `Q15_2_mean_formal` ("Trust in institutions index") y `Q15_2_mean_people` ("Trust in people index"), medidos en el brazo de seguimiento del experimento, se parecen por nombre a `confianza_institucional` (`G1`) y `radio_confianza` (`G1`/`G5`) del modelo. Esa nota explícitamente **no afirmaba** identidad de constructo — la decisión de mesa aquí es tomar ese mismo candidato **por instrucción directa**, no por el juicio del acto: mesa asume el riesgo que el acto se negó a asumir solo, que es exactamente la clase de decisión que `ADR-57(c)` reserva a mesa.

**Qué queda escrito, y qué falta.** `θ = confianza_institucional (G1) / radio_confianza (G1, G5)` queda registrado aquí como el objeto que reclama la evidencia de `EXP-COMPARTAMOS-1`, citando esta firma. **Esto no ejerce la llave**: la columna `preregistro_ref` de la fila `EXP-COMPARTAMOS-1` (§3) sigue diciendo `NINGUNO` — verificar que `Q15_2_mean_formal`/`Q15_2_mean_people` midan de verdad el mismo constructo (abrir el reactivo, verificar escala y universo) y escribir la spec B-bis que declare el diseño son trabajo de un acto propio, no de esta firma. `FP-132` cierra como decisión de mesa (`FIRMADA`), pero la llave sigue `SELLADA_NO_EJERCIDA` hasta que ese acto corra.

**`data/curacion-registro/necesidad-objeto-modelo.tsv` — no se edita en esta firma.** El encargo pedía escribir el mapeo también ahí. Verificado: sus 37 filas no traen hoy una fila para `confianza_institucional` (`G1`) ni `radio_confianza` (`G1`/`G5`) a la que asignarle esta fuente — crear una fila nueva es un acto de curación de datos con su propio proceso (`data/curacion-registro/`), no una anotación de registro. Escribirla aquí sin ese proceso fabricaría una fila del curador por fuera de su propio mecanismo. Se declara pendiente, no se fabrica.
