...
> **v4.0 — 3/ago/2026. MAYOR: la unidad de población deja de ser el perfil. §1.1 pasa de tabla a diseño de síntesis; los seis perfiles pasan de BASES a DESCRIPTORES.** *(ADR-51 · `revision-programa-2026-07-31.md` §2 · P1 `forense/notas/2026-07-31-p1-enigh-semilla.md` · P2 `forense/notas/2026-08-01-p2-momentos-atributos.md`.)* El motor sigue en **49 reglas**; el perímetro del Hito D sigue en **27**; los siete generadores de §2 y sus quince coeficientes **no cambian de contenido**. Lo que cambia es **sobre qué se define un agente**: ya no pertenece a uno de seis perfiles con quince valores puntuales, sino que porta un **vector de atributos observables** y los parámetros del modelo son **distribuciones condicionales sobre ese vector**. Las 10 reglas que citaban número de perfil citan ahora el atributo que ya codificaban, **sin cambio predictivo** (§1.6). El contador nuevo del programa es **"condicionales medidas sobre atributos: ~~11~~ 12 de 15"** *(Numerador 9 → 10 el 13/ago/2026, `PROC-10-bis` ejecutando ADR-79(a): sella la séptima clase de procedencia, `MEDIDO·NACIONAL` — condicional medida sin eje condicionante, `x = ∅` — en `milpa/procedencia.yaml` y le entra la primera θ, `norma_de_género` (ENASIC 2022 `P7_12_7`); ver §1.1.F Paso 5. Numerador 10 → 11 el 13/ago/2026, `ACTO PROD-P638`: reproduce `obligación_medida` (la segunda marginal nacional de la misma familia, ENASIC 2022 `P6_38`) por el motor formal (`prepare→produce→integrate`, `hashes_analista_confiados: false`) contra las cinco proporciones e IC de `forense/notas/2026-08-13-enasic-split-verificacion.md` §2.2, y le entra a `milpa/procedencia.yaml`. Numerador 11 → 12 el 18/ago/2026, `ACTO COND-ATRIB`: `condicionales_escalares_confianza_generica.confianza_institucional_generico_servidores_publicos` pasa de `PENDIENTE` a `MEDIDO·PARCIAL(edad,formalidad,sexo,escolaridad,estrato)` (ENCIG 2023 `P11_1_23`, cinco ejes, `forense/notas/2026-08-18-cond-atrib-confianza-generico.md`). Denominador 14 → 15 el 13/ago/2026, `ACTO PROC-11` ejecutando ADR-75(b): entra `obligación_medida` como condicional nueva, ver §1.1.F Paso 6. Antes, corregido 4/ago/2026, Encargo K — `forense/notas/2026-08-04-medicion-exposicion-violencia-envipe.md` §3-5: mide `exposicion_violencia` (ENVIPE 2025 `TPer_Vic2`). Esa línea no se había propagado en el mismo acto que corrigió §1.1.F/§6.1/§7 — cerrado en Acto 1, 4/ago/2026.)*, con `D`=15 derivado en §1.1.F.
>
> | # | Cambio | Origen |
> |---|---|---|
...
> | 38 | **Corte de `edad` para «joven»: convención declarada, 15-29 años (INEGI/ENOE), propagada a los nueve sitios que lo citaban `PENDIENTE`** — descriptor 5, H-02, H-06, H-07 (§1.1.D/§1.1.E), `R1.4`/`R2.4`/`R5.4` en §1.6 y sus dos reglas operativas en §3.2/§3.5. No re-adjudica ninguna hipótesis ni regla — cambia estatus de determinabilidad, no veredicto. Definición y procedencia citada en §1.1.A; declarado explícitamente como convención, no derivación empírica, revisable por `CORTE-EDAD-EMPIRICO` | **`ADR-110(b)` (mesa) · `ADR-116` · `ACTO CORTE-EDAD-CONVENCION`, 19/ago/2026** |
>
...
**Estado de falsación de las REGLAS del motor (49 total; perímetro pre-registrado del Hito D = 27, subconjunto de las 49):** vocabulario `RX.Y → veredicto A-E`, fuente única el bloque append-only de `hitoD-preregistro §Registro de veredictos archivados`. **26 de 27 corridas archivadas**<!-- T20:HITO-D pob=reglas --> — `R1.1` → veredicto `D`, `R3.2` → veredicto `B`, `R7.2` → veredicto `D`, `R4.2` → veredicto `D`, `R4.1` → veredicto `D`, `R9.1` → veredicto `D`, `R4.3` → veredicto `D` (mitad A y mitad B), `R9.2` → veredicto `D`, `R5.1` → veredicto `A`, `R5.2` → veredicto `A`, `R1.2` → veredicto `E`, `R3.1` → veredicto `B`, `R1.3` → veredicto `E`, `R8.1` → veredicto `D`, `R7.1` → veredicto `A`, `R7.3` → veredicto `C`, `R7.4` → veredicto `D`, `R7.5` → veredicto `D`, `R8.3` → veredicto `A`, `R1.4` → veredicto `D`, `R10.2` → veredicto `D`, `R8.2` → veredicto `B`, `R2.2` → veredicto `D`, `R3.4` → veredicto `B`, `R10.1` → veredicto `C`, `R2.1` → veredicto `D` (§7). Un veredicto `D` es una corrida igual que cualquier otra: cuenta en el conteo, no se descarta por ser inejecutable. *(Corregido de nuevo 25/ago/2026, `ACTO PROPAGA-LETRAS`, `ADR-196` — archiva `R10.2` → `D` por firma de mesa «R10.2: a», cruce `#363` confirma `SIN-DATO-GENUINO`; `R2.2` gana enmienda de censo sin archivar; `R8.2` sin archivar por RANURA ausente; 20 de 27 → 21 de 27.)* *(Corregido de nuevo 26/ago/2026, `ACTO CIERRA-4-FIRMAS`, `ADR-199` — archiva `R8.2` → `B` por firma de mesa L1 y `R2.2` → `D` por firma de mesa L2, tras correr `forense/R22-censo-liderazgo-v1_0.md` (veredicto `SIN-DATO`); 21 de 27 → 23 de 27. `R2.2` queda `[MEDIA-FUERTE]`, el ataque contra la lectura fuerte nunca corrió. Corregido de nuevo 26/ago/2026, `ACTO CIERRA-FP157`, `ADR-201` — archiva `R3.4` → `B` por firma de mesa, verbatim «FIRMO FP-157: gate R3.4 = fila B; Respaldo 2 corregido por enmienda fechada.»; 23 de 27 → 24 de 27, cifra derivada de `python3 tests/check.py` T18/T20, no tecleada.)* *(Corregido 4/ago/2026 — propaga `R7.2` → `D`, archivado en `hitoD-preregistro`, ADR-45; propaga `R4.2` → `D`, ADR-55, Encargo Y. Corregido de nuevo 4/ago/2026 — propaga `R4.1`/`R9.1`/`R4.3`(A,B)/`R9.2` → `D`, ADR-56, Encargo Z. Corregido de nuevo 4/ago/2026, Acto 1 — adjudica `R5.1`/`R5.2` → `A` y, MESA-R12=SÍ, `R1.2` → `E` (excepción acotada), ADR-58; 8 de 27 → 11 de 27. Corregido de nuevo 4/ago/2026, ENCARGO M-1 — adjudica `R3.1` → `B`, ADR-60; 11 de 27 → 12 de 27. El tier `[FUERTE]` de `R3.1` en `§3.B`/`§7` no se toca — ADR-60(b). Corregido de nuevo 5/ago/2026, ADR-63, Encargo CU — adjudica `R1.3` → `E`; 12 de 27 → 14 de 27. Este sitio no vivía en el perímetro acotado de ningún ADR anterior a Encargo CU (no aparece en las notas de deuda de `gobernanza:786` ni `:647`, que solo nombran `gobernanza:358,810`) — hallazgo propio de este acto, no debt declarada previamente. Derivado de `python3 tests/check.py` T18, no tecleado. Corregido de nuevo 20/ago/2026, `ACTO RETRIAGE-4` — archiva `R8.1` → `D`, ADR-138/ADR-143; 13 de 27 → 14 de 27. Corregido de nuevo 24/ago/2026, `ACTO SELLA-AGO24` — adjudica `R7.1` → `A` por firma de mesa D1 (verbatim "D1- A"), ADR-145; 14 de 27 → 15 de 27. Derivado de `python3 tests/check.py` T18, no tecleado. Corregido de nuevo 24/ago/2026, `ACTO SELLA-AGO24-C-v2` — adjudica `R7.3` → `C`, ADR-155; 15 de 27 → 16 de 27. Corregido de nuevo 24/ago/2026, `ACTO ADQ-CORRE-R74R75` — archiva `R7.4`/`R7.5` → `D`, falsador compartido inejecutable por diseño de instrumento, `ADR-158`; 16 de 27 → 18 de 27. Derivado de `python3 tests/check.py` T18, no tecleado. Corregido de nuevo 25/ago/2026, `ACTO SELLA-ABRIDORES-R83-R14` — archiva `R8.3` → `A` por firma de mesa RANURA 1, ADR-186; 18 de 27 → 19 de 27. Corregido de nuevo 25/ago/2026, mismo acto — archiva `R1.4` → `D` por firma de mesa RANURA 2, ADR-187, resolviendo la colisión de `ADR-182`; 19 de 27 → 20 de 27. Derivado de `python3 tests/check.py` T18/T20, no tecleado.)* *(Corregido de nuevo 26/ago/2026, `ACTO CORRE-R10.1-v2` Fase B, `ADR-205` — archiva `R10.1` → veredicto `C` bajo la RANURA 2 de mesa; rama 4 de la spec sucesora (los dos IC cruzan 15 pp) con ancla universitaria/elicitada, y gate de `κ` aprobado (Nivel 1 = 0.7209) entre dos codificadores; 24 de 27 → 25 de 27.)*
...
| 13 | **Siete variables nuevas declaradas** (esfuerzo/horas, salud mental, emprendimiento, canal de compra, colorismo, religiosidad, ítem actitudinal de deferencia a la autoridad) — dan objeto a siete de las ocho refutaciones sin objeto; ninguna calibrada. Octava (`prestamista`) queda **sin objeto, bloqueada** por la frontera de ADR-35 | 117 | Atributos/dominio |
| 14 | **Entidad `prestamista` abierta, lado de la oferta de crédito** (`fondeo`, `gobierno_corporativo`, enlace `tasa_ofrecida`/`cat`) — enmienda completa (no acotada) a `ADR-35`, firma de dirección "(a)" sobre `FP-61`. Da objeto a la octava refutación (`ref.A.04`); ninguna calibrada. Las ocho quedan con objeto | 124 (ADR-35(b)) | Atributos/dominio |
...
#### Corte de `edad` para «joven» — convención declarada, no derivación empírica

**`edad` joven = 15 a 29 años.** Corte oficial adoptado por firma de mesa D-2 de `MESA-19AGO` — vía **(c) ambas**, verbatim: *"Convención declarada ahora para desbloquear los 9 sitios, más derivación empírica con dato mexicano propio como acto en cola que puede corregirla"* (`ADR-110(b)`, fila `FP-53`) — y propagado por `ACTO CORTE-EDAD-CONVENCION` (19/ago/2026, `ADR-116`). Es el corte de «población joven» que INEGI usa en la ENOE para estadística laboral — el mismo registro en que se mide `R2.4` (rotación) y en que se observa el descriptor 5. Procedencia citada con URL y fecha de consulta, per el aviso del propio encargo (ninguna de las dos fuentes vivía citada en el árbol antes de este acto): INEGI, *Panorámica de la población joven por condición de actividad* (ENOE), <https://www.inegi.org.mx/investigacion/pobjoven/>, consultado 19/ago/2026.

**Alterno declarado, no adoptado aquí:** 12 a 29 años — Ley del Instituto Mexicano de la Juventud, art. 2 (DOF 06/ene/1999, últimas reformas 02/abr/2015), <https://www.diputados.gob.mx/LeyesBiblio/pdf/LIMJ.pdf>, consultado 19/ago/2026.

⚠️ **Convención, no verdad final — la vía (c) obliga a dejarlo escrito.** `CORTE-EDAD-EMPIRICO` (acto gemelo, en cola, sesión UBUNTU con microdato mexicano propio) puede **corregir este corte** con derivación empírica; su alcance puede vencer al de esta convención (mismo principio que la estampa de universo, `ADR-67(b)`/`ADR-78(e)`, rótulo `A.10`). Los nueve sitios que este corte desbloquea (descriptor 5 · H-02 · H-06 · H-07 · `R1.4` · `R2.4` ×2 · `R5.4` ×2, §1.1.D/§1.6/§3.2/§3.5) están escritos contra **esta** convención, no contra un hallazgo empírico que todavía no existe. `P1` (`forense/notas/2026-07-31-p1-enigh-semilla.md`) no verificó partición canónica de `edad` en la semilla ENIGH — eso sigue cierto y no lo cambia esta convención: el corte es una regla externa aplicada sobre la variable `edad` ya inventariada (§1.1.A #2), no una partición descubierta en el microdato.
...
| **5 · Joven Gen Z urbano conectado** | `edad` joven (**15-29 años, convención declarada — §1.1.A**) **∧** `tam_loc`=1 **∧** `conex_inte`=1 | Región definible. **Eje transversal a la formalidad**, no un rival de ella — por eso ya no hay solapamiento que resolver |
...
| **H-02** | E[`horizonte_temporal` \| joven urbano conectado] **<** E[`horizonte_temporal` \| adulto urbano formal] | Edad (**15-29, convención declarada — §1.1.A**) × `tam_loc`=1 × `conex_inte` | Mismo proxy, mismo defecto | `[HIPÓTESIS]` — el "corto (inmediatez)" del perfil 5 nunca tuvo dato mexicano citado |
...
| **H-06** | E[`sens_estatus` \| joven urbano conectado] **>** E[`sens_estatus` \| resto urbano] | Edad (**15-29, convención declarada — §1.1.A**) × `tam_loc`=1 × `conex_inte` | **NO DETERMINABLE EN ESTE RÉGIMEN** (`sens_estatus` sin reactivo, búsqueda cerrada por límite de régimen — §1.1.F Paso 5; el corte de edad ya no es la razón) | `[HIPÓTESIS]` · **no comprobable hoy** |
| **H-07** | E[`deferencia` \| joven] **<** E[`deferencia` \| mayor] | `edad` (**15-29, convención declarada — §1.1.A**) | ~~**NO DETERMINABLE**, cola de verificación **C-bis** declarada y no corrida (ADR-51 (c))~~ → **PROXY CON SUPUESTO DECLARADO (parcial)** — Latinobarómetro 2024 `P4NOIJ` ("Obediencia" entre cualidades a inculcar en los niños), México n=1200, C-bis corrida (ADR-51 (f), `forense/notas/2026-08-03-cbis-deferencia-externas.md`) | `[MEDIA]` · **forma PENDIENTE** (funcional, no del corte de edad — ya no bloqueada por él) |
...
| **H-11** | E[`norma_de_género` \| informal, ingreso bajo] **>** E[`norma_de_género` \| formal, ingreso medio-alto] | `segsoc` × `est_socio` | **PROXY CON SUPUESTO DECLARADO** — ENUT 6.11/6.11a, carga de cuidado a nivel persona (ADR-51 (c), M2) | `[HIPÓTESIS]` **(b)** · **forma PENDIENTE y sin magnitud** |
| **H-12** | E[`exposicion_violencia` \| ingreso bajo] **>** E[`exposicion_violencia` \| ingreso alto] | `est_socio` × `tam_loc` | ~~**Reactivo directo** ENVIPE `BP1_20`/`BP1_23`/`BP1_28`~~ *(retirado por `PR #57`, medía conducta de denuncia, no exposición — corrección in situ 4/ago/2026, Encargo K)* → **Reactivo directo** ENVIPE 2025 `TPer_Vic2`, `AP7_3_10`-`_14` (`forense/notas/2026-08-04-medicion-exposicion-violencia-envipe.md`) | `[MEDIA]` **(a)** · ⚠️ **malla truncada**: ENVIPE no tiene digital ni migración, y el ingreso solo entra como `ESTRATO` de área — *(cita del reactivo corregida, no re-evaluada: probar H-12 contra el nuevo reactivo es adjudicación, fuera del alcance de un acto de medición)* |
...
1 `horizonte_temporal` · 2 `radio_confianza` · 3 `aversion_riesgo` · 4 `sens_estatus` · 5 `deferencia` · 6 `familismo_apoyo` · 7 `norma_de_género` *(nombrada `familismo_obligacion` hasta ADR-75(b), 13/ago/2026 — el renombre es de la condicional, no del coeficiente de generador homónimo)* · 8 `exposicion_violencia` · 9 **`acceso_digital`** · 10–15 `confianza_institucional` × **6 componentes** (seguridad-fuerzas armadas · educación · salud · electoral-partidos · justicia-policía · financiera, §1.3).
...
**Paso 2 — de 15 parámetros a 14 condicionales.** `acceso_digital` **sale de la lista de parámetros**: bajo el reencuadre es el **eje 5 del vector de atributos** (§1.1.A). Un parámetro no puede ser una condicional sobre sí mismo — es el criterio C3 de P2 §2.b, el mismo que inhabilita los pares circulares del ajuste. **La salida no toca §2:** los nueve parámetros que los generadores multiplican son `confianza_institucional`, `radio_confianza`, `sens_estatus`, `aversion_riesgo`, `horizonte_temporal`, `familismo_apoyo`, `norma_de_género`, `exposicion_violencia` y `deferencia` (§2.2) — `acceso_digital` no está entre ellos.

> ### **D = 15 condicionales** = 9 escalares + 6 componentes de `confianza_institucional`
>
> *(Eran **14** = 8 + 6 hasta el 13/ago/2026. El noveno escalar es `obligación_medida`, que **no** entra por los pasos 1-2 — ésos reconstruyen la tabla 6×15 del v3.4 y no se mueven, ver Paso 6 — sino por ADR-75(b), ejecutado por `ACTO PROC-11`.)*
...
|---|---|---|
| **MEDIDO·PARCIAL(x)** *(nuevo 3/ago/2026, ver clase sellada en `milpa/procedencia.yaml`)* | `confianza_institucional[salud]` (ENCIG `P11_1_3`, condicionada a edad) · `confianza_institucional[educación]` (ENCUCI `AP5_2_6`, formalidad×edad + marginales urbanización/ingreso/acceso digital) · `confianza_institucional[financiera]` (ENIF `P11_1_1`-`P11_1_5`, formalidad×edad + marginales urbanización/migración/ingreso) — `forense/notas/2026-08-03-cal-conf-faseb-medicion.md` §4-5 · `confianza_institucional[seguridad-FFAA]` (ENVIPE `AP5_4_04/08/09/10`, por institución, condicionada a edad/dominio) · `confianza_institucional[justicia-policía]` (ENVIPE `AP5_4_01/02/03/05/06/07/11`, por institución, condicionada a edad/dominio) · `confianza_institucional[electoral-partidos]` (ENCUCI `AP5_2_5`/`AP5_3_6/7/8`, por ítem, condicionada a formalidad×edad/dominio) — `forense/notas/2026-08-03-cal-conf-faseb-medicion-ola2.md` §3-5 · `radio_confianza` (ENCUCI `AP5_1_1/2/3`, por ítem, condicionada a formalidad×edad + marginal dominio) · `familismo_apoyo` (ENIF `P9_9_1..6`, por ítem — `p9_9_4` opera el constructo, condicionada a formalidad×edad + marginal urbanización) — `forense/notas/2026-08-03-cal-conf-faseb-pos5-6-radio-familismo.md` §1-5 · `exposicion_violencia` (ENVIPE 2025 `TPer_Vic2`, `AP7_3_10`-`_14`, binario núcleo + condicional con `AP7_3_09`/extorsión reportada aparte, condicionada a edad×dominio + marginales formalidad/ESTRATO) — `forense/notas/2026-08-04-medicion-exposicion-violencia-envipe.md` §3-5 | ~~**8**~~ **9** |
| **Sin reactivo verificado — búsqueda abierta** | *(vacía desde 4/ago/2026 — ver nota abajo: `exposicion_violencia` sale de esta clase por medición exitosa, no por agotamiento de los tres actos de ADR-52 A, que sigue intacto como criterio si algo vuelve a caer aquí)* | **0** |
| **Sin reactivo — búsqueda cerrada (límite de régimen)** | `aversion_riesgo` — único candidato examinado y descartado con argumento: ENIF `P5_23`/`P5_24` mide conocimiento de protección de depósitos IPAB, el moderador que `dinero.ahorro.seguro_deposito_atenua_aversion` pone en el `SI`, no una medida de aversión (`hitoE §17`) · `sens_estatus` — sale de "búsqueda abierta" (ADR-54, `gobernanza` §4): el examen de descriptor de `PR #64` (`forense/notas/2026-08-04-sens-estatus-examen-descriptor.md`) recorrió los cinco instrumentos permitidos del régimen y **ninguno sirve**, fundado en descriptor literal — vía de cierre del precedente `aversion_riesgo` de ADR-52 A (candidato examinado y descartado con argumento, no la caducidad de tres actos) | **2** |
| **Proxy declarado, pendiente de medición** | `horizonte_temporal` (ENIF `P4_10`, ⚠️ falla C3) · `deferencia` (M3, Latinobarómetro `P4NOIJ`, ADR-51 (f)) | **2** |
| **MEDIDO·NACIONAL** *(séptima clase, nueva 13/ago/2026 — ADR-79(a), `PROC-10-bis`; clase sellada en `milpa/procedencia.yaml`, ver header de clases)* | `norma_de_género` (ENASIC 2022 `P7_12_7`, tabla `TPER_ELE` — θ=0.6933, IC95 [0.6725,0.7140], n=5,579, `data/curacion-registro/produccion-modelo.tsv` fila `PROD-cca3ea0bccd54d70083728b2` — ADR-67(b) · ADR-75(a); **cita corregida 13/ago/2026 por `ACTO PROC-11`**: decía "ENUT 6.11/6.11a, M2", obsoleta desde que ADR-67(b) fijó `P7_12_7` como la θ real de este generador el 10/ago; ENUT queda como capa conductual, no como θ — límite de constructo de ADR-75(a): norma de deber de cuidado diferenciada por sexo, no obligación familiar genérica; marginal nacional, sin eje condicionante, `x = ∅`) · `obligación_medida` (ENASIC 2022 `P6_38`, tabla `TPOB_CUI` — categórica de 5 vías: obligación 0.0765/decisión familiar 0.0671/única que podía 0.0356/petición 0.0123/iniciativa propia 0.8086, n=5,677, `produccion-modelo.tsv` fila `PROD-ea28cc177894157ca113388b` — entrada 13/ago/2026 por `ACTO PROD-P638`, que reprodujo por el motor formal el cálculo de `ENASIC-SPLIT`; misma familia que `norma_de_género`, mismo `x = ∅`; límite de escala declarado — no se compara con `norma_de_género`, ver `milpa/procedencia.yaml`) | **2** |
| **Medida sin clase de procedencia — decisión D3 pendiente** *(vacía desde 13/ago/2026 — `obligación_medida` salió por entrada a `MEDIDO·NACIONAL`, `ACTO PROD-P638`; categoría queda nombrada para cualquier medida futura que caiga en el mismo hueco antes de que su propio expediente cierre)* | — | **0** |
| | | **15** ✔ *(9+0+2+2+2+0)* |
| **MEDIDAS** | los seis componentes de `confianza_institucional` (arriba) más `radio_confianza`, `familismo_apoyo` y `exposicion_violencia` — `MEDIDO·PARCIAL(x)`, distribución condicional empírica sobre un subconjunto declarado de `x` en los nueve casos (nunca los seis ejes a la vez) — más `norma_de_género` y `obligación_medida` — `MEDIDO·NACIONAL`, marginal nacional sin eje (`x = ∅`), clase nueva 13/ago/2026 (ADR-79(a), `PROC-10-bis`), segunda entrada 13/ago/2026 (`ACTO PROD-P638`); los tres de la ola 2 medidos **por institución**, `radio_confianza`/`familismo_apoyo` medidos **por ítem**, `exposicion_violencia` medido como binario único con desglose de cinco ítems reportado como intensidad, `norma_de_género` medido como escalar único de cuatro categorías oficiales, `obligación_medida` medido como categórica de cinco vías, ninguno como escalar único sin declarar (`forense/notas/2026-08-03-cal-conf-faseb-pos5-6-radio-familismo.md` §1.0, §4-5; `forense/notas/2026-08-04-medicion-exposicion-violencia-envipe.md` §3-5). Marca C3 declarada, viaja con el número: `familismo_apoyo` no identifica `G5`; `radio_confianza` no identifica `cooperacion.confianza.puente_personal` (misma nota, §1.0); `exposicion_violencia` pasa C3 limpio (ninguna candidata de Tabla B) pero trae **límite de C2 declarado** — no usar junto con `BP1_23` en la misma ecuación (ver `milpa/procedencia.yaml`); `norma_de_género` trae **límite de constructo declarado** (ADR-75(a)) — norma de deber de cuidado diferenciada por sexo, no obligación familiar genérica, no se usa sin esa restricción; `obligación_medida` trae **límite de escala declarado** (`enasic-split-verificacion.md` §2.2) — no se resta ni se compara con `norma_de_género` como si fueran la misma escala | ~~**10**~~ **11** |

**Criterio, sellado (decisión de mesa, 4/ago/2026, ver `gobernanza` §4, ADR-52 A).** La clase no la determina qué le pasó al reactivo, sino si queda algún acto nombrado que pueda producirlo. **Condición de caducidad:** una búsqueda se cierra —pasa a "límite de régimen"— si tres actos nombrados vuelven sin reactivo; sin esa condición la clase "búsqueda abierta" es un eufemismo permanente. **Corrección 4/ago/2026 (Encargo K):** `exposicion_violencia` sale de "búsqueda abierta" y entra a `MEDIDO·PARCIAL(x)` — la clase "búsqueda abierta" queda **vacía**, no por agotar los tres actos de la condición de caducidad, sino porque el segundo acto de búsqueda (`ENVIPE TPer_Vic2`, `forense/notas/2026-08-04-envipe-tper-vic2-tmod-vic-paso1.md`) encontró candidato válido y este acto lo midió — la caducidad de tres actos **sigue intacta como criterio** para cualquier búsqueda que caiga en esta clase en el futuro, simplemente no aplicó aquí porque no hubo agotamiento.

**Paso 6 — condicionales registradas después del reencuadre** *(nuevo 13/ago/2026, `ACTO PROC-11`, ejecutando ADR-75(b))*. Los pasos 1–5 derivan `D` de la tabla 6×15 del v3.4 y son **historia cerrada: no se mueven**. Una condicional que **no existía en esa tabla** no cabe en ellos sin falsificar su propio control aritmético — el Paso 1 verifica `15 × 6 perfiles = 90` contra el `+36` y el `90` que §6 declara, y meter una decimosexta lo rompería. Por eso este paso.

`obligación_medida` (ENASIC 2022 `P6_38`, tabla `TPOB_CUI`, n=5,677) entra como **condicional nueva** por la política que ADR-75(b) firmó, verbatim: *"entra, cuando se registre, como **condicional nueva** (`D`: 14 → 15), no como sub-componente de la existente (denominador intacto)"*. Su razón está escrita ahí y no se repite aquí: `norma_de_género` y `obligación_medida` no comparten tipo de constructo —una norma-sobre-terceros y un motivo-propio-reportado—, a diferencia de los seis componentes de `confianza_institucional`, que sí lo comparten y por eso cuentan cada uno su lugar en `D`. Su celda-D queda registrada en `data/curacion-registro/celdas-d/` por este mismo acto.

**`D`: 14 → 15.**

> ## 📊 **Condicionales medidas sobre atributos: ~~11~~ 12 de 15**
> *(Corregido 4/ago/2026, Encargo K — `forense/notas/2026-08-04-medicion-exposicion-violencia-envipe.md` §3-5: mide `exposicion_violencia` (ENVIPE 2025 `TPer_Vic2`, `AP7_3_10`-`_14`). `4 de 144` sigue congelado y no se mueve — ver §6.1. **Nota de reparto, actualizada 4/ago/2026:** `exposicion_violencia` sale de "sin reactivo verificado — búsqueda abierta" (que queda vacía, 0) y entra a `MEDIDO·PARCIAL(edad,dominio,formalidad,ESTRATO)`; `aversion_riesgo` y `sens_estatus` siguen en "sin reactivo — búsqueda cerrada (límite de régimen)". **Denominador 14 → 15 el 13/ago/2026, `ACTO PROC-11`** (ADR-75(b), Paso 6 arriba): entra `obligación_medida`. **Numerador 9 → 10 el 13/ago/2026, `PROC-10-bis`** (ADR-79(a)): sella `MEDIDO·NACIONAL` y le entra `norma_de_género`. **Numerador 10 → 11 el 13/ago/2026, `ACTO PROD-P638`**: reproduce `obligación_medida` por el motor formal y le entra a la misma clase. **Numerador 11 → 12 el 18/ago/2026, `ACTO COND-ATRIB`** (ADR-104): `confianza_institucional_generico_servidores_publicos` pasa de `PENDIENTE` a `MEDIDO·PARCIAL(edad,formalidad,sexo,escolaridad,estrato)` (ENCIG 2023 `P11_1_23`). Propagado 24/ago/2026 por `ACTO SELLA-AGO24` sobre este banner que había quedado rezagado — solo el numerador de este titular se corrige aquí; el reparto de categorías no se toca, fuera de perímetro de este acto.)*
>
> ✅ **El NUMERADOR se mueve dos veces el mismo día, 13/ago/2026 — la razón es taxonómica: la clase séptima ya existe.** `PROC-10-bis`, ejecutando `ADR-79(a)` (D-A, firma de mesa verbatim: *"crear una clase séptima para marginales medidas sin eje"*), sella `MEDIDO·NACIONAL` en `milpa/procedencia.yaml` y le entra la primera θ, `norma_de_género` (ENASIC 2022 `P7_12_7`) — de donde T19b/T19c derivan el numerador. `ACTO PROD-P638` cierra la segunda: reproduce `obligación_medida` (ENASIC 2022 `P6_38`) por el motor formal (`prepare→produce→integrate`, `hashes_analista_confiados: false`), verificando byte a byte contra las cinco proporciones e IC que `ENASIC-SPLIT` ya había calculado (`forense/notas/2026-08-13-enasic-split-verificacion.md` §2.2), y le entra a la misma clase — misma familia (adenda B.5.3 de `forense/notas/2026-08-13-proc-11.md`), numerador 10 → 11. **Decisión D3, resuelta `D3-A`** (`ADR-79(a)`, en sustancia — ver `forense/notas/2026-08-13-proc-11.md` ADENDA B.2): la puerta **D3-B** (condicionar `norma_de_género` POR eje, en vez de admitirla marginal) queda nombrada dentro de la propia definición de la clase, dueño "acto futuro por θ", no ejecutada aquí — `MEDIDO·NACIONAL` no la cierra, solo no la exige.
>
> **`D`=15 es derivado del diseño de §1.1, nunca estimado.** Su aritmética completa está arriba, pasos 1–6. Si alguien mueve `D`, tiene que mover uno de esos pasos y decir cuál. *(Quien movió `D` de 14 a 15 fue `ACTO PROC-11`, y la respuesta a "cuál paso" es: **ninguno de los cinco originales — se añadió el Paso 6**, precisamente para no falsificar el control aritmético del Paso 1. El numerador no depende de ningún paso 1-6 — depende de `milpa/procedencia.yaml`, ver la nota de arriba.)*

**Qué le pasa al `144`.** 144 = 90 `params_base` + 15 coeficientes + 39 probabilidades. Los **90 salen del conteo de números** y son sustituidos por **15 condicionales de dimensión indeterminada** — indeterminada porque la forma funcional está PENDIENTE, no porque no se haya contado. Quedan **54 números enumerables** (15 + 39), de los cuales **4 son `MEDIDO`**. ⚠️ **Este documento NO mueve el titular `4 de 144`**: sigue **congelado** por decisión de mesa (`forense/hallazgos.md`, 31/jul/2026), y elegir entre congelarlo como cifra histórica o retirarlo del titular es una decisión que `revision §6.3` deja abierta y que este encargo no implementa. Se deriva la aritmética y se deja la decisión donde está.
...
#### 1.1.G · Ocho variables nuevas, para darles objeto a las refutaciones sin objeto *(ADR-117, ejecuta `FP-56`/`ADR-110(a)`/`ADR-111(c)`)*
...
**Por qué no entran a `D`.** El Paso 1 de §1.1.F ancla `D` a la tabla 6×15 del v3.4 (`15 × 6 = 90`, control aritmético íntegro); el Paso 6 ya estableció el precedente de que una condicional **nueva**, ajena a esa tabla, entra por acto propio y mueve `D` solo cuando un generador la multiplica (`obligación_medida`, ADR-75(b)). Las ocho de abajo no tienen generador que las multiplique todavía — **son atributo/dominio declarado, no coeficiente de generador** — así que viven fuera de `D` hasta que ese acto futuro las reclame. Tocarlo aquí falsificaría el control aritmético del Paso 1 sin que exista el generador que lo justifique.

| id refutación | Variable nueva | Tier (`corrida-refutaciones §3`) | Base declarada u hueco de evidencia | Objeto |
|---|---|---|---|---|
| `ref.A.02` esfuerzo laboral | `esfuerzo_laboral` / `horas_trabajadas` | **MUY_FUERTE** (única de las 49) | Corpus ya cita el dato: **2,207 h/año, el mayor de la OCDE, 26% sobre el promedio** — la baja productividad es déficit de capital, no de esfuerzo (`corrida-refutaciones.md:92`). Reactivo mexicano (candidato: ENOE, horas habituales trabajadas) **no se buscó en este acto** — declarar la variable no es medirla; la búsqueda del reactivo es acto propio nombrado. | Existe como atributo declarado del dominio §3.2 (Trabajo y carrera). Forma PENDIENTE. |
| `ref.A.04` pobres no pagan | `prestamista` (`fondeo`, `gobierno_corporativo`, enlace `tasa_ofrecida`/`cat`) — **§1.1.H, nueva** | FUERTE | **RESUELTA 19/ago/2026.** El gate que este mismo acto exigió (deep research auditado de todo el sector, `corpus/forense/compass-4-e29a28d4-credito-popular-2026.md`, N=11) llegó, y dirección firmó **"(a) completa"** sobre `FP-61`: `ADR-125` sella `ADR-35(b)` sin acotar. | **Con objeto — `ADR-125`/`ADR-35(b)`.** Ver §1.1.H para el detalle de los tres atributos. `ref.A.04` re-redactada en `milpa/refutations.yaml` (veredicto `REFUTADA-FUERTE`, N=11). |
| `ref.A.14` no cree en terapia | `salud_mental` (dominio nuevo) | FUERTE | Hueco de evidencia dicho: existe un report entero sobre el tema (`corrida-refutaciones.md:83`) pero este acto no extrajo reactivo de él — extracción y medición son actos propios. | Existe como dominio nuevo, sin regla en §3 todavía. Forma PENDIENTE. |
| `ref.A.20` emprendimiento vibrante | `emprendimiento` | MEDIA | Hueco de evidencia dicho — no se buscó reactivo en este acto. | Existe como atributo declarado del dominio §3.2. Forma PENDIENTE. |
| `ref.A.28` e-commerce transformador | `canal_de_compra` | MEDIA | Hueco de evidencia dicho — no se buscó reactivo en este acto. | Existe como atributo declarado del dominio §3.1 (Dinero, ahorro, crédito y consumo). Forma PENDIENTE. |
| `ref.B.04` colorismo estructural | `colorismo` / tono de piel | FUERTE | Hueco de evidencia dicho — no se buscó reactivo en este acto (candidato de literatura: escalas tipo PERLA, no verificado contra microdato mexicano esta sesión). | Existe como séptimo eje candidato del vector de atributos (§1.1.A), **fuera de los seis verificados por P1** — no se añade a la tabla de §1.1.A hasta que un acto con red verifique un módulo/llave real, mismo criterio que A.6. |
| `ref.B.06` fatalismo no es religioso | `religiosidad` | FUERTE | Ya nombrada como modificador transversal en §1.4, sin constructo propio ni marca de procedencia — este acto la promueve de mención a variable declarada del dominio §3.9 (Información y creencia). Hueco de evidencia dicho para su reactivo. | Existe como modificador transversal con objeto propio. Forma PENDIENTE. |
| `ref.A.17` líder fuerte ≠ autoritarismo | `deferencia_autoridad` (ítem actitudinal) | MEDIA | Parcial ya existente: §3.2 distingue liderazgo benévolo de autoritario, sin ítem actitudinal (`corrida-refutaciones.md:88`). Este acto declara el ítem; no lo mide — el corpus ya trae un candidato de deferencia general (H-07, §1.1.E, Latinobarómetro `P4NOIJ`), que **no es el mismo constructo** (deferencia a la autoridad en general ≠ evaluación de liderazgo fuerte vs. autoritario) y no se reutiliza sin decirlo. | Existe como ítem actitudinal declarado del dominio §3.3 (Autoridad, trámite y relación con el Estado), distinto de H-07. Forma PENDIENTE. |

⚠️ **Historia de esta tabla, no estado vigente: al escribirse (`ADR-117`, 19/ago/2026 temprano), siete de ocho quedaban CON objeto y la octava bloqueada por `ADR-35`, escalada a mesa como `FP-61`.** La lectura de `FP-56`/`ADR-111(c)` decía "las ocho... para darles objeto"; la letra del propio acuerdo, en la misma firma, decía que `ADR-35` declaraba la frontera del prestamista y que ampliar al lado de la oferta era "una decisión distinta y mayor" que ese ADR no tomaba. `ADR-117` siguió entonces la instrucción específica sobre la general. **Esa octava quedó resuelta el mismo día, más tarde: `ADR-125` sella la enmienda completa a `ADR-35` (`ADR-35(b)`) sobre firma de dirección, y las ocho quedan con objeto — ver §1.1.H.**

⚠️ **Ninguna de las ocho se calibra aquí (ni las siete de `ADR-117` ni la octava de `ADR-125`).** Forma funcional PENDIENTE en las ocho, igual que las 15 condicionales de §1.1.F antes de tener reactivo. Buscar y verificar reactivo, fijar forma funcional y correr la batería ampliada contra estas ocho son actos propios, no ejecutados aquí.

#### 1.1.H · Entidad `prestamista` — lado de la oferta de crédito, para dar objeto a `ref.A.04` *(ADR-125, sella `ADR-35(b)` completa, firma de dirección "(a)" sobre `FP-61`)*

**Qué es esto y qué no es.** `ADR-35` (28/jul/2026) declaró a propósito una frontera: el motor modela al **decisor**, no al **oferente** — *"ampliar al lado de la oferta es una decisión distinta y mayor, que este ADR NO toma."* `ADR-117` (19/ago) dejó `ref.A.04` sin objeto por esa misma razón y subió la enmienda redactada a mesa (`FP-61`). El gate que esa escalada exigía — un deep research con datos auditados de todo el sector, no solo los dos casos (Nu/Kueski) que la mesa había juzgado insuficientes — llegó el mismo día (`corpus/forense/compass-4-e29a28d4-credito-popular-2026.md`, 11 casos independientes, 0 `CONFIRMA` la forma fuerte del mito, `ROMPE/MATIZA` mayoritario), y dirección firmó **"(a) completa"**: se sella la enmienda **sin acotar**, tal como el borrador la proponía tal cual (`forense/notas/2026-08-19-adr35-enmienda-borrador.md`, opción 1).

**La entidad.** `prestamista`, distinta del agente-decisor de §1.1.A (no es un séptimo eje del vector de atributos poblacionales — es un tipo de entidad aparte, del lado de la oferta). Tres atributos, los tres declarados y **ninguno calibrado**:

| Atributo | Qué es | Base declarada | Forma funcional |
|---|---|---|---|
| `fondeo` | Estructura de financiamiento de la entidad (deuda, capital, subsidio) | Las tres quiebras que el gate analizó (Famsa, Crédito Real, AlphaCredit) se adjudican a riesgo del *emisor*, no del acreditado; Crédito Real específicamente cae por un muro de refinanciamiento en francos suizos y un portafolio con ~47% de interés capitalizado ocultando el descalce real de caja — ficha-puente §2, `forense/notas/2026-08-19-fp61-ficha-puente-a04.md` | PENDIENTE |
| `gobierno_corporativo` | Proxy de supervisión/regulación de la entidad | Famsa (partes relacionadas, Ps. 8,589 M registrados para evadir reservas) y AlphaCredit (falla de control contable sobre derivados) — misma ficha-puente §2 | PENDIENTE |
| `tasa_ofrecida` / `cat` | Enlace al parámetro ya existente del lado del decisor (§3.1, techo cuantificado de disposición a pagar, sellado por `ADR-35` original) | No es un número nuevo — es la declaración de que el precio que cobra el `prestamista` y el techo de disposición a pagar del decisor son la **misma** cantidad, vista desde los dos lados de la transacción | PENDIENTE (el número ya existe en §3.1; aquí solo se declara el enlace) |

**El costo de "completa" sobre "acotada" — dicho, no omitido.** La versión acotada del borrador (solo `fondeo`+`gobierno_corporativo`, sin el tercer atributo) no calibra nada tampoco — la diferencia entre las dos versiones no es de calibración ni de "dinámica propia" (ninguna de las dos tiene generador todavía). Es exactamente el enlace `tasa_ofrecida`/`cat`: declararlo compromete a que cualquier acto futuro que toque el parámetro de precio del decisor (§3.1) tenga que reconciliarlo con lo que `fondeo`/`gobierno_corporativo` terminen implicando del lado del prestamista, en vez de tratar los dos como cifras independientes que casualmente describen la misma transacción real. Es la opción que evita que el motor termine con dos precios no coordinados para el mismo crédito — pero es una promesa que **este acto no paga**: la paga quien calibre.

**Por qué no entra a `D`, no reabre §3.1, y qué exige antes de fijar forma funcional — misma disciplina que las siete de §1.1.G.** No entra a `D` (§1.1.F) hasta que un generador la multiplique: es atributo/dominio declarado, no coeficiente de generador. No reabre las dos reglas de `§3.1` que `ADR-35` original ya selló del lado del decisor (`dinero.credito.scoring_alternativo`, `dinero.credito.baja_friccion_usura_dano_downstream`) — ninguna se toca aquí. No decide si `prestamista` entra al vector de atributos de §1.1.A — es una entidad aparte, no un séptimo eje del decisor. Exige, antes de fijar forma funcional, un mecanismo estructural nombrado y con fuente para cualquier diferencial que introduzca (mismo criterio de §1.5, `ADR-28.c`) y un reactivo verificado — declarar la entidad no mide `fondeo` ni `gobierno_corporativo`; buscar y verificar reactivo mexicano es acto propio, no ejecutado aquí.

**`ref.A.04` re-redactada.** `milpa/refutations.yaml`: la forma fuerte del mito ("los pobres no pagan") queda `REFUTADA-FUERTE` con N=11 (veredicto agregado del gate: `ROMPE/MATIZA`, 0 `CONFIRMA`); el enunciado vigente pasa a la frase del informe — *paga a tasas comparables o solo modestamente peores que el consumo bancario, condicionado a mecanismo (fuente/prenda/cobranza/grupo) y a un pricing que precarga pérdida de 6–20%* — con sus benchmarks de reversión registrados como Ficha B-bis en `forense/notas/2026-08-19-fp61-ficha-puente-a04.md §5`. Detalle completo del gate y su síntesis: misma ficha-puente, §§1-4.

⚠️ **No se calibra nada aquí.** Los tres atributos quedan `[HIPÓTESIS]`, forma PENDIENTE — mismo trato que las siete variables de §1.1.G. `ref.A.04` gana objeto, no pasa: corribles no es pasadas, mismo criterio que `ADR-117` aplicó a las otras siete.
...
⚠️ **El check sigue sin poder correr**, y ahora se sabe por qué con precisión: exige que cada una de las **15 condicionales** declare familia de distribución y dispersión con clase de procedencia, y **ninguna la declara hoy** (§1.1.F).
...
| `R1.4` · `dinero.consumo.estatus_mediado_por_credito` | perfiles 2, 3, 5 | (`segsoc`=2 **∧** `est_socio` ∈ {1,2}) **∨** (`edad` joven, **15-29, convención — §1.1.A** **∧** `tam_loc`=1 **∧** `conex_inte`=1) | **No.** Unión de dos regiones = unión de los tres perfiles |
...
| `R2.4` · `trabajo.rotacion.joven_urbano_sin_culpa` | perfil 5 | `edad` joven (**15-29, convención — §1.1.A**) **∧** `tam_loc`=1 — el `SI` ya decía *"joven urbano"* | **No.** |
...
| `R5.4` · `familia.cortejo.urbano_joven_apps` | perfil 5 | `edad` joven (**15-29, convención — §1.1.A**) **∧** `tam_loc`=1 **∧** `conex_inte`=1 — el `SI` ya decía *"urbano-joven-conectado"* | **No.** |
...
| **G2** · Desigualdad + baja movilidad | Ansiedad de estatus, consumo compensatorio (rama estatus) | **Se refuta si** un segmento de movilidad bloqueada **no** señaliza estatus y prioriza precio. ⚠️ **CONTESTADO en D/E** — Caso 5 (Bodega Aurrera), leído como confirmación en su momento |
...
⚠️ **G1a: homogeneidad de pendientes DECLARADA, no medida (ADR-49, D3).** El `−0.60` de `confianza_institucional[dominio]` se aplica al componente que el dominio seleccione — eso **asume una elasticidad común a los seis componentes** del vector de ADR-28.b. Es un supuesto declarado, no una medición; **no se desdobla en seis `ASIGNADO` donde hoy hay uno.** `G4` no comparte este problema: usa `confianza_institucional[justicia]`, componente nombrado, sin selección variable. **Pre-registro:** la salida de `CAL-CONF` Fase A (los seis niveles de confianza institucional por perfil) es lo que revisita el supuesto — si la dispersión entre componentes es la que ADR-28.b sostiene (Marina 89% vs. partidos 23.9%), la pendiente común queda implausible y el desdoblamiento pasa a justificado. Condición escrita **antes** de tener el dato. ~~**No cierra `D-12`** (`forense/hallazgos-congelados-2026-07-30.yaml`): D-12 es la operacionalización única de Hito E que derogaría ADR-28.b sin nombrarlo — sigue abierta, es otro asunto.~~ *(Corregido 3/ago/2026 — redacción heredada, no contradicción sustantiva. `D-12` sí cierra hoy, pero por una causa distinta de la que esta nota discute: `gobernanza` ADR-51 (d) lo cierra porque el reencuadre perfiles→atributos elimina la tabla que forzaba a tratar `confianza_institucional` como escalar — esa era la causa original de `D-12`. La pregunta que esta nota deja abierta —si el `−0.60` de G1a debe desdoblarse en seis `ASIGNADO` nombrados, como hace `G4`— nunca fue literalmente `D-12`: es una pregunta adyacente sobre la homogeneidad de pendientes, distinta de si el vector de 6 componentes sobrevive. `forense/hitoE-campana-medicion-v2_0.md` §12.2 señaló esta misma tensión sin resolverla ("esta adenda no la resuelve — no edita `canon/`"); esta sesión sí puede tocar `canon/` y la concilia aquí. **La pregunta de homogeneidad de pendientes sigue sin decidir** — el pre-registro de este párrafo sigue vigente y esperando la salida de `CAL-CONF` Fase A por dominio-perfil, que no es lo mismo que la Fase B por atributos que ya corrió (`forense/notas/2026-08-03-cal-conf-faseb-medicion.md`).)* **Enmienda in situ, 4/ago/2026 — el pre-registro corrió, y su resultado es que no puede llegar en este régimen.** `hitoE §14.3` posición 8 (P4) llevó este pre-registro a paso 1: verificar si un solo instrumento trae los seis componentes de `confianza_institucional` en la misma batería, condición necesaria para medir la dispersión *entre* componentes sin fabricar la conjunta que §1.1.C prohíbe. `PR #58` (`forense/notas/2026-08-04-cal-conf-faseb-pos8-encig-battxi.md`) corrió ese chequeo contra la batería XI de ENCIG 2021 (25 ítems, sección "Confianza en instituciones"): **CUBIERTO** salud, educación, seguridad-FFAA, justicia-policía y electoral-partidos — **NO CUBIERTO** financiera (ningún ítem de los 25; el candidato más cercano por nombre, "Empresarios", es confianza en un actor social, no en una institución financiera). Ningún instrumento del corpus trae los seis en una sola batería. **Resultado, no tarea pendiente:** la dispersión entre componentes no es estimable en este régimen sin fabricar la conjunta prohibida — el dato que este pre-registro esperaba no puede llegar por esta vía. **Esto NO decide si `G1a` se desdobla en seis `ASIGNADO` nombrados** — esa pregunta no se resuelve, **vuelve a mesa**: el pre-registro suponía que el dato revisitaría el supuesto de homogeneidad; lo que se verificó es que el dato no puede producirse en este régimen, así que mesa decide con el supuesto declarado intacto, no con evidencia nueva. Ver cierre de fila en `hitoE §14.3` posición 8 (adenda fechada).* **Enmienda in situ, 4/ago/2026 (mesa) — vuelve de mesa: `G1a` nombra su componente, no se desdobla (`gobernanza` §4, ADR-52 B).** La pregunta que quedó abierta arriba se resuelve sin el dato que el pre-registro esperaba: el `−0.60` de `confianza_institucional[dominio]` pasa a `confianza_institucional[financiera] −0.60` (tabla de coeficientes, arriba). Regla que va con la decisión: **si una regla de otro dominio invoca `G1`, esa regla nombra su componente; el coeficiente no vuelve a ser genérico** — derivado contra archivo (`grep -n "PORQUE G1" canon/modelo-decision-v4_0.md`): tres reglas invocan `G1` (`dinero.ahorro.informal_sin_puente`/`con_puente_y_respaldo`, `dinero.ahorro.seguro_deposito_atenua_aversion`, ambas de dominio financiero, más `cooperacion.confianza.puente_personal`, que corre por `radio_confianza`, no por `confianza_institucional`) — ninguna fuera de dominio financiero invoca el componente `confianza_institucional` de `G1`, así que nombrarlo `financiera` no dejó ninguna regla sin componente. **Por qué esto no es "sin dato, todo sigue igual":** `[dominio]` era infalsable — se aplicaba "al componente que el dominio seleccione", así que siempre había un componente donde encajaba; nombrarlo lo vuelve refutable, el mismo criterio de falsabilidad que el Bloque A exige. **La asimetría que se corrige:** este mismo párrafo decía que "`G4` no comparte este problema: usa `confianza_institucional[justicia]`, componente nombrado" — la diferencia nunca estuvo justificada por evidencia, era de redacción; los dos generadores nombran componente ahora. **Lo que NO cambia:** el `−0.60` sigue siendo `ASIGNADO` — nombrar el componente no lo mide. **Nota derivada, verificada contra `milpa/procedencia.yaml`:** `confianza_institucional[financiera]` está `MEDIDO·PARCIAL(formalidad,edad,urbanización,migración,ingreso)` desde la ola 1 de Fase B (ENIF `P11_1_1`-`P11_1_5`) — el coeficiente ahora apunta a una condicional con número detrás, no a un vector cuyo componente relevante nadie había nombrado.*

**Enmienda in situ, 28/ago/2026 (mesa) — `ADR-220`: enlace identidad sellado para los pares con θ ELEGIDA-CIEGA y β̂ medido; "los quince coeficientes son `ASIGNADO`" deja de ser el estado completo para 3 de los 15, desde hoy.** Mismo patrón que la enmienda de `G1a` arriba (`gobernanza` §4, `ADR-52 B`): la tabla de coeficientes y la frase "los quince coeficientes son `ASIGNADO`" **no se borran** — siguen siendo lo que el modelo declara como punto de partida. Lo que cambia es que, para los pares `gen.coef` cuya θ ya resolvió `ELEGIDA-CIEGA` (Paso 2 de `ESCALAS-COMPLETAS`, `ADR-173`, `forense/escalas-eleccion-ciega-v1_0.md` §4) **y** que además tienen un β̂ medido en `milpa/procedencia.yaml:coeficientes_generador_medidos` (4/ago/2026), mesa selló hoy la función de enlace del lado del generador: **la escala de salida del generador se declara IGUAL a la escala de la θ pareja — proporción ponderada `[0,1]`, enlace identidad** (firma de mesa `M-ENLACE=A`, `ADR-220`; resuelve `FP-177`). Son exactamente las 5 filas `RUTA-A` de `rutas_estimabilidad_coeficiente.detalle` (`forense/estado-motor-v1_0.md` §1): `G1a.confianza_institucional[financiera]`, `G1.radio_confianza`, `G3.familismo_apoyo`, `G4.exposicion_violencia`, `G4.confianza_institucional[justicia]`. De esas 5, **3 quedan escritas en el ejecutable** (`milpa/procedencia.yaml:coeficientes_generador_sellados`, consumido por `milpa/src/matriz.py` como override sobre el `ASIGNADO` de esta tabla): `confianza_institucional[financiera] -0.0645`, `familismo_apoyo 0.0279`, `exposicion_violencia 0.16614` (convertido de `+16.614pp`). Las otras 2 (`G1.radio_confianza`, 3 ítems; `G4.confianza_institucional[justicia]`, 7 ítems) quedan `SELLADO-ESCALA·SIN-AGREGACION` — mismo enlace identidad, pero sin escalar a un solo número hasta que mesa firme cómo agregar (`M-AGREGA=(a)`, firma de mesa, `ADR-220`). **Rótulo que viaja con los tres valores escritos, firma de mesa `M-176` verbatim: `ASOCIACION-MEDIDA`, nunca coeficiente identificado.** `ADR-57(a)` ya había establecido, para `G1`/`G3`, que condicionar (Encargo X) revierte el signo del marginal en la mayoría de sus celdas — asociar ≠ identificar — y esta firma trae a `G4` al mismo tratamiento (con la salvedad declarada de que el marginal de `G4.exposicion_violencia` sí sobrevive condicionar sin invertir signo, `forense/estado-motor-v1_0.md` §1 fila 8; se rotula igual de todos modos, porque `ADR-57(a)` gobierna la clase entera, no el resultado célula por célula). `A-bis` 3 sigue prohibiendo comparar cualquiera de estos tres valores contra su `ASIGNADO` correspondiente (`-0.60`/`0.20`/`0.70` de la tabla de arriba) como si fueran la misma magnitud: son la escala de la θ, nunca la escala de una elasticidad — la tabla de coeficientes de arriba **no cambia ningún número**, el valor ejecutable vive aparte, en `procedencia.yaml`. **Los 10 `ASIGNADO` restantes** (`G1b`, `G2.sens_estatus`, `G2.aversion_riesgo`, `G3.horizonte_temporal` — bloqueado por `GATE·ID-X`, `G3.aversion_riesgo`, `G4.horizonte_temporal`, `G4.sens_estatus`, `G5.familismo_apoyo`, `G5.familismo_obligacion`, `G5.radio_confianza`, `G6.deferencia`) quedan **`SOLO-SIGNO·NO-COMPARABLE`** — su historia intacta, ningún campo tocado, y `FP-149`/`ADR-173` sigue siendo el gate activo para ellos. Detalle completo, script y salida real: `forense/notas/2026-08-28-sella-enlace-cierre.md`.
...
- **SI** hay movilidad real bloqueada + presión de estatus ((`segsoc`=2 **∧** `est_socio` ∈ {1,2}) **∨** (`edad` joven **∧** `tam_loc`=1 **∧** `conex_inte`=1)) **ENTONCES** consumo compensatorio/aspiracional (rama estatus; marca, logo, mensualidades), aun apalancado — PORQUE G2 — `[FUERTE como correlación]`. *(v2: sin cambio de tier. V1 forense lo rompió "como driver decisivo aislado" — **afirmación distinta** de "fuerte como correlación" — y **omitió el perfil 5**. Hito 2.)* · **id:** `dinero.consumo.estatus_mediado_por_credito`
...
- **SI** el trabajador es joven urbano (`edad` joven, **15-29 años, convención declarada — §1.1.A** **∧** `tam_loc`=1) **ENTONCES** cambia de empleo sin culpa (no es deslealtad) y exige que las decisiones se justifiquen — PORQUE recalibración de G6 — `[MEDIA]`. · **id:** `trabajo.rotacion.joven_urbano_sin_culpa`
...
- **SI** el cortejo es urbano-joven-conectado (`edad` joven, **15-29 años, convención declarada — §1.1.A** **∧** `tam_loc`=1 **∧** `conex_inte`=1) **ENTONCES** apps + lógica de mercado, pero los guiones de género se reconfiguran **desigual** (actitud rápida, conducta lenta) — PORQUE cohorte + exposición — `[MEDIA / HIPÓTESIS]`. · **id:** `familia.cortejo.urbano_joven_apps`
...
3. **Ninguna cifra de confianza interpersonal como establecida — salvo tres, cerradas por ADR-64.** `conf.06` cerró (5/ago/2026): 21.8% (`AP5_1_1`, "la mayoría"), 32.1% (`AP5_1_3`, vecinos) y 62.1% (`AP5_1_2`, conocidos) son ENCUCI 2020 a corte ≥8/10, cada una confirmada contra microdato y **quedan establecidas con esa atribución y ese corte**. Las otras tres del racimo (12% WVS 2012, 22% Latinobarómetro/LAPOP, 18% Pew 2025) **no son ENCUCI y siguen sin establecer** — este ADR no las toca. ⚠️ El motor mide `radio_confianza` a corte ≥6/10 (`milpa/procedencia.yaml`): las tres cifras establecidas, a ≥8/10, **no son comparables contra esa medición sin ajustar el corte** — pregunta abierta, no decidida por ADR-64. **El 22% quedó adjudicado por `ADR-111`/`FP-58` (18-19/ago/2026), no sustituido por otra cifra: sin procedencia sostenible** — WVS 2018 (`Q57`) REFUTADA (10.51% `IC95%[8.86,12.15]`), Latinobarómetro 2024 (`P10STGBS`) NO SOSTENIDA en su única ola (26.06% `IC95%[22.45,29.67]`), LAPOP REFUTADA POR ERROR DE CATEGORÍA (no fielda el reactivo generalizado), ENAFI INDECIDIBLE (no adquirido). El 18% de Pew (`Q104`, Spring 2025) **reproduce exacto**; el 12% (WVS Wave 6, 2012) queda **indecidible** por falta de la ola — el punto 2018 de esa misma serie es 10.51%. Rango medible por escala, nunca promediado entre sí: binarios 10.5%–26.1% (2018–2025) · ENCUCI 21.9% a ≥8/10 (2020). Detalle: `forense/notas/2026-08-18-fp29-adjudicacion.md`.
...
| Bloque | v3.4 | **v4.0** |
|---|---|---|
| `params_base` | **90** números (15 parámetros × 6 perfiles) | **15 condicionales** θ_k( · \| x ) de dimensión indeterminada — la forma funcional está PENDIENTE (§1.1.B) |
...
> ## 📊 **Condicionales medidas sobre atributos: ~~11~~ 12 de 15**
> *(Numerador 9 → 10 el 13/ago/2026, `PROC-10-bis` — ADR-79(a) sella la clase séptima `MEDIDO·NACIONAL` y entra `norma_de_género`; numerador 10 → 11 el mismo día, `ACTO PROD-P638` — reproduce `obligación_medida` por el motor formal y le entra a la misma clase. Numerador 11 → 12 el 18/ago/2026, `ACTO COND-ATRIB` — `confianza_institucional_generico_servidores_publicos` pasa de `PENDIENTE` a `MEDIDO·PARCIAL(edad,formalidad,sexo,escolaridad,estrato)` (ENCIG 2023 `P11_1_23`), `ADR-104`. Denominador 14 → 15 el 13/ago/2026, `ACTO PROC-11` — ADR-75(b) registra `obligación_medida` como condicional nueva, ver §1.1.F Paso 6. Corregido antes, 4/ago/2026 — `CAL-CONF` Fase B posiciones 5-6, `forense/notas/2026-08-03-cal-conf-faseb-pos5-6-radio-familismo.md` §5: mide `radio_confianza` (ENCUCI) y `familismo_apoyo` (ENIF). Corregido de nuevo el mismo día, Encargo K, `forense/notas/2026-08-04-medicion-exposicion-violencia-envipe.md` §3-5: mide `exposicion_violencia` (ENVIPE `TPer_Vic2`). Ver §1.1.F, Paso 5. `4 de 144` sigue congelado y no se toca, ver abajo.)*

**`D`=15 sale del diseño de §1.1, no de una estimación.** Su aritmética: 15 parámetros por perfil (nombrados y controlados contra el `+36` de la tabla de arriba) **menos** `acceso_digital`, que bajo el reencuadre es un **atributo observable** y no un parámetro —lo que da 14—, **más** `obligación_medida`, condicional registrada después del reencuadre y por tanto ajena a la tabla 6×15 del v3.4 (ADR-75(b), `ACTO PROC-11`). Ver §1.1.F, pasos 1–6.
...
- **49 reglas** *(42 en v2 · 43 en v2.1 por conf.07 · 44 en v2.3 al partir la diagonal)*. **Hito D (perímetro de 27 reglas, subconjunto de las 49): 26 de 27 corridas archivadas**<!-- T20:HITO-D pob=reglas --> — `R1.1` → veredicto `D` (inejecutable por hueco de mercado, no de dato), `R3.2` → veredicto `B` (gate de 20pp aritméticamente inalcanzable), `R7.2` → veredicto `D` (inejecutable por hueco de instrumento, no de mercado ni de muestra), `R4.2` → veredicto `D` (inejecutable por hueco de instrumento: ENSANUT no cruza permiso laboral con conducta preventiva), `R4.1` → veredicto `D` (sin diseño antes/después de una mejora de acceso documentada), `R9.1` → veredicto `D` (sin variable de distancia en km ni muestra de quien no consulta a nadie), `R4.3` → veredicto `D` mitad A y mitad B (adherencia solo disponible auto-reportada en el catálogo entero), `R9.2` → veredicto `D` (abasto/campaña solo lo reporta el prestador, ninguna fuente independiente en el catálogo), `R5.1` → veredicto `A` (satisface la fila con IC95% que no despeja el umbral, reserva escrita en la línea), `R5.2` → veredicto `A` (mismo patrón que `R5.1`, IC95%=[14.39%,33.57%] no despeja 20%), `R1.2` → veredicto `E` (corrió limpio y no se satisfizo — la regla sobrevive a la prueba; excepción acotada, MESA-R12=SÍ), `R3.1` → veredicto `B` (dirección confirmada en 6/6 cómputos, brecha 9.28pp-32.73pp, resuelta por Respaldo 1 sin pareo por tipo de trámite — degrada a `[MEDIA]`, no refuta; tier `[FUERTE]` de la regla sin tocar, ADR-60(b)), `R1.3` → veredicto `E` (corroboración prospectiva acotada, penetración 3.86% del segmento popular no cruza ≥10%, ADR-63), `R8.1` → veredicto `D` (hueco de instrumento medido: `Q3`, ausencia identificable de sanción, cobertura cero en los cuatro instrumentos abiertos, ADR-138), `R7.1` → veredicto `A` (IC95%=[0.7331,12.3329] entero bajo el Umbral de 15pp, firma de mesa D1, ADR-145), `R7.3` → veredicto `C` (RDD no construible con ninguna fuente pública en disco, firma de mesa `FP-112`, ADR-155), `R7.4`/`R7.5` → veredicto `D` (falsador compartido: ninguna de las tres fuentes reales adquiridas —Mass Mobilization, UCDP, GDELT— codifica conjuntamente entorno y forma de respuesta sobre el mismo caso, `ADR-158`), `R8.3` → veredicto `A` (estrato SIN PUENTE, `d = −0.4422 pp`, IC95%=[−3.3572,+2.4727] despeja el Umbral por ambos extremos, firma de mesa RANURA 1, ADR-186; eje 3 ISSP multipaís en desacuerdo, abierto y declarado), `R1.4` → veredicto `D` (Umbral no construible sobre ENNViH: marca y sustituto funcional con cobertura cero en 425 archivos contra control positivo de 1274 columnas de compra/gasto, firma de mesa RANURA 2, ADR-187, resolviendo la colisión de `ADR-182`; fila `C` de su escala reescrita por SÍ de mesa). `R10.2` → veredicto `D` (variable de exposición retro pública/privada sin instrumento censado, ni público ni propietario; cruce `#363` confirma `SIN-DATO-GENUINO`, firma de mesa «R10.2: a», `ADR-196`). `R8.2` → veredicto `B` (verificación puntual confirma apps organizadoras entre conocidos, sin tasa de incumplimiento publicada; firma de mesa L1, `ADR-199`). `R2.2` → veredicto `D` (censo `forense/R22-censo-liderazgo-v1_0.md`, `SIN-DATO`: la literatura mexicana mide transformacional, no la escisión benévolo/autoritario; firma de mesa L2, `ADR-199`; ataque contra la lectura fuerte nunca corrió, regla queda `[MEDIA-FUERTE]`). `R3.4` → veredicto `B` (Respaldo 2 del pre-registro disparado como se anticipó, pero el aterrizaje corregido por firma de mesa de "cae en fila D" a fila `B`: A sellada `ADR-177`, B/C inejecutables por diseño de fuente, gate de Fase 1 sostenido no cerrado; firma de mesa, `ADR-201`). `R10.1` → veredicto `C` (spec sucesora v2, dos codificadores, `κ` Nivel 1 = 0.7209, gate de `§3.4` aprobado; los dos IC cruzan 15pp, ancla universitaria/elicitada; RANURA 2 de mesa, `ADR-205`). `R2.1` → veredicto `D` (censo `forense/R21-censo-fuentes-v1_0.md`, cero de 12 candidatas `EXISTE-SATISFACE` para el falsador `v2`; letra derivada de la fila `Demasiado débil` de la escala `B-bis` post-`ADR-190`; firma de mesa RANURA M-R21, `ADR-208`; `v1` techo histórico intacto, tier `[FUERTE]`/`v2` `[FUERTE como correlación]` sin tocar). Fuente única: `hitoD-preregistro §Registro de veredictos archivados`. *(Corregido 4/ago/2026 — propaga `R7.2` → `D`; propaga `R4.2` → `D`, ADR-55, Encargo Y. Corregido de nuevo 4/ago/2026 — propaga `R4.1`/`R9.1`/`R4.3`(A,B)/`R9.2` → `D`, ADR-56, Encargo Z. Corregido de nuevo 4/ago/2026, Acto 1 — adjudica `R5.1`/`R5.2` → `A` y `R1.2` → `E`, ADR-58; 8 de 27 → 11 de 27. Corregido de nuevo 4/ago/2026, ENCARGO M-1 — adjudica `R3.1` → `B`, ADR-60; 11 de 27 → 12 de 27. Corregido de nuevo 5/ago/2026, ADR-63, Encargo CU — adjudica `R1.3` → `E`; 12 de 27 → 14 de 27. Mismo hallazgo que `modelo-decision:64`: sitio sin debt declarada previamente, cerrado aquí. Corregido de nuevo 20/ago/2026, `ACTO RETRIAGE-4` — archiva `R8.1` → `D`, ADR-138; 13 de 27 → 14 de 27. Corregido de nuevo 24/ago/2026, `ACTO SELLA-AGO24` — adjudica `R7.1` → `A`, ADR-145; 14 de 27 → 15 de 27. Derivado de `python3 tests/check.py` T18, no tecleado.)* *(Corregido de nuevo 24/ago/2026, `ACTO SELLA-AGO24-C-v2`, `ADR-155` — adjudica `R7.3` → `C` por firma de mesa `FP-112`, primera fila `C` del programa; 15 de 27 → 16 de 27, cifra derivada del bloque append-only, no tecleada.)* *(Corregido de nuevo 24/ago/2026, `ACTO ADQ-CORRE-R74R75`, `ADR-158` — archiva `R7.4`/`R7.5` → `D`; 16 de 27 → 18 de 27, cifra derivada de `python3 tests/check.py` T18, no tecleada.)* *(Corregido de nuevo 25/ago/2026, `ACTO SELLA-ABRIDORES-R83-R14`, `ADR-186` — archiva `R8.3` → `A` por firma de mesa RANURA 1; 18 de 27 → 19 de 27. Corregido de nuevo 25/ago/2026, mismo acto, `ADR-187` — archiva `R1.4` → `D` por firma de mesa RANURA 2, resolviendo la colisión de `ADR-182`; 19 de 27 → 20 de 27, cifra derivada de `python3 tests/check.py` T18/T20, no tecleada.)* *(Corregido de nuevo 25/ago/2026, `ACTO PROPAGA-LETRAS`, `ADR-196` — archiva `R10.2` → `D` por firma de mesa «R10.2: a», cruce `#363`; `R2.2` gana enmienda de censo sin archivar; `R8.2` sin archivar por RANURA ausente; 20 de 27 → 21 de 27, cifra derivada de `python3 tests/check.py` T18/T20, no tecleada.)* *(Corregido de nuevo 26/ago/2026, `ACTO CIERRA-4-FIRMAS`, `ADR-199` — archiva `R8.2` → `B` por firma de mesa L1 y `R2.2` → `D` por firma de mesa L2, tras correr el censo `forense/R22-censo-liderazgo-v1_0.md` (veredicto `SIN-DATO`); 21 de 27 → 23 de 27, cifra derivada de `python3 tests/check.py` T18/T20, no tecleada. Corregido de nuevo 26/ago/2026, `ACTO CIERRA-FP157`, `ADR-201` — archiva `R3.4` → `B` por firma de mesa; 23 de 27 → 24 de 27, cifra derivada de `python3 tests/check.py` T18/T20, no tecleada.)* *(Corregido de nuevo 26/ago/2026, `ACTO CORRE-R10.1-v2` Fase B, `ADR-205` — archiva `R10.1` → veredicto `C` bajo la RANURA 2 de mesa; 24 de 27 → 25 de 27.)* *(Corregido de nuevo 26/ago/2026, `ACTO MAESTRA30-E10 · R21-ADJUDICA`, `ADR-208` — archiva `R2.1` → veredicto `D` por firma de mesa RANURA M-R21; 25 de 27 → 26 de 27, cifra derivada de `python3 tests/check.py` T18/T20, no tecleada.)*
- **Ejercicio distinto, no confundir con lo anterior (ADR-45, D-05):** `glosario §6/§10` corrió por su cuenta, el 27/jul/2026 — antes de que el Hito D existiera —, un ejercicio informal sobre "utilidad + fricción baja > confianza" (dominio seguro agrícola), **veredicto informal B**: sin contraejemplo limpio, candidato irresuelto, techo estructural que impide alcanzar el veredicto C con fuentes públicas. No tiene ID de regla, no vive en el bloque append-only de `hitoD-preregistro`, y no cuenta en el "13 de 27" de arriba.
...
| `R1.4` | L216 | Movilidad bloqueada + presión de estatus → consumo compensatorio (rama estatus) | `[FUERTE como correlación]` | Sí |
...
- **Condicionales medidas sobre atributos: ~~11~~ 12 de 15** (§1.1.F · §6.1; numerador 9 → 10 el 13/ago/2026, `PROC-10-bis` ejecutando ADR-79(a) — sella la clase séptima `MEDIDO·NACIONAL` y entra `norma_de_género`, ver §1.1.F Paso 5. Numerador 10 → 11 el mismo día, `ACTO PROD-P638` — reproduce `obligación_medida` (ENASIC 2022 `P6_38`) por el motor formal (`prepare→produce→integrate`, `hashes_analista_confiados: false`) contra las cinco proporciones e IC ya calculadas por `ENASIC-SPLIT`, y le entra a la misma clase. Numerador 11 → 12 el 18/ago/2026, `ACTO COND-ATRIB` — `confianza_institucional_generico_servidores_publicos` pasa de `PENDIENTE` a `MEDIDO·PARCIAL(edad,formalidad,sexo,escolaridad,estrato)` (ENCIG 2023 `P11_1_23`), ADR-104; propagado a este banner 24/ago/2026 por `ACTO SELLA-AGO24`, que había quedado rezagado. Denominador 14 → 15 el 13/ago/2026, `ACTO PROC-11` ejecutando ADR-75(b) — ver §1.1.F Paso 6. Antes, corregido 4/ago/2026, Encargo K — `forense/notas/2026-08-04-medicion-exposicion-violencia-envipe.md` §3-5). De las 15 (desglose de categorías sin recontar en este acto — solo el numerador de este titular se corrige aquí, fuera de perímetro tocar el resto): **9 MEDIDO·PARCIAL** (`confianza_institucional[salud/educación/financiera/seguridad-FFAA/justicia-policía/electoral-partidos]`, 6 de 6, más `radio_confianza`, `familismo_apoyo` y `exposicion_violencia`), **2 MEDIDO·NACIONAL** (`norma_de_género`, ENASIC 2022 `P7_12_7`; `obligación_medida`, ENASIC 2022 `P6_38` — ambas marginales nacionales, sin eje condicionante, `x = ∅`, límite de escala declarado entre ellas — no se comparan), 0 sin reactivo verificado — búsqueda abierta (vacía desde 4/ago/2026: `exposicion_violencia` salió por medición exitosa, no por agotamiento de la condición de caducidad de ADR-52 A, que sigue intacta como criterio, ver §1.1.F Paso 5), 2 sin reactivo — búsqueda cerrada, límite de régimen (`aversion_riesgo`, `sens_estatus`; `sens_estatus` cerrada 4/ago/2026 por ADR-54, `gobernanza` §4, ver §1.1.F Paso 5), 2 solo proxy, y 0 medida sin clase de procedencia (categoría vacía desde 13/ago/2026 — `obligación_medida` salió por entrada a `MEDIDO·NACIONAL`, `ACTO PROD-P638`).
...
- **Las 15 familias de distribución exigidas por ADR-28.d no están declaradas** — mientras falten, el check de varianza intra-celda no puede correr (§1.2, §6).
...
- **El corte de `edad`**: tres de las doce hipótesis (H-02, H-06, H-07), tres de las diez reglas traducidas (`R1.4`, `R2.4`, `R5.4`) y el descriptor 5 dependían de un umbral de "joven" que ningún inventario fijaba — la intuición de que existe una cohorte divergente era fuerte en el corpus; el corte que la operacionaliza no existía. **Fijado 19/ago/2026 por `ACTO CORTE-EDAD-CONVENCION`**: `edad` joven = 15-29 años, **convención declarada** (INEGI/ENOE, firma de mesa D-2 `ADR-110(b)`, §1.1.A), no derivación empírica — `CORTE-EDAD-EMPIRICO` (acto gemelo, en cola) puede corregirla con dato mexicano propio. Los nueve sitios textuales quedan propagados (§1.1.A); ninguna hipótesis ni regla cambió de veredicto, solo de estatus de determinabilidad donde el corte era el único bloqueo.
...
| **`D` = 15 condicionales** | 15 − 1 (`acceso_digital` pasa a atributo, criterio C3) **+ 1** (`obligación_medida`, registrada tras el reencuadre — ADR-75(b)) | **Sí** (§1.1.F, pasos 2 y 6) |
...
| **49 reglas · 27 de perímetro · 26 de 27 corridas · 20 `[FUERTE]`**<!-- T20:HITO-D pob=reglas --> | 49/27/20 sin cambio respecto del v3.4, recuento mecánico sobre §3.B, reproducible con `python3 tests/validador_registro_ids.py`; corridas 2→3 por `R7.2` → `D`, archivado 4/ago/2026 (ADR-45); 3→4 por `R4.2` → `D`, archivado 4/ago/2026 (ADR-55, Encargo Y); 4→8 por `R4.1`/`R9.1`/`R4.3`(A,B)/`R9.2` → `D`, archivados 4/ago/2026 (ADR-56, Encargo Z); 8→11 por `R5.1`/`R5.2` → `A` y `R1.2` → `E`, archivados 4/ago/2026 (ADR-58); 11→12 por `R3.1` → `B`, archivado 4/ago/2026 (ADR-60) — tier `[FUERTE]` de `R3.1` sin tocar, 20 `[FUERTE]` sin cambio; 12→13 por `R1.3` → `E`, archivado 5/ago/2026 (ADR-63, Encargo CU); 13→14 por `R8.1` → `D`, archivado 20/ago/2026 (ADR-138); 14→15 por `R7.1` → `A`, archivado 24/ago/2026 (ADR-145); 15→16 por `R7.3` → `C`, archivado 24/ago/2026 (ADR-155, `ACTO SELLA-AGO24-C-v2`, firma de mesa `FP-112`, primera fila `C` del programa); 16→18 por `R7.4`/`R7.5` → `D`, archivados 24/ago/2026 (ADR-158, `ACTO ADQ-CORRE-R74R75`, falsador compartido inejecutable por diseño de instrumento en las tres fuentes reales adquiridas para probarlo) — tiers `[MEDIA-FUERTE]` de ambas sin tocar; 18→19 por `R8.3` → `A`, archivado 25/ago/2026 (ADR-186, `ACTO SELLA-ABRIDORES-R83-R14`, firma de mesa RANURA 1) — tier `[FUERTE]` de `R8.3` sin tocar; 19→20 por `R1.4` → `D`, archivado 25/ago/2026 (ADR-187, mismo acto, firma de mesa RANURA 2, resolviendo la colisión de `ADR-182`; fila `C` de la escala de `R1.4` reescrita por SÍ de mesa, enmienda fechada en `hitoD-preregistro`); 20→21 por `R10.2` → `D`, archivado 25/ago/2026 (ADR-196, `ACTO PROPAGA-LETRAS`, firma de mesa «R10.2: a», cruce `#363` confirma `SIN-DATO-GENUINO`) — tier `[MEDIA-FUERTE]` de `R10.2` sin tocar, 20 `[FUERTE]` sin cambio; 21→23 por `R8.2` → `B` y `R2.2` → `D`, archivados 26/ago/2026 (ADR-199, `ACTO CIERRA-4-FIRMAS`, firmas de mesa L1/L2; censo `forense/R22-censo-liderazgo-v1_0.md` corrido para `R2.2`, veredicto `SIN-DATO`) — tier `[MEDIA-FUERTE]` de `R2.2` sin tocar (el ataque nunca corrió), 20 `[FUERTE]` sin cambio; 23→24 por `R3.4` → `B`, archivado 26/ago/2026 (ADR-201, `ACTO CIERRA-FP157`, firma de mesa, enmienda fechada al Respaldo 2 del pre-registro) — tier `[MEDIA-FUERTE]` de `R3.4` sin tocar, 20 `[FUERTE]` sin cambio; 24→25 por `R10.1` → `C`, archivado 26/ago/2026 (ADR-205, `ACTO CORRE-R10.1-v2` Fase B, RANURA 2 de mesa) — 20 `[FUERTE]` sin cambio; 25→26 por `R2.1` → `D`, archivado 26/ago/2026 (ADR-208, `ACTO MAESTRA30-E10 · R21-ADJUDICA`, firma de mesa RANURA M-R21, censo `forense/R21-censo-fuentes-v1_0.md` en cero `EXISTE-SATISFACE`) — tier `[FUERTE]`/`v2` `[FUERTE como correlación]` de `R2.1` sin tocar, 20 `[FUERTE]` sin cambio | **Heredado, con receta ejecutable — corridas actualizado 26/ago/2026 (`ACTO MAESTRA30-E10 · R21-ADJUDICA`)** |
