...
FP/ADR: ADR-329 (deriva) · FP-289 recibo · FP-290 sólo si INDETERMINADO > 0 (lista para mesa, con fecha vence a 7 días). CONTADOR: encargos sin marca 166 → declara el real · medición: cero.

## CONSUMIDO

Ejecutado el 3/sep/2026 en entorno NUBE con la skill `/acto` (`ADR-237`),
rama `claude/auditoria-encargos-maestra37-p8k51b`, cascada en `ADR-333`
(candidato original `ADR-328`; renumerado tres veces por colisiones
sucesivas de merge — `origin/main`/`PR #523` (`MAESTRA37-N8`, `ADR-328`/
`ADR-329`) → candidato `330`; `PR #526` (`MAESTRA37-A2`, mismo candidato
independiente) → candidato `331`; `PR #524` (`MAESTRA38-A1`, `ADR-331`/
`ADR-332` propios) → **`ADR-333`** final — regla de la casa, "renumera
quien fusiona segundo", aplicada tres veces; ver detalle en `canon/gobernanza-v1_15.md`).
...
`FP-290`, vence 2026-09-10) · **13 `## NO-EJECUTADO`** · **0
...
Recibo `FP-289`. Deuda abierta `FP-290` (los 46 `INDETERMINADO`, mesa
