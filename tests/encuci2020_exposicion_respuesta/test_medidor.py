...
SCRIPT = ROOT / "data/corrida0/CALC-ENCUCI2020-EXPOSICION-RESPUESTA-0001/medidor.py"
...
def test_partitions_and_empty_conditional_denominator():
    ...
    assert abs(result["RESULT-ENCUCI2020-ER-VALIDACION-PARTICION"]) < 1e-12
    assert abs(result["RESULT-ENCUCI2020-ER-VALIDACION-UNION"]) < 1e-12
    counts = json.loads(result["RESULT-ENCUCI2020-ER-CONTEO-TIPOS"])
    assert {x["numero_tipos"]: x["n_categoria"] for x in counts} == {
        "0": 0, "1": 2, "2": 1, "3+": 1}
    joint = {x["grupo"]: x for x in json.loads(
        result["RESULT-ENCUCI2020-ER-RESPUESTA-CONJUNTA"])}
    assert joint["tipos_2"]["p_entrega_dado_solicitud"]["p"] is None
    assert joint["total_contacto"]["distribucion"]["ambas"]["p"] == 0.25
