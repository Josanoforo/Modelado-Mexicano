...
_T25_ARCHIVOS_CONOCIDOS = {
    ...
    # ACTO MAESTRA38-N22, 7/sep/2026: encargo archivado VERBATIM (A.3,
    # por cherry-pick de d8deb11). Su bloque CARRILES nombra "la rama de
    # M13 ENMIENDA-1" entre los carriles de nube que corren en paralelo,
    # para declarar que ninguno toca `forense/prereg-caja/S6-*`. `M13`
    # pelado es mencion de perimetro ajeno (forense/prereg-duelo-v2/), no
    # un rotulo que este acto instancie -- mismo caso que L16-BIS-2,
    # arriba. El encargo verbatim no se edita para complacer un test.
    ...
    # encargo archivado VERBATIM (A.3). Direccion nombra tres veces al
    # acto de tramite en vuelo como "E1" pelado (dos veces al declarar
    # la concurrencia entre los dos actos, una en la linea de A.8), y
    # "E1" pelado ya es habitante censado del espacio E en
    ...
    # declara ACTO MAESTRA33-E2 en todo archivo que escribe y queda
    # censado en canon/registro-rotulos.tsv; el encargo NO se edita,
...
GUARDIA-TSV-Y-CAPA2-LISTAS, 3/sep/2026. Un round-trip
    ...
    dentro de una nota, ninguna intencional). Dos actos re-midieron el
    mismo día por caminos distintos y ambos quedaron parcialmente
    obsoletos al fusionar juntos: `ACTO MAESTRA37-A2 · REVISA-COLA-A-DETALLE`
    (28 `nota` con texto de agentes, muchas con comillas dobles como
...
_T_YAMEDIDO_ARCHIVOS_CONOCIDOS = {
    ...
    # ACTO MAESTRA38-N22, 7/sep/2026: el encargo se archiva VERBATIM
    # (A.3) y cita `R4.4` en su cuerpo original, redactado por direccion
    # antes de esta sesion -- un encargo verbatim nunca se edita para
    # complacer un test (misma regla que rige T25). La verificacion A.8 de
    # este acto NO es una corrida de `tools/ya_medido.py`: es la
    # verificacion de EXISTENCIA DEL REACTIVO contra
    # `data/inventario-reactivos-ext-v1_0.tsv` (comandos y salidas en el
    # §0.2 de `forense/prereg-caja/S6-L16-spec-v1_2.md`), que es lo que el
    # encargo pide. `NUNCA-MEDIDA` para `salud.atencion.grave`/`R4.4` ya lo
    # establecio `S6 v1.1 §0.1` con la salida cruda de `ya_medido.py`
    # pegada, y `v1.2` no la re-corre ni la contradice.
    "forense/encargos/2026-09-07-MAESTRA38-N22.md",
