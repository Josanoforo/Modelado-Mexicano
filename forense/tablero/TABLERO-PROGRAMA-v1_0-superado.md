**SUPERADO POR `forense/tablero/TABLERO-PROGRAMA.md` — ACTO MAESTRA38-TRAMITE-3, 7/sep/2026, decisión de mesa "1 solo archivo con un solo nombre, TODO que haga referencia se alinea a ello".** Este archivo (contenido v1.0, snapshot 2/sep/2026) queda congelado, no se borra (historia recuperable, además preservada íntegra en `git log`); el archivo vigente para toda referencia nueva es `forense/tablero/TABLERO-PROGRAMA.md`, que a partir de este acto lleva el contenido v1.1 (snapshot 2/sep/2026, más reciente que este v1.0) bajo un solo nombre sin sufijo de versión — el adjunto TABLERO-PROGRAMA-v1_5.md (adjunto nunca llegado al repo) que el encargo citaba como fuente NO llegó a esta sesión ni está en el repo; ver `## CONSUMIDO` de `forense/encargos/2026-09-07-MAESTRA38-TRAMITE-3.md` para el PARO parcial declarado sobre esa pieza.
...
**Nota 2026-09-04 (recibo, `ACTO MAESTRA38-N5 · DISEÑO-9-REGLAS-SIN-INSTRUMENTO`, NUBE, `FP-297` — recibo, no requiere firma).** Clasifica con evidencia las 9 reglas `NO-ENCONTRADO` que el censo de cierre de `MAESTRA38-A1` dejó sin falsador (`forense/notas/2026-09-03-MAESTRA38-A1-censo-9-no-encontrado.md`): **2 REFORMULABLE** (`civico.voto.clientelar_si_observable` vía LAPOP AmericasBarometer México 2019, ítems `clien1n`/`clien1na`/`clien4a`/`clien4b`, ya en corpus; `civico.protesta.agravio_urbano` vía LAPOP multi-ola, `PROT1`/`PROT2`/`prot3` + `VIC1`/`vicbar4a` + `AOJ12` + `CP6`/`CP9`/`LAPOP-E8` + `TAMANO`), **5 SIN-INSTRUMENTO** (`tramite.evasion.norma_inutil_sancion_improbable`, `dinero.ahorro.seguro_deposito_atenua_aversion`, `civico.voto.agencia_con_secreto`, `civico.transferencia.atribucion_lider`, `familia.cortejo.urbano_joven_apps`, cada una con instrumento hipotético mínimo y recomendación `MANTENER-COMO-HIPÓTESIS`), **2 CON-CANDIDATA** (`dinero.credito.scoring_alternativo` vía CNBV — objeto administrativo, estructuralmente invisible a `busca_reactivos.py`, que solo indexa reactivo de hogar; `dinero.credito.baja_friccion_usura_dano_downstream`/`N34` vía ENCRIGE FD completo + CONDUSEF). Universo de búsqueda: `data/inventario-reactivos-descargas-mx-v1_1.tsv` (42536 filas, superset de la tabla que usó el censo de A1 — la diferencia es por qué esta pieza sí encuentra señal donde A1 declaró cero). Cero medición, cero regla cerrada, cero canon tocado (perímetro explícito del encargo: `NO toca: canon/**` sin la excepción "salvo ADR" que sí traía `N3` — no se abre `ADR-338`, la propagación a canon queda para el sucesor que dispare mesa). Tabla `PENDIENTE-DE-MESA` completa en `forense/notas/2026-09-04-MAESTRA38-N5-diseno-9-reglas.md` §3; `FP-298` (decisión de mesa, vence 7 días, 11/sep/2026). Este snapshot del tablero sigue sin regenerarse (mismo `adr_max`/`fp_max` obsoletos de las notas de arriba).

**Nota 2026-09-04 (recibo, `ACTO MAESTRA38-N6 · PROPAGA-FP298-TESTS-Y-A3`, NUBE, `FP-299` — recibo, no requiere firma; `ADR-338`).** `FP-298` → `EJECUTADA`: mesa acepta la clasificación de la nota de arriba sin excepción de origen. Carga en `milpa/tramite-ola5-propuesta-v0.yaml`: **3 `HIPÓTESIS-SIN-INSTRUMENTO`** (`tramite.evasion.norma_inutil_sancion_improbable`, `civico.transferencia.atribucion_lider`, `familia.cortejo.urbano_joven_apps` — vocabulario nuevo, documentado en `.claude/commands/mapea.md` §4) y **2 `REFORMULABLE`** como tercera formulación complementaria (`civico.voto.clientelar_si_observable_lapop2019`, `civico.protesta.agravio_urbano_multiola`), sin reabrir los sellos ya vigentes de `MAESTRA35-L9`/`L11`. Dos filas nuevas en la cola de adquisición (`CNBV_PORTAFOLIO_INFORMACION_IMOR_CONSUMO`, `ENCRIGE_2020_FD_COMPLETO_MAS_CONDUSEF`, vía el writer canónico), `PAQUETE-RECETAS-6` (0 de 2 recetas verificables — red bloqueada en `NUBE`, declarado). **Hallazgo (D-13):** las otras 2 `SIN-INSTRUMENTO` de la nota de arriba (`dinero.ahorro.seguro_deposito_atenua_aversion`, `civico.voto.agencia_con_secreto`) NO se cargan — ya tenían instrumento medido en `MAESTRA35-L9`/`L11` (`R1.5` `NO-DISCRIMINA`; `R7.3` `CONTRARIA-REPLICADA`, degradada a `[MEDIA]` por la Enmienda D2-f) que `MAESTRA38-N5` no cruzó al clasificar. `T-A3`/`T-FIRMAS-2` nuevos en `tests/check.py`, `B24` cerrado (§5): repara el A.3 perdido de `MAESTRA38-N1-lite` (archivado post-hoc, `## CONSUMIDO` con `PR #530`/`ADR-335`). `tests/baseline.json` sin recifrar por `P2`/`P3`: LÍNEA BASE **VERDE**, 3 FAIL / 170 WARN. **Enmienda post-cierre, mismo día (CI de `PR #535`).** Subir `gobernanza` a `338 ADR` desincronizó `canon/estado-programa-v1_11.md` (`T15` a `ROJO`, 3 entradas) — el acto original no lo tocó por perímetro (carril `N8`, disjunto); con la CI del PR realmente roja, dirección pidió resolverlo ahí mismo, autorización puntual para esta cifra mecánica, y `canon/estado-programa-v1_11.md` se recifró (`337`→`338 ADR`). Detalle: `forense/encargos/2026-09-04-MAESTRA38-N6-PROPAGA-FP298-TESTS-Y-A3.md` `## CONSUMIDO`.

**Nota 2026-09-04 (recibo, `ACTO MAESTRA38-N7 · PRE-REGISTRO-CIVICO-LAPOP`, NUBE, `FP-300` — recibo, no requiere firma; renumerado desde `FP-299`/sin ADR al fusionar `origin/main` — `PR #535`/`ACTO MAESTRA38-N6` fusionó primero contra `main` y tomó, de forma independiente, los mismos candidatos `FP-299`/`ADR-338`; regla de la casa, "renumera quien fusiona segundo").** Dos specs de pre-registro selladas, cero medición, ampliando el conteo de `forense/prereg-caja/` de 3 a 5: `S4-L4-spec-v1_0.md` (`civico.voto.clientelar_si_observable`, objeto de `N5 §2.6`, LAPOP 2019 `clien1n`/`clien1na` × `vb3n`) y `S5-L5-spec-v1_0.md` (`civico.protesta.agravio_urbano`, objeto de `N5 §2.8`, multi-ola 2004/2006/2019, `TAMANO` como estrato), cada una con su `.sha256`. **Hallazgo central de esta pieza, no citado por el encargo ni por `N5`, y sólo parcialmente por `N6`:** `MAESTRA35-L9`/`L11` (2/sep/2026) ya corrieron falsaciones reales de estos mismos dos `id` — `civico.voto.agencia_con_secreto` (`R7.3`, gemela de `clientelar_si_observable` en la misma disyunción) `CONTRARIA-REPLICADA` y degradada `[FUERTE]`→`[MEDIA]` por la Enmienda `D2-f` (**ya vigente en canon desde el 3/sep, antes del `SHA` de redacción de este encargo** — esta pieza no la había citado en su primer sello; corregido aquí, tras el merge con `PR #535`), y `civico.protesta.agravio_urbano` con su contraste `C2` (agravio×urbano) **`CARGADA-A-MOTOR`, tier `FUERTE`, `CORROBORADA-REPLICADA`** por la Enmienda `D2-d` (también vigente desde el 3/sep) — el contraste `C1` (entorno solo) queda `SELLADA-SIN-CARGA`, `[MEDIA]`, `AMBIGUA-ENTRE-INSTRUMENTOS`. `S4`/`S5` no repiten esas corridas ni las reabren: `S4` ataca el otro brazo de la disyunción de `civico.voto.clientelar_si_observable` (proximidad/focalización del reparto, único medible en la ola 2019 que trae la batería `clien*`, sin `D2-x` que lo toque) y declara por qué la celda de tres factores que pedía el encargo no es construible en una sola ola; `S5` completa el diseño de cuatro antecedentes que `L9`/`D2-d` dejaron en dos (agravio×urbano), con los dos reactivos (`AOJ12` falla estatal, `CP6`/`CP9` red previa) que el inventario `v1_1` —nacido un día después de `L9`— sí trae. `S5` corrige además la caracterización de `N5` sobre `LAPOP-E8` (ítem de aprobación normativa, no de asistencia propia). Ambas specs actualizadas para citar los ids estables que `FP-298`/`MAESTRA38-N6` ya cargó en la propuesta (`civico.voto.clientelar_si_observable_lapop2019`, `civico.protesta.agravio_urbano_multiola`) como el destino de su medición. `B24` — cerrado por `N6` (nota de arriba), no por esta pieza: la referencia a "`B24` parcial" del primer sello de esta nota se retira, D-13. No se abre ningún ADR (perímetro excluye `canon`; `ADR-338` ya no está libre — lo tomó `N6`). Este snapshot del tablero sigue sin regenerarse (mismo `adr_max`/`fp_max` obsoletos de las notas de arriba).

**Nota 2026-09-04 (recibo, `ACTO MAESTRA38-N8 · ESTADO-PROGRAMA-v1_12`, NUBE, `FP-301` — recibo, no requiere firma; `ADR-339`).** Crea `canon/estado-programa-v1_12.md`: re-deriva por comando cada cifra que `v1_11` (2/sep) citaba sin comando al lado o dejó envejecer — motor `10`→`20` reglas (`grep -cE '^  - id: ' milpa/tramite.yaml`), 19 con dato/1 sin dato, dominios activos `4` (`ADR-265`), corredor con `14` de `14` celdas sorteadas puntuables (`R∩M∩L` completa), Ola 6 `0` de `6` con `salud` en `2` de `5` (`ADR-327`), manifiesto `1 040`→`1 281` payloads, relaciones activas `222`/procedencias aceptadas `223`/`utilidad-modelo.tsv` 1:1 sin error (`tools/curador_registro/baseline.py`), ADR máximo `338`→`339` y FP máximo `300`→`301` (con las propias entradas de este acto), las 9 reglas `NO-ENCONTRADO` clasificadas `2/5/2` (`MAESTRA38-N5`), specs de caja selladas `3`→`5`, FAIL absorbidos `3` (`tests/baseline.json`), y 6 filas `PENDIENTE-DE-MESA` de `PAQUETE-RECETAS-5`/`-6`. Añade §9 «Qué espera a la caja» (`MAESTRA38-A2`/`L2`/`C1`/`L4`/`L5`) y §10 «Qué no se sabe sin caja» (`C1` físico real, `[CENSO]`, `ENFIH-4`/`FP-288`). **Desviación D-13 declarada contra el propio encargo:** `estado-programa-v1_11.md` **no** queda intacta — `T01` (una sola versión viva de `canon/estado-programa-v*.md`) exige retirarla del árbol, mismo mecanismo y mismo precedente que retiró `v1_10` al nacer `v1_11` (`tests/check.py:239-248`); `git rm canon/estado-programa-v1_11.md`, historia recuperable con `git show 7574008:canon/estado-programa-v1_11.md` (commit de A.3 de este mismo acto). ADR re-derivado a `339`, no el `340` que citaba el encargo (`338` era el máximo real; `MAESTRA38-N7` no tomó ADR); `FP-301` sí coincide con el encargo. Cero medición, cero regla del motor tocada. Este snapshot del tablero sigue sin regenerarse (mismo `adr_max`/`fp_max` obsoletos de las notas de arriba — la única fuente de estado vigente es ahora `canon/estado-programa-v1_12.md`, no este tablero). Detalle completo en el `## CONSUMIDO` del encargo archivado: `forense/encargos/2026-09-04-MAESTRA38-N8-ESTADO-PROGRAMA-v1_12.md`.

**Nota 2026-09-05 (recibo, `ACTO MAESTRA38-N9 · YA-MEDIDO`, NUBE, `FP-302` — recibo, no requiere firma; `ADR-340`).** Instrumenta el defecto que `MAESTRA38-N5` y el encargo de `MAESTRA38-N7` repitieron la misma semana — clasificar/pre-registrar una regla sin cruzar antes las fuentes que ya la miden. `tools/ya_medido.py <id-de-regla|R-n>` (nuevo) cruza mecánicamente `milpa/tramite.yaml`, `milpa/tramite-ola5-propuesta-v0.yaml`, `canon/modelo-decision-v4_0.md` §7, `forense/notas/*-L*-*.md` y `forense/prereg-caja/S*-spec-*.md`, sin heurística de parecido — la equivalencia `id`↔`R-n` sale del registro congelado de `tests/validador_registro_ids.py` cruzado con el tag `**id:**` del propio canon. Control positivo verificado: los dos `id` de `MAESTRA38-N7` (`civico.voto.clientelar_si_observable`, `civico.protesta.agravio_urbano`) devuelven `MEDIDA-EN: ...L9...L11...`; control negativo: `familia.cortejo.urbano_joven_apps` devuelve `NUNCA-MEDIDA` (solo la hipótesis que `MAESTRA38-N6` cargó vía `FP-298`). `.claude/commands/mapea.md`/`acto.md` ahora piden pegar esa salida en A.8 antes de clasificar/pre-registrar/cargar/sellar una regla; `T-YAMEDIDO` (`tests/check.py`) lo exige mecánicamente para todo encargo archivado desde hoy que cite un `id`/`R-n` en su SPEC. `FP-301` (recibo de `MAESTRA38-N8`) recifrado `ABIERTA`→`FIRMADA-POR-MERGE` (`#537`, ya fusionado — regla 1 de maestra-34). Cero medición de México, cero regla del motor tocada; `data/INFRAESTRUCTURA-v1_0.md` gana una sección. Este snapshot del tablero sigue sin regenerarse (mismo `adr_max`/`fp_max` obsoletos de las notas de arriba). Detalle completo en el `## CONSUMIDO` del encargo archivado: `forense/encargos/2026-09-05-MAESTRA38-N9-YA-MEDIDO.md`.

**Nota 2026-09-05 (`ACTO MAESTRA38-N10 · COBERTURA-COMPLETA-OLA6`, NUBE, `FP-304` recibo + `FP-303` decisión de mesa pendiente, vence 7 días; `ADR-341`).** Mapa completo de las 25 reglas de los 6 dominios candidatos de Ola 6 (universo derivado y congelado por comando — `4+5+4+4+4+4`, no `~30` como estimaba el encargo) y plan de cobertura por dominio, mandato de mesa del 4/sep («no quiero hacerlo al mínimo»). `tools/ya_medido.py` corrido en las 25 antes de clasificar: `NUNCA-MEDIDA` en las 25, sin discrepancia contra `MAESTRA34-N5`/`MAESTRA36-N6` (preguntan cosas distintas: existencia de reactivo vs. falsación real corrida). Clasificación con evidencia cruzando **tres** universos independientes (encuestas `v1_2`/`ext` de `MAESTRA34-N5`; administrativo `manifiesto.yaml` de `MAESTRA36-N6`; `descargas_mx_v1_1`, **nuevo de este acto**, 75 corridas): **3 `MEDIBLE-COMO-ESTÁ`** (`salud.atencion.grave`, `salud.vacunacion.disponible` — ya conocidas — y `comunicacion.inseguridad.ver_oir_callar`, **hallazgo nuevo** vía módulo `AOJ` de LAPOP AmericasBarometer, población general no restringida a violencia de género, que ni N5 ni N6 tenían asignado), **3 `CON-CANDIDATA`** (`salud.adherencia.desabasto_vs_cuidadora` vía `N36`/Cero Desabasto, ya registrada; `cooperacion.comite.monitoreo_sancion_visible`/`cooperacion.faena.sancion_social_pueblo_mestizo` vía CNGMD, pendientes de abrir bytes), **19 `HIPÓTESIS-SIN-INSTRUMENTO`** con instrumento mínimo escrito cada una. **Criterio 2 de `motor-nucleo-medible-v1_0.md` §3.a, como consecuencia, no optimizado:** `0` de `6` dominios con lo medible hoy; **sigue `0` de `6`** aun sumando toda adquisición identificable. **6 de 6 dominios `COMPLETABLE`** — las 25 reglas, sin excepción, tienen ruta escrita (medir hoy / adquirir con ficha / instrumento mínimo de hipótesis). Cero canon tocado salvo la cascada mecánica de ADR/L0 (perímetro explícito: `N-siguiente` propaga lo que mesa firme, sella specs por dominio). Este snapshot del tablero sigue sin regenerarse. Detalle completo, seis tablas por dominio y tabla resumen: `forense/notas/2026-09-05-MAESTRA38-N10-cobertura-ola6.md`.

**Nota 2026-09-05 (recibo, `ACTO MAESTRA38-N11 · PRE-REGISTRO-OLA6-MEDIBLES-Y-FICHAS`, NUBE, `FP-305` — recibo, no requiere firma; `ADR-342`).** Sella tres specs de pre-registro (patrón `N7`) en `forense/prereg-caja/` para las tres reglas `MEDIBLE-COMO-ESTÁ` de `N10`: `S6-L16` (`salud.atencion.grave`), `S7-L17` (`salud.vacunacion.disponible`), `S8-L18` (`comunicacion.inseguridad.ver_oir_callar`). `tools/ya_medido.py` corrido en las tres antes de escribir: `NUNCA-MEDIDA`, sin excepción. **Tres correcciones de premisa (A.8/D-13):** `S6` declara que el árbol trae **dos** `EXISTE-SATISFACE` ya sellados para `salud.atencion.grave` sobre reactivos que no se solapan (`ENNVIH`+`ENDIREH` de `N5`/`MAESTRA37-L1` vs. `ENSANUT2024` de `MAESTRA37-L3`/`L3-BIS`), sin reconciliar entre sí — la afirmación de `L3` de heredar de `N5` no se sostiene contra el texto real de `N5`; pre-registra las dos ramas en paralelo. `S7` corrige que la ficha original de `N5`/`N10` cita variables que viven en `data/inventario-reactivos-ext-v1_0.tsv`, no en `v1_1`, y aporta un **hallazgo nuevo**: el bloque `a0927a1`-`a0927e4` de `adultos_ensanut2024_w.dta` prueba el `PORQUE` de la regla más directamente que la ficha original. `S8` corrige que el desenlace del módulo `AOJ` (`aoj1`/`aoj1a`/`aoj1b`) existe solo en la ola 2004, no en las cinco olas que `N10 §2.6` describe de corrido. Ficha, vía el escritor canónico, las tres `CON-CANDIDATA` en la cola de adquisición (cita `N36`/`N28` ya registradas; propone `N42` en nota para `cooperacion.faena.sancion_social_pueblo_mestizo`, que no tiene `N` asignada — verificado, sin editar `necesidad-objeto-modelo.tsv`, fuera de perímetro). `PAQUETE-RECETAS-7`: `SIN-FETCH` — las tres URLs de las fichas devuelven `000` desde NUBE (proxy de egreso, política de organización), 3 examinadas, 0 alcanzables. Corrige mapa (`canon/registro-rotulos.tsv`): `salud.vacunacion.disponible` es regla de §3.9, no de §3.4. No sella ninguna clasificación `MEDIBLE-COMO-ESTÁ`, no depende de `FP-303`. Cero medición de México, cero canon sustantivo tocado. Detalle completo en el `## CONSUMIDO` del encargo archivado: `forense/encargos/2026-09-05-MAESTRA38-N11-PRE-REGISTRO-OLA6-MEDIBLES-Y-FICHAS.md`.

**Nota 2026-09-05 (recibo, `ACTO MAESTRA38-N12 · SONDA-INSTRUMENTOS-DE-PERCEPCION`, NUBE, `FP-306` — recibo, no requiere firma; `ADR-343`).** Sonda, no sello. `COMMIT-1` congela los 19 instrumentos mínimos `HIPÓTESIS-SIN-INSTRUMENTO` de `N10 §2` (verbatim, una excepción declarada), la lista cerrada de 4 fuentes (*Los mexicanos vistos por sí mismos*/UNAM-IIJ 2015, `ECOPRED`/INEGI 2014, *Cultura Constitucional*/UNAM-IIJ, `MxFLS`/`ENNViH`) y el criterio de acierto; cero fuentes derivadas por homonimia de `N10 §5c` tras buscar en 15 catálogos locales. `COMMIT-2` sondea red por tres mecanismos independientes (`curl`, estado del proxy, `WebFetch`): **INEGI y UNAM bloqueados por igual** — las 3 fuentes externas quedan `SIN-FETCH`, fichadas `PENDIENTE` en la cola; `PAQUETE-RECETAS-8`: `0` recetas verificadas de ≤1 minuto. `MxFLS`/`ENNViH` corrige la premisa (e) del encargo: sí está indexada (`inventario-reactivos-ext-v1_0.tsv`, 17 181 filas) y ya fue agotada por `N5`/`N10` contra estas 19 filas — no «fuera del inventario». Las 19 quedan `SIN-COBERTURA-EN-ESTAS-FUENTES`, universo declarado; cuántas cambiarían de clase si las 3 fuentes se adquieren: **no estimable** sin abrirlas, no inventado. Módulo propio v0 (los 19 como ítem/escala/población): declarado, no lanzado. Enmienda a `FP-303` puesta en `forense/tablero/TABLERO-PROGRAMA-v1_1.md §8.11` (no en su fila de `firmas-pendientes.tsv`, fuera de perímetro). Cero medición de México, cero canon sustantivo tocado. Este snapshot del tablero sigue sin regenerarse. Detalle completo en el `## CONSUMIDO` del encargo archivado: `forense/encargos/2026-09-05-MAESTRA38-N12-SONDA-INSTRUMENTOS-DE-PERCEPCION.md`.
