...
Ejecutado por `PR #636` (`ACTO GEN2-FIRMA-CONTADOR · tres filas en decisiones.tsv`), rama `claude/hopeful-dirac-nftpw4`. Cascada de cierre: `ADR-413` (`canon/gobernanza-v1_15.md`), `L0` recifrado (`canon/estado-programa-v1_12.md`), rótulo `GEN2-FIRMA-CONTADOR` censado en `canon/registro-rotulos.tsv`. `python3 tests/check.py --baseline` → ROJO, con exactamente las dos entradas declaradas y esperadas (T35 211 fail, T-CORRIDA0 2 fail, ambas `NC-0053`) frente a la línea base — ninguna otra nueva.
