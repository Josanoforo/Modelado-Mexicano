...
SPEC_SUFIJO = "celda-d-piloto-v1_0"
...
def spec_de(celda: str) -> SpecCelda:
...
def ruta_salida(celda: str, variante: str, indice: int) -> Path:
    return CORRIDAS_L / f"CD-DIN-{celda}__{variante}__{indice:02d}__{SPEC_SUFIJO}.json"
...
def main() -> int:
    ...
    if args.variante == "L+corpus":
        print("L+corpus: PARO-PREMISA -- `paquete-corpus-F5-v1_0/manifiesto.json` "
              "no tiene entrada para ninguna de las 8 celdas de este piloto; "
              "sin ella el runner cae al placeholder historico y la dieta "
              "declarada no se cumple. No se captura.", file=sys.stderr)
        return 2

    tuplas = plan(args.variante)
    params = params_de(args.variante)
    print(f"plan: {len(tuplas)} capturas · variante={args.variante} · "
          f"k={K_CORRIDAS} · modelo={MODELO_ALIAS} · orden semilla={SEMILLA_ORDEN}")
    if args.dry_run or not args.correr:
        muestra = construir_prompt(spec_de(tuplas[0][0]), params)
    ...
    for n, (celda, variante, indice) in enumerate(tuplas, 1):
        destino = ruta_salida(celda, variante, indice)
        if destino.exists():
            saltadas += 1
            continue
        spec = spec_de(celda)
