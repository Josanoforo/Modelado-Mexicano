> **PROCEDENCIA — archivado histórico, ACTO LOTE·NUBE-DECISIONES-1/T2, 19/ago/2026 (FP-62).**
> Texto verbatim de `forense/notas/2026-08-13-reconcilia-puertas.md` tal como vivía en la rama
> `rescate/reconcilia-puertas-local` (`f169abd`, empujada 19/ago/2026 por `ACTO CAJA-RESIDUOS`,
> nunca fusionada) — sha256 `49e9f31d533e5a51b08a45f198ae8d3b91db730650349c74e0e562227c6d7d6d`,
> 165 líneas. Es una de **dos ejecuciones independientes y ciegas entre sí** del mismo
> `ENCARGO RECONCILIA-PUERTAS` (13/ago/2026): esta corrió en `caja` (worktree local); la otra
> corrió en `nube` y se fusionó como `PR #208`, y su texto — con método más estricto (evidencia
> por URL/cita, nunca por parecido de nombre) y 16 pares identificados en vez de 21 — es el que
> vive hoy en `forense/notas/2026-08-13-reconcilia-puertas.md` en `main`.
>
> **Veredicto de `FP-62`, derivado por comparación de contenido, no heredado:** el defecto de
> fondo que ambas versiones diagnostican — filas `gap_mapeo_map_b` (universo interno, sin
> sondeo) sin retirar frente a su fila real gemela en `data/universo-puertas-*.tsv` — **ya está
> resuelto en el árbol actual**, por un tercer acto que ninguna de las dos versiones conocía:
> `ACTO FP10-PRECEDENCIA` (`ADR-115`, 19/ago/2026) aplicó el diff de 16 pares — el conteo y el
> método de la versión de `main`/`PR #208`, no los 21 de esta rama — contra
> `data/universo-puertas-2026-08-14.tsv`: `122→106` filas, `gap_mapeo_map_b` `61→45`, verificado
> comando por comando en `forense/notas/2026-08-19-fp10-precedencia.md`. La propuesta propia de
> esta rama (§7-§9 abajo: columna `superseded_por`, marcar sin borrar, `ADR-74` sin sellar) **no
> es la que se ejecutó** — `FP10-PRECEDENCIA` borró las filas stale en vez de marcarlas — pero el
> hallazgo de fondo (cuáles filas están duplicadas y por qué) coincide en 16 de los 21 pares que
> esta rama cuenta, así que **DUPLICA** sobre esos 16: mismo diagnóstico, ejecutado por otra vía.
> Y `FUSION-PUERTAS` (`ADR` de hoy, `FP-12` → `CERRADA`) reafirma, citando expresamente a
> `ACTO RECONCILIA-PUERTAS` del 13/ago, la misma conclusión de no-fusibilidad de esquema que
> ambas versiones — ésta en su §1, la de `main` en su §2.3 — ya habían medido: tampoco ahí hay
> contradicción.
>
> **Lo que esta rama traía y ningún acto posterior tocó — rescatado, no descartado:** su
> Addendum 2 (§11 abajo) documenta, en vivo, que `P·LOTE-2` creó filas `_adquisicion_PLOTE2`
> duplicadas (real contra real, no gap contra real) para 4 fuentes — `INEGI_ENCOAP_2023`,
> `Banxico_EncuestaCompetenciasFinancieras`, `Zenodo_ElectoralPrecinctLevel_MexicoMunicipal`,
> `OSF_InteractingAsEquals_PartisanPolarizacion_Mexico` — mientras esta rama corría. Verificado
> hoy, en fresco, contra `data/universo-puertas-2026-08-14.tsv`: **las 4 parejas siguen presentes
...
> "`FP-29` (`PR #275`, `ADR-111`)" — verificado contra el árbol: `FP-29` es la fila de
> reconciliación de fuentes externas de `confianza radial` (el 22%, WVS/Pew/Latinobarómetro),
> **sin relación temática con `universo-puertas`/ADR-69/ADR-70**. Las filas que sí gobiernan este
> solape, cerradas ambas hoy mismo, son `FP-10` (`ADR-115`, `ACTO FP10-PRECEDENCIA`) y `FP-12`
> (`ACTO FUSION-PUERTAS`) — usadas arriba. No se edita `FP-29` ni se le atribuye nada de esto.
>
