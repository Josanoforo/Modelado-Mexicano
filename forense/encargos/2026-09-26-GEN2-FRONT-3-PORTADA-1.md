...
Con `pyproject.toml` ya en la raíz, `requirements-dev.txt` sale a `archivo/` (`40a1829a`; rama prevista en P1). `requirements.txt` se queda porque la CI (`verify.yml`, ajeno) lo instala, y `uv.lock` acompaña a `pyproject.toml`: la raíz queda con 12 archivos. El bloque T-MEM de `canon/MEMORIA-OPERATIVA.md` y `herramientas.tsv` se regeneraron por comando (`tools/memoria_operativa.py --escribe`). Detalle en la nota, §Tras el merge.
