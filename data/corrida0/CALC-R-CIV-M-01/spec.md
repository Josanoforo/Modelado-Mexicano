...
**Celda:** `CIV-M-01` — ENVIPE 2012, **delitos de 2011**.
**Releva:** `CORR-0024` → `RES-0093`, el árbitro `R` de esta celda del marco `M`.
Consumidor natural: `forense/prereg-duelo-v2/marco-M-sorteado-v1_3.tsv:CIV-M-01:R`.
...
1. **PRIMARIO — el árbitro `R`.** Proporción ponderada de delitos con razón principal de
   ...
   ponderador `FAC_DEL`, diseño `EST`/`UPM`. Codificación, universo, ponderador y
...
2. **SECUNDARIO HOMOLOGADO — el punto de serie.** Mismo desenlace bajo `U1` y `C1` de
   `prereg-caja-ENVIPE-DENUNCIA`: delitos **personales** (`BPCOD ∈ {04,…,14}` **en esta ola**),
   `BP1_20 = 2`, denominador `{01…08}`. Se emite también `C2` (partición GEN1) y el delta.

**⚠️ Esta ola lleva el corrimiento de `BPCOD`.** Su catálogo tiene **catorce** códigos (`01…14`), no quince, y su bloque personal es **`{04,…,14}`**, que equivale uno a uno al `{05,…,15}` de 2013/2015. Las once parejas de etiquetas están verbatim en la spec sellada §3.1, y el residuo (código moderno `03`, vandalismo, del bloque hogar, fuera de `U1` por construcción) en §3.2.
...
| campo | valor |
|---|---|
| `id` de manifiesto | **`envipe_2012_base_de_datos_envipe_2012_dbf`** |
| miembro del ZIP | `Tmod_Vic.DBF` — literal, nunca por plantilla |
| filas (cabecera DBF) | 32,493 |
| descriptor citado | `FD_ENVIPE12.zip -> FD_ENVIPE_2012.xls, hoja TMod_Vic` — `BP1_23` fila 231, `BPCOD` fila 26, diseño filas 621 (FAC_DEL), 627 (EST), 628 (UPM) |
...
| `FAC_DEL` | `FAC_DEL` | Caracter(6) |
| `ESTRATO` | **`EST`** | Caracter(3) |
| `UPM` | **`UPM`** | Caracter(5) |
...
No abre `tools/arbitra.py` ni `corridas-R/CIV-M-01.json`: el control positivo es script
