...
GUARDIA-TSV-Y-CAPA2-LISTAS, 3/sep/2026. Un round-trip
    ...
    147, 148, 149).
    Re-medido el 16/sep/2026 al añadir el residual público
    `ENPOL_2016_MICRODATOS_CSV` por la misma vía canónica: **30 líneas**; la
    nueva línea 153 contiene metadata JSON con comillas dobles.
    Este test es DOBLE:
    ...
    (1) CONTROL, documenta que el defecto sigue vivo con `csv`: si algún
        ...
        quedar en silencio -- se esperan EXACTAMENTE 30 líneas distintas
    ...
    if len(diffs_csv) != 30:
        fail("T26-bis", f"control: round-trip csv sobre cola-adquisicion-registro.tsv debe dar "
                         f"30 líneas distintas (20, 29, 35, 37, 38, 40, 41, 47, 50, 51, 63, 94, "
                         f"97, 114, 117, 119, 121, 123, 124, 125, 136, 139, 140, 144, 145, 146, "
                         f"147, 148, 149, 153), re-medidas el 16/sep/2026 tras añadir el residual "
                         f"ENPOL_2016_MICRODATOS_CSV con `tsv_crudo.upsert_fila`; hoy da "
...
def t42_digesto_candidatas():
    """T42 · T-CANDIDATAS -- P1 de `ACTO GEN2-OPERACION-1 ·
    CICLO-CONCILIACION-PAGABLE`
    (`forense/encargos/2026-09-09-GEN2-OPERACION-1-CICLO-CONCILIACION-PAGABLE.md`).
    Corre `tests/test_digesto_candidatas.py`: falsadores dirigidos de la
    sección K de `tools/digesto_tramite.py` (localización de candidatas)
    -- mismo arnés que T39/T40/T41."""
    ruta = os.path.join(ROOT, "tests", "test_digesto_candidatas.py")
    if not os.path.exists(ruta):
        fail("T-CANDIDATAS", "no existe `tests/test_digesto_candidatas.py`")
        return
    try:
        import importlib.util as _iu
        _spec = _iu.spec_from_file_location("test_digesto_candidatas_desde_check", ruta)
        _mod = _iu.module_from_spec(_spec)
        sys.modules[_spec.name] = _mod
        _spec.loader.exec_module(_mod)
        fallos = _mod.corre()
    except Exception as exc:
        fail("T-CANDIDATAS", f"`tests/test_digesto_candidatas.py` no pudo correr: "
                              f"{type(exc).__name__}: {exc}")
        return
    for f in fallos:
        fail("T-CANDIDATAS", f)
