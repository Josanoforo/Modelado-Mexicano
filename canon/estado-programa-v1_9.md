...
**L5 · Validación — el estrato más problemático.** **Hito D** (reglas del motor; perímetro de 27, subconjunto de las 49 del motor): **4 de 27 corridas archivadas** — `R1.1` → veredicto `D`, `R3.2` → veredicto `B`, `R7.2` → veredicto `D`, `R4.2` → veredicto `D` (bloque append-only de `hitoD-preregistro`, ver §7). *(Corregido 4/ago/2026, Encargo Y — propaga `R4.2` → `D`, archivado en `hitoD-preregistro` Nota 17, ADR-55. `R5.2` y `R1.2` corrieron completas en el mismo acto pero no se archivaron — Notas 18/19, propuestas, no adjudicadas.)* 49 refutaciones narrativas corridas *(población distinta, `milpa/refutations.yaml`, ref.A.*)*: 27 pasan, 3 fallan, 8 sin objeto, 11 requieren el ejecutable. **15 coeficientes, cero medidos.** Cuatro generadores sin falsar (**Hito C**, distinto de lo anterior — ver `hitoC-prueba-generadores`).
...
- **45 de 49 reglas del motor sin prueba de falsación pre-registrada corrida** (4 corridas archivadas: `R1.1` → `D`, `R3.2` → `B`, `R7.2` → `D`, `R4.2` → `D`). **Dentro del perímetro decidido del Hito D (27 reglas, subconjunto de las 49): 23 de 27 sin corrida.** *(Corregido 4/ago/2026, Encargo Y — propaga `R4.2` → `D`, ADR-55.)*
...
**Paso 2 — EN CURSO. 4 de 27 corridas.** *(Corregido 4/ago/2026, Encargo Y — propaga `R4.2` → `D`, ADR-55.)*
