# Informe del programa · v1.4 — Benchmark del Mexicano
...
26 de septiembre de 2026 (cierre de la semana del 22 al 26).

### `informe-programa` · **v1.4** · DOCUMENTO DEL PROGRAMA
...
Dictamen por serie de `GEN2-DONDE-CAMBIO-EL-MEXICANO-1` (`canon/donde-cambio-el-mexicano-v1_0.md`, RETROSPECTIVA, sin adopción): de **7 872** series <!-- comando: python3 forense/analisis/informe-v1_4/cifra.py cat:dc:series -->, **8** muestran `CAMBIO-SOSTENIDO` (3 suben, 5 bajan) <!-- comando: python3 forense/analisis/informe-v1_4/cifra.py cat:dc:CAMBIO-SOSTENIDO -->, **583** son `ESTABLE` <!-- comando: python3 forense/analisis/informe-v1_4/cifra.py cat:dc:ESTABLE -->, **81** tienen un `SALTO-SIN-EXPLICAR` <!-- comando: python3 forense/analisis/informe-v1_4/cifra.py cat:dc:SALTO-SIN-EXPLICAR --> y **7 200** no tienen tres olas comparables (`SIN-SERIE`) <!-- comando: python3 forense/analisis/informe-v1_4/cifra.py cat:dc:SIN-SERIE -->.

Lectura: los cambios sostenidos son de ENOE y de décimas de punto. Fuera de esas series, la frase «el mexicano cambió en X» **no tiene respaldo en este corpus**. «Cambió» describe una serie, no una psicología.

## C · Cobertura por clase

`GEN2-CLASE-AMAI-2` (`forense/analisis/clase-amai/cobertura-por-clase-v1_1.md`) calibró el eje NSE AMAI contra ENIGH 2024, en una apertura acotada a los seis componentes AMAI. Por la firma A4, el eje entra solo en ENIGH 2022, ENIF 2024 y ENDUTIH 2023 (esta última como aproximación rotulada). El catálogo v1.2 trae pisos por NSE en **144** celdas publicadas por el acto emisor <!-- comando: python3 forense/analisis/informe-v1_4/cifra.py cat:nse:celdas -->, y la tabla de piso v1.1 las marca con `eje_nse_o_region = NSE` <!-- comando: python3 -c "import csv,sys;csv.field_size_limit(sys.maxsize);print(sum(r['eje_nse_o_region']=='NSE' for r in csv.DictReader(open('canon/tabla-de-piso-v1_1.tsv'),delimiter='\t')))" -->.

Hoy el corte de clase solo es posible en dinero, remesas, consumo y tecnología. En lo cívico y en el trato con el Estado (ENVIPE, ENCIG) no se puede construir por diseño del cuestionario: ahí el único corte socioeconómico es la escolaridad. Los instrumentos internacionales (WVS, Latinobarómetro) traen clase o ingreso **subjetivos**, que **no son NSE AMAI** y no se funden con él.

## D · Pisos por origen: antes y después

- **Antes** (censo de `GEN2-ENCIG-PISOS-GEN2-1`, 24/sep): de los pisos puntuales de las celdas-D, **36** eran `HEREDADO-DE-LEGACY` <!-- comando: python3 forense/analisis/informe-v1_4/cifra.py censo:HEREDADO-DE-LEGACY --> y **16** `NUEVO` <!-- comando: python3 forense/analisis/informe-v1_4/cifra.py censo:NUEVO -->.
- **Después** (catálogo v1.2): **43 141** filas `NUEVO` <!-- comando: python3 forense/analisis/informe-v1_4/cifra.py cat_origen:NUEVO -->, **47** `HEREDADO-DE-GEN2` <!-- comando: python3 forense/analisis/informe-v1_4/cifra.py cat_origen:HEREDADO-DE-GEN2 --> y **0** `HEREDADO-DE-LEGACY` <!-- comando: python3 forense/analisis/informe-v1_4/cifra.py cat_origen:HEREDADO-DE-LEGACY -->. `GEN2-PISOS-GEN2-2` cerró (`PR #1123`) re-midiendo los pisos legacy con sucesores `-0002`. Los RESULT del censo con consumo activo quedan fuera de la tabla por regla: **20** <!-- comando: python3 forense/analisis/informe-v1_4/cifra.py cat:excluidos:PISO-HEREDADO-DE-LEGACY -->.
...
- **¿Clase media urbana sobre-representada?** Los instrumentos internacionales nuevos tienen muestras nacionales pequeñas y sin estrato público (WVS) o sin UPM (Latinobarómetro). Por eso entran con reserva de ancho y no se leen como rasgo nacional.
- **¿Marcos importados?** PEW, WVS, LAPOP y Latinobarómetro son marcos de clase (c) con muestra en México (a). PEW migración mide disposiciones, no flujos.
- **¿PROSPECTIVA y RETROSPECTIVA mezcladas?** No. §A–§E son RETROSPECTIVAS, salvo las familias 2027 y las celdas prospectivas, que se nombran aparte.
- **¿Unidades?** Hogar (ENIGH consumo, ENGASTO), persona y delito/trámite nunca se promedian entre sí.
- **¿Qué cifra se escribió a mano?** Ninguna: cada una tiene su `comando:`.

¹ *Vistas.* Las vistas derivadas en `main` son del 23/sep (el `[deriva]` fresco no se ha fusionado). Las cifras de `corrida0.py status` que cita este informe **no dependen de esas vistas**: `status` reconstruye el registro en memoria desde las corridas selladas (`derivado de 434 corridas`). Por eso no hizo falta re-correr `registro --escribe` ni commitear vistas.

---

# Heredado de v1.3 sin editar (universo `8358b891`)
...
`N_corridas_selladas` sí avanzó mientras este acto estaba abierto (246→263,
en dos oleadas) — por `GEN2-CLASE-AMAI-1` (seis `CALC-AMAI-NSE-*`, `PR
#1127`), `GEN2-DONDE-CAMBIO-EL-MEXICANO-1` (nueve `CALC-*-SERIE-DICTAMEN-
0001`, `PR #1125`) y `GEN2-RELEVO-MOTOR-34-1` (dos CALC derivados, `PR
#1128`), los tres fusionados mientras esta sesión seguía en curso, ajenos a
este acto y sin celdas-D nuevas (siguen 21) ni cambio en `celdas_validadas`.
**Comando único
...
congelado)
<!-- comando: grep -c "^## CONSUMIDO" forense/encargos/2026-09-24-GEN2-PISOS-GEN2-2.md -->. (Nota 25/sep/2026, GEN2-TRAMITE-FIRMAS-16: la rama se borró al fusionar; el comando original sobre `git ls-remote` caducó; el checkout de CI no trae refs remotas, por eso se verifica sobre el árbol.)
...
seguía en curso** — ninguno cambia ninguna cifra de §0–§4
(`celdas_validadas`, `celdas_d_adoptadas_activas` y las 21 celdas-D no se
movieron, verificado):

- **`GEN2-CLASE-AMAI-1`** (`PR #1127`): seis `CALC-AMAI-NSE-*` (clase
  AMAI/NSE por instrumento, `forense/analisis/clase-amai/`), un eje
  descriptivo de clase social, no una séptima evaluación de la etapa de
  retadores.
- **`GEN2-DONDE-CAMBIO-EL-MEXICANO-1`** (`PR #1125`): nueve
  `CALC-*-SERIE-DICTAMEN-0001`, 7872 series dictaminadas (583 ESTABLE · 8
  CAMBIO-SOSTENIDO · 0 SALTO-DE-INSTRUMENTO · 81 SALTO-SIN-EXPLICAR · 7200
  SIN-SERIE), todo **RETROSPECTIVA**, `adopta: NO`
  (`canon/donde-cambio-el-mexicano-v1_0.md`) — es exactamente el «dónde sí
  cambió el mexicano» que dirección cita como lo que viene, ya con primer
  documento; no es una evaluación de retadores ni mueve ningún candidato de
  §2.
- **`GEN2-RELEVO-MOTOR-34-1`** (`PR #1128`): dos CALC derivados de clase
  iii, relevo de nueve lecturas del motor — mecánico, sin tocar celda-D.

Las tres ramas se borraron al fusionar (política de cero ramas). v1.4
incorpora `GEN2-DONDE-CAMBIO-EL-MEXICANO-1` y `GEN2-CLASE-AMAI-1` como
secciones propias.
...
- **`GEN2-DONDE-CAMBIO-EL-MEXICANO-1` cerró (`PR #1125`) mientras este acto
  seguía abierto** (§5): su documento (`canon/donde-cambio-el-mexicano-
  v1_0.md`, 7872 series, todo RETROSPECTIVA) no está descrito en este
  informe más allá de la nota de §5; v1.4 le da su propia sección.
