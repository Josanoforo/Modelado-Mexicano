...
VARIABLES = {
    ...
    "ingocup", "t_tra", "tip_con", "remune2c", "t_loc_tri", "cve_ent",
}
...
def _segmentos(f):
    ...
    loc = _numero(f, "t_loc_tri" if "t_loc_tri" in f else "t_loc")
    ent = _numero(f, "ent" if "ent" in f else "cve_ent")
    return {
...
def _lee_sdem(ruta):
    with zipfile.ZipFile(ruta) as z:
        ...
        with z.open(miembros[0]) as fh:
            ...
            fh.seek(0)
            return pd.read_csv(fh, encoding="utf-8-sig" if bom else "latin-1",
...
def medir_ola(ruta, ola, era, replicas=200, semilla=42):
    ...
    oblig = {"r_def", "c_res", "eda", "sex", "upm", fac, est, "clase1", "clase2"}
    if "ent" not in f and "cve_ent" not in f:
        oblig.add("ent|cve_ent")
    falta = oblig - set(f)
    if falta:
