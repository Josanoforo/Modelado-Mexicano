...
- **SHA de redacción**: `f8eb2e3` (base declarada por el propio documento: "base origin/main = f8eb2e3"; verificado en esta sesión — `f8eb2e3` es ancestro de `origin/main`, HEAD actual `e078e46` tras el merge de PR #183).
- **Entorno asignado**: por acto, ver §0 ORDEN MAESTRO del texto abajo. **P·Lote-1, P·Lote-2, Q, M-APERTURA, R**: caja obligatoria (`sin_variable` + sonda 200), nunca nube. **M-ADQ**: nube o caja, el que esté libre, nunca los dos. Esta sesión ejecuta **ACTO P·LOTE-1** en caja (`CLAUDE_CODE_REMOTE_ENVIRONMENT_TYPE` sin definir, sonda 200 a `https://www.inegi.org.mx/`, corpus compartido montado en `data/raw` → `/home/pc0/mm-corpus/raw`, 241 entradas).
- **Estado**: `CONSUMIDO (parcial)` — **§0a/§1 (FIRMA DE CORTE)** ya estaban `CONSUMIDO` antes de este archivado: PR #183 fusionado en `origin/main` (commit `e078e46`, 2026-08-12 10:23:10 -0600) con la firma de corte de mesa verbatim-coincidente con el texto de §1 (Lote 1 = ISSP/WVS/EARLY_CHILDHOOD/GPS/CSES; BRASDEFER y MOBILE_TUTORS retirados del lote por relación NEGATIVA). Esta sesión ejecuta **ACTO P·LOTE-1 (§2)**, detalle en `forense/notas/2026-08-12-acto-p-lote1-adquisicion.md`. **M-ADQ (§3), P·LOTE-2 (§4), Q (§5), M-APERTURA (§6), R (§7) no se ejecutan en esta sesión** — quedan `VIVO`, gateados como el documento especifica.

Archivado por convención de este directorio (`forense/encargos/convencion.md`), como primer commit de este acto, antes de ejecutar el resto del bloque de ARRANQUE — Regla A.3 (`instrucciones-proyecto-v2_6.md`, Bloque D-bis).
