...
- **`T03` — mía, real.** La nota citaba `` `raices.local.yaml` `` **a secas**, entre acentos graves y sin ruta. `T03` vigila nombres de archivo desnudos y ese archivo está gitignorado: **nunca existe en un checkout limpio**, así que la cita es colgante por construcción. Es el mismo trampolín que `MAP-2` ya había pisado el 6/ago con este archivo exacto, y la solución es la misma que aquella nota dejó escrita: **citarlo con ruta** (`data/raices.local.yaml`), porque el patrón de `T03` no admite `/`. Corregido.
