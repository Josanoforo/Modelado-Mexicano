"""Perfil estructural de personas ENIGH 2016, conforme a spec v1.0.

Generalizacion, byte-identica en 2022, de CALC-ENIGH2022-PERFIL-ESTRUCTURAL-0003/medidor.py
(ACTO GEN2-ENIGH2024-SERIE-Y-COMMIT-1, P1). Unica diferencia sustantiva declarada:
en la ola 2016, poblacion.csv NO trae factor/est_dis/upm (verificado por columna
contra el header real del zip: 2016 vs 2022) -- se toman de concentradohogar por
folioviv+foliohog, y se probo exactamente equivalente contra 2022 (donde poblacion
SI trae su propia copia): 1667/1667 personas de una muestra de 500 hogares con
poblacion.factor == concentradohogar.factor y poblacion.(est_dis,upm) ==
concentradohogar.(est_dis,upm), cero discordancias.
"""
...
ZIP_ID = "enigh2016_nc_csv"
P = "RESULT-ENIGH16-PERFIL-"
MIEMBROS = {
    "poblacion": "conjunto_de_datos_poblacion_enigh_2016_ns/conjunto_de_datos/conjunto_de_datos_poblacion_enigh_2016_ns.csv",
    "concentrado": "conjunto_de_datos_concentradohogar_enigh_2016_ns/conjunto_de_datos/conjunto_de_datos_concentradohogar_enigh_2016_ns.csv",
    "hogares": "conjunto_de_datos_hogares_enigh_2016_ns/conjunto_de_datos/conjunto_de_datos_hogares_enigh_2016_ns.csv",
}
COLUMNAS = {
    "poblacion": ["folioviv", "foliohog", "numren", "parentesco", "edad", "segsoc"],
    "concentrado": ["folioviv", "foliohog", "tam_loc", "est_socio", "factor", "est_dis", "upm"],
...
FUENTES = {
    "segsoc": "Codificación heredada de ENIGH 2022 Descripción de la base p. 75 (1=Sí, 2=No); 2016 no tiene descriptor propio en el corpus, no verificado independientemente contra su codebook -- ASUME continuidad de nueva serie 2016-2022 (ADR de corte de serie de este acto)",
    "tramo_edad": "Tramos operativos propios de este medidor (18-96), no de INEGI; corte igual en todas las olas",
    "tam_loc": "Codificación heredada de ENIGH 2022 Descripción de la base pp. 45 y 186 (1..4); 2016 no tiene descriptor propio en el corpus, no verificado independientemente",
    "est_socio": "Codificación heredada de ENIGH 2022 Descripción de la base pp. 46 y 186 (1..4); 2016 no tiene descriptor propio en el corpus, no verificado independientemente",
    "conex_inte": "Codificación heredada de ENIGH 2022 Descripción de la base p. 54 (1=Sí, 2=No); 2016 no tiene descriptor propio en el corpus, no verificado independientemente",
    "celular": "Codificación heredada de ENIGH 2022 Descripción de la base p. 54 (1=Sí, 2=No); 2016 no tiene descriptor propio en el corpus, no verificado independientemente",
}
...
def calcular(poblacion, concentrado, hogares, replicas=1000, seed=20260919):
    ...
    embudo = {k: {"n": 0, "masa": 0.0} for k in (
        ...
        "excl_parentesco_otro", "excl_sin_diseno_hogar", "excl_edad_invalida", "excl_menor_18",
        "excl_mayor_96", "excl_peso_invalido", "universo_principal")}
    principales = []
    for raw in poblacion:
        r = dict(raw)
        cr0 = conc.get(_clave_hogar(r))
        r["factor"] = _texto(cr0["factor"]) if cr0 else ""
        r["est_dis"] = _texto(cr0["est_dis"]) if cr0 else ""
        r["upm"] = _texto(cr0["upm"]) if cr0 else ""
        sin_diseno_hogar = cr0 is None
        r["_w"] = _peso(r["factor"])
        ...
        if sin_diseno_hogar:
            embudo["excl_sin_diseno_hogar"]["n"] += 1
            continue
        edad = _entero(r["edad"])
        if edad is None:
...
def medir(inputs, contrato):
    ...
    return {
        ...
        P + "P3-EVIDENCIA": "ENIGH2022-DESCRIPCION-BASE-PP75:5-ANOS-VS-OCTUBRE-2005;UNIVERSO-Y-SALTOS-NO-ACREDITADOS;OLA-2016-SIN-DESCRIPTOR-PROPIO-EN-CORPUS-NO-VERIFICABLE",
        P + "EMBUDO-JSON": emb,
