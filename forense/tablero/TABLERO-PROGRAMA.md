...
**Nota 2026-09-05 (`ACTO MAESTRA38-N11 · PRE-REGISTRO-OLA6-MEDIBLES-Y-FICHAS`, NUBE, `FP-305` recibo; `ADR-342`).** Sella tres specs de pre-registro en `forense/prereg-caja/` (5 → 8) para las tres reglas que `N10` clasificó `MEDIBLE-COMO-ESTÁ`: `S6-L6` (`salud.atencion.grave`, ENNViH-3 `es09`×`cen10*`), `S7-L7` (`salud.vacunacion.disponible`, ENNViH costo×recepción de vacuna) y `S8-L8` (`comunicacion.inseguridad.ver_oir_callar`, módulo `AOJ` de LAPOP). **Hallazgo de A.8/D-13, no pedido explícitamente por el encargo:** `canon/modelo-decision-v4_0.md §7` ya archiva veredicto `D` de Hito D para `R9.2` (`ADR-56`, 4/ago/2026) — `tools/ya_medido.py` no lo clasifica como medición por vocabulario (letras `A`–`E` de Hito D vs. `CORROBORADA`/`CONTRARIA` de espacio `L`), aunque su propio cruce sí encuentra la línea; declarado en `S7 §0.2`, sin reabrirse — el diseño de esta pieza (autorreporte de hogar) es distinto del que Hito D bloqueó (auditor institucional tercero de abasto/campaña). Tres filas `PENDIENTE` nuevas en `data/curacion-registro/cola-adquisicion-registro.tsv` (vista regenerada) para las tres `CON-CANDIDATA`: `salud.adherencia.desabasto_vs_cuidadora` (necesidad `N36`, ya asignada; Hito D también archiva veredicto `D` en ambas mitades, `ADR-56`), `cooperacion.comite.monitoreo_sancion_visible` (Hito D archiva veredicto `D`, `ADR-138`; CNGMD es un instrumento nuevo no examinado por ese veredicto, pero no resuelve el descalce de unidad de análisis) y `cooperacion.faena.sancion_social_pueblo_mestizo` (sin veredicto Hito D) — cooperación no tiene necesidad `N` asignada, se propone `N42`/`N43` en la nota de cada fila sin insertarla (`data/curacion-registro/necesidad-objeto-modelo.tsv` fuera de perímetro). Red bloqueada en esta sesión NUBE (verificado, `403` del proxy de agente) — `SIN-FETCH` declarado en las tres filas, sin `PAQUETE-RECETAS-7`. `canon/registro-rotulos.tsv` gana la corrección de mapa `salud.vacunacion.disponible` es regla de `§3.9`, no de `§3.4` (el `id` conserva el prefijo por historia). Ninguna hipótesis de `N10` se toca; criterio 2 no se persigue — se sella lo medible porque es medible. Este snapshot del tablero sigue sin regenerarse. Detalle completo en el `## CONSUMIDO` del encargo archivado: `forense/encargos/2026-09-05-MAESTRA38-N11-PRE-REGISTRO-OLA6-MEDIBLES-Y-FICHAS.md`.
