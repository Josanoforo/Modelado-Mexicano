...
| **Confianza radial — magnitud** | ⚠️ **NO ESTABLECIDA** | (a) | `LEÍDO` | `conf.06` **cerrado por ADR-64** (5/ago/2026): las tres cifras de ENCUCI 2020 no competían por el mismo reactivo — son **tres reactivos distintos** de la pregunta 5.1 (`AP5_1_1/2/3`), todos a corte **≥8/10**: **21.8%** = "la mayoría" · **32.1%** = vecinos · **62.1%** = conocidos (§11). **El constructo como magnitud única sigue sin establecer:** quedan 12% (WVS 2012), 22% (Latinobarómetro/LAPOP) y 18% (Pew 2025) sin reconciliar contra ENCUCI — no son ENCUCI, su reactivo y corte no están verificados contra microdato, y no son comparables entre sí ni contra las tres de ENCUCI sin esa verificación. **Resolver antes de que uno de esos tres entre a un modelo** |
