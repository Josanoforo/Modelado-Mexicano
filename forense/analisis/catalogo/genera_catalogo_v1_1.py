#!/usr/bin/env python3
"""Catálogo del mexicano v1.1 (ACTO GEN2-CATALOGO-V1-1-1).

Deriva, sin teclear una sola cifra, las tablas del catálogo v1.1:

  canon/catalogo-del-mexicano-v1_1.tsv            estimadores (P1)
  forense/analisis/catalogo/v1_1/calcs.tsv         CALC citados, con sus hashes
  forense/analisis/catalogo/v1_1/pendientes-de-firma.tsv
  forense/analisis/catalogo/v1_1/excluidos.tsv     fuera por regla, con nota
  forense/analisis/catalogo/v1_1/cobertura-31.tsv  cobertura por report (P3)
  forense/analisis/catalogo/v1_1/conteos.json      toda cifra de la portada

y renderiza la portada canon/catalogo-del-mexicano-v1_1.md desde
forense/analisis/catalogo/v1_1/plantilla.md ({{c:clave}} -> conteos.json;
...
Uso:  python3 forense/analisis/catalogo/genera_catalogo_v1_1.py [--sin-registro]
      --sin-registro reutiliza v1_1/adoptados-activos.tsv en vez de derivar
...
DIR = ROOT / "forense/analisis/catalogo/v1_1"
TSV = ROOT / "canon/catalogo-del-mexicano-v1_1.tsv"
MD = ROOT / "canon/catalogo-del-mexicano-v1_1.md"
    ...
    for fp in FIRMAS16:
        ...
        if not estado.startswith("FIRMADA"):
            pend.append({"id": fp, "objeto": "pisos/eje del acto emisor", "calc": "",
                         "estado_fp": estado, "nota": "no entra a v1.1 (encargo §2)"})
...
def cobertura(lista: list[dict], get: Calcs) -> list[dict]:
    ...
    for rep in reportes:
        ...
        out.append({
            ...
            "estimadores_v1_1": medidos[dom], "calc_gen2_sin_adoptar": en_med[dom],
...
def _tabla_cobertura() -> str:
    ...
    out = ["| report | dominio (mapa U0) | estado | estimadores v1.1 | CALC GEN2 sin adoptar | afirmaciones (en corpus / con adquisición / no medibles) |",
           "|---|---|---|---:|---:|---|"]
    for r in rows:
        nombre = r["report"].removesuffix(".md").replace("_", " ").strip()[:70]
        out.append(f"| {nombre} | `{r['dominio']}` | **{r['estado']}** | {r['estimadores_v1_1']} | "
