...
- **49 reglas** *(42 en v2 · 43 en v2.1 por conf.07 · 44 en v2.3 al partir la diagonal)*. **Hito D (perímetro de 27 reglas, subconjunto de las 49): 11 de 27 corridas archivadas** — `R1.1` → veredicto `D` (inejecutable por hueco de mercado, no de dato), `R3.2` → veredicto `B` (gate de 20pp aritméticamente inalcanzable), `R7.2` → veredicto `D` (inejecutable por hueco de instrumento, no de mercado ni de muestra), `R4.2` → veredicto `D` (inejecutable por hueco de instrumento: ENSANUT no cruza permiso laboral con conducta preventiva), `R4.1` → veredicto `D` (sin diseño antes/después de una mejora de acceso documentada), `R9.1` → veredicto `D` (sin variable de distancia en km ni muestra de quien no consulta a nadie), `R4.3` → veredicto `D` mitad A y mitad B (adherencia solo disponible auto-reportada en el catálogo entero), `R9.2` → veredicto `D` (abasto/campaña solo lo reporta el prestador, ninguna fuente independiente en el catálogo), `R5.1` → veredicto `A` (satisface la fila con IC95% que no despeja el umbral, reserva escrita en la línea), `R5.2` → veredicto `A` (mismo patrón que `R5.1`, IC95%=[14.39%,33.57%] no despeja 20%), `R1.2` → veredicto `E` (corrió limpio y no se satisfizo — la regla sobrevive a la prueba; excepción acotada, MESA-R12=SÍ). Fuente única: `hitoD-preregistro §Registro de veredictos archivados`. *(Corregido 4/ago/2026 — propaga `R7.2` → `D`; propaga `R4.2` → `D`, ADR-55, Encargo Y. Corregido de nuevo 4/ago/2026 — propaga `R4.1`/`R9.1`/`R4.3`(A,B)/`R9.2` → `D`, ADR-56, Encargo Z. Corregido de nuevo 4/ago/2026, Acto 1 — adjudica `R5.1`/`R5.2` → `A` y `R1.2` → `E`, ADR-58; 8 de 27 → 11 de 27.)*
- **Ejercicio distinto, no confundir con lo anterior (ADR-45, D-05):** `glosario §6/§10` corrió por su cuenta, el 27/jul/2026 — antes de que el Hito D existiera —, un ejercicio informal sobre "utilidad + fricción baja > confianza" (dominio seguro agrícola), **veredicto informal B**: sin contraejemplo limpio, candidato irresuelto, techo estructural que impide alcanzar el veredicto C con fuentes públicas. No tiene ID de regla, no vive en el bloque append-only de `hitoD-preregistro`, y no cuenta en el "11 de 27" de arriba.
...
| **49 reglas · 27 de perímetro · 11 de 27 corridas · 20 `[FUERTE]`** | 49/27/20 sin cambio respecto del v3.4, recuento mecánico sobre §3.B, reproducible con `python3 tests/validador_registro_ids.py`; corridas 2→3 por `R7.2` → `D`, archivado 4/ago/2026 (ADR-45); 3→4 por `R4.2` → `D`, archivado 4/ago/2026 (ADR-55, Encargo Y); 4→8 por `R4.1`/`R9.1`/`R4.3`(A,B)/`R9.2` → `D`, archivados 4/ago/2026 (ADR-56, Encargo Z); 8→11 por `R5.1`/`R5.2` → `A` y `R1.2` → `E`, archivados 4/ago/2026 (ADR-58) | **Heredado, con receta ejecutable — corridas actualizado 4/ago/2026** |
