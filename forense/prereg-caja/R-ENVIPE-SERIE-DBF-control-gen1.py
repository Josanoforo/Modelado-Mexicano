#!/usr/bin/env python3
"""Control positivo externo de `prereg-caja-R-ENVIPE-SERIE-DBF` — GEN2 contra GEN1.

`ACTO GEN2-R-SERIE-DBF`, 9/sep/2026. Congelado en el COMMIT-1 junto con la
spec y los tres medidores, **antes de abrir un solo byte de microdato** y
antes de leer un solo JSON de `corridas-R/`.

La LOGICA DE COMPARACION es la del control del trio CSV
(`R-ENVIPE-SERIE-control-gen1.py`), copiada sin cambios: mismas cuatro ramas,
mismos umbrales, misma lista cerrada de campos. Lo unico distinto son las tres
celdas. Que el control de las siete olas sea el mismo codigo es parte de que la
serie sea comparable consigo misma.
...
por ningun parametro (spec sellada DBF, §0 y §7).
...
## Las cuatro ramas, pre-declaradas (spec sellada DBF §7)
...
CELDAS = ["CIV-M-01", "CIV-M-02", "CIV-M-04"]
TOL_EXACTA = 1.0e-9
TOL_FLOTANTE = 1.0e-6
...
def main() -> int:
    print("CONTROL POSITIVO EXTERNO · prereg-caja-R-ENVIPE-SERIE-DBF · GEN2 vs GEN1")
