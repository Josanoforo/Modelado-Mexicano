...
Encargo: `forense/encargos/2026-09-19-GEN2-PISOS-ENUT2019-EJES-1.md`. ADR: `ADR-554`
...
tomó `ADR-553` y `NC-0361` — renumerado aquí dos veces, a `ADR-554` y `NC-0362`..`NC-0367`,
regla de la casa: renumera quien fusiona después).
...
Lo que sí colisiona (`ADR-554`, `NC-0362..0364`) se resuelve por la regla de la casa. Si
...
| `tools/marcador_segmento.py` | enlace de la segunda tabla (**tres sitios**, no una línea — `NC-0366`) |
...
| cascada | `hallazgos.md` (una entrada), `no-corrido.tsv` (`NC-0362..0364`), `gobernanza-v1_15.md` (`ADR-554`), `estado-programa-v1_14.md` (L0), `registro-rotulos.tsv` (una fila) |
...
FAIL nuevos**, todos propios: 3 × `T-NO-CORRIDO` (`NC-0363`/`NC-0366`/`NC-0367` con sucesor
...
(el sucesor de `NC-0362`; mesa al fusionar; el acto que cierre `NC-0364`). `T16` no se
