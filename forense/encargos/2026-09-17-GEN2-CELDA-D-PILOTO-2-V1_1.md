...
|---|---|---|---|
| P4 · «Fila NC: edad × dominio ENVIPE 2025: RESERVA-CONSUMIDA-SIN-PILOTO, 17/sep/2026, script exploratorio en scratchpad antes de COMMIT-1 (sesión cerrada por mesa); sin commit; los valores no entran al canon; no usar como reserva» — asentada como `NC-0328`, sin números | DECISIÓN-DE-MESA-PENDIENTE | ese cruce ya no puede ser reserva de evaluación de ningún piloto | SIN-ASIGNAR — si mesa lo quiere como celda-D, con árbitro abierto desde el inicio |
| COMMIT-3 · «registro en la vista» de `CALC-TRA-EVADE-NORMA-SXD-EMISIONES-0001--2ff028319eed` y `…-ARBITRO-CRUCE-0001--7e86d93722ec`: `python3 tools/corrida0.py registro --escribe --lote CALC-TRA-EVADE-NORMA-SXD-EMISIONES-0001,CALC-TRA-EVADE-NORMA-SXD-ARBITRO-CRUCE-0001` → `PARO · REPLAY-PISADO (NC-0094)` sobre 25 corridas ajenas, cero vistas escritas (`NC-0329`) | NO-VERIFICABLE-AQUÍ | las dos CALC quedan SELLADA-EN-DISCO, NO-REGISTRADA en el CONTADOR (`N_corridas_selladas` 80 → 82 sólo por disco); 883 RESULT sin fila en `resultados.tsv` | el mismo de `NC-0315` (bandera `--excluye` o asientos de replay para las 25 ajenas) |
| COMMIT-3 · fila `M05` a `DERIVADO-Y-SELLADO-GEN2` deja `tests/test_celda_d_piloto_consumidor.py` (primer piloto) en FAIL: cablea «22 filas NO-VERIFICADO» (`NC-0330`) | FUERA-DE-PERÍMETRO | un test del primer piloto queda en FAIL silencioso (2 de 9; ni `check.py` ni CI lo corren) | acto de mantenimiento: sustituir el conteo literal por la propiedad y comparar contra SHA fijo |
| `tests/test_marginales_una_variable.py` y `tests/test_celda_d_piloto2_consumidor.py` no están cableados en `.github/workflows/verify.yml` (`NC-0331`) | FUERA-DE-PERÍMETRO | la guardia de Firma 2 y el falsador de orden sólo se ejercen a mano (15/15 y 10/10 al cerrar) | acto de mantenimiento de CI: dos pasos `run:` |

## CONSUMIDO

Ejecutado por **PR #858** (`acto/gen2-celda-d-piloto-2`), 17/sep/2026, CAJA. Commits: `e2d252d` (0-bis) → `2ff0283` (COMMIT-1, spec + reserva como código, sin microdato) → `cdd78f9` (COMMIT-2, emisiones selladas, R inexistente) → `7e86d93` (COMMIT-3a, contrato del árbitro) → `9b5c371` (COMMIT-3, R, adjudicación SIN-CANDIDATO-SUPERIOR, celda-D, M05, test) → `194b5e8` (cascada, ADR-542 candidato). Nota: `forense/notas/2026-09-17-GEN2-CELDA-D-PILOTO-2-cierre.md`. Mesa fusiona.
