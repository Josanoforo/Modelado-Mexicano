...
> ## 📊 **Condicionales medidas sobre atributos: ~~8~~ 9 de 15**
> *(Corregido 4/ago/2026, Encargo K — `forense/notas/2026-08-04-medicion-exposicion-violencia-envipe.md` §3-5: mide `exposicion_violencia` (ENVIPE 2025 `TPer_Vic2`, `AP7_3_10`-`_14`). `4 de 144` sigue congelado y no se mueve — ver §6.1. **Nota de reparto, actualizada 4/ago/2026:** `exposicion_violencia` sale de "sin reactivo verificado — búsqueda abierta" (que queda vacía, 0) y entra a `MEDIDO·PARCIAL(edad,dominio,formalidad,ESTRATO)`; `aversion_riesgo` y `sens_estatus` siguen en "sin reactivo — búsqueda cerrada (límite de régimen)". **Denominador 14 → 15 el 13/ago/2026, `ACTO PROC-11`** (ADR-75(b), Paso 6 arriba): entra `obligación_medida`. El reparto cierra ahora en **15 = 9+0+2+3+1**.)*
>
> ⚠️ **El NUMERADOR no se movió, y la razón es taxonómica, no de perímetro.** Sube el denominador y no el `9`: ni `norma_de_género` ni `obligación_medida` entran hoy a `milpa/procedencia.yaml`, de donde T19b/T19c derivan el numerador (`grep -c 'clase: "MEDIDO·PARCIAL'`). La clase `MEDIDO·PARCIAL(x)` **exige eje condicionante declarado** y ambas θ son marginales nacionales (`x = ∅`) — `ACTO PROC-10` lo verificó y lo dejó escrito (`forense/notas/2026-08-13-proc-10.md` §3). Escribir `MEDIDO·PARCIAL()`, o declarar un eje que el expediente no trae, falsearía qué mide el número. **Abrir una clase que sí las describa es la decisión D3, nombrada y no firmada al cierre de este acto.** Un `9 de 15` honesto vale más que un `10` inventado.
>
> **`D`=15 es derivado del diseño de §1.1, nunca estimado.** Su aritmética completa está arriba, pasos 1–6. Si alguien mueve `D`, tiene que mover uno de esos pasos y decir cuál. *(Quien movió `D` de 14 a 15 fue `ACTO PROC-11`, y la respuesta a "cuál paso" es: **ninguno de los cinco originales — se añadió el Paso 6**, precisamente para no falsificar el control aritmético del Paso 1.)*
...
> ## 📊 **Condicionales medidas sobre atributos: ~~8~~ 9 de 15**
> *(Denominador 14 → 15 el 13/ago/2026, `ACTO PROC-11` — ADR-75(b) registra `obligación_medida` como condicional nueva, ver §1.1.F Paso 6. El numerador no se mueve: ambas θ nuevas son marginales nacionales y `MEDIDO·PARCIAL(x)` exige eje condicionante, decisión D3 pendiente. Corregido antes, 4/ago/2026 — `CAL-CONF` Fase B posiciones 5-6, `forense/notas/2026-08-03-cal-conf-faseb-pos5-6-radio-familismo.md` §5: mide `radio_confianza` (ENCUCI) y `familismo_apoyo` (ENIF). Corregido de nuevo el mismo día, Encargo K, `forense/notas/2026-08-04-medicion-exposicion-violencia-envipe.md` §3-5: mide `exposicion_violencia` (ENVIPE `TPer_Vic2`). Ver §1.1.F, Paso 5. `4 de 144` sigue congelado y no se toca, ver abajo.)*
...
- **Condicionales medidas sobre atributos: ~~8~~ 9 de 15** (§1.1.F · §6.1; denominador 14 → 15 el 13/ago/2026, `ACTO PROC-11` ejecutando ADR-75(b) — ver §1.1.F Paso 6. Antes, corregido 4/ago/2026, Encargo K — `forense/notas/2026-08-04-medicion-exposicion-violencia-envipe.md` §3-5). De las 15: **9 MEDIDO·PARCIAL** (`confianza_institucional[salud/educación/financiera/seguridad-FFAA/justicia-policía/electoral-partidos]`, 6 de 6, más `radio_confianza`, `familismo_apoyo` y `exposicion_violencia`), 0 sin reactivo verificado — búsqueda abierta (vacía desde 4/ago/2026: `exposicion_violencia` salió por medición exitosa, no por agotamiento de la condición de caducidad de ADR-52 A, que sigue intacta como criterio, ver §1.1.F Paso 5), 2 sin reactivo — búsqueda cerrada, límite de régimen (`aversion_riesgo`, `sens_estatus`; `sens_estatus` cerrada 4/ago/2026 por ADR-54, `gobernanza` §4, ver §1.1.F Paso 5), 3 solo proxy, y **1 medida sin clase de procedencia** (`obligación_medida`, ENASIC 2022 `P6_38` — calculada y verificada, pero marginal nacional: `x = ∅` y `MEDIDO·PARCIAL(x)` exige eje condicionante, así que no sube el numerador; decisión D3 pendiente, ver §1.1.F Paso 5 y Paso 6).
