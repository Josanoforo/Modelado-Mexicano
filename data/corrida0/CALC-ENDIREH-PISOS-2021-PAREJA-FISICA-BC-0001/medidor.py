"""ENDIREH 2021, violencia física de última relación B y relación actual o última C1.
...
def measure_rows(rows, replicas=500, seed=20260923):
    ...
    axes = {"nacional": ["MX"], "edad": ["15-29", "30-44", "45-59", "60+"],
            ...
            "localidad": ["U", "C", "R"], "pareja": ["B1", "B2", "C1"],
...
def medir(inputs, contrato):
    with zipfile.ZipFile(inputs[PAYLOAD]["ruta_absoluta"]) as archive:
        ...
        with _member(archive, MEMBER) as stream:
            for r in csv.DictReader(stream):
                if r.get("T_INSTRUM") not in {"B1", "B2", "C1"}:
                    continue
    ...
    return {"RESULT-ENDIREH2021-PF-BC-TABLA": json.dumps(table, ensure_ascii=False, sort_keys=True),
            "RESULT-ENDIREH2021-PF-BC-N-ELEGIBLES": len(rows)}
