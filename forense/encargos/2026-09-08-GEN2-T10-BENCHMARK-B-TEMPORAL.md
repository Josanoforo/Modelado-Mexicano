...
|---|---|---|---|
| **B sobre datos reales** — el selector queda instalado, no consumido; ninguna cifra B real se calculó. | `DIFERIDO-A:C0-B` | Sin cifra B ni mejora medida de M; la spec del CALC consumidor (con snapshot y `disponible_desde` documentado) queda pendiente. | `C0-B`. `NC-0028`. |
| **Nuevos R/L, ensamble E y adopciones** — ningún corredor R/L nuevo, ningún ensamble E, ninguna adopción de parámetros. | `FUERA-DE-PERÍMETRO` | `NC-0018`/`NC-0019` sin cambio; el selector no mueve ninguna cifra de R, L ni E. | Cola vigente (`NC-0018`/`NC-0019`). `NC-0029`. |
| **P3(c) tal como el encargo la pedía** — una línea en `hallazgos.md` que atribuyera a esta propuesta el envío/fusión de la adenda de mesa a T9 (fuga temporal LOO + `ORIGEN-ARBITRO`). | `SUSTITUIDO-POR:PR #618` — esa adenda se envió y fusionó de forma **independiente**, antes de que este acto se ejecutara: `PR #618` («ADENDA GEN2-T9 · ola previa estricta y ORIGEN-ARBITRO», asentada bajo `ADR-401`), con sus propias líneas en `hallazgos.md` y `NC-0026`/`NC-0027`. `tools/emite_m.py::ola_previa_estricta`/`origen_de_entrada_serie` ya implementan la regla que la propuesta (§«Corrección material a P1») describe en futuro. | Ninguna cifra de GEN2 se mueve; `hallazgos.md` no gana una línea redundante con las que `PR #618` ya escribió; el documento aplicado por `P1` (verbatim, A.3) queda con esa sección describiendo como pendiente una regla que el árbol ya tiene — no se edita in situ, se señala aquí y en la línea de `hallazgos.md` que sí se escribió, corregida. | Ninguno vivo — la corrección es esta misma fila y la línea de `hallazgos.md` que la acompaña. `NC-0030` (`CERRADA`). |
| **Rama del acto con su rótulo** — la convención (P4/A.14) es abrir una rama nueva nombrada por el rótulo del acto y fusionarla o borrarla al cerrar. | `DECISIÓN-DE-MESA-PENDIENTE` — se trabajó sobre `claude/new-session-xhtzkj`, la rama que el entorno de ejecución (Claude Code Remote) asigna a esta sesión, con instrucción explícita de no empujar a otra rama sin permiso. | La «política de cero ramas» (A.14) no se ejecuta con su mecánica normal (fusionar/borrar la rama propia del acto) al cerrar; ningún contador GEN2 se mueve por esto. | Mesa decide, en el merge, si acepta esta rama o pide reabrir el acto bajo una rama con el rótulo `GEN2-T10`. `NC-0031`. |

## CONSUMIDO

Ejecutado por `PR #620` (`ACTO GEN2-T10 · BENCHMARK-B-TEMPORAL — selector B temporal + propuesta corregida`), 8/sep/2026.
