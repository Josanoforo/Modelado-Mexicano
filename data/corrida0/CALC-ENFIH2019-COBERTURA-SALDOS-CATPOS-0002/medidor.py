...
PREFIX = "RESULT-ENFIH2019-COBERTURA-SALDOS-CATPOS-0002-"
...
def _ci(v):
    a = np.asarray(v, float); valid = np.isfinite(a); nvalid = int(valid.sum()); invalid = int((~valid).sum())
    # Una réplica inválida no se elimina: el IC de esa celda es no estimable.
    return {"ic95": ([float(z) for z in np.percentile(a,[2.5,97.5])] if invalid == 0 and nvalid else [None,None]),
            "replicas_validas": nvalid, "replicas_invalidas": invalid,
            "causa_no_estimable": (None if invalid == 0 and nvalid else "DOMINIO_VACIO_O_DENOMINADOR_CERO_EN_REPLICA")}


def _clean(value):
    """JSON estricto: NaN no es un valor publicado."""
...
def medir(inputs, contrato):
    ...
    cats_all = NATIVE+["DESCONOCIDO"]
    metrics = ["cobertura_completa", "media", "p25", "mediana", "p75", "p90", "media_positivos", "mediana_positivos"]
    keys = [(c,k) for c in cats_all for k in metrics]
    statekeys = [(c,s) for c in cats_all for s in ["COMPLETO-CERO", "COMPLETO-POSITIVO", "PARCIAL", "DESCONOCIDO-TOTAL"]]
    ckeys = [(c,k) for c in NATIVE for k in ["delta_cobertura","delta_media","delta_mediana"]]
    samples = {("t",)+k: [] for k in keys} | {("s",)+k: [] for k in statekeys} | {("c",)+k: [] for k in ckeys}
    for mult in _multipliers(d,int(p["bootstrap_replicas"]),int(contrato["seed"]["valor"])):
        ...
        for c,s in statekeys: samples[("s",c,s)].append(rt[c]["estados"][s]["p_tenedores"])
        for c,k in ckeys: samples[("c",c,k)].append(rc[c][k])
    for c,k in keys: table[c].setdefault("precision", {})[k] = _ci(samples[("t",c,k)])
    for c,s in statekeys: table[c]["estados"][s]["precision_p_tenedores"] = _ci(samples[("s",c,s)])
    for c,k in ckeys: con[c][k+"_precision"] = _ci(samples[("c",c,k)])
    ...
    allstates=["COMPLETO-CERO", "COMPLETO-POSITIVO", "PARCIAL", "DESCONOCIDO-TOTAL"]
    national["reconstruye_estados"] = all(
        sum(v["estados"][s]["n"] for v in table.values()) == int(d.state.eq(s).sum()) and
        np.isclose(sum(v["estados"][s]["masa"] for v in table.values()), _mass(w,d.state.eq(s).to_numpy())) for s in allstates)
    # Cálculo independiente para categoría 1: agregados aritméticos, no point_table.
    m=d.cat.eq("1").to_numpy(); h=m & holder; comp=m & complete; iw=_mass(w,h)
    independent_cov=_mass(w,comp)/iw if iw else float("nan")
    independent_mean=_mean(d.loc[comp,"v"].to_numpy(float),w[comp])
    national["control_independiente_cat_1"]={"cobertura":independent_cov,"media":independent_mean,
        "coincide":bool(np.isclose(independent_cov,table["1"]["cobertura_completa"]) and np.isclose(independent_mean,table["1"]["media"]))}
    out={"TABLA-CATEGORIAS-JSON":table,"TABLA-CONTRASTES-JSON":con,"CONTROL-NACIONAL-JSON":national,
         ...
         "CONTROL-RECONCILIACION":"SI" if national["reconstruye_marco"] and national["reconstruye_masa"] and national["reconstruye_estados"] and national["control_independiente_cat_1"]["coincide"] else "NO"}
    return {PREFIX+k:(json.dumps(_clean(v),sort_keys=True,separators=(",",":"),allow_nan=False) if isinstance(v,(dict,list)) else v) for k,v in out.items()}
