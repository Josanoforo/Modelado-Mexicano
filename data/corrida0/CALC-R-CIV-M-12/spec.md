...
**Celda:** `CIV-M-12` — ENVIPE 2023, **delitos de 2022**.
**Releva:** `CORR-0040` → `RES-0113`, el árbitro `R` de esta celda del marco `M`.
Consumidor natural: `forense/prereg-duelo-v2/marco-M-sorteado-v1_3.tsv:CIV-M-12:R`.
...
| campo | valor |
|---|---|
| `id` de manifiesto | **`envipe2023_csv`** — resuelto por `tests/payload_resolver.py`, nunca por heurística |
| miembro | `tmod_vic_envipe2023/conjunto_de_datos/conjunto_de_datos_tmod_vic_envipe2023.csv` |
| `Identifier` (metadato) | `MEX-INEGI.EGS3.02-ENVIPE-2023` |
| `Temporal` (metadato) | `2022-01-01-2022-12-31, 2023-03-01-2023-04-30 y 2023-03-01-2023-12-31` → **delitos de 2022** |
...
**Redacción del reactivo en esta ola:** anade 'o Fiscalia Estatal'; sigue 'Por miedo al agresor' — ninguna de esas diferencias mueve un código.

## Diseño

`EST_DIS` y `UPM_DIS` se agrupan como **cadenas opacas** (nunca `int()`, nunca `zfill()`). El
descriptor de esta ola declara `EST_DIS Alfanumerico(3), 001..595 SEGUN DESCRIPTOR` y `UPM_DIS Alfanumerico(5) SEGUN DESCRIPTOR`; el perfil **real**
se emite en `RESULT-R-CIV-M-12-PERFIL-DISENO`, porque el descriptor ya mintió sobre esto en la
...
**Este medidor no lo calcula.** No abre `forense/prereg-duelo-v2/corridas-R/CIV-M-12.json` y no
