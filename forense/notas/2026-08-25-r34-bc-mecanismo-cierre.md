...
**Encargo:** `forense/encargos/2026-08-25-R34-BC-MECANISMO.md` (archivado verbatim por este acto, `A.3`). **ADR:** `ADR-189`. **Entorno:** UBUNTU. **Base:** `2b7d787` (= `origin/main`, `PR #356`). **Rama:** `acto/r34-bc-mecanismo`.
...
4. **`F3` · Commit 2** — propuesta por condición, el defecto del pre-registro, la vía, `ADR-189`, `estado`, tablero, esta nota.
...
4. Una **contradicción interna del pre-registro sellado** de `R3.4`: su `Respaldo 2` manda este desenlace a la fila `D`, pero la fila `D` exige que `A` falle y `A` está `SELLADA`. Va a mesa (`FP-157`), no se adjudica.
5. `tools/censo_r34_bc.py`, reejecutable y reanudable, con sus límites escritos en el propio docstring.

## 6 · Contadores

**CONTADOR: cero, declarado.** No se movió ningún contador de medición sobre México. `Hito D` sin cambio. **Base medida de `B`/`C`: sigue en `0 de 2`** — este acto no produjo dato mexicano sobre esos umbrales porque no hay fuente de la que producirlo; lo que produjo es el universo del negativo. El contador de `ADR` pasa de `185` a `186` (recifrado en `estado` L0). El tablero pasa de **0 filas `ABIERTA`** a **1** (`FP-157`).
...
`python3 tests/check.py --baseline` → **LÍNEA BASE: VERDE**, nada nuevo frente a `tests/baseline.json`. Cifras: **19 FAIL núcleo (sin cambio) · 128 WARN** (`127 → 128`), medido y no derivado por aritmética. El `+1` de WARN es **`FP-157` naciendo `ABIERTA`** y `T22` señalándola — que es exactamente lo que `A.12` existe para hacer: el WARN sigue gritando hasta que mesa la atienda.

**La corrida que vale es la de después de fusionar, no la de antes.** El acto midió primero `128 → 129` contra su base (`2b7d787`); mientras corría, `origin/main` avanzó **9 commits** y movió la cifra vigente a `127`. Las declaraciones de `estado` se recifraron **con la corrida real sobre el árbol fusionado**, no arrastrando la medición anterior ni ajustándola por aritmética.
...
`FP-157`, `ABIERTA`, con tres decisiones separables para mesa: (1) si se adopta `B`/`C` `INDETERMINADA`; (2) en qué fila aterriza el gate —el acto propone `B` y explica por qué la `D` que el `Respaldo 2` nombra es imposible con `A` sellada—; y (3) si se autoriza la vía, cuyo primer paso es **una descarga**: el `FD` de `ENSAFI 2023`, que `FP-115`(c) ya tenía identificado con su patrón de URL.


## 10 · Renumeración al fusionar

`origin/main` avanzó **9 commits** durante el acto y tomó los tres números siguientes de `ADR` y los dos de `FP`. Regla de la casa aplicada tal cual — **renumera quien fusiona segundo**, y el segundo es éste:

| lo que este acto reservó | máximo re-derivado contra `origin/main` | renumerado a |
|---|---|---|
| `ADR-186` | `ADR-188` (sin huecos; `ACTO SORTEA`/Acto B y su cadena tomaron 186-188) | **`ADR-189`** |
| `FP-155` | `FP-156` (sin huecos) | **`FP-157`** |

Los máximos se re-derivaron **contra `origin/main`, no contra la base del acto**, con dígitos extraídos y `sort -n` sobre el campo numérico limpio — nunca `sort -t- -k2`, que parte en el primer guion y devuelve un máximo falso. La renumeración se propagó a los cinco archivos que citan los rótulos (`gobernanza`, `estado`, tablero, ficha y esta nota); verificado por `grep` que no queda ninguna referencia a `ADR-186` ni a `FP-155` en los archivos de este acto.
