...
- **Estado**: `CONSUMIDO` — ejecutado en la rama `claude/new-session-qwc8wn`, commits `c26bdba` (COMMIT 1: crea `instrucciones-proyecto-v2_7.md`) · `d03c408` (archiva este mismo encargo, corrigiendo la brecha de perímetro de §2 abajo) · `85b6856` (COMMIT 2: sella `ADR-77` y propaga la cascada) · `0a01cab` (`forense/hallazgos.md` + nota de cierre `forense/notas/2026-08-13-a8-land.md`). PR aún no abierto al momento de este commit — mismo criterio que `2026-08-13-adr-provisionalidad.md` ya sentó: ningún acto de esta convención exige PR abierto para marcar `CONSUMIDO`, el registro es el commit. `tests/check.py --baseline`: antes `18 FAIL · 104 WARN` VERDE, después `19 FAIL · 107 WARN` ROJO por dos hallazgos declarados, no congelados unilateralmente — detalle completo en `forense/notas/2026-08-13-a8-land.md` §4.
