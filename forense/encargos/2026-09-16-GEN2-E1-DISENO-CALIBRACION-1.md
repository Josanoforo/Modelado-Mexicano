...
Ejecutado en [PR #811](https://github.com/Josanoforo/Modelado-Mexicano/pull/811), abierto contra `main` el 16/sep/2026 y **no fusionado por el ejecutor** (el merge es de mesa). Cascada de cierre completa: `ADR-526` en `canon/gobernanza-v1_15.md` (renumerado de `524` a `526` al fusionar `origin/main` — `PR #809`/`GEN2-PINS-REPRODUCE-1` y `PR #810`/`GEN2-MANTENIMIENTO-3` fusionaron antes y tomaron `ADR-524`/`ADR-525`; enmienda histórica sobre el `Suite:` de `ADR-525`, cuyo conteo de WARN quedó vencido), anotación `L0` en `canon/estado-programa-v1_13.md`, los tres contadores mecánicos reconciliados por `tools/cierre_acto.py --aplica` (523→524, antes del merge) más la corrección manual a `526` al resolver el conflicto, rótulo `GEN2-E1-DISENO-CALIBRACION-1` censado en `canon/registro-rotulos.tsv` con la colisión `GEN2-E1` declarada, exención T25 para los tres archivos propios que citan `E1` pelado (verbatim de `F-18`), y `NC-0258`/`NC-0259` nuevas en `forense/no-corrido.tsv` (renumeradas de `0253`/`0254` por la misma razón; ver `## NO-CORRIDO / RESERVAS` arriba) más la enmienda fechada sobre `NC-0239` (no se cierra: diseño ejecutado, corrida pendiente).
