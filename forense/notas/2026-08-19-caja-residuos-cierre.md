...
`PR #284` · `ADR-120` · Base: `origin/main` = `10ddb20` (`PR #285`/`PR #286`) al cerrar; `20c7dee` al numerar la primera vez; `35c9c9f` al redactarse el encargo · Sesión **UBUNTU** · Modelo: Opus · Sin `--freeze`. **La caja no fue de dueña única — ver §4.**
...
2. **`ADR` re-derivado tres veces**, receta `T15`: `118` al escribir, y de nuevo tras fusionar `PR #283` y tras fusionar `PR #285`/`PR #286` — que **ocuparon el 118 y el 119** mientras esta rama esperaba revisión. Final: únicos `120` · máx `120` · huecos `[]`, sin duplicados; **este acto sella `ADR-120`**. Cascada de `T15` a tres sitios (`estado:27`, `estado:101`, `gobernanza:2`), los únicos que no exime `{cita-historica}`. El tablero **no** colisionó: las dos fusiones cerraron `FP-58` y `FP-60`, no abrieron filas, así que `FP-62`/`FP-63` siguen libres.
3. **Cascada de `T16`, y una decisión pequeña que no es cosmética.** Sobre el árbol **ya fusionado**, las dos filas nuevas suben `T22` de 3 a 5 WARN y el total de **116 a 118**; recifrados `estado:205` y `estado:297`. Ninguno de los dos lados tenía la cifra buena —esta rama decía 120 sobre una base con `FP-58`/`FP-60` abiertas, `origin/main` decía 116—, así que se re-derivó de la corrida real y no por aritmética. Pero `:296` traía una **segunda cita interna** —*«Fusionado: **21 FAIL · 118 WARN** — neto de las dos causas»*, de `ACTO REFUTACIONES-SIN-OBJETO`— que **era cierta cuando se escribió** y que `T16` empezó a marcar `FAIL` sólo porque la cifra vigente se movió. Se **marcó `{cita-historica}`** (mecanismo de `ADR-90`) en vez de sobrescribirla hacia adelante. Sobrescribir es justo la conducta que `FP-52` tiene abierta sobre cinco narraciones de `gobernanza` reescritas entre 7 y 17 veces cada una para mantener `T16` verde; este acto declina añadir la sexta. Fueron **dos** en la misma línea —la de `ACTO REFUTACIONES-SIN-OBJETO` y, tras la fusión, la de `PR #285`/`PR #286`, rancia en cuestión de horas por la misma causa—, más una tercera en el propio `ADR-120` al citar la cifra para explicar la decisión. Son los primeros de `estado-programa-v1_10.md`, y el ritmo es el dato: **cada acto que mueve el conteo de WARN convierte la narración de fusión del acto anterior en un `FAIL` de `T16`**. Evidencia dura para `FP-52` y `FP-48`.
4. **Corrida final: `21 FAIL · 118 WARN` · `LÍNEA BASE: VERDE`**, sin `--freeze` —prohibido por el encargo— y `tests/baseline.json` verificado sin tocar (`git status --porcelain -- tests/` vacío). Una entrada de la línea base dejó de aparecer (mejora); no se bajó la cifra congelada.
