...
- **Corrida en frío, tras F1 (benchmark aterrizado, nada más tocado):** 24 FAIL total (T16 auto-referencial incluido) / real subyacente (subproceso sin T16) = 20 FAIL · 132 WARN. Dos hallazgos nuevos frente a la línea base congelada: `T25` (rótulo `§E5` pelado en el benchmark, cita de un espacio ya existente en `forense/EDGE-CASES-y-literatura-reciente.md`, no un espacio nuevo) y `T16` (las citas de FAIL/WARN vigente en `estado-programa` quedaron desactualizadas por el propio cambio).
- **`T25`:** declarado y **deliberadamente no arreglado** — su fix (censar el archivo en `_T25_ARCHIVOS_CONOCIDOS`, `tests/check.py`) cae fuera de la lista cerrada de archivos que este acto toca; el propio encargo ordena "si la suite rompe por algo que esta lista no cubre, PARA y reporta en vez de ajustar fuera de perímetro". Sigue como entrada nueva contra `tests/baseline.json` — no bloqueante (no se congela con `--freeze`), pendiente de un acto futuro con perímetro sobre `tests/check.py`.
- **`T16`:** dentro de perímetro (`canon/estado-programa-v1_10.md`, cascada L0 + línea de suite) — corregido: cabecera de conteo de ADR (176→177), las dos citas de suite (§L0 y §"Suite de verificación") recifradas con las cifras reales medidas, y se marcó `{cita-historica}` la cita inmediatamente anterior (`ACTO PURGA-2`) que dejó de ser la vigente.
- **Después (real, medido, corrida final):** **20 FAIL · 132 WARN**. Neto de WARN: **−1**, exactamente el esperado por `FP-104` saliendo de `ABIERTA`. Neto de FAIL: **+1** (`T25`, ya explicado arriba — no atribuible a un defecto del par CoDi/SPEI, sino a un artefacto del propio verbatim del benchmark chocando con una convención de rotulado que este acto no tiene perímetro para resolver).
- Línea base (`tests/baseline.json`, HEAD congelado `e24d033ed3c095f1e81c2fbb8248f108e9d3ef65`): **ROJO — 1 entrada nueva** (`T25`, arriba) y **5 entradas de la línea base que ya no aparecen** (mejora, no forzada, no se bajó la cifra congelada — no se corrió `--freeze`).
