"""ENDIREH 2021, violencia escolar, elegibilidad por asistencia.
...
MEMBER = "TB_SEC_VII.csv"
DEM = "TSDem.csv"
ACTS = tuple(range(1, 19))
...
def medir(inputs, contrato):
        ...
        with _member(archive, MEMBER) as stream:
            for r in csv.DictReader(stream):
                ...
                if not r.get("EST_DIS") or not r.get("UPM_DIS"):
                    continue
                life = (classify_binary_union([r.get(f"P7_6_{i}", "") for i in ACTS])
                        if r.get("P7_1") == "1" else None)
                recent_values = []
                for i in ACTS:
                    ever = r.get(f"P7_6_{i}", "").strip()
                    recent_values.append("4" if ever == "2" else r.get(f"P7_8_{i}", ""))
                recent = (classify(recent_values) if life is not None and r.get("P7_2") == "1"
                          else None)
    ...
    return {"RESULT-ENDIREH2021-ESC-TABLA": json.dumps(table, ensure_ascii=False, sort_keys=True),
            "RESULT-ENDIREH2021-ESC-N-ELEGIBLES": len(rows)}
