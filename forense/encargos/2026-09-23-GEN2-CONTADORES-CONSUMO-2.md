...
- **qué:** P2 completo (publicar el marcador re-derivado y las 5 aserciones de `T-C2-COMPUESTO` en este mismo PR). · **por qué:** PARO-PREMISA descubierto por CI (`enrutamiento-pr` → `derivados_protegidos.py --toca`): commitear `data/corrida0/marcador-segmento.tsv` / `milpa/estimadores-por-segmento.yaml` a mano en este PR es exactamente el PARO b) del encargo — el mismo motivo por el que `GEN2-CONTADORES-CONSUMO-1` ya había revertido ese cambio local. Se revierten los dos derivados y las 5 aserciones a como están en `origin/main` (correctas contra el archivo commiteado hoy: 30/30 OK, T-REPRO 0 FAIL). El OBJETIVO del propio encargo ya preveía esta secuencia: "el siguiente PR `[deriva]` publica el marcador". · **impacto:** el marcador de `GOB.gobierno_digital.edad_x_escolaridad` (16 usos) no se publica en este PR; sus 5 aserciones de `T-C2-COMPUESTO` no se actualizan aquí. · **sucesor:** el próximo PR `derivados/auto-*` (ya wireado por P3) publica el marcador; un acto posterior (o el mismo `GEN2-CONTADORES-CONSUMO-3`) actualiza las 5 aserciones de estado contra ese marcador re-derivado.

## CONSUMIDO

Ejecutado por `ACTO GEN2-CONTADORES-CONSUMO-2`, PR #1096 (`Josanoforo/Modelado-Mexicano`). Rama `acto/gen2-contadores-consumo-2`. ADR `ADR-260923-GEN2-CONTADORES-CONSUMO-2-749c-01`. P1 (acreditación de `origen_numerico`, 16/32 usos vía `CALC-GOB-DIGITAL-EXE-EMISIONES-0002`) y P3 (canal en `verify.yml`) y P4 (sufijo `-D-C2` general) hechos. P2 (publicar el marcador re-derivado + actualizar `T-C2-COMPUESTO`) diferido al `PR derivados/auto-*` que sigue a este, por el PARO-PREMISA de `derivados_protegidos.py` — ver `## NO-CORRIDO / RESERVAS`. Los 16 usos restantes, dependientes de `CALC-ENCIG-DUELO-2025-ADJUDICACION-0001` (fuera de perímetro), también quedan PARO-PREMISA. Mesa fusiona.
