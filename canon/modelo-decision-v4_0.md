...
| `ref.A.04` pobres no pagan | `prestamista` (`fondeo`, `gobierno_corporativo`, enlace `tasa_ofrecida`/`cat`) — **§1.1.H, nueva** | FUERTE | **RESUELTA 19/ago/2026.** El gate que este mismo acto exigió (deep research auditado de todo el sector, `corpus/forense/compass-4-e29a28d4-credito-popular-2026.md`, N=11) llegó, y dirección firmó **"(a) completa"** sobre `FP-61`: `ADR-124` sella `ADR-35(b)` sin acotar. | **Con objeto — `ADR-124`/`ADR-35(b)`.** Ver §1.1.H para el detalle de los tres atributos. `ref.A.04` re-redactada en `milpa/refutations.yaml` (veredicto `REFUTADA-FUERTE`, N=11). |
