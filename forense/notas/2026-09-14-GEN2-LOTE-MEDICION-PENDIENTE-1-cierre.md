...
**Actualización 15/sep/2026 — `FP-375` firmada.** Mesa, 15/sep/2026 · OBJETO: `cuenta_gen2 = SI` para las cuatro corridas selladas por el lote; no adopta al motor. Escrita en `data/corrida0/decisiones.tsv` (una fila por `calc_id`) y en `etiquetas.cuenta_gen2`/`firma_de_contador` de cada `spec.yaml`; registro re-derivado (`corrida0.py demanda` + `registro --escribe`). `N_corridas_selladas` 63→67, `N_resultados_gen2_sellados` 2882→3142 (+260 `RESULT`, las mismas cuatro corridas). La re-derivación en esta sesión nueva exigió declarar `--lote` sobre las 12 corridas cuyo veredicto de replay bajó de `REPRODUCE`/`IDENTICO` a `NO-VERIFICADO` (esta sesión no corrió `verify`; ninguna de esas 12 cambió de contenido, solo de estado de verificación) — no las cuatro de `FP-375` únicamente, sino todas las que el guard de `registro` encontró sin verificar en esta sesión. `N_resultados_gen2_pendientes_adopcion` sin cambio (5): esta firma sella, no adopta.
