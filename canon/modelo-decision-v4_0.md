...
- **49 reglas** *(42 en v2 · 43 en v2.1 por conf.07 · 44 en v2.3 al partir la diagonal)*. **Hito D (perímetro de 27 reglas, subconjunto de las 49): 4 de 27 corridas archivadas** — `R1.1` → veredicto `D` (inejecutable por hueco de mercado, no de dato), `R3.2` → veredicto `B` (gate de 20pp aritméticamente inalcanzable), `R7.2` → veredicto `D` (inejecutable por hueco de instrumento, no de mercado ni de muestra), `R4.2` → veredicto `D` (inejecutable por hueco de instrumento: ENSANUT no cruza permiso laboral con conducta preventiva). Fuente única: `hitoD-preregistro §Registro de veredictos archivados`. *(Corregido 4/ago/2026 — propaga `R7.2` → `D`; propaga `R4.2` → `D`, ADR-55, Encargo Y.)*
...
| **49 reglas · 27 de perímetro · 4 de 27 corridas · 20 `[FUERTE]`** | 49/27/20 sin cambio respecto del v3.4, recuento mecánico sobre §3.B, reproducible con `python3 tests/validador_registro_ids.py`; corridas 2→3 por `R7.2` → `D`, archivado 4/ago/2026 (ADR-45); 3→4 por `R4.2` → `D`, archivado 4/ago/2026 (ADR-55, Encargo Y) | **Heredado, con receta ejecutable — corridas actualizado 4/ago/2026** |
