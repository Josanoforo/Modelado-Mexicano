...
| **Confianza radial — magnitud** | ⚠️ **NO ESTABLECIDA** | (a) | `LEÍDO` | **Cinco cifras en circulación:** 12% (WVS 2012) · **21.8%** (ENCUCI 2020) · 22% (Latinobarómetro/LAPOP) · **32.1%** (ENCUCI 2020) · 18% (Pew 2025, 2º más bajo de 25 países tras Turquía 14%). **Dos dicen ser la misma encuesta y difieren 10.3 puntos.** No es "item-dependiente": es contradicción abierta. **Resolver antes de que cualquier número de confianza entre a un modelo** |
...
| **Consumo compensatorio** | **Fuerte** (consumidor) / **Hipótesis** (salud) ⚠️ | (a)+(c) | `LEÍDO` | Velandia-Morales 2022 (base latinoamericana). **Choque no reconciliado entre reports — no promediar.** ⚠️ El modelo lo marca `FUERTE como correlación`; V1 lo rompió *"como driver decisivo aislado"* — **no es la misma afirmación**, y V1 omitió el perfil 5 |
...
- **Utilidad + fricción baja > confianza** → la falsaría un producto de alta utilidad y baja fricción que fracasara por desconfianza pura. **Prueba corrida: veredicto B.** No apareció contraejemplo limpio; queda un candidato irresuelto y un techo estructural.
...
| **conf.06** ⭐ | **Magnitud de la confianza interpersonal**: 21.8% vs. 32.1% en la **misma ENCUCI 2020** | ⚠️ **Abierto.** Cinco cifras en circulación; **ninguna es usable** hasta resolver cuál corresponde a qué reactivo |
...
| **conf.07** ⭐ | **`modelo §3.7` empaqueta dos afirmaciones de tier distinto bajo un solo `[FUERTE]`**: *"se vive como derecho/gratitud al líder"* (**Hipótesis**, sin identificación causal) + *"conserva autonomía de voto"* (**Fuerte**, con RCT y contrafactual) | ⚠️ **Abierto — nuevo en v5.1.** Detectado al dar entrada al constructo. **No invalida el ascenso HIPÓTESIS→FUERTE del cambio 10**, que se apoya en la segunda mitad; obliga a **partir la regla en dos** o a tierarla por su mitad más débil. Requisito de salida: la regla no puede citarse como `[FUERTE]` completa en un pre-registro del Hito D hasta resolverse |
...
- **conf.02, conf.05, conf.06, conf.07** — sin ADR. *(conf.04 salió de esta lista: resuelto por ADR-27.)*
...
- **conf.07** — `modelo §3.7` empaqueta Fuerte + Hipótesis bajo un solo tier. Nuevo en v5.1.
