"""`CALC-0003-v3` — S6 **v1.3** Rama A' sobre ENNViH-1 2002: 4 celdas x brazos.

`repite_de: CALC-0003-v2`. UNICA diferencia operativa con el medidor sellado
por `ACTO GEN2-E5-1`, declarada aqui y en spec.md / spec.yaml:

    el bootstrap remuestrea LOCALIDADES (`id_loc` de `c_portad.dta`) dentro de
    estrato, no HOGARES (`folio`) dentro de estrato.

Es lo que `S6 v1.3 §3.5` pre-registra al corregir la premisa falsa de `v1.2`
(`FP-351`: la localidad SI esta en los archivos). Todo lo demas —universo,
disparadores, desenlaces, las tres tablas de clasificacion, `P_PUB` y su
sensibilidad, el umbral de `n`, la pareja de ponderadores, `seed`, replicas y
el filtro de llave ausente de `c_portad` que v2 introdujo— es byte por byte el
de v2. Los 128 `RESULT` de v2 se reproducen con los MISMOS ids; los 14 nuevos
son diagnostico de esta unica diferencia y ninguno es un `Delta`, un IC ni un
veredicto.

Interfaz estable: `medir(inputs, contrato) -> {"RESULT-…": v}`.

Codigos: `1 = Si`, `3 = No`, `8 = NS` (solo brazo bx). NO existe el codigo `0`
-- `S6 v1.3 §1` ya lo escribe asi (`FP-349`): en v1.2 el papel decia `0` y el
medidor de v2 tuvo que declarar la discrepancia; aqui papel y codigo coinciden.
"""
...
def medir(inputs, contrato):
    ...
    portad = _llave(portad_crudo[~_sin_llave], ["folio", "ls"])[
    ...
    _n_hogares = int(portad["folio"].nunique())
    ...
    _hog_multi_loc = int((portad.groupby("folio")[_cong].nunique() > 1).sum())
    ...
    out["RESULT-LLAVE-UNICA-PORTAD"] = (
        f"{out['RESULT-LLAVE-UNICA-PORTAD']} · A.13 c_portad.dta: "
        f"{_portad_leidas} filas leidas · {_portad_filtradas} filtradas por "
        f"llave (folio/ls) ausente · {len(portad)} al join")
    ...
    def fila(clave, disparador, universo, tabla, pesos, columnas_peso, filtro_txt):
        ...
        d = d.merge(portad[["folio", "ls", "estrato", _cong]], on=["folio", "ls"],
