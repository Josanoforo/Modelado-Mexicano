...
CALC_ID = "CALC-ENADID2023-UNION-SEXO-EDAD-0002"
...
def calculate(frame: pd.DataFrame, replicates: int = REPLICATES, seed: int = SEED) -> dict:
    ...
    for age_slot, (age_id, _, _) in enumerate(AGES):
        ...
        row = {"tipo": "peso_estandar_edad", "sexo": "ambos", "edad": age_id,
               "n_denominador": n, "masa_denominador": all_den,
               "masa_numerador": den, "punto": point,
...
def medir(inputs: dict, contrato: dict) -> dict:
    ...
    return {
        "RESULT-ENADID-USE2-P1-TABLA-SHA256": _sha(outputs["distribucion.csv"]),
        "RESULT-ENADID-USE2-P2-TABLA-SHA256": _sha(outputs["union_condicional.csv"]),
        "RESULT-ENADID-USE2-P3-TABLA-SHA256": _sha(outputs["estandarizacion.csv"]),
        "RESULT-ENADID-USE2-FLUJO-SHA256": _sha(outputs["flujo.csv"]),
        "RESULT-ENADID-USE2-AUDITORIA-SHA256": _sha(outputs["auditoria.json"]),
        "RESULT-ENADID-USE2-DIF-BRUTA-MH": std["diferencia_bruta"]["punto"],
        "RESULT-ENADID-USE2-DIF-ESTANDAR-MH": std["diferencia_estandarizada"]["punto"],
        "RESULT-ENADID-USE2-BRUTA-MENOS-ESTANDAR": std["bruta_menos_estandarizada"]["punto"],
        "RESULT-ENADID-USE2-REPLICAS": REPLICATES,
        "RESULT-ENADID-USE2-SEMILLA": SEED,
        "RESULT-ENADID-USE2-SALIDA": "forense/analisis/enadid2023-union-sexo-edad-cli-2",
    }
