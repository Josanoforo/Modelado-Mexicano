...
El árbol determinó 6 árbitros R ya sellados y 8 faltantes ejecutables. Los ocho pasaron `spec-check → preflight VERDE → run → verify REPRODUCE/CONTEXTO=IDENTICO`. Al aplicar la corrección puramente registral de estado/fecha exigida por FP-370, la guardia de inmutabilidad impidió re-sellar los mismos IDs; por eso nacieron ocho sucesores `-v2` ligados por `repite_de`. La revisión adversarial de PR #680 conserva esos sellos históricos intactos y añade siete sucesores técnicos `-v3` y `CALC-R-DIN-M-01-v4` sólo para completar las dependencias materiales del replay; el intento sellado `CALC-R-DIN-M-01-v3` también se preserva y queda superado. No hubo celda bloqueada ni sin spec. `UR` queda congelado en **14/14** mediante `universo-triada-v1_4.tsv`, bajo la sucesora de reconciliación `F5-contrato-triada-spec-v1_1.md`; esto habilita el panel para el `ENCARGO 5/5`, pero no decide `U3` ni adjudica contendiente alguno.

La spec TRIADA v1.0 queda intacta como fotografía del orden 2/5: cerró prematuramente UR en 6/14 cuando los otros ocho R aún no existían. La v1.1 cambia exclusivamente el momento de cierre: completa R en 3/5, cierra UR antes de cualquier cálculo de error y prohíbe modificarlo después. No cambia MAE, `δ=0.5 pp`, bootstrap, semilla, `k=8`, mediana L, reglas pareadas/de ganador, roles de R/B ni transferencia secundaria. Su entrada en vigor se perfecciona por el merge de mesa; no se inventa firma ni cita verbatim.
...
Ningún target R requiere una fuente nueva: **0 bloqueados, 0 sin spec, 0 NC nuevas**. Se mantienen las reservas estadísticas nombradas arriba. `NC-0143` sigue abierta: el extractor L v1.3 y el snapshot M v1.0 existen, pero disponibilidad no equivale a punto válido. `ENCARGO 5/5` debe consumir la spec TRIADA v1.1 y `universo-triada-v1_4.tsv` para aplicar la intersección y derivar U3. Este acto no calcula U3 ni errores TRIADA.
...
- Sucesoras vigentes tras la corrección adversarial: `codificacion-R-v1_2.tsv`; `F5-contrato-triada-spec-v1_1.md` (`db6b24c579e72dc705c772200ca2c4066bd9708cc6c06c85c5101d56b081b4f5`); y `universo-triada-v1_4.tsv` (`840fc68ce7261686426221effbf9df313ef17e2876fa6345b3c00589bab80f80`). En el universo, `L_punto_valido` y `M_punto_valido` quedan `PENDIENTE-DE-DERIVAR`: registra que extractor L v1.3 y snapshot M v1.0 existen sin confundir esa disponibilidad con validez. U3 sigue sin derivar y UR permanece congelado en 14/14.
...
- `ENCARGO 5/5` debe consumir `F5-contrato-triada-spec-v1_1.md` y `universo-triada-v1_4.tsv`, y aplicar, sin ampliar ni reducir UR, la intersección U3 ya fijada.
