...
> **Actualización 19/ago/2026 (`FP-56`, `ADR-110(a)`/`ADR-111(c)`, ejecutado por `ADR-114`, `ACTO REFUTACIONES-SIN-OBJETO`).** Mesa firmó `(a)`: se amplía el modelo, no se retiran. Siete de las ocho de arriba ya tienen variable declarada en `canon/modelo-decision-v4_0.md §1.1.G` — ninguna calibrada, ninguna corrida; vuelven a la batería con veredicto pendiente. `ref.A.04` (pobres no pagan) **sigue sin objeto**: darle una exige entidad `prestamista`, que choca con la frontera declarada de `ADR-35` ("el motor modela al decisor, no al oferente"); la enmienda está redactada y sin ejecutar (`forense/notas/2026-08-19-adr35-enmienda-borrador.md`), escalada a mesa como `FP-61`. Esta tabla no se reescribe — es historia de la primera corrida, y en esa corrida las ocho seguían sin objeto.
...
4. ~~**Decidir sobre las ocho sin objeto.** Dos rutas: ampliar el modelo (esfuerzo/horas, salud mental, colorismo, prestamista) o **retirarlas de la batería declarando el alcance**. Lo que no se vale es dejarlas contando como refutaciones activas cuando no pueden fallar.~~ ✅ **DECIDIDO 19/ago/2026** — mesa firmó ampliar (`FP-56`, `ADR-111(c)`). **Ejecutado para siete de ocho** (`ADR-114`, `modelo §1.1.G`): variable declarada, sin calibrar, veredicto pendiente. La octava (`ref.A.04`, prestamista) sigue sin objeto, bloqueada por `ADR-35`, escalada a mesa (`FP-61`, enmienda redactada y no ejecutada).
