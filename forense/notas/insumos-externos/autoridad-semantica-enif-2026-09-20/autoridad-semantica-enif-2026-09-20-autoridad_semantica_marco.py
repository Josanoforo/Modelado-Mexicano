#!/usr/bin/env python3
"""Contrato fail-closed para autoridades semánticas del marco candidato.

Una autoridad enlaza exactamente una semilla E2 material con las afirmaciones
necesarias para derivar una spec. No admite defaults, comodines, herencia entre
representaciones ni emparejamientos nominales.
"""
...
OPTIONAL_CITABLE_FIELDS = frozenset(
    {"unidad_medida", "rango_valido", "operacion_estimador", "no_aplica_ponderador_documentado"}
)
ALL_CITABLE_FIELDS = BASE_CITABLE_FIELDS | OPTIONAL_CITABLE_FIELDS


class AutoridadSemanticaError(ValueError):
    """Fallo cerrado del contrato de autoridad semántica."""
...
def validate_authority_row(row: Mapping[str, Any]) -> None:
    ...
    if kind in {"ADMINISTRATIVA", "CENSAL"}:
        ...
        if _text(row.get("ponderador_scope_tipo")) not in SCOPE_TYPES:
...
class SemanticAuthorityIndex:
    """Índice O(1) con aserciones anti-deriva y control de orfandad."""
