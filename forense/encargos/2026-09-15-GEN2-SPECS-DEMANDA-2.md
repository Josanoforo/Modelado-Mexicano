...
|---|---|---|---|
| escribir y correr el `medidor.py` de las 6 `CALC` nuevas (`CALC-L8-CONVERSION-0001`, `CALC-S7-L17-0001`, `CALC-EVASION-NORMA-0001`, `CALC-TIENE-AHORROS-0001`, `CALC-DINERO-FAMILIARES-VEJEZ-0001`, `CALC-HORIZONTE-VIA-DERIVADOS-0001`) (`NC-0200`) | `DIFERIDO-A:acto de CAJA/registro`. El encargo declara `CONTADOR: cero mediciones propias` y `no corre ningún CALC` sin excepción por facilidad técnica — dos de las seis (`L8-CONVERSION`, `HORIZONTE-VIA-DERIVADOS`) son aritmética pura sin `numpy`/`pandas`, pero se mantiene la misma disciplina que el resto | `preflight` reportará `BLOQUEADO:script_ausente` (las que necesitan medidor nuevo) o `BLOQUEADO:resultados_ausente` (las que transcriben una medición ya corrida: `S7-L17`, `EVASION-NORMA`, `TIENE-AHORROS`, `DINERO-FAMILIARES-VEJEZ`) en las seis — ninguna corre ni adopta cifra | `ACTO GEN2-MEDICION-DEMANDA-1` (CAJA, en vuelo) para las que necesitan microdato/`pandas`; cualquier acto en NUBE para las dos deterministas |
...
Cinco specs congeladas (`D-15` capa 1+2 donde faltaba alguna): `CORR-0016` → `CALC-L8-CONVERSION-0001`, `CORR-0017` → `CALC-S7-L17-0001`, `CORR-0007` (parcial) → `CALC-EVASION-NORMA-0001`, `CORR-0009` → `CALC-TIENE-AHORROS-0001` (agotada), `CORR-0010` → `CALC-DINERO-FAMILIARES-VEJEZ-0001` (agotada), `CORR-0015` → `CALC-HORIZONTE-VIA-DERIVADOS-0001` (agotada). Dos sondas (`CORR-0018`, `CORR-0008`) con tabla de candidatas. Una corrección de premisa sin sonda (`CORR-0001`).
