...
> | **QUÉ ES** | Pre-registro, congelado antes de abrir ningún `.dta`, de una pieza **multi-ola** que mide los **cuatro** antecedentes del `SI` de `civico.protesta.agravio_urbano` — agravio, falla estatal, red previa, entorno urbano — donde `MAESTRA35-L9`/`L11` (ya corridos y ya sellados en canon vía la Enmienda `D2-d`, `C2` `CARGADA-A-MOTOR` tier `FUERTE`) sólo midieron **dos**. |
...
**Corrección post-merge (`PR #536`, tras `origin/main` = `7c04069`, `PR #535`/`ACTO MAESTRA38-N6`).** El primer sello de esta pieza citaba las dos corridas como `PENDIENTE-DE-MESA` (`FP-298`, `ABIERTA`) y "ningún sello de canon movió la línea 558" — desactualizado, y en parte ya lo estaba antes de escribirse: `canon/modelo-decision-v4_0.md §7`, **Enmienda `D2-d`** (firma de mesa **3/sep/2026**, propagada por `ACTO MAESTRA37-N8`, un día **antes** del `SHA` de redacción `a0e06da4`) ya había cargado el contraste `C2` (agravio × urbano, exactamente la fila de arriba) **al motor**: `civico.protesta.agravio_urbano_encuci2020`, `situacion: CARGADA-A-MOTOR`, `tier: FUERTE`, `CORROBORADA-REPLICADA` (LAPOP 2019 + ENCUCI 2020, copiada verbatim a `milpa/tramite.yaml`). `C1` (entorno solo, la columna que cae por guardia en `L9` y queda ambigua en `L11`) quedó `SELLADA-SIN-CARGA`, `tier: MEDIA`, `AMBIGUA-ENTRE-INSTRUMENTOS` — sellado, no `PENDIENTE-DE-MESA`, y sin cargar al motor. `FP-298` pasó además a `EJECUTADA` el 4/sep (`ACTO MAESTRA38-N6`), cargando `civico.protesta.agravio_urbano_multiola` — **el mismo diseño que este documento pre-registra** — como tercera formulación complementaria en `milpa/tramite-ola5-propuesta-v0.yaml`, `situacion`/`tier` `PENDIENTE-DE-MESA` (diseño aceptado, falsador de cuatro factores sin correr), sin reabrir `D2-d`. `L9 §4` es explícito: *"mide **dos de los cuatro** antecedentes de la regla — «red previa» y «falla estatal palpable» **no están en el instrumento**"* (verbatim, `resultados.md:174-175`) — esa limitación sigue vigente en `D2-d` (que sólo carga agravio×urbano, los mismos dos antecedentes de `L9`/`L11`), y es exactamente el hueco que esta pieza pre-registra cerrar.
...
No abre ningún archivo de §6. No calcula ninguna celda ni IC95. No mueve el tier de `civico.protesta.agravio_urbano`, ni el de `civico.protesta.agravio_urbano_encuci2020` (`[FUERTE]`, `CARGADA-A-MOTOR` vía `D2-d`) ni el de `_lapop2019` (`SELLADA-SIN-CARGA`, `[MEDIA]`). No reabre `D2-d` ni la fila `D` de `R7.4` (`ADR-158`), ni dice nada sobre `civico.autodefensa.agravio_rural`. No repite como hallazgo nuevo lo que `L9`/`L11` ya corrieron y `D2-d` ya selló (`C_agravio`/`C2`) — lo cita como evidencia existente, ya sellada. `FP-298` ya está `EJECUTADA` — esta pieza no la reabre. No resuelve los cortes de `FALLA_ESTATAL`/`RED_PREVIA`/`URBANO` — quedan pendientes de codebook, declarado, no inventado.
