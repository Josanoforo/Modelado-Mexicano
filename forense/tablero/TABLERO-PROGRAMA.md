...
- **Procedencia.** SHA `3f48be30` · fecha del commit `2026-09-22` · ¿árbol == origin/main? `True`.
...
- **Ramas presentes en origin.** 0 ramas presentes en origin (política de cero cumplida):
  (ninguna)
...
- **Gobernanza operativa.** ADR máximo del espacio numérico CERRADO `593` · FP máximo del mismo espacio `409` · ids con raíz de acto (época vigente) `{'ADR': 48, 'FP': 62, 'NC': 146}` · FP abiertas: FP-260921-GEN2-CORPUS-INTEGRIDAD-Y-RESPALDO-1-3d56-01, FP-260922-GEN2-MARCO-M-CONSUMIDOR-1-02e6-01, FP-260922-GEN2-ENIF-PERSISTENCIA-IC-CALIBRADO-1-2868-01, FP-260922-GEN2-TRAMITE-PENDIENTES-1-18fa-01 · encargos archivados `666` (consumidos `606`) · instrucciones vigentes `v2.16` · cola de encargos (solo estados != CONSUMIDO; consumidos `36`):
...
- **NC abiertas por razón (token A.14, prefijo exacto).** abiertas `210` · por token: `DECISIÓN-DE-MESA-PENDIENTE` 22 · `DIFERIDO-A` 34 · `FUERA-DE-PERÍMETRO` 46 · `NO-VERIFICABLE-AQUÍ` 9 · `PARO-ENTORNO` 1 · `PARO-PREMISA` 20 · `SUSTITUIDO-POR` 3 · prosa (sin token reconocible) `75`.
- **GEN2 (derivado de `corrida0 status`).** corridas selladas `164` / requeridas `87` · resultados sellados `49254` / activos `211` · pendientes `211` · dependencias numéricas legacy activas `146` · validación independiente `215` · diferencias materiales `0` · NC- abiertas `210` · replays LEGACY-GEN1 sellados `5` (no cuentan). El `0 / N` es la lectura correcta: el aparato se construyó antes que las corridas.
