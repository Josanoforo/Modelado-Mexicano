...
Tu último mensaje debe cumplir tools/adq-resultado.schema.json. Entrega los objetos inicialmente elegidos primero y después sólo candidatas de esta investigación que hayas adquirido o intentado. Cada evidencia debe ser una ruta local existente; toda adquisición debe acreditar archivos e ids pertinentes de data/manifiesto.yaml. Para cada intento, el valor completo de intentos[].resultado debe aparecer VERBATIM en al menos una ruta de resultados_por_objeto[].evidencias o en la nota de su fila canónica; una paráfrasis no acredita el intento. Evalúa por separado identidad, concepto, población, selección/no respuesta, unidad, temporalidad, diseño e identificación; no uses una nota agregada. Deriva resultado_sustantivo de la estructura, no de la prosa: investigaciones+adquiridos=descubrimiento_y_adquisicion; investigaciones sin adquiridos=descubrimiento_documentado; sin investigaciones y con adquiridos=adquisicion_obtenida; sin investigaciones y con objetos sólo intentados=intentos_documentados; ambas selecciones vacías=cola_vacia; publicación fallida=fallo. Nunca rellenes cifras de utilidad desde prosa. Con cualquier trabajo, la publicación exige refs/heads/<rama> y SHA remoto exacto: el recibo posterior del wrapper no la sustituye. Si falla, conserva resultados, declara resultado_sustantivo=fallo y publicacion_trabajo=fallida.
HANDOFF REDUNDANTE OBLIGATORIO: después de conocer las refs y SHA remotos, serializa exactamente el mismo objeto JSON final en ${HANDOFF_RESULTADO}. Escríbelo de forma atómica mediante un temporal creado en ese mismo directorio, fsync y os.replace; no escribas prosa ni otro formato en esa ruta. ANTES de entregar el último mensaje, valida ese handoff con el contrato productivo: `python3 tools/adq_handoff.py --selecciona-resultado --last-message ${LOGDIR}/${RUN_ID}-preentrega-ausente.json --handoff ${HANDOFF_RESULTADO} --seleccion-archivo ${SELECCION_ARCHIVO} --seleccion-investigacion-archivo ${INVESTIGACION_ARCHIVO} --resultado-aceptado ${LOGDIR}/${RUN_ID}-preentrega-aceptado.json --informe ${LOGDIR}/${RUN_ID}-preentrega-validacion.json`. Si el comando no termina en 0, lee el informe, corrige el candidato o su evidencia mediante las interfaces autorizadas y repite; no entregues un resultado que el validador acaba de rechazar. Luego entrega exactamente el mismo objeto del handoff como último mensaje. El wrapper volverá a validar ambos canales por separado y fallará si son válidos pero difieren."
