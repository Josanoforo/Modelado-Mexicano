...
| `R5.1` · `familia.seguro.volatilidad_ausencia_estado` | perfiles 2, 3, 6 | `segsoc`=2 **∨** `residencia` ∈ {EUA, Otro país} **∨** hogar con `remesas` (P041) — el `SI` ya decía *"ingreso volátil / ausencia de Estado"* | **No.** ⚠️ **REFUTADA — `R5.1` → veredicto `A`, 4/ago/2026 (ADR-55).** Alcance exacto: refutada **en el nivel donde se usa** — la familia **no se retira cuando el Estado entra**. 6 olas ENIGH, hogares con adulto mayor, beneficiario vs. no beneficiario de la Pensión del Bienestar: ni corresidencia ni transferencia intrafamiliar hacia mayores bajan en ninguna ola ni en ningún tercil de ingreso — si acaso **suben**. La condición de traducción de esta fila **no cambia**; lo que cae es la afirmación de **sustitución**. El tier `[FUERTE]` no se toca aquí (ADR-02, acto aparte) y G5 no se apaga |
...
- **SI** hay ingreso volátil / ausencia de Estado (`segsoc`=2 **∨** `residencia` ∈ {EUA, Otro país} **∨** hogar con `remesas` P041) **ENTONCES** la familia opera como seguro (corresidencia, pooling, remesas) — PORQUE G5 — `[FUERTE]`. · **id:** `familia.seguro.volatilidad_ausencia_estado`
  - ⚠️ **REFUTADA — `R5.1` → veredicto `A`, archivado 4/ago/2026 (ADR-55, `hitoD-preregistro §Registro de veredictos archivados`).** Es la **única** regla del motor que declara `PORQUE G5`, y la primera regla refutada con **dato primario propio (a)**. **Alcance exacto de la refutación:** cae la lectura de **sustitución** — *"la familia opera como seguro **porque el Estado está ausente**, y retrocede cuando el Estado entra"*. **No cae** el hecho de que la familia opere como seguro. Medido sobre 6 olas de ENIGH (2012-2022), hogares con adulto mayor, beneficiario vs. no beneficiario de la Pensión del Bienestar: corresidencia y transferencia intrafamiliar hacia mayores **no se reducen en ninguna ola ni en ningún tercil de ingreso post-reforma — si acaso suben** (transferencia: brecha positiva y significativa 2016-2022; corresidencia: +9.1 a +9.8pp, significativa, en el tercil alto). Es refutación por **inversión de signo**, no por sustitución pequeña.
  - **Lo que este veredicto NO hace:** el tier `[FUERTE]` **se queda** — un tier entra por el mapa de evidencia de un report al glosario (ADR-02) y re-tierar desde una corrida de falsación se saltaría esa cadena; acto aparte. **G5 no se apaga:** `familismo_apoyo` (0.50) sigue **ASIGNADO** y medido desde la Fase B — la corrida refuta la sustitución, no la existencia del canal familiar. Apagar el generador sería inferir de más.
...
- **49 reglas** *(42 en v2 · 43 en v2.1 por conf.07 · 44 en v2.3 al partir la diagonal)*. **Hito D (perímetro de 27 reglas, subconjunto de las 49): 4 de 27 corridas archivadas** — `R1.1` → veredicto `D` (inejecutable por hueco de mercado, no de dato), `R3.2` → veredicto `B` (gate de 20pp aritméticamente inalcanzable), `R7.2` → veredicto `D` (inejecutable por hueco de instrumento, no de mercado ni de muestra), `R5.1` → veredicto `A` (**refutada** sobre 6 olas ENIGH — por inversión de signo, no por reducción pequeña: la familia no se retira cuando el Estado entra). Fuente única: `hitoD-preregistro §Registro de veredictos archivados`. *(Corregido 4/ago/2026 — propaga `R7.2` → `D` y `R5.1` → `A`, ADR-55.)*
- **Ejercicio distinto, no confundir con lo anterior (ADR-45, D-05):** `glosario §6/§10` corrió por su cuenta, el 27/jul/2026 — antes de que el Hito D existiera —, un ejercicio informal sobre "utilidad + fricción baja > confianza" (dominio seguro agrícola), **veredicto informal B**: sin contraejemplo limpio, candidato irresuelto, techo estructural que impide alcanzar el veredicto C con fuentes públicas. No tiene ID de regla, no vive en el bloque append-only de `hitoD-preregistro`, y no cuenta en el "4 de 27" de arriba.
...
| **49 reglas · 27 de perímetro · 4 de 27 corridas · 20 `[FUERTE]`** | 49/27/20 sin cambio respecto del v3.4, recuento mecánico sobre §3.B, reproducible con `python3 tests/validador_registro_ids.py`; corridas 2→3 por `R7.2` → `D` y **3→4 por `R5.1` → `A`**, ambos archivados 4/ago/2026 (ADR-45, ADR-55) | **Heredado, con receta ejecutable — corridas actualizado 4/ago/2026** |
