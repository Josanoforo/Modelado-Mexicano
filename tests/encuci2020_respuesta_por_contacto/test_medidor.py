...
SCRIPT = ROOT / "data/corrida0/CALC-ENCUCI2020-RESPUESTA-POR-CONTACTO-0001/medidor.py"
...
def test_partitions_and_empty_conditional_denominator():
    ...
    assert abs(result["RESULT-ENCUCI2020-RPC-VALIDACION-PARTICION"]) < 1e-12
    assert abs(result["RESULT-ENCUCI2020-RPC-VALIDACION-UNION"]) < 1e-12
    profile = {x["variable"]: x for x in json.loads(
        result["RESULT-ENCUCI2020-RPC-PERFIL-POR-CONTACTO"])}
    assert profile["AP5_16_10"]["p_entrega_dado_solicitud"]["p"] is None
    assert profile["AP5_16_1"]["distribucion"]["ambas"]["p"] == 0.25
    overlap = json.loads(result["RESULT-ENCUCI2020-RPC-SOLAPAMIENTO-OTROS-CONTACTOS"])
