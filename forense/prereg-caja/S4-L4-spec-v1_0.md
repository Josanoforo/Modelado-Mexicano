...
Las dos corridas están **`PENDIENTE-DE-MESA`** (`forense/firmas-pendientes.tsv` fila `FP-298`, `ABIERTA`; el ítem que cubre este `id` específico es distinto de `#6`/`#8` de `N5` — `L9`/`L11` corrieron contra el `id` de canon directamente, sin pasar por la clasificación de `N5`) — **ningún sello de canon se ha movido todavía**, así que esta pieza no reabre nada archivado (`modelo-decision-v4_0.md` línea 554 sigue en `[MEDIA]`).
...
No abre ningún archivo de §6. No calcula `Δ_elección` ni ningún IC95. No mueve el tier de `civico.voto.clientelar_si_observable` ni el de `civico.voto.agencia_con_secreto`. No sella `FP-298` para los 7 ítems de `N5 §3` distintos de `#6`/`#8`. No reabre ni corrige los veredictos `CONTRARIA` de `L9`/`L11` — los cita como lo que son, evidencia ya corrida sobre el mismo `id`, pendiente de sello de mesa.
