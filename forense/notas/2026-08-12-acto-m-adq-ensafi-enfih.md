...
Tres nemónicos (`FAC_VIV`, `FAC_HOG`, `FACTOR`) cubriendo 15 tablas nombradas explícitamente (recontado en revisión: TViviendas/THogares/TSDem/TModulo/TPropiedad/TComparte/TUnico/TAuto/TMoto/TDepar/TBanca/TNomina/TEduca/TPersonal/TGrupal — el "14" original omitía una).
...
## 3 · Correcciones post-revisión (commit `611dd55` en adelante) — no se edita lo anterior, se declara aquí

**§2.3/§2.4 quedaron stale desde `611dd55`, nunca retractadas.** Este párrafo de cierre (arriba) y §2.3 dicen "cero filas escritas" / filas "PROPUESTAS, NO ESCRITAS" — cierto cuando se escribió, falso desde `611dd55` (mesa secuenció y escribió las dos filas ese mismo día). Detectado en revisión, no en su momento. Precedente de este mismo archivo (`forense/hallazgos.md:202`) para este caso: se declara la corrección, no se edita el texto que quedó atrás. Aplicado aquí: §0-§2 arriba quedan como el registro de lo que este acto sabía y proponía en su momento; este §3 es lo que cambió después.

**Clasificación de las dos filas de puerta corregida — de `EXISTE-SATISFACE` a `EXISTE-NO-SATISFACE`.** Las filas que `611dd55` escribió en `data/universo-puertas-2026-08-12.tsv` (líneas 33-34) copiaron el borrador de §2.3 literal, incluyendo su `EXISTE-SATISFACE` de una sola palabra por ficha. Ese token colapsaba lo que §2.1/§2.2/§2.4 arriba ya documentaban: ENSAFI tiene 8/10 piezas satisfechas pero 2 no (FD/descriptor sin cerrar, "Conociendo la base de datos" NO-ENCONTRADO); ENFIH tiene 8/9 pero 1 no (mismo NO-ENCONTRADO). Un lector de la fila de puerta — sin abrir esta nota — no tenía forma de saberlo; un gate futuro (p. ej. M-APERTURA §6, que explícitamente busca "si algo quedó NO-ENCONTRADO") tampoco. Corregido directamente en el TSV (no es un `hallazgo` sellado, es una clasificación operativa que se actualiza como la de WVS ya se actualizó en el worktree hermano) a `EXISTE-NO-SATISFACE`, con las piezas faltantes nombradas en la columna `universo_declarado`.

**Correcciones menores, mismo commit:** "3 tablas" → 4 (ENSAFI, línea 112 arriba ya decía 4; el conteo de 3 solo vivía en `hallazgos.md:205`, corregido ahí también) y "14 tablas" → 15 (ENFIH, arriba en este mismo archivo — recontado contra el export JSON real, ninguna tabla inventada, solo una que faltaba contar) · typo "geniunos"→"genuinos" (§2.3, la fila propuesta ya no coincidía byte a byte con la fila escrita) · cita de línea `inventario-fuentes-credito-ahorro-finanzas-hogar-mexico.md:120`→`:119` para la cifra "ajustadas a 23 000" (línea correcta confirmada por `sed`).

**Procedencia de la regla de concurrencia (ARRANQUE 5-bis), citada explícitamente ahora.** "Un solo escritor a la vez" no viene del encargo archivado en este mismo commit — ese documento, en su propio §3 PERÍMETRO, dice lo contrario para el puntero de puertas ("cada quien añade filas, nadie edita filas ajenas", sin gate de secuenciación). La regla vino de una instrucción directa de mesa a la sesión que orquestó este acto y su hermano P·Lote-1 en paralelo (mismo día, 12/ago/2026, fuera de este documento y del encargo co-archivado), motivada por que `data/manifiesto.yaml` y `data/universo-puertas-*.tsv` no tienen `merge=union` y `tests/manifiesto.py --registra` reescribe el manifiesto completo. No es una desviación oculta del encargo — es una instrucción posterior y más estricta que él, y debía citarse como tal en vez de presentarse como si fuera parte del bloque ARRANQUE estándar.

**La concurrencia NO quedó resuelta a nivel de merge — corregido el sobre-alcance de esa afirmación.** El cierre de §2.4 arriba y el mensaje de `611dd55` dicen que la concurrencia con P·Lote-1 "se resolvió por secuenciación." Verificado en revisión con `git merge-tree` contra las puntas reales de las dos ramas: **sí hay conflicto textual real** en `data/universo-puertas-2026-08-12.tsv` — las dos ramas anexan filas nuevas inmediatamente después de la misma última línea (`RNM_ENCUCI_2020_ficha647`), y un merge de 3 vías no puede ordenar dos inserciones ancladas en el mismo renglón de contexto, sin importar en qué orden se escribieron. Lo que la secuenciación sí logró: cero corrupción por escritura simultánea, cero ids de puerta duplicados entre las dos ramas (verificado, sigue siendo cierto). Lo que NO logró: un merge limpio — quien fusione la segunda de las dos PRs (M-ADQ o P·Lote-1, la que llegue después) va a encontrar un conflicto real en ese archivo y tiene que resolverlo a mano, no es una sorpresa del botón de GitHub, es esperado. Mismo defecto, segunda instancia: las dos ramas también archivaron `forense/encargos/2026-08-12-encargos-finales-plan-descargas-completo.md` (regla A.3) con preámbulos distintos — commit `31c4ec3` aquí, `8cf0320` en P·Lote-1 — mismo cuerpo del encargo (187 líneas idénticas), preámbulo distinto: conflicto add/add, también sin resolver todavía, también sin bandera previa.

**Desviación no declarada, ahora declarada: la fila de WVS del acto hermano se editó en sitio, no solo se añadió.** No es un hallazgo de este acto (vive en `mm-p-lote1-adquisicion`), pero el mismo patrón de "perímetro dice solo filas nuevas, la ejecución edita una existente" ocurre en ambas ramas y vale la pena nombrarlo una vez: el perímetro declarado en el encargo (§2/§3, "cada quien añade filas, nadie edita filas ajenas") se interpretó correctamente en este acto (M-ADQ solo añadió) pero no en el hermano (P·Lote-1 actualizó la fila WVS existente en vez de solo añadir) — sustantivamente la decisión correcta (evita una fila duplicada/huérfana para la misma fuente), pero debió declararse como desviación del perímetro en su momento, no quedar implícita.

**No corregido aquí, declarado para mesa:** el front-matter del encargo co-archivado (`forense/encargos/2026-08-12-encargos-finales-plan-descargas-completo.md`, línea 5) llama a PR #184 "ACTO K ya corrido y fusionado." Verificado en revisión (`gh pr view 184`): PR #184 es la corrida de E4c/R5.1-D2 (rama `mesa/e4c-paso3-corrida`, "NO FUSIONAR sin mesa" en su propio título) — no tiene relación con ningún "ACTO K", y la cadena "ACTO K" no aparece en ningún otro archivo del repo. El texto archivado no se edita (regla A.3: es el registro de lo que se recibió, no un resumen corregible) — se declara aquí porque un lector futuro de ese front-matter creería que el acto K se cerró con esa PR, y no hay ningún otro documento que diga qué es realmente "ACTO K" o si corrió alguna vez.
