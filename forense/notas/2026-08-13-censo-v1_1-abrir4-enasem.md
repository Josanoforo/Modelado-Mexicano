...
## 12 · Merge de `origin/main` (VENTANA 2, mesa) — resultado crudo, y la propuesta de cierre de la Entrada 1 de `registro-recalculo-v1_0.md`

**Merge.** `origin/main` se movió 19 commits desde la base de este acto (`dcc4f6a`) hasta `49e987f` (PR #201) — `SONDA-1` (#197), `ADR-72`/`registro-recalculo-v1_0.md` (#199), `Censo de explotación` (#201). `git merge origin/main --no-edit`: **auto-merge limpio**, sin conflictos (`forense/hallazgos.md` se fusionó solo — ambas ramas apendieron al final del archivo, sin tocar la misma línea). Re-corrida de la suite tras el merge:
...
Baja de `22 FAIL` a `18 FAIL` por el merge (mejora ajena a este acto, viene de los commits fusionados de `ADR-72`/censo de explotación — no se investiga aquí, fuera de perímetro).

**`APERTURA-ISSP` (PR #200) — NO incorporado, por instrucción explícita de mesa.** Verificado antes de decidir nada: `PR #200` sigue `OPEN`, no fusionado a `main` (`gh pr view 200`) — no llegó con este merge y no habría nada que incorporar aunque quisiera. Aun si hubiera fusionado, la instrucción de mesa es no absorberlo: `APERTURA-ISSP` corrobora la fila 14 con un segundo instrumento (ZA6980 v35) además de ENBIARE — es convergencia sobre una fila que ya está en `RUTA-C`, no una reclasificación, y la foto del corpus de este acto (§8 de `censo-estimabilidad-coeficientes-v1_1.md`, declarada contra `dcc4f6a`) es exactamente lo que protege contra tener que re-derivar después del hecho. No se toca `censo-estimabilidad-coeficientes-v1_1.md` en este commit.

**Propuesta de cierre — Entrada 1 de `forense/registro-recalculo-v1_0.md` §1 (no escrita en el registro por este acto; mesa/acto propio la aplica).** Texto propuesto para la columna `estado` de la fila `1 · Censo v1.1`:

> `RECALCULADO — CAMBIA` — PR #198. Universo declarado: `gobernanza:623` (ADR-57(c), las tres llaves, citadas íntegras) + 6 payloads ENASEM 2018/2021/2024 (`data/manifiesto.yaml`, hashes 6/6 `COINCIDE`, diccionarios abiertos completos — 6,471 variables) + 28 celdas de `data/abrir4-variables-2026-08-08.tsv` + `data/curacion-registro/relaciones.tsv` (filas N12/N13/N14), @ `dcc4f6a`. Tres filas del censo (12·`familismo_apoyo`, 13·`familismo_obligacion`, 14·`radio_confianza`) reclasifican `SIN-RUTA` → `RUTA-C` — ninguna a `RUTA-I` (regla de precedencia: co-observación sin llave de identificación no es identificación; verificada contra la definición propia de `RUTA-C` en `censo-estimabilidad-coeficientes-v1_0.md` §1, que ya cubre el caso sin necesidad de clase nueva). Cruce directo de ENASEM: negativo definitivo para `radio_confianza` (0/6,471 variables de confianza en las tres rondas) y para `familismo_obligacion` (único candidato es un ítem de personalidad general, no de deber familiar); precondición de panel real para `familismo_apoyo` (`G17`, tres olas, `UNHHIDNP` persistente) propuesta como candidata a diseño futuro, no ejercida como llave. Reparto: `RUTA-A`=3 (sin cambio) · `RUTA-C`=2→5 · `RUTA-I`=1 (sin cambio, `CAL-G3` no tocada) · `SIN-RUTA`=9→6. Universo de llaves declarado en las 9 filas `SIN-RUTA` originales de v1.0: **9 de 9** (v1.0: 1 de 9). Contador `llaves de identificación ejercidas: 0 de 2` (`registro-llaves-identificacion-v1_0.md`) **no se mueve** — ninguna fila alcanzó `SELLADA_NO_EJERCIDA`, sin propuesta de movimiento. Detalle completo: `forense/censo-estimabilidad-coeficientes-v1_1.md`, `forense/notas/2026-08-13-censo-v1_1-abrir4-enasem.md`.

**Sobre la Entrada 0 (mismo §1, "Cotejo censo ↔ relaciones.tsv"), no incluida en la instrucción de mesa de esta ventana — se declara la relación, no se propone su cierre.** Esa fila ya se lee `ABIERTA — la absorbe CENSO-v1.1 (entrada 1, mismo acto)`: las tres filas que motivaron su apertura (N12/N13/N14) son exactamente las tres que este acto reclasificó, y quedan resueltas en el mismo sentido. Pero su alcance declarado es más ancho que el de este acto — "el censo no trae un campo `necesidad_id` cruzable por máquina contra `relaciones.tsv`... identificar la necesidad conceptual de cada una de sus 15 filas exige la misma lectura semántica" — y este acto no hizo ese mapeo para las 12 filas restantes (fuera del perímetro del encargo `CENSO-v1.1`, que se limitó a las 9 `SIN-RUTA` ya nombradas). No se propone texto de cierre para la Entrada 0 aquí — se deja nombrada para que mesa decida si cierra junto con la Entrada 1 (mismas tres filas, mismo acto) o si permanece abierta hasta el cotejo completo de las 15.
