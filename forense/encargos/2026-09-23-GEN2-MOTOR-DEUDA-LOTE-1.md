...
Ejecutado por PR #1054 (`acto/gen2-motor-deuda-lote-1`). P2 resuelto: dictamen `NO-PROCEDE-HISTÓRICO`, `tests/test_motor_holdout.py` pasa a 0 FAIL (`saltar()` con razón citada); `NC-...-e8fa-02` ya estaba CERRADA en `main` por `ACTO GEN2-TRAMITE-FIRMAS-11` al sincronizar — este acto aporta el fix de código real, sin re-editar esa fila. P1/P3/P4 en `## NO-CORRIDO / RESERVAS` arriba. Enmienda fechada 23/sep: se reconstruyó `forense/no-corrido.tsv` línea a línea tras detectar que un round-trip con el módulo `csv` de Python rompió `tests/test_adq_demanda_vigente.py` en CI (defecto ya documentado en ese test); corregido, 0 fallos. Detalle: `canon/gobernanza-v1_15.md` `ADR-260923-GEN2-MOTOR-DEUDA-LOTE-1-e270-01`, `forense/notas/2026-09-23-GEN2-MOTOR-DEUDA-LOTE-1-cierre.md`.
