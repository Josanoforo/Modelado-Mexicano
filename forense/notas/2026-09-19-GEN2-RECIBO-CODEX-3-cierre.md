...
Los adjuntos disponibles se archivaron byte a byte en `forense/encargos/insumos-gen2-recibo-codex-3-2026-09-19/`, con `MANIFIESTO-RECIBO3-SHA256.txt`; tres insumos idénticos ya archivados se citan por su ruta canónica y no se duplican. El original nombrado `ENCARGO-GEN2-RECIBO-CODEX-3-2026-09-19.md` no vino en el directorio de adjuntos, no estaba en el árbol ni apareció en su historia: no se fabricó. Por la misma razón no se reconstruyen de memoria sus seis semillas `PARA-v2.15`; la ausencia queda como reserva explícita.
...
| D3 / ENCIG original | `FP-386` ABIERTA. La relación técnica de sucesión no decide `cuenta_gen2`, `SUPERADO` ni clasificación. |
...
| DEM-AHORRO / #855 | `NC-0333` ABIERTA con las tres disposiciones de mesa; el handoff no elige una. |
...
| #855 | `89610c54429cca3e0f181902f23400ea878aee79` | handoff de resultado/salud, prueba y cableado de automatización | 0; deja `NC-0333` |
...
La fuente de #866 contiene 41 asientos: 38 `REPRODUCE/IDENTICO`, un DIN `NO-REPRODUCE/IDENTICO` y dos `NO-VERIFICABLE` por acceso (ENCRIGE y ENSANUT). En la base de apertura las vistas aún proponían 82 cambios de campo —dos por corrida—; por eso un `verify` que imprime y no asienta es media verificación. La publicación y los cierres dependientes se completan únicamente después del producto integrado del carril A, con el lote exacto y comprobando en memoria cero transiciones más el guardia ordinario.

Los cuatro pisos originales de #866 quedan preservados y vetados. Sus defectos se consolidan en un expediente: persistencia/código real, cardinalidad y salida por celda, dos universos incompatibles en ENCIG, uso inválido de `BP1_20`, y semántica de `P5_6` de ENIF 2021 — tarjeta de débito, ya documentada por la spec DIN y confirmada contra FD. El eventual sucesor corregido será otra identidad: no hereda ni veto ni adopción.
...
- Falta el adjunto original y, con él, las seis semillas exactas `PARA-v2.15`; no se inventan.
- `FP-386` requiere decisión de mesa sobre el ENCIG original y el contador.
- `NC-0237`, `NC-0024`, `NC-0076`, `NC-0239`, `NC-0300` y `NC-0333` permanecen abiertas por sus objetos propios.
...
- Los cierres NC-0315/0284–0288/0329 se adjudican individualmente tras incorporar A; un asiento no resuelve acceso, procedencia o producto distinto por asociación.
