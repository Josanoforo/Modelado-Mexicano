**CONSUMIDO** — `ADR-137` (`canon/gobernanza-v1_15.md`), `ACTO LANDING-EMISOR-M1`, 21/ago/2026. Ejecutado: §1 (los cuatro adjuntos de código/test/tsv, hashes verificados exactos), §2 (las tres salidas crudas coinciden con lo esperado), §3 (ADR-137), §4 (`FP-99` FIRMADA, `FP-100` ABIERTA, tres líneas en `hallazgos.md`). Hueco declarado, no oculto: los cuatro documentos de sesión de §1 (diseño/benchmark/nota) no llegaron adjuntos a este acto y no se archivaron — ver `ADR-137(c)`.
