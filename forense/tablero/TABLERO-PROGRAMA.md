...
- **NC abiertas por razón (token A.14, prefijo exacto).** abiertas `278` · por token: `DECISIÓN-DE-MESA-PENDIENTE` 23 · `DIFERIDO-A` 43 · `FUERA-DE-PERÍMETRO` 54 · `NO-VERIFICABLE-AQUÍ` 16 · `PARO-ENTORNO` 2 · `PARO-PREMISA` 30 · `SUSTITUIDO-POR` 4 · prosa (sin token reconocible) `106`.
- **GEN2 (derivado de `corrida0 status`).** corridas selladas `234` / requeridas `87` · resultados sellados `66202` / activos `211` · pendientes `211` · dependencias numéricas legacy activas `146` · validación independiente `215` · diferencias materiales `0` · NC- abiertas `278` · replays LEGACY-GEN1 sellados `5` (no cuentan). El `0 / N` es la lectura correcta: el aparato se construyó antes que las corridas.
