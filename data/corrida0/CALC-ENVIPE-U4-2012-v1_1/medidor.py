"""`CALC-ENVIPE-U4-2012-v1_1` — v1.1 de `CALC-ENVIPE-U4-2012`: el par de
diseño (EST, UPM) de la persona sale de `Tmod_Vic.DBF` por hogar (el `EST` de
`tper_vic.dbf`/`tsdem.DBF` no es el estrato de diseño: 4 valores). `U4` (unidad PERSONA, `FAC_ELE`) en ENVIPE 2012 por
...
`forense/prereg-caja/ENVIPE-2012-U4-spec-v1_1.md` (sucesora de v1_0, intacta). Lector DBF copiado de
...
def medir(inputs, contrato):
    ...
    P = "RESULT-ENVIPE-U4-12-V11-"
    out = {P + s: None for s in par["sufijos_result"]}
    for s in par["sufijos_result"]:
        if s.startswith(("N-", "G-N-", "G-PER-N-", "G-TSDEM-N-", "G-MOD-N-")):
            out[P + s] = 0
        elif s in ("ESTADO", "METODO-IC", "G-CONTROL-U1", "G-CONTROL-V1-0") or s.startswith(("PERFIL", "G-TSDEM-PERFIL", "G-PERFIL")):
    ...
    # ── Tmod_Vic ──────────────────────────────────────────────────────────
    mod, falt, perfil_mod = _lee_dbf(zf.read(T_MOD), list(HOGAR_KEYS) + ["R_SEL", "BPCOD", "BP1_20", "BP1_23", "FAC_DEL", "EST", "UPM"])
    ...
    # v1.1: par de diseno por hogar desde Tmod_Vic (todas las filas del hogar)
    dis_mod = {}
    for f in mod:
        h = tuple(f[k].strip() for k in HOGAR_KEYS)
        dis_mod.setdefault(h, set()).add((f["EST"].strip(), f["UPM"].strip()))
    out[P + "G-MOD-N-HOGARES"] = len(dis_mod)
    out[P + "G-MOD-N-HOGARES-EST-UPM-MULTI"] = sum(1 for v in dis_mod.values() if len(v) > 1)
    out[P + "G-N-EST-DISTINTOS-MODULO"] = len({e for v in dis_mod.values() for e, _ in v})
    out[P + "G-PERFIL-DISENO-MODULO"] = ("ESTRATO[" + _perfil_texto([f["EST"] for f in mod]) + "] UPM["
                                        + _perfil_texto([f["UPM"] for f in mod]) + "]")
    ...
    out[P + "G-N-EST-DISTINTOS-PERSONAS"] = len({e for g in hog.values() for e, _ in g["dis"]})
    out[P + "G-N-HOGARES-UPM-DISCORDA-MOD-VS-PER"] = sum(
        1 for h, v in dis_mod.items() if len(v) == 1 and h in resuelto and resuelto[h]["dis"] is not None
        and next(iter(v))[1] != resuelto[h]["dis"][1])
    ...
    v10 = par["control_v1_0"]
    ok = (out[P + "P-C2-U4"] == float(v10["P_C2_U4"]) and out[P + "P-C1-U4"] == float(v10["P_C1_U4"])
          and out[P + "N-U4"] == int(v10["N_U4"]) and out[P + "MASA-FAC-ELE-U4"] == float(v10["MASA_FAC_ELE_U4"]))
    out[P + "G-CONTROL-V1-0"] = "REPRODUCE" if ok else "NO-REPRODUCE"
