...
**Base:** `2e25e6c` al abrir (PR #772) → `b983988` tras merge (PR #773). Cero conflictos.
...
**Firma de contador con OBJETO.** El lanzamiento la trae verbatim («Firma de contador con OBJETO embebida desde el lanzamiento — para no repetir el hueco de FP-375 …»); autoridad mesa, fecha 15/sep/2026, OBJETO = cada corrida sellada aquí sobre spec del encargo 1. Se escribió como **cuatro filas en `data/corrida0/decisiones.tsv`** (`3ab7278`) citando ese párrafo; `_cuenta_gen2_resuelto` da precedencia a `decisiones.tsv` sobre la etiqueta `PENDIENTE-DE-MESA` que traen los cuatro `spec.yaml` del encargo 1 (que **no se editan**, E.3). Proyección en seco del registro: las cuatro `OFERTA · SELLADA · GEN2 · cuenta_gen2=SI · «decision de mesa (decisiones.tsv)»`. El merge de mesa perfecciona la firma. Esto satisface por producto `NC-0192` (medidores) y `NC-0198` (firma) de `PR #775`, que todavía no están en `origin/main` — ver `## NO-CORRIDO`.

**Registro (`corrida0 registro --verifica --escribe`).** Primera pasada: `PARO · REPLAY-PISADO (NC-0094)` por `CALC-M-marco-M-sorteado-v1_3` (proyectó `NO-REPRODUCE · CONTEXTO-DISTINTO`); `verify` aislado 2/2 → `REPLICA-RESULTADO · CONTEXTO-DISTINTO`: el mismo veredicto inestable in-process que `NC-0182` ya asienta. **No se nombró en `--lote`**; la segunda pasada proyectó su veredicto estable y escribió: `corridas.tsv` 168 → **172** filas, `resultados.tsv` 4 743 → **4 862** (+119 `RESULT`: 26 + 31 + 33 + 29), `usos.tsv` **207, idéntico a `origin/main`** (cero adopciones, probado). Pisadas medidas contra `origin/main` excluyendo las cuatro filas propias: `resultados.tsv` **0**; `corridas.tsv` **1 fila ajena, 1 columna** — `CORR-0008.input_ids`, que pasa de la glosa «NO-LOCALIZADO …» al texto ya corregido por `NC-0188` (`ACTO GEN2-ADOPCION-VENTANILLA-2`, PR #762): es la vista derivada alcanzando al manifiesto que `origin/main` ya tiene, no evidencia de replay. `fuente_replay`/`resultado_replay`/`contexto_replay` de las 168 corridas ajenas: sin cambios.

## 4 · Contador, sin adornos

`python3 tools/corrida0.py status`, antes → después (mismo comando, mismo árbol salvo los commits de este acto):

| contador | antes | después | Δ |
|---|---|---|---|
| `N_corridas_requeridas` | 82 | 82 | 0 |
| **`N_corridas_selladas`** | **63** | **67** | **+4** (`CALC-ENIGH-0001`, `CALC-ENFIH-0001`, `CALC-EDER-0003`, `CALC-ENUT-0001`, todas `cuenta_gen2 = SI` por la firma del lanzamiento) |
| **`N_resultados_gen2_sellados`** | **2 882** | **3 001** | **+119** |
...
| `no_corrido_abiertas` | 57 | 57 | 0 antes de la cascada |
...
- **No cerró `NC-0192`/`NC-0198`** en `forense/no-corrido.tsv`: las filas viven en `PR #775`, no en `origin/main`; se cierran por producto en el merge (ver `## NO-CORRIDO`).
