"""ENDIREH 2021, violencia laboral, elegibilidad por trabajo remunerado.
...
MEMBER = "TB_SEC_VIII.csv"
DEM = "TSDem.csv"
ACTS = tuple(range(1, 20))
...
def classify_binary_union(values):
    vals = [str(x).strip() for x in values]
...
def medir(inputs, contrato):
    with zipfile.ZipFile(inputs[PAYLOAD]["ruta_absoluta"]) as archive:
        ...
        with _member(archive, MEMBER) as stream:
            for r in csv.DictReader(stream):
                if r.get("T_INSTRUM") not in {"A1", "A2", "B1", "B2", "C1", "C2"}:
                    continue
                ...
                if not r.get("EST_DIS") or not r.get("UPM_DIS"):
                    continue
                life = (classify_binary_union([r.get(f"P8_9_{i}", "") for i in ACTS])
                        if r.get("P8_1") == "1" else None)
                recent_values = []
                for i in ACTS:
                    ever = r.get(f"P8_9_{i}", "").strip()
                    recent_values.append("4" if ever == "2" else r.get(f"P8_11_{i}", ""))
                recent = (classify(recent_values) if life is not None and r.get("P8_4") == "1"
                          else None)
    ...
    return {"RESULT-ENDIREH2021-LAB-TABLA": json.dumps(table, ensure_ascii=False, sort_keys=True),
            "RESULT-ENDIREH2021-LAB-N-ELEGIBLES": len(rows)}
