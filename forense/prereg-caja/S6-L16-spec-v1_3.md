...
### `prereg-caja-S6-L16` · **v1.3** · 8 de septiembre de 2026 · `sucesora_de: v1_2`

> | | |
> |---|---|
> | **ARCHIVO** | `forense/prereg-caja/S6-L16-spec-v1_3.md` |
> | **NOMBRE ESTABLE** | **`prereg-caja-S6-L16`** — cítalo así, nunca por nombre de archivo |
> | **SUCESORA DE** | **`v1.2`** (`forense/prereg-caja/S6-L16-spec-v1_2.md`, `sha256 = c1cd3b6367d4edc4d38401e19752adb9656d238a81ae6cff9db11dacaf73da1c`). **El sello de `v1.2` no se edita** (E.3): sus bytes quedan intactos y su sidecar sigue verificando. `v1.3` la **supera**; `CALC-0003`/`CALC-0003-v2`, que corrieron contra `v1.2`, siguen siendo corridas válidas de `v1.2` y **no se retiran**. |
> | **QUÉ ES** | El mismo pre-registro de `v1.2` —Rama A′ para `salud.atencion.grave` (`R4.4`) sobre `ENNViH` 2002: dos disparadores (`es09`; `ec01*`) × dos desenlaces (`HS`; `CE`) = 4 celdas × 2 brazos = **6 filas efectivas**— con **DOS diferencias, y ninguna más**, listadas abajo y en §0.5. |
> | **LAS DOS DIFERENCIAS, Y NINGUNA MÁS** | **(a) §1 — la codificación de `T1`.** `v1.2` congeló «`T1 = 0` si `es09 == 0`». **El código `0` no existe en esta encuesta**: el manual de codificación fija «No» en el código **`3`** (`ehh02cb_b3b.pdf` pág. 8; `ehh02cb_bx.pdf` pág. 49). `v1.3` escribe `3`. **`FP-349`.** · **(b) §3.5 — el conglomerado del IC95.** `v1.2` afirmó que la unidad primaria de `ENNViH` (la localidad) «no está en los archivos» y por eso pre-registró conglomerado = hogar **con reserva de varianza subestimada**. **La premisa es falsa**: `ehh02dta_bc/c_portad.dta` —el mismo archivo del que la spec toma `estrato`— trae `loc` («LOCALIDAD») e `id_loc` («ID LOCALIDAD»). `v1.3` pre-registra **conglomerado = localidad** y **levanta la reserva**. **`FP-351`.** |
> | **QUÉ NO CAMBIA** | Todo lo demás, verbatim de `v1.2`: §0 (corrección de premisa y reclasificación de `FP-332` `D2`), §1 `T2`, §2 (los dos desenlaces, las **tres** tablas de clasificación, el estimando `P_PUB` y su sensibilidad, las 4 celdas y la cláusula de multiplicidad), §3.1–§3.4 y §3.6, §4 (la escala de veredicto y §4.1), §5 (`se_mueve_si`), §6, §7 y §8. **Ni una tabla de clasificación se toca, ni un universo, ni un ponderador, ni el umbral de `n`, ni la regla de pareja del brazo `bx`, ni cuál fila es la primaria.** |
> | **QUÉ NO ES** | **No es una corrección de resultados.** No mira ningún `Δ` ni ningún IC95 de `CALC-0003-v2` para decidir nada de lo de arriba: las dos diferencias salen del **codebook** (a) y del **metadato de un archivo** (b), no del desenlace. No mueve el tier de `R4.4` (hoy `[MEDIA]`, `canon/modelo-decision-v4_0.md:747`) ni toca `canon/`. No edita `v1.0`, `v1.1` ni `v1.2` — las tres quedan íntegras en sus rutas. No reabre `FP-332`. No pre-registra 2005 ni 2009 (§7). **No contiene la pre-declaración `B-bis` del acto que la estrena** — vive, deliberadamente, en `data/corrida0/CALC-0003-v3/spec.md` §0.3, congelada en el mismo `COMMIT-1` y **antes** de correr: así esta spec conserva **exactamente dos** diferencias con `v1.2` y la lectura del re-corrido no se cuela dentro del contrato sellado. |
...
### 0.5 · Las dos diferencias con `v1.2`, en una tabla — y por qué son dos y no tres

| # | § | `v1.2` decía | `v1.3` dice | de dónde sale | firma |
|---|---|---|---|---|---|
| **(a)** | §1, `T1` | «`T1 = 0` si `es09 == 0`» | «`T1 = 0` si `es09 == 3`» | `ehh02cb_b3b.pdf` **pág. 8** · `ehh02cb_bx.pdf` **pág. 49** — el código `0` no existe en esta encuesta | **`FP-349`** |
| **(b)** | §3.5, varianza | conglomerado = **hogar** (`folio`), *«la localidad no está en los archivos»*, **con reserva** de varianza subestimada | conglomerado = **localidad** (`id_loc` de `c_portad.dta`), **reserva levantada** | metadato de `ehh02dta_bc/c_portad.dta`: `edo`/`mpio`/`loc`/`id_loc` existen | **`FP-351`** |

⚠️ **La diferencia (a) toca DOS sitios, no uno — y sigue siendo una.** `v1.2` escribió el código «No» en §1 («`T1 = 0` si `es09 == 0`») **y otra vez** en la cláusula `se_mueve_si` de §5 («`Δ = P_PUB(es09=1) − P_PUB(es09=0)`»). Corregir sólo la primera dejaría una `v1.3` que se contradice a sí misma en la cláusula que decide si la regla se mueve. Se corrigen las dos; el hecho de codebook es el mismo y la cuenta de diferencias no sube.

**Las dos son correcciones de HECHO, no de criterio.** (a) no cambia qué se mide, cambia con qué código se lee un «No» que el papel escribió mal; sin ella el contraste sale degenerado en silencio. (b) no cambia qué se mide ni sobre quién, cambia **la unidad de remuestreo del IC95**; la dirección firmada de la spec —qué es público, qué es privado, cuál celda es primaria, qué umbral de `n`— no se toca en ninguna de las dos.

**Y por qué no hay una tercera.** La pre-declaración `B-bis` que gobierna cómo se lee el re-corrido —qué significa que `C3`/`C4` conserven o pierdan su intervalo, y qué significa que `C1` cambie de estado— **no está aquí**: vive en `data/corrida0/CALC-0003-v3/spec.md` §0.3, congelada en el mismo `COMMIT-1` y antes de abrir un solo `.dta`. Es una decisión declarada, no un olvido: **el contrato sellado conserva exactamente dos diferencias con su antecesora, y la lectura de una corrida concreta no se cuela dentro del contrato.**

**Qué pasa con `CALC-0003` y `CALC-0003-v2`.** Corrieron contra `v1.2` y **siguen siendo corridas válidas de `v1.2`**: no se retiran, no se re-etiquetan, sus bytes no se tocan. `CALC-0003-v2` es, además, el punto de comparación de la corrida de `v1.3` — misma escala, mismo universo, misma `seed`, comparables sin enlace. Que una corrida de `v1.2` haya reportado `no: 3` **por su cuenta**, declarando la discrepancia sin editar la spec sellada, es precisamente lo que hace que (a) sea una puesta al día del papel y no una corrección de resultados.
...
`T1 = 1` si `es09 == 1`; **`T1 = 0` si `es09 == 3`**; NS/NC (`8`) y faltante quedan **fuera** de los dos grupos y **se cuentan** en el reporte (`v1.1` midió 15 NS/NC en `p_es.dta`). **El adjetivo difiere por brazo** («SERIO» en `b3b`, «GRAVE» en `bx`) — ya declarado en `v1.1` §1.1, se hereda con la cita, no se borra: es una razón más para no agrupar los brazos.

🛑 **DIFERENCIA (a) CON `v1.2` — `FP-349`, y de dónde sale el `3`.** `v1.2` escribió «`T1 = 0` si `es09 == 0`». **Estos `.dta` no traen ninguna etiqueta de valor** (`variable_to_label` vacío en las variables de interés de los ocho archivos), así que **el manual de codificación es la única autoridad sobre los códigos**, y dice:
...
**«No» se codifica `3`. El código `0` no existe.** Corrida verbatim, la forma de `v1.2` habría seleccionado el **conjunto vacío** —sin error y sin aviso— y las seis filas habrían salido degeneradas: `T1 = 0` vacío ⇒ `P_PUB(T=0)` indefinido ⇒ `Δ` indefinido. **`CALC-0003` ya congeló `no: 3` y declaró la discrepancia sin editar la spec sellada; esta `v1.3` pone el papel al día con el código que ya corrió.** El mismo `3` gobierna `ce01` (pág. 21 / 56) y `hs01` (pág. 28 / 60), que esta spec usa como filtros de universo. **Corroboración independiente de que esos manuales describen ESTOS datos:** los totales del manual coinciden exactamente con las filas de cada `.dta` (19 804 / 19 803 / 19 799 / 1 848) y el `8. NS` = **15** de `es09` en `bx` reproduce, al caso, los «15 NS/NC en `p_es.dta`» que `v1.1` midió.
...
**Escala de reporte (`A-bis 3`): proporción ponderada**, en los términos de §2.4.

🛑 **DIFERENCIA (b) CON `v1.2` — `FP-351`. La premisa de `v1.2` §3.5 es falsa, y se corrige aquí.** `v1.2` escribió: *«La unidad primaria real de `ENNViH` (localidad) **no está en los archivos**, de modo que el IC por hogar **subestima la varianza** respecto del diseño verdadero»*, y de ahí pre-registró **conglomerado = hogar** con la reserva de que *«un `Δ` cuyo IC95 apenas excluya 0 se reporta como `PROPUESTA con reserva`, nunca como `CORROBORADA`»*.

**La localidad SÍ está en los archivos.** `ehh02dta_bc/c_portad.dta` —**el mismo archivo del que `v1.2` ya tomaba `estrato`**— trae, en su metadato, cinco columnas de geografía además de la llave: `edo` («ESTADO»), `mpio` («MUNICIPIO»), `loc` («LOCALIDAD»), `control` («NO. DE CONTROL DE HOGARES») e `id_loc` («ID LOCALIDAD»). No hubo que descargar nada ni abrir un archivo nuevo: la premisa se cayó al mirar el descriptor del que ya se leía otra columna.

**Diseño pre-registrado por `v1.3`, congelado antes de correr:**

> **Conglomerado = LOCALIDAD**, identificada por **`id_loc`** de `c_portad.dta`. **Estrato = `estrato`** del mismo archivo, sin cambio. **IC95 por bootstrap de LOCALIDADES dentro de estrato**, percentil 2.5/97.5, mismas réplicas y misma `seed` que `v1.2` usó para el bootstrap de hogares — de modo que **lo único que cambia entre las dos corridas es la unidad de remuestreo**.

**Por qué `id_loc` y no `loc` a secas — la trampa, nombrada antes de tocar el dato.** Bajo la convención geoestadística de INEGI, `loc` es un código de localidad **único dentro de su municipio, no a nivel nacional**: usarlo solo colapsaría en un mismo conglomerado localidades de estados distintos que comparten número, **inflando** el conglomerado y ensanchando el IC por un artefacto de codificación, no por diseño. La llave que identifica una localidad sin ambigüedad es la compuesta `(edo, mpio, loc)` — o `id_loc`, si resulta ser la misma cosa. **Cuál de las dos se usa no se decide a ojo: se decide por un conteo, y el conteo se publica.**

**Guardia de conglomerado, obligatoria y publicada como `RESULT` (nunca sólo en prosa).** La corrida emite `RESULT-CONGLOMERADO-LOCALIDAD` con los tres conteos sobre las filas de `c_portad.dta` que sobreviven al filtro de llave: número de conglomerados bajo `id_loc`, bajo `(edo, mpio, loc)` y bajo `loc` a secas, más el número de estratos, el de hogares y las localidades por estrato. **Si `id_loc` y `(edo, mpio, loc)` no producen el mismo particionamiento, la llave compuesta manda** —es la que la convención documenta— y la corrida lo declara en el mismo `RESULT`. Y emite también `RESULT-CONGLOMERADO-ANIDAMIENTO`: **una localidad que cruzara más de un estrato, o un hogar que cruzara más de una localidad, romperían el anidamiento que el remuestreo supone**; si eso ocurre, se reporta y **no** se corrige inventando un anidamiento que el archivo no tiene.

**La reserva de `v1.2` §3.5 QUEDA LEVANTADA.** Con el conglomerado correcto, el IC95 ya no subestima la varianza por omisión de la unidad primaria, y por tanto **la cláusula «un `Δ` cuyo IC95 apenas excluya 0 se reporta como `PROPUESTA con reserva`, nunca como `CORROBORADA`» deja de aplicar por esta causa**. Se levanta **sólo** esa reserva y **sólo** por esa causa: siguen íntegras, y ninguna de ellas se toca, (i) la cláusula de multiplicidad de §2.5 —`C1`/`b3b` decide, una fila secundaria sola es `PROPUESTA con reserva`—, (ii) la regla de pareja de ponderadores de §3.2, (iii) la coincidencia de signo entre definición base y sensibilidad de §2.4, (iv) el umbral de `n < 10` de §4, y (v) la prohibición causal de §3.6 (las cuatro celdas miden co-ocurrencia, no secuencia). **Levantar una reserva de varianza no convierte una co-ocurrencia en una causa.**

⚠️ **Y lo que esta diferencia NO es.** No es un cambio de estimando: `Δ` y `P_PUB` son los de §2.4, sin tocar. No es un cambio de universo: entran exactamente las mismas personas. **Es un cambio de estimador de varianza, y sólo eso** — la dirección esperada de su efecto sobre los IC (conglomerados más grandes ⇒ intervalos más anchos, salvo correlación intraclase nula) **se declara aquí, antes de correr, para que nadie la lea después como si fuera un hallazgo**. `ENNViH` 2002 sigue **sin traer UPM/PSU declarada** en ningún archivo (buscado en `v1.2`, cero): la localidad es la unidad primaria del diseño según la documentación de la encuesta, y es la mejor aproximación disponible en el microdato — se usa como tal y se dice.
...
**`se_mueve_si`, congelada en `v1.2` y heredada por `v1.3` sin cambio de contenido — con la notación del código «No» corregida donde reaparece (§0.5, diferencia **(a)**, *segunda ocurrencia*: el mismo hecho de codebook aplicado al otro sitio donde `v1.2` lo escribió; no es una tercera diferencia, es la misma llevada hasta donde llegaba):**

> **`R4.4` se mueve si, en la fila primaria `C1`/`b3b` (`T1` = `es09`, `D-HS`, Tabla A, `fac_3b`), `Δ = P_PUB(es09=1) − P_PUB(es09=3)` resulta `CONTRARIA` o `NO-DISCRIMINA` bajo las dos definiciones de §2.4.** El `ENTONCES` de la regla —«busca el sistema público»— predice que el antecedente **desplaza** la elección hacia lo público; un `Δ` que no lo haga, medido sobre el instrumento que sí tiene síntoma y lugar en la misma persona, es evidencia contra la regla, no contra el instrumento.
>
...
**Esta pieza —el documento de spec— no calcula ninguna celda, ninguna proporción, ningún IC95, y no mueve el tier de `R4.4` ni toca `canon/`. No edita `v1.0`, `v1.1` ni `v1.2`.** (En `v1.2` esta línea decía además «no abre ningún archivo de §6», porque aquella se redactó en NUBE sin corpus. `v1.3` se redacta en CAJA: para fijar la diferencia (b) se leyó el **metadato** de `c_portad.dta` y, como guardia marginal, **sus cinco columnas de geografía y su estrato** — `edo`, `mpio`, `loc`, `id_loc`, `estrato` — con el conteo declarado en `CALC-0003-v3/spec.md` §0.2. **Ni una variable de disparador ni de desenlace se leyó como observación al redactar esta spec.** Se declara aquí, no se omite.) **No reabre `FP-332`** — la reclasificación de §0.3 es del alcance de un veredicto de existencia, no de la firma. No corre el brazo `ENDIREH` de `v1.1` (sigue como está: corroboración de otro antecedente, nunca sumada). No propone bajar el tier de `R4.4` (§5 dice cuándo eso sería materia de mesa; proponerlo aquí sería el ejecutor decidiendo). **No adjudica el ponderador del `bx`** — lo corre en pareja, que es lo contrario de adjudicar.
...
- **`CALC-0003-v3`** (caja, UBUNTU, corpus montado) — **el sucesor que estrena esta versión, lanzado por el mismo acto que la congela.** Corre las 4 celdas × brazos de §2.5 contra `v1.3`, gateado **por producto** al `sha256` de este archivo. Dos commits: (1) spec citada antes de abrir un `.dta`, (2) resultados. Ocupa el lugar que `v1.2` §8 reservaba para `L21`; `L21` como rótulo **no se usa** — el sucesor tiene id de registro `GEN2`, que es el que los contadores leen.
- **Si alguna celda de `T2` corrobora, es el primer dato que vuelve medible el «crónico complejo» de la regla** — con la reserva de §1 intacta: corroboraría «crónico», no «crónico complejo».
