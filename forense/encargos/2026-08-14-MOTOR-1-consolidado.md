...
- **Bloque VERIFICACIÓN DE EXISTENCIA (A.8), contestado** contra el árbol real (`84b2acf`, antes de escribir nada):
  ...
  0
  $ git branch -r
  origin/main · origin/claude/motor-1-acto-ejecucion-1xn00f    (ninguna otra)
...
4. **La suite base que el encargo v2 declara está vencida por partida doble.** El encargo cita `3d0d1e5, 20 FAIL · 107 WARN`; `gobernanza:856` declara `18 FAIL · 107 WARN`; la corrida real de hoy da **`24 FAIL · 119 WARN`, LÍNEA BASE VERDE** contra `tests/baseline.json` (HEAD congelado `0ad9b7b`). La discrepancia `gobernanza:856` ↔ corrida real ya es un `T16` vigente y baselineado, no un defecto nuevo de este acto; se declara porque **RONDA-M §5(a) manda citar la base al juzgar** y citaría una cifra muerta.
