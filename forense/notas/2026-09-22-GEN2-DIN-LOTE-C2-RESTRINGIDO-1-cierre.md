...
| FP `…6c10-02` (Q1) | [LEÍDO] | `firmas-pendientes.tsv:433` | ABIERTA al arrancar; durante el acto `GEN2-TRAMITE-FIRMAS-6` (#1001, fusionado) la asentó FIRMADA: Q1 queda resuelta por este acto (C2-restringido), texto verbatim en `forense/encargos/2026-09-22-GEN2-TRAMITE-FIRMAS-6.md:15`, que designa este acto por su rótulo corto de dirección. Ese mismo trámite (`:19`) lista `…8e53-02` como consumida por este acto («citar, no re-asentar»); la asienta este acto. |
...
## 8 · Adenda de cierre (22/sep, tras fusionar `origin/main` en `31da26e0`)

- Entró `GEN2-TRAMITE-FIRMAS-6` (#1001) con Q1 y Q2 firmadas (arriba). Nada de este acto cambia: el universo y el régimen congelados ya son los firmados.
- La fusión `merge=union` de `forense/firmas-pendientes.tsv` resucitó las versiones ABIERTA de `…8e53-01`, `…8e53-02` y `…7866-01` junto a las FIRMADA de main (el defecto T47 que main acaba de limpiar). Se quitaron por línea; `git diff origin/main --numstat` del archivo = `2 1` (1 fila cambiada `…8e53-02`, 1 fila nueva `…4e12-01`).
- El canal para publicar la vista sigue pendiente en mesa: hay dos FP abiertas en PR sin fusionar (#1004 `…c09b-02`, #1005 `…0af9-01`, lectura de tipo (3), no se citan como hecho). NC `-01` queda como está.
- **Firma de mesa sobre `FP-260922-GEN2-DIN-LOTE-C2-RESTRINGIDO-1-4e12-01`** (22/sep, en la sesión de este acto), verbatim: «Opción A: enmienda con archivo propio y CALC de adjudicación SECUNDARIO para los 5 pares con formalidad, en un estrato «universo T» aparte que nunca se suma al ΔMAE primario ni a la frase de producto de las 44 celdas; C2R citado del CALC restringido, réplicas re-derivadas dentro del lote, régimen ARBITRO-2024 (firma Q2). Lo ejecuta la conversación PRODUCTO-DINERO.» Asentada `FIRMADA` en el tablero por este acto (A.12); la ejecución queda PENDIENTE en PRODUCTO-DINERO, cuyo encargo cita la fila y no la re-asienta (§0). El secundario `informal_cualquiera` no está en la firma: NC `-02` sigue abierta con sucesor actualizado.
- **Canal de la vista firmado** (tras fusionar `origin/main` en `0f833085`, #1004/#1005): `FP-260922-GEN2-PENDIENTES-CAJA-1-c09b-02` y `FP-260922-GEN2-DIN-CREDITO-ESCOLARIDAD-2-0af9-01` FIRMADAS, opción (a): el job del push a `main` correrá `registro --verifica --escribe --lote` sobre los CALC con asiento nuevo en `forense/replay-evidencia.tsv`. Ejecución en la conversación TUBERÍA; al fusionar este PR, `verify.yml` aún no lo trae (`grep -n registro .github/workflows/verify.yml` → sólo el comentario de la línea 370). NC `-01` queda ABIERTA con ese sucesor; el asiento propio ya viaja.
