...
- El [ADR raíz](../analisis/dominios/genero/endireh-adr-raiz.md) y [FP/NC](../analisis/dominios/genero/endireh-fp-nc.md) fijan alcance, falsadores, dependencia y estado de publicación. Sobre la base de la rama, los seis CALC nuevos llevaron el registro de **219 a 225 corridas selladas** y de **65,567 a 65,579 RESULT GEN2 sellados**. Tras integrar el `origin/main` concurrente, los contadores finales derivados son **228** y **65,586**; `celdas_validadas=219` no cambia.
...
- **NC-260923-ASTRA5-U2-ENDIREH-6a2c-01:** sin tabla 2003 hasta disponer de llave oficial por registro para UPM/estrato o pesos replicados. Los demás ámbitos 2003 se excluyen por el diseño del cuestionario verificado, no por convertir blancos en ceros.
- **NC-260923-ASTRA5-U2-ENDIREH-6a2c-02 / -03:** sin IC predictivo calibrado, prueba de cambio temporal ni reconstrucción del agregado 70.1%; requiere actos y población homologados y transición independiente, o un CALC nuevo de unión por mujer con definición oficial.
- **Publicación pendiente:** las tablas están **selladas en disco, no registradas**. El canal de publicación y las cinco FP abiertas de mesa son operaciones posteriores; un PR sin fusionar no las ejecuta. Ninguna otra reserva ENDIREH explícita bloqueó 2006–2021; ENCO, ENVIPE 2026 y ENOE reservado permanecieron cerrados.

## CONSUMIDO Y PRUEBAS

Corpus y manifiesto por alias/archivo; FD y cuestionarios primarios de 2003, 2006, 2011, 2016 y 2021; especificaciones, nueve CALC, recibo y replay de #1093; contrato U0 GEN-001 solo como afirmación de contraste. Ningún número GEN1 fue origen del producto. Los seis RESULT nuevos están sellados y `verify` dio **REPRODUCE, CONTEXTO=IDENTICO** en cada uno; sus seis asientos REAL figuran en `forense/replay-evidencia.tsv` y el registro local de cierre. Las pruebas sintéticas nuevas pasan **18/18** con pytest. Tras integrar `origin/main`, `tests/check.py --baseline` dio **VERDE, 0 FAIL nuevos, 3 heredados**; las guardias corrieron **105**, saltaron **71** y fallaron **0**. Cinco FP quedaron abiertas en `forense/firmas-pendientes.tsv` y tres NC en `forense/no-corrido.tsv`. El paquete se entrega en PR sucesor sin merge.
