...
| `ref.A.04` pobres no pagan | *(bloqueada — ver abajo)* | FUERTE | El objeto exige entidad `prestamista`, y eso choca de frente con la frontera de `ADR-35` ("el motor modela al decisor, no al oferente"; *"ampliar al lado de la oferta es una decisión distinta y mayor, que este ADR NO toma"*). **Este acto no toma esa decisión por su cuenta.** | **Sin objeto — BLOQUEADA por `ADR-35`.** Enmienda redactada y no ejecutada, sube a mesa (`FP-61`, `forense/notas/2026-08-19-adr35-enmienda-borrador.md`). |
...
⚠️ **Siete de ocho quedan CON objeto; una queda sin objeto, bloqueada por ADR-35 y escalada a mesa.** La lectura de `FP-56`/`ADR-111(c)` dice "las ocho... para darles objeto"; la letra del propio acuerdo, en la misma firma, dice que `ADR-35` declara la frontera del prestamista y que ampliar al lado de la oferta es "una decisión distinta y mayor" que ese ADR no toma. Las dos frases conviven en la misma firma sin que la primera resuelva la segunda: este acto sigue la instrucción específica (ADR-35, enmienda redactada y sin ejecutar) sobre la general (siete de ocho, no las ocho), porque ejecutar la ampliación de alcance por cuenta propia sería el ejecutor tomando una decisión de mesa — el mismo defecto que la enmienda existe para evitar.

⚠️ **Ninguna de las siete se calibra aquí.** Forma funcional PENDIENTE en las siete, igual que las 15 condicionales de §1.1.F antes de tener reactivo. Buscar y verificar reactivo, fijar forma funcional y correr la batería ampliada contra estas ocho son actos propios, no ejecutados aquí (§4 del encargo, "Lo que NO hace").
