...
## 7 · Firmas de mesa recibidas con el acto abierto (22/sep/2026), verbatim

**Sobre `FP-260922-GEN2-PENDIENTES-CAJA-1-c09b-01`:** «Opción (b): edad sigue EQUIVALENTE entre los tres instrumentos (misma función, mismo tope); el rótulo 60+ se re-declara 60–96 con la exclusión de 97+ escrita como reserva en el crosswalk y en las celdas afectadas, sin tocar ningún sello. Un corte sucesor con 97 dentro (opción c) solo si el marcador lo pide con dato.» → FP `FIRMADA`; ejecución fuera de perímetro: `NC-260922-GEN2-PENDIENTES-CAJA-1-c09b-04`.

**Sobre `FP-260922-GEN2-PENDIENTES-CAJA-1-c09b-02`:** «Opción (a) en las dos: el job del push a main corre corrida0 registro --verifica --escribe --lote <CALC con asiento nuevo en ese push>, con el lote derivado mecánicamente del diff de forense/replay-evidencia.tsv — sin juicio: solo lo que un acto asentó. Lo diseña y ejecuta la conversación TUBERÍA; mientras, cada acto sigue asentando en replay-evidencia.tsv y el CONTADOR dice «sellada en disco, no registrada» (E.7). Hoy son 22 CALC sin fila por este hueco; la firma cierra los dos FP con una sola decisión.» → FP `FIRMADA`; «las dos» incluye `FP-260922-GEN2-DIN-CREDITO-ESCOLARIDAD-2-0af9-01`, que vive en la rama de #1005 y la asienta su acto. `NC-260922-GEN2-PENDIENTES-CAJA-1-c09b-01` sigue `ABIERTA` con sucesor TUBERÍA. El «22» es la cifra de mesa; este acto midió 21 sobre `ccd7c0eb` (18 sin fila + 3 todavía `SPEC-FIJADA`), antes de que se sellaran las corridas de #1005 y #1003.
