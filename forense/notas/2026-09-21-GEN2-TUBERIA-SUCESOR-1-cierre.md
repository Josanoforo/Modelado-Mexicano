...
Consecuencia: el `ADR` de este acto es **`ADR-575`**, no el `ADR-572` que el encargo proponía
...
en silencio a otro `ADR`; `T15` sale verde antes y después. `ADR-575` **no afirma lo contrario**.
