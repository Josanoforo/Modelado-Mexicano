...
- **P1 · Citas de `motor.py` (e8fa-01)** — `PARO-PREMISA`. La firma D7 («A.», 23/sep, «asentada por FIRMAS-11») que autoriza esta pieza solo existe en `origin/acto/gen2-tramite-firmas-11` (PR #1049), sin fusionar: `git merge-base --is-ancestor origin/acto/gen2-tramite-firmas-11 origin/main` → no ancestro; `git show origin/main:forense/firmas-pendientes.tsv | grep -c FIRMAS-11` → 0. Lectura tipo (3) (§2): no cuenta como hecho hasta re-leerse en main. Impacto: `motor.py:20`/`:129` siguen citando `ADR-531` en vez de `BARRIDO-2`; ningún CALC se re-sella. Sucesor: `GEN2-MOTOR-DEUDA-LOTE-2`, una vez que PR #1049 fusione y D7 sea verificable contra `origin/main`. Fila: `NC-260923-GEN2-MOTOR-DEUDA-LOTE-1-e270-01`.
