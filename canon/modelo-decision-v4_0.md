...
> ## 📊 **Condicionales medidas sobre atributos: ~~9~~ 10 de 15**
> *(Corregido 4/ago/2026, Encargo K — `forense/notas/2026-08-04-medicion-exposicion-violencia-envipe.md` §3-5: mide `exposicion_violencia` (ENVIPE 2025 `TPer_Vic2`, `AP7_3_10`-`_14`). `4 de 144` sigue congelado y no se mueve — ver §6.1. **Nota de reparto, actualizada 4/ago/2026:** `exposicion_violencia` sale de "sin reactivo verificado — búsqueda abierta" (que queda vacía, 0) y entra a `MEDIDO·PARCIAL(edad,dominio,formalidad,ESTRATO)`; `aversion_riesgo` y `sens_estatus` siguen en "sin reactivo — búsqueda cerrada (límite de régimen)". **Denominador 14 → 15 el 13/ago/2026, `ACTO PROC-11`** (ADR-75(b), Paso 6 arriba): entra `obligación_medida`. **Numerador 9 → 10 el 13/ago/2026, `PROC-10-bis`** (ADR-79(a)): sella `MEDIDO·NACIONAL` y le entra `norma_de_género`. El reparto cierra ahora en **15 = 9+0+2+2+1+1**.)*
>
> ✅ **El NUMERADOR se mueve por primera vez, 13/ago/2026 — la razón es taxonómica: la clase séptima ya existe.** `PROC-10-bis`, ejecutando `ADR-79(a)` (D-A, firma de mesa verbatim: *"crear una clase séptima para marginales medidas sin eje"*), sella `MEDIDO·NACIONAL` en `milpa/procedencia.yaml` y le entra la primera θ, `norma_de_género` (ENASIC 2022 `P7_12_7`) — de donde T19b/T19c derivan el numerador. `obligación_medida` sigue sin entrar: la misma clase la admitiría en principio (misma familia, adenda B.5.3 de `forense/notas/2026-08-13-proc-11.md`), pero su expediente propio lo abre `PROD-P638`, no este acto — numerador a **11** cuando cierre. **Decisión D3, resuelta `D3-A`** (`ADR-79(a)`, en sustancia — ver `forense/notas/2026-08-13-proc-11.md` ADENDA B.2): la puerta **D3-B** (condicionar `norma_de_género` POR eje, en vez de admitirla marginal) queda nombrada dentro de la propia definición de la clase, dueño "acto futuro por θ", no ejecutada aquí — `MEDIDO·NACIONAL` no la cierra, solo no la exige.
>
...
- **Condicionales medidas sobre atributos: ~~9~~ 10 de 15** (§1.1.F · §6.1; numerador 9 → 10 el 13/ago/2026, `PROC-10-bis` ejecutando ADR-79(a) — sella la clase séptima `MEDIDO·NACIONAL` y entra `norma_de_género`, ver §1.1.F Paso 5. Denominador 14 → 15 el 13/ago/2026, `ACTO PROC-11` ejecutando ADR-75(b) — ver §1.1.F Paso 6. Antes, corregido 4/ago/2026, Encargo K — `forense/notas/2026-08-04-medicion-exposicion-violencia-envipe.md` §3-5). De las 15: **9 MEDIDO·PARCIAL** (`confianza_institucional[salud/educación/financiera/seguridad-FFAA/justicia-policía/electoral-partidos]`, 6 de 6, más `radio_confianza`, `familismo_apoyo` y `exposicion_violencia`), **1 MEDIDO·NACIONAL** (`norma_de_género`, ENASIC 2022 `P7_12_7` — marginal nacional, sin eje condicionante, `x = ∅`), 0 sin reactivo verificado — búsqueda abierta (vacía desde 4/ago/2026: `exposicion_violencia` salió por medición exitosa, no por agotamiento de la condición de caducidad de ADR-52 A, que sigue intacta como criterio, ver §1.1.F Paso 5), 2 sin reactivo — búsqueda cerrada, límite de régimen (`aversion_riesgo`, `sens_estatus`; `sens_estatus` cerrada 4/ago/2026 por ADR-54, `gobernanza` §4, ver §1.1.F Paso 5), 2 solo proxy, y **1 medida sin clase de procedencia** (`obligación_medida`, ENASIC 2022 `P6_38` — calculada y verificada, pero marginal nacional: `x = ∅`; la misma clase séptima que admitió `norma_de_género` la admitiría en principio, expediente propio pendiente — `PROD-P638`, numerador a 11 cuando cierre).
