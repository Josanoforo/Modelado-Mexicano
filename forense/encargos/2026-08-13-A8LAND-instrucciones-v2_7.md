...
- **Estado**: `CONSUMIDO` — ejecutado en la rama `claude/new-session-qwc8wn`, commits `c26bdba` (COMMIT 1: crea `instrucciones-proyecto-v2_7.md`) · `d03c408` (archiva este mismo encargo) · `85b6856` (COMMIT 2 original: sella `ADR-77` y propaga la cascada) · `0a01cab` (`forense/hallazgos.md` + nota de cierre) · `5f4095f` (marca `CONSUMIDO`, PR #218 abierto contra `8b30306`). **Reconciliado después de abrir el PR**: `origin/main` avanzó a `a97dc28` (PR #217, `ENCARGO A8-LAND`/`ADR-77`, un encargo A8-LAND anterior y más corto — solo A.8 — que fusionó primero). Merge local de `origin/main` hacia esta rama (commit de merge que sigue a éste); `ADR-77` de `PR#217` queda intacto y sin editar; A.9 se re-sella como **`ADR-78`**, con `instrucciones-proyecto-v2_8.md` nuevo (v2.7 + A.9). Detalle completo de la colisión y su resolución en `forense/notas/2026-08-13-a8-land-adr78.md` (renombrada desde `2026-08-13-a8-land.md`, que colisionaba por nombre con la nota de la otra sesión). `tests/check.py --baseline`, corrido antes y después del merge: ver esa misma nota para la cifra final y cualquier hallazgo de T03/T16 declarado y no forzado a VERDE.
