...
1. **REPO.** Clon existente `/home/pc0/Modelado-Mexicano`; worktree nuevo `/home/pc0/mm-sonda1` (`git worktree add -b sonda1-mapa-barreras-lote2 /home/pc0/mm-sonda1 origin/main`). `git log -1`: `b17a6f6 Merge pull request #195 from Josanoforo/z/inventario-curador-20260812-184630`. `git status`: árbol limpio al abrir.
   - `git worktree add` emitió dos veces `error: could not write config file .git/config: Device or resource busy` — misma contención conocida de esta máquina (memoria de sesión del agente, fuera de este repositorio — no un artefacto citable aquí). Verificado independientemente: `git log -1` quedó en `b17a6f6`, `git status` limpio, `git worktree list` lo lista — la creación no falló, solo la escritura de metadato de tracking.
