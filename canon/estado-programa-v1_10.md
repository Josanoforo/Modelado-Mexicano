...
**L5 · Validación — el estrato más problemático.** **Hito D** (reglas del motor; perímetro de 27, subconjunto de las 49 del motor): **8 de 27 corridas archivadas** — `R1.1` → veredicto `D`, `R3.2` → veredicto `B`, `R7.2` → veredicto `D`, `R4.2` → veredicto `D`, `R4.1` → veredicto `D`, `R9.1` → veredicto `D`, `R4.3` → veredicto `D` (mitad A y mitad B), `R9.2` → veredicto `D` (bloque append-only de `hitoD-preregistro`, ver §7). *(Corregido 4/ago/2026, Encargo Y — propaga `R4.2` → `D`, archivado en `hitoD-preregistro` Nota 17, ADR-55. `R5.2` y `R1.2` corrieron completas en el mismo acto pero no se archivaron — Notas 18/19, propuestas, no adjudicadas. Corregido de nuevo 4/ago/2026, Encargo Z — propaga `R4.1`/`R9.1`/`R4.3`(A,B)/`R9.2` → `D`, archivados en `hitoD-preregistro` Notas 23-25, ADR-56.)* 49 refutaciones narrativas corridas *(población distinta, `milpa/refutations.yaml`, ref.A.*)*: 27 pasan, 3 fallan, 8 sin objeto, 11 requieren el ejecutable. **15 coeficientes, cero medidos.** Cuatro generadores sin falsar (**Hito C**, distinto de lo anterior — ver `hitoC-prueba-generadores`).
...
- **41 de 49 reglas del motor sin prueba de falsación pre-registrada corrida** (8 corridas archivadas: `R1.1` → `D`, `R3.2` → `B`, `R7.2` → `D`, `R4.2` → `D`, `R4.1` → `D`, `R9.1` → `D`, `R4.3` → `D` mitad A y mitad B, `R9.2` → `D`). **Dentro del perímetro decidido del Hito D (27 reglas, subconjunto de las 49): 19 de 27 sin corrida.** *(Corregido 4/ago/2026, Encargo Y — propaga `R4.2` → `D`, ADR-55. Corregido de nuevo 4/ago/2026, Encargo Z — propaga `R4.1`/`R9.1`/`R4.3`(A,B)/`R9.2` → `D`, ADR-56.)*
