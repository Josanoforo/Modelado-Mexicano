...
> ⚠️ **Enmienda in situ (20/ago/2026, `SELLA-C`, `ADR-140`).** Los «7 umbrales con conteos por comando» de esta línea quedan `VENCIDOS EN ALCANCE` — no refutados, no borrados, no vigentes para el territorio nuevo (`A.10`, las tres cosas a la vez; mismo patrón que `ADR-112`→`ADR-110(d)`). El criterio de cierre de `PILOTO-E1E3 T4` es, desde este ADR, `ADV1-M6 v2` (`forense/adv-duelo/ADV1-M6-v2-2026-08-20.md`): los tres criterios de resultado que `M6` ya nombraba, presentados como conjunto completo. Texto original arriba, intacto.
...
## §5 · DELTA FECHADO — 20/ago/2026 (`T-SELLO`, `ADR-131`), citado verbatim de `canon/APERTURA-FASE-CALCULO-v1_2.md §1`
