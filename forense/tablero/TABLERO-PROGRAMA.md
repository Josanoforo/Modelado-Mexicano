...
- **Procedencia.** SHA `82b85026` · fecha del commit `2026-09-24` · ¿árbol == origin/main? `True`.
...
- **Gobernanza operativa.** ADR máximo del espacio numérico CERRADO `593` · FP máximo del mismo espacio `409` · ids con raíz de acto (época vigente) `{'ADR': 113, 'FP': 138, 'NC': 356}` · FP abiertas: FP-260921-GEN2-CORPUS-INTEGRIDAD-Y-RESPALDO-1-3d56-01, FP-260923-GEN2-TRAMITE-FIRMAS-12-c3fa-05, FP-260923-GEN2-FRONT-1-4296-01, FP-260924-GEN2-ASTRA5-U5-ADQUISICION-1-43d6-01 · encargos archivados `752` (consumidos `674`) · instrucciones vigentes `v2.16` · cola de encargos (solo estados != CONSUMIDO; consumidos `61`):
...
- **NC abiertas por razón (token A.14, prefijo exacto).** abiertas `352` · por token: `DECISIÓN-DE-MESA-PENDIENTE` 45 · `DIFERIDO-A` 80 · `FUERA-DE-PERÍMETRO` 56 · `NO-VERIFICABLE-AQUÍ` 19 · `PARO-ENTORNO` 2 · `PARO-PREMISA` 34 · `SUSTITUIDO-POR` 5 · prosa (sin token reconocible) `111`.
- **GEN2 (derivado de `corrida0 status`, valores EN ÁRBOL; vistas del árbol distintas de HEAD: `ninguna`).** adoptados activos `81` · pendientes de adopción `10` · corridas selladas `272` / requeridas `87` · resultados sellados `79352` / activos `211` · pendientes `211` · dependencias numéricas legacy activas `137` · validación independiente `215` · diferencias materiales `0` · NC- abiertas `352` · replays LEGACY-GEN1 sellados `5` (no cuentan). El `0 / N` es la lectura correcta: el aparato se construyó antes que las corridas.
