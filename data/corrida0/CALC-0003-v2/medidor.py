"""`CALC-0003-v2` — S6 v1.2 Rama A' sobre ENNViH-1 2002: 4 celdas x brazos.

`repite_de: CALC-0003`. UNICA diferencia con el medidor congelado por
`ACTO GEN2-E5-0`, declarada aqui y en spec.yaml / spec.md: se filtran de
`c_portad.dta` las filas cuya LLAVE DE JOIN (`folio`, `ls`) esta ausente,
ANTES de construir la llave. Todo lo demas es byte por byte el de v1.

Interfaz estable: `medir(inputs, contrato) -> {"RESULT-…": v}`.

ESCRITO EN `ACTO GEN2-E5-0` Y NO EJECUTADO EN EL. Lo corre `ACTO GEN2-E5-1`.
