...
2. **El archivo de la cola ES su propio archivo `A.3`; no se duplica al
   ...
   **FAIL de la suite**: `T02` normaliza el nombre de archivo *sin
   distinguir directorio* (`tests/check.py:187`, y la propia
   `convencion.md` lo advierte), así que las dos copias colisionarían.
...
2. **CANDADO** — mecánico, dos comprobaciones. Si **cualquiera** de las
   dos da positivo: reporta y **termina sin tocar nada**. Una sesión de
   nube a la vez.
...
Medido el 31/ago/2026 en la nube: **`gh` no existe** (`which gh` →
`command not found`), así que el candado **no puede** depender de la API
de PRs. Se hace con `git` puro, y son dos:

- **(a) ¿Hay un `EN-CURSO` en la cola?**
  `grep -l '^ESTADO: EN-CURSO' forense/encargos/cola/*.md`
  Uno o más → otra sesión está trabajando. Se reporta y se termina.
...
nada no es un negativo. Al 31/ago/2026 el remoto tiene **1** rama viva
(`main`), así que el candado abre.
...
- **CANDADO CERRADO.** "Hay un `EN-CURSO`" o "hay una rama de acto
  abierta", con su conteo de archivos y ramas examinados, y **cero
  commits**. Es el resultado más común y es el correcto: quiere decir
  que la pieza está haciendo justo lo que se le pidió.
