...
- **Nuevas** (`forense/no-corrido.tsv`): **`NC-0179`** las 4 celdas no-construibles — `PARO-PREMISA`, sucesor: mesa, si autoriza un `B` por crosswalk (fuera de la familia); **`NC-0180`** la próxima tríada con `B` de cobertura real — `DECISIÓN-DE-MESA-PENDIENTE`; **`NC-0181`** los tres asientos de `replay-evidencia.tsv` desactualizados por inputs cambiados en `origin/main` — `FUERA-DE-PERÍMETRO`; **`NC-0182`** el veredicto inestable de `CALC-M-marco-M-sorteado-v1_3` dentro de `registro --verifica` — `NO-VERIFICABLE-AQUÍ`. (Nacieron como `NC-0177`–`NC-0180` y se renumeraron al fusionar `main`: `PR #756` tomó `NC-0177`/`NC-0178` y `ADR-498`; este acto queda en `ADR-499`.)
