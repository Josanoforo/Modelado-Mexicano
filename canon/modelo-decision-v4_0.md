...
| **Sin reactivo verificado — búsqueda abierta** | `exposicion_violencia` — su reactivo reportado (ENVIPE `BP1_20/23/28`) fue **retirado**: mide conducta de denuncia condicionada a victimización, no exposición a violencia (`PR #57`, `forense/hitoE-campana-medicion-v2_0.md` §15); dos actos de búsqueda ya en curso (posición 4 rehecha sobre `TPer_Vic1`; barrido de alcanzabilidad ENDIREH/ENSU); un tercero se nombra cuando esos dos cierren | **1** |
...
**Criterio, sellado (decisión de mesa, 4/ago/2026, ver `gobernanza` §4, ADR-52 A).** La clase no la determina qué le pasó al reactivo, sino si queda algún acto nombrado que pueda producirlo. **Condición de caducidad:** una búsqueda se cierra —pasa a "límite de régimen"— si tres actos nombrados vuelven sin reactivo; sin esa condición la clase "búsqueda abierta" es un eufemismo permanente.

> ## 📊 **Condicionales medidas sobre atributos: ~~6~~ 8 de 14**
> *(Corregido 4/ago/2026 — `forense/notas/2026-08-03-cal-conf-faseb-pos5-6-radio-familismo.md` §5, Fase B posiciones 5-6 midió `radio_confianza` (ENCUCI) y `familismo_apoyo` (ENIF). `4 de 144` sigue congelado y no se mueve — ver §6.1. **Nota de reparto, sellada 4/ago/2026 (mesa, `gobernanza` §4, ADR-52 A):** `exposicion_violencia` sale de "reactivo directo reportado, sin medir" porque `PR #57` retiró su reactivo (`hitoE` §15) y cae en "sin reactivo verificado — búsqueda abierta"; `aversion_riesgo` cae en "sin reactivo — búsqueda cerrada (límite de régimen)". **Corrección 4/ago/2026 (ADR-54, `gobernanza` §4):** `sens_estatus` sale de "búsqueda abierta" y cae también en "sin reactivo — búsqueda cerrada (límite de régimen)", por el examen de descriptor de `PR #64` (`forense/notas/2026-08-04-sens-estatus-examen-descriptor.md`) contra los cinco instrumentos permitidos del régimen — ninguno sirve — cerrado por la vía del precedente `aversion_riesgo` (ADR-52 A), no por la caducidad de tres actos. El reparto sigue cerrando en 14.)*
>
