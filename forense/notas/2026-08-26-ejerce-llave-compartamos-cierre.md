...
**Entorno UBUNTU · caja del acto `/home/pc0/mm-e3-ejerce-compartamos`, rama `acto/e3-ejerce-llave-compartamos` · base `186f090` · `ADR-202`**
...
**Colisión declarada al escribir, y ocurrida exactamente como se declaró.** Al arrancar, `gh pr list --state open` salió **vacío**. Antes de cerrar aparecieron y **fusionaron** dos actos hermanos: `PR #371` (`ACTO E2-PREP-L-RUN`) se quedó con `ADR-200`, y `PR #372` (`ACTO CIERRA-FP157`) renumeró a `ADR-201` al fusionar segundo. **Regla de la casa aplicada tal cual, y este acto es el que fusiona después:** máximo re-derivado por conteo entero **contra el árbol ya fusionado** (`re.findall(r'ADR-(\d+)')` sobre `canon/gobernanza-v1_15.md` → **201**, sin huecos) → **`ADR-202`**. Las contribuciones ajenas se conservan **íntegras**: los bloques de `ADR-200` y `ADR-201` quedan sin editar, y en `canon/registro-rotulos.tsv` conviven la fila censal de `E1` que trajo `PR #372` y la de `MAESTRA30-E1..E4` de este acto. **Colisión que sigue viva:** `PR #373` (`ACTO DISEÑO-ENSAFI`) candidatea también `ADR-202`; si fusiona primero, este acto renumera otra vez al resolver. `FP-164` no colisionó: máximo de `forense/firmas-pendientes.tsv` re-derivado por el mismo método → **163** (con los huecos históricos `137-140`), y el merge de `PR #371`/`PR #372` no abrió filas nuevas. Los conflictos de `git merge` cayeron donde se anticipó —`canon/gobernanza-v1_15.md`, `canon/estado-programa-v1_10.md`, `canon/registro-rotulos.tsv`— y se resolvieron a mano conservando ambos lados; la línea `L0`, que es la que se duplica al resolver a mano, quedó **una sola** (verificado por conteo: 1 línea, 75 recifrados, 55,936 caracteres = la de ellos más la inserción de este acto).

---

## 7 · Perímetro — lo que se tocó y lo que no

**Tocado:** `forense/resultado-exp-compartamos-v1_0.md` [nuevo] · `forense/registro-llaves-identificacion-v1_0.md` (la fila `EXP-COMPARTAMOS-1`, el contador y §13 nuevo) · `forense/firmas-pendientes.tsv` (fila `FP-164`) · `canon/gobernanza-v1_15.md` (`ADR-202` + cabecera de conteo) · `canon/estado-programa-v1_10.md` (recifrado de ADR, línea de llaves, tabla de canon, suite) · esta nota · `forense/encargos/2026-08-26-E3-EJERCE-LLAVE-COMPARTAMOS.md` (`A.3`). Nada fuera de la lista del encargo.
