...
| `data/curacion-registro/cola-adquisicion-registro.tsv` | `tools/curador_registro/registra_cola_adquisicion.py --cola data/cola-adquisicion-v1_0.tsv --aliases data/curacion-registro/aliases-fuentes.tsv --output <ruta>` (`ACTO MAESTRA33-A5`, 1/sep/2026, P2) | 10 campos: `fila_origen, fuente_canonica, fuente_canonica_normalizada, discordancia_alias, estado_A4A5, prioridad, url_conocida, ids_manifiesto, origen, nota` | `tools/vista_cola_adquisicion.py`, `tests/check.py` (T26, T26-bis) | **Es el hogar nuevo** para las cinco columnas de la cola que no tenían tabla del registro antes de este acto (`estado_A4A5, prioridad, url_conocida, origen, nota` — ver `data/curacion-registro/MAPA-COLA-ADQUISICION-v1_0.md`, P1). `fuente_canonica_normalizada` resuelve contra `aliases-fuentes.tsv`; hoy 0/79 filas tienen alias (`discordancia_alias=SIN_ALIAS`), declarado, no forzado. **`tools/curador_registro/tsv_crudo.py`** (nuevo, `ACTO MAESTRA37-N2`, 3/sep/2026, FP-258): un round-trip `csv.reader`→`csv.writer` sobre este archivo corrompe 4 de sus 112 líneas (comillas sueltas ajenas a CSV real dentro de `nota`); `tsv_crudo.leer_lineas`/`escribir_lineas` trata cada línea como texto opaco (0 líneas distintas garantizado, T26-bis) en vez de reserializar todo el archivo. `vista_cola_adquisicion.py` sigue leyendo VALORES con `csv`/`tsv_crudo.leer_dicts` (necesario: al menos una fila del registro sí está correctamente citada en CSV — un split naïve por tab la rompería, medido al implementar) — el módulo nuevo cubre el round-trip, no la lectura de columnas. |
| `data/pdn-adquisicion-resumen-v1_0.tsv` | A MANO, sin script propio — escrito directo por `ACTO MAESTRA38-A5 · PDN-BULK-Y-PROXY`, 6/sep/2026, como resumen por objeto de la adquisición `PDN_SESNA_S1_S2_S3_S6` que exigía la spec `prereg-caja-S9-A5` | 7 campos: `sistema, ruta_que_funciono, objeto, bytes, sha256_crudo, sha256_contenido, fecha` | ninguno en `tools/`/`tests/` hoy — solo prosa forense (`forense/notas/2026-09-06-MAESTRA38-A5-resultados.md`) | Sin escritor de script: una fila nueva se edita a mano, con el mismo riesgo de comillas que motivó `tsv_crudo.py` en la fila de arriba si algún día trae una columna de texto libre. `sha256_crudo` == `sha256_contenido` en las cuatro filas de hoy porque `A4` ya cumplió A.7 (doble descarga, curl+wget, hash idéntico) — una fila futura con los dos valores distintos señalaría un bulk con marca de generación variable en el propio archivo, caso que `S9-A5 §5`/R2 ya anticipa. |
...
**Actualización 11/sep/2026, ADR-484.** Ya no es necesario reconstruir esa
...
| `data/l3-ensanut2024-catalogos-v1_0.tsv` | `openpyxl 3.1.5` sobre los 5 `.Catlogo.xlsx` de ENSANUT 2024 en la raíz `descargas_mx` (hojas `Variables` y `Valores`) — `ACTO MAESTRA37-L3`, `P0(b)`, `COMMIT-1` | TSV: `modulo, variable_id, etiqueta_variable, codigo, etiqueta_valor` — **8 790 filas** sobre **1 914 variables** de `utilizadores` (122), `hogar` (204), `integrantes` (261), `adultos` (843) y `menores` (484); 8 179 filas de valor + **611 filas de variable SIN catálogo de valores** (continua, abierta o identificador) | lectura dirigida de instrumento (`A.4` con texto a la vista); `L3-bis` cuando llegue el microdato de `adultos` | Es **censo, no búsqueda**: se lee todo el catálogo y no se eligen términos. Las 611 variables sin códigos entran **a propósito** — si sólo se anexaran las que tienen valores, el censo perdería 611 variables en silencio y un `NO-ENCONTRADO` posterior no sería auditable. `adultos` entra **sólo por catálogo**: su microdato está `AUSENTE-EN-RAIZ`, y poder leer *qué se preguntó* es lo que distingue una brecha de adquisición de una de instrumento |
| `data/l3-ensanut2024-cuestionarios-v1_0.txt` | `pdftotext -layout <pdf> -` sobre los 5 PDF de cuestionario ENSANUT 2024 de la raíz `descargas_mx`, concatenados — `ACTO MAESTRA37-L3`, `P0(c)`, `COMMIT-1` | Texto plano, **8 973 líneas** sobre **136 páginas**, con una cabecera por PDF que lleva su `sha256_12` y su número de páginas (hogar `adc873843b79`/28 · niños 0-9 `af65f922094c`/29 · adolescentes `344f32ef0f87`/28 · adultos `0bc30c3b7f08`/44 · utilizadores `004aacee3729`/7) | completar etiquetas que **Stata trunca a 80 caracteres** antes de fallar una regla; leer las opciones de respuesta con su número; leer universos y saltos declarados en portada | Estos 5 PDF produjeron **0 filas** en `data/inventario-reactivos-descargas-mx-v1_0.tsv` (defecto `DE2`, registrado por `MAESTRA37-N4`): que el extractor no los leyera no dice nada sobre lo que contienen. De aquí salió, por ejemplo, que el cuestionario de utilizadores es «solo para las personas que SÍ recibieron atención médica» — el universo que excluye la automedicación **por construcción** |
...
|---|---|---|---|---|
| `forense/censo-raiz/AAAA-MM-DD.txt` | `tools/adquiere_cron.sh`, paso `2.5`, corrido diariamente por el cron de mesa (`ADR-281`) ANTES de `claude -p`, independiente de que `data/raw` esté montado | salida cruda de `python3 tests/manifiesto.py --escanea descargas_mx`, con el bloque `Total en disco · nuevos · ya registrados · conflicto` como primera línea | `.claude/commands/acto.md` (ARRANQUE, punto 3) y `.claude/commands/encola.md` (§4-bis) — cita obligatoria para todo `AUSENTE-EN-RAIZ`/`NO-ENCONTRADO` o toda petición de descarga a mesa | un `AUSENTE-EN-RAIZ` sin censo del día citado es `NO-VERIFICADO`, no `AUSENTE`; el commit `[CENSO] AAAA-MM-DD` es propio, separado del `[ADQ]`, empujado directo a `main` — si la raíz no resuelve en la máquina (`data/raices.local.yaml`), el log trae una línea `PARO-RAIZ` y el resto del cron sigue — **un clon git = un objeto** (`ACTO MAESTRA38-CENSO-CLON`, `COMMIT-1`): una carpeta con `.git/` se reporta como una sola línea `CLON <ruta> · commit <sha> · <n> archivos` en `CLONES (k):`, nunca archivo por archivo, defecto medido el 6/sep (136 residuos de un clon contados como "nuevos") — **perímetro físico de raíces** (`ACTO AUTOMATIZA-1-E1`, `ADR-358`): sólo `data_raw` y `descargas_mx` son recorribles por I/O físico general del corpus (`raiz_escaneable()`, `tests/manifiesto.py`); `downloads` queda excluida por diseño (contuvo información personal ajena al proyecto), no por "no configurada" — puede seguir citada en entradas históricas del manifiesto y en `data/raices.local.yaml`, pero ningún `--escanea`/`--verifica`/`tests/corpus.py` la camina, abre ni hashea |
...
| `tools/adquiere_cron.sh` (el cron completo, no solo su paso `2.5`) | crontab de mesa en la caja `mm-adq`, lunes-viernes 07:30 CST | identidad, línea de crontab, calendario, regla mecánica y playbook de diagnóstico en `forense/cron/REGISTRO-CRON-v1_0.md` (`ACTO MAESTRA38-CRON-2 · REGISTRO-Y-HUELLA`, 6/sep/2026) | `tests/check.py` (`T31 T-CRON`, WARN si el último día hábil no dejó huella) y quien diagnostique el cron desde §5 de ese registro | WARN nunca FAIL — no hay forma de confirmar desde el repo que el cron corrió completo, solo que llegó al paso `2.5`; excepción declarada antes del 4/sep/2026 (instalación) |
...
## `forense/prereg-caja/` — pre-registros congelados antes de que caja corra (`ACTO MAESTRA38-N3 · PRE-REGISTRO-DE-CAJA`, 4/sep/2026; ampliado por `ACTO MAESTRA38-N7 · PRE-REGISTRO-CIVICO-LAPOP`, mismo día)
...
| `forense/prereg-caja/S4-L4-spec-v1_0.md` + `.sha256` | dirección, a mano, en NUBE, sin corpus montado — mismo patrón COMMIT-1-por-spec que `S1-S3`, objeto de `forense/notas/2026-09-04-MAESTRA38-N5-diseno-9-reglas.md §2.6` | mismo contrato de `.sha256` que la fila de arriba | el futuro acto `MAESTRA38-L4` (`civico.voto.clientelar_si_observable`, LAPOP 2019) — compara su primer resultado contra la spec congelada aquí | corrige la misma clase de premisa que `S1`/`S3`: `MAESTRA35-L9 §3`/`L11 §1` (`forense/notas/2026-09-02-MAESTRA35-L9-*`, `-L11-*`) ya corrieron una falsación real de este mismo `id` de canon (otro brazo de su disyunción), veredicto `CONTRARIA` replicado, `PENDIENTE-DE-MESA` en `FP-298` sin sellar — ni el encargo de `N7` ni `N5` la citaban; `S4` declara esa continuidad en vez de pre-registrar como si fuera la primera medición (A.8) |
| `forense/prereg-caja/S5-L5-spec-v1_0.md` + `.sha256` | dirección, a mano, en NUBE, sin corpus montado — mismo patrón COMMIT-1-por-spec que `S1-S3`, objeto de `forense/notas/2026-09-04-MAESTRA38-N5-diseno-9-reglas.md §2.8` | mismo contrato de `.sha256` que la fila de arriba | el futuro acto `MAESTRA38-L5` (`civico.protesta.agravio_urbano`, multi-ola 2004/2006/2019) — compara su primer resultado contra la spec congelada aquí | corrige la misma clase de premisa que `S1`/`S3`: `MAESTRA35-L9 §4`/`L11 §2` ya corrieron un diseño de dos de los cuatro antecedentes sobre este mismo `id`, veredicto `CORROBORADA-PARCIAL` replicado, `PENDIENTE-DE-MESA` en `FP-298` sin sellar; `S5` completa el diseño con los dos antecedentes (falla estatal, red previa) que `L9` declaró ausentes del instrumento y que el inventario de reactivos v1_1 —nacido un día después de `L9`— sí trae (A.8) |
| `forense/prereg-caja/S6-L16-spec-v1_0.md` + `.sha256` (`ACTO MAESTRA38-N11 · PRE-REGISTRO-OLA6-MEDIBLES-Y-FICHAS`, 5/sep/2026) | dirección, a mano, en NUBE, sin corpus montado — mismo patrón COMMIT-1-por-spec, objeto de `forense/notas/2026-09-05-MAESTRA38-N10-cobertura-ola6.md §2.2` | mismo contrato de `.sha256` que las filas de arriba | el futuro acto `MAESTRA38-L16` (`salud.atencion.grave`) — compara su primer resultado, de cada rama por separado, contra la spec congelada aquí | corrige que el árbol trae **dos** `EXISTE-SATISFACE` ya sellados para este `id` sobre reactivos que no se solapan (`ENNVIH`+`ENDIREH` de `MAESTRA34-N5`/`MAESTRA37-L1` vs. `ENSANUT2024` de `MAESTRA37-L3`/`L3-BIS`), sin que ninguna nota los reconcilie — la afirmación de `L3` de heredar de `N5` no se sostiene contra el texto real de `N5`; pre-registra las dos ramas en paralelo sin adjudicar cuál prevalece (A.8) |
| `forense/prereg-caja/S7-L17-spec-v1_0.md` + `.sha256` (`ACTO MAESTRA38-N11`, 5/sep/2026) | dirección, a mano, en NUBE, sin corpus montado — mismo patrón, objeto de `forense/notas/2026-09-05-MAESTRA38-N10-cobertura-ola6.md §2.5` | mismo contrato de `.sha256` | el futuro acto `MAESTRA38-L17` (`salud.vacunacion.disponible`) — compara su primer resultado, de cada rama, contra la spec congelada aquí | corrige la fuente real del inventario de la ficha original (`data/inventario-reactivos-ext-v1_0.tsv`, no `v1_1`) y aporta el bloque `a0927a1`-`a0927e4` de `adultos_ensanut2024_w.dta` (razón de no vacunación) como prueba directa del `PORQUE` de la regla — hallazgo nuevo, ni `N5` ni `N10` lo habían citado (A.8) |
| `forense/prereg-caja/S8-L18-spec-v1_0.md` + `.sha256` (`ACTO MAESTRA38-N11`, 5/sep/2026) | dirección, a mano, en NUBE, sin corpus montado — mismo patrón, objeto de `forense/notas/2026-09-05-MAESTRA38-N10-cobertura-ola6.md §2.6` | mismo contrato de `.sha256` | el futuro acto `MAESTRA38-L18` (`comunicacion.inseguridad.ver_oir_callar`) — compara su primer resultado contra la spec congelada aquí | corrige que el desenlace del módulo `AOJ` (`aoj1`/`aoj1a`/`aoj1b`) existe solo en la ola 2004 de LAPOP, no en "las mismas cinco olas" que `N10 §2.6` describe de corrido; acota el falsador a esa sola ola, citando 2006/2019/2021/2023 solo como contexto del antecedente (A.8) |

## `canon/estado-programa-v1_12.md` — recibo (`ACTO MAESTRA38-N8`, 4/sep/2026)

No es una tabla de dominio nuevo — es un recibo de que la ÚNICA FUENTE DE
ESTADO cambió de nombre estable de archivo. `canon/estado-programa-v1_11.md`
fue retirada del árbol por `T01` (`git rm`, mismo mecanismo que retiró
`estado-programa-v1_10.md`); el nombre estable sigue siendo `estado`, cítalo así, nunca por
...
## Coexistencia física `data_raw`/`descargas_mx` regularizada (`ACTO MAESTRA38-A2-bis · REGULARIZA-PERIMETRO-REAL`, 5/sep/2026)

Las 73 entradas de `data/manifiesto.yaml` declaradas `raiz: data_raw` que `tests/corpus.py` reporta `presente_bajo_otra_raiz` en `descargas_mx` (residuo categoría C de `forense/prereg-caja/S1-A2-spec-v1_0.md` §4) son coexistencia física deliberada, no error de captura — verificado archivo por archivo por `sha256` — y quedan sin recifrar (`FP-308` firmada (iii): *"coexistencia deliberada; no se recifra raíz en ninguna de las 73"*).

`tests/corpus.py` gana la reclasificación **patrón B** (`S1-A2-spec-v1_0.md` §3, `_es_patron_b()`): un huérfano `sin_registro` cuyo sha256 ya duplica contenido de la MISMA raíz se excluye del conteo de candidatas nuevas —no del reporte— sólo si además su nombre trae el sufijo de copia de navegador ` (N)` o es una página guardada; `C1` bajó de `185` a `132` WARN tras esta corrida (34 candidatas reales promovidas + 19 patrón B reclasificados), sin ocultar ninguno de los dos.

## Corpus, red y TLS tras `ACTO MAESTRA38-A4 · ADQUIERE-TODO-LO-PUBLICO` (6/sep/2026)

- **2026-09-06 · `ACTO MAESTRA38-A4`** — **17 subcarpetas nuevas bajo `data_raw`** (`/home/pc0/mm-corpus/raw`, corpus compartido, `data/raw` es symlink): `lapop_earthquake_2017` (2) · `imss_bienestar_infraestructura` (3) · `condusef_redeco_reune` (3) · `inegi_rnm_ddi` (2) · `tepjf_elecciones_concurrentes` (1) · `ietam_tamaulipas_municipal` (46) · `milk_rct_microseguro` (1) · `inegi_cngf_2025` (3) · `ieepco_oaxaca_municipal` (1) · `pdn_declaranet_sfp` (6) · `compranet_historico` (1) · `pdn_bulk_2026_09` (4) · `ecopred2014` (1) · `wb_enterprise_surveys_mx2023` (2) · `cultura_constitucional_unam_iij` (6) · `losmexicanos_unam_iij` (113, en 24 subcarpetas por módulo) · `enjuve_imjuve` (4); más 2 archivos nuevos en `academico_icpsr35024`. Total **201 archivos, 5.39 GB**. Manifiesto `1315` → `1515`; `--verifica` sobre `data_raw`: `coincide=1195 · no_coincide=0 · ausente=0`.
- **2026-09-06 · `ACTO MAESTRA38-A4`** — **la red de esta caja no pasa por el sandbox de bash.** `curl -s -o /dev/null -w "%{http_code}" --max-time 25 https://www.inegi.org.mx/` da `000` **dentro** del sandbox y `200` **fuera**. Consecuencia operativa: todo acto que descargue desde Ubuntu corre sus `curl`/`wget` con el sandbox deshabilitado, y **un `000` medido dentro del sandbox no es un hecho sobre el portal** — es un hecho sobre el sandbox. Mismo patrón ya anotado para `descargas_mx` (que vive bajo `/mnt/c`, fuera del árbol de lectura por defecto).
- **2026-09-06 · `ACTO MAESTRA38-A4`** — **`ieepco.org.mx` exige el intermedio TLS a mano.** El servidor sirve el *leaf* dos veces y omite `GeoTrust TLS RSA CA G1`; `curl`/`wget`/`requests` fallan con `unable to get local issuer certificate` mientras un navegador lo resuelve solo por AIA. Receta reutilizable, sin `-k`: `curl -o geotrust.crt http://cacerts.geotrust.com/GeoTrustTLSRSACAG1.crt && openssl x509 -inform DER -in geotrust.crt -out geotrust.pem && cat /etc/ssl/certs/ca-certificates.crt geotrust.pem > bundle.pem` y luego `curl --cacert bundle.pem` / `wget --ca-certificate=bundle.pem`. Misma clase que el defecto ya registrado para CNBV.
- **2026-09-06 · `ACTO MAESTRA38-A4`** — **`losmexicanos.unam.mx` sólo responde por `http://www.`**; `https://` sin `www` resetea la conexión (`curl (35) Recv failure`), medido tres veces con 20 s entre intentos. Cualquier sonda futura a ese host (y al módulo `MexicanosConstitucion` que vive en él) debe usar `http://www.losmexicanos.unam.mx/…`. El listado de `…/base_datos/` da `403` pero los archivos responden `200`.
## Tres falsadores de caja sobre LAPOP AmericasBarometer México (`ACTO MAESTRA38-LOTE-LAPOP · L4 + L5 + L18`, 6/sep/2026)

| archivo | productor | contrato | quién lo lee | advertencia |
|---|---|---|---|---|
| `data/l4-l5-l18-censo-v1_0.json` | `python3 tools/censo_lote_lapop.py --json …` (censo A.4 compartido por las tres piezas) | JSON: una entrada por payload (`2004_sav`, `2004_dta`, `2006_sav`, `2006_dta`, `2019`, `2021`, `2023`) con `id`, `sha256`, `sha256_manifiesto`, `coincide_manifiesto`, `n_filas`, `n_columnas` y, por variable pedida, `estado` (`EXISTE`/`NO-ENCONTRADO`), `columna_real`, `etiqueta` verbatim, `n_validos`, `marginal` y `codigos` (mapa de *value labels*) | las tres piezas de este lote, y cualquier acto sucesor que quiera plantear algo sobre LAPOP México | **Describe el instrumento, no el resultado**: no cruza ninguna variable contra ningún desenlace. Es el archivo que **resuelve los cortes que las tres specs dejaron «pendientes de codebook»** (`aoj11`, `aoj12`, `cp6`/`cp9`, `tamano`, `prot1`) contra el mapa de códigos del propio `.dta`/`.sav`. Trae tres hallazgos que ninguna spec anticipó y que invalidan supuestos heredados: **`wt` de 2004 está VACÍA** (0 válidos de 1 556 — la ola no se puede ponderar, aunque el inventario liste la columna); **2004 no trae `upm`/`estratopri`/`cluster`** y su conglomerado es `msec` («Sección electoral», 131 pares con `mestrat`); y **`ur`/`UR` NO es «`tamano`=5»** (en 2019 `ur`=rural incluye `tamano` 1..4). Quien reuse una de esas tres suposiciones sin leer esta tabla producirá un IC estrechado de mentira o una dicotomización que no es la que cree. |
| `data/l4-clientelar-lapop2019-v1_0.json` | `python3 tools/medidor_l4_clientelar_lapop2019.py --mide --json …` (`ACTO MAESTRA38-L4`) | JSON: `payload`, `estimador`, `diseno`, `clasificacion_tratamiento`, `universo`, `celdas{PRI_principal, MORENA_secundario}`, `contrastes`, `veredicto`, `robustez_vb10`, `comparacion_otro_brazo_no_celda_conjunta`. Cada celda: `p`, `ic95`, `n`, `numerador`, `n_estratos`, `n_upm`, `control_regresion` | la nota de resultados del acto y `milpa/tramite-ola5-propuesta-v0.yaml` (entrada `civico.voto.clientelar_si_observable_lapop2019`) | **`CONTRARIA`: el signo va al revés de lo que `R7.6` predice** — quien fue blanco de una oferta clientelar vota **menos** PRI (4.94 % contra 9.01 %). Quien cite este JSON buscando apoyo para la cesión de autonomía estará citando su refutación. **Los dos campos de comparación con `L9`/`L11` dicen cosas distintas y hay que leer los dos**: `mismo_signo_del_efecto_que_L9_L11` es **`False`** (esta pieza da −4.07 pp; las otras, +14.37/+17.98/+6.38/+11.57) y `mismo_rotulo_de_veredicto_que_L9_L11` es `True`. Coincidir en el rótulo `CONTRARIA` **no** es coincidir en la dirección empírica: cada pieza refuta la predicción de su propio diseño. La primera versión de este archivo colapsaba los dos en un solo campo y afirmaba «misma dirección»; lo corrigió la verificación adversarial del propio acto. `Δ(MORENA)` **contiene 0** y no activa la cláusula de precedencia. Es asociación transversal, y `vb3n` está condicionado a haber votado: la celda es descriptiva **dentro de los votantes**. La rama expuesta se apoya en **17** casos. |
| `data/l5-protesta-multiola-v1_0.json` | `python3 tools/medidor_l5_protesta_multiola.py --mide --json …` (`ACTO MAESTRA38-L5`) | JSON: `diseno_por_ola`, `guardia_2019`, `min_numerador`, `piezas{2004,2006,2019}` — cada ola con `C_completo`, `C_agravio`, `C_falla`, `C_red` (celdas + contraste), `marginales`, `urbano_verificacion_cruzada`, `lapop_e8`, y 2006 además `PROT2_pieza_separada` —, `veredicto_falsador_Bbis`, `notas_spec` | ídem, entrada `civico.protesta.agravio_urbano_multiola` | **`C_completo` es `NO-ESTIMABLE` en las TRES olas y `C_completo` es el corazón de la regla.** Citar los cuatro contrastes estimables (`C_falla` y `C_red` de 2004 y 2006: +2.28, +0.96, +3.92, +1.11 pp) como si corroboraran `R7.4` afirma algo que este dato no midió — **los cuatro contienen 0**, y la propia spec prohíbe sustituir lo compuesto por la suma de lo simple. El hueco es de **tamaño de celda** (rural de alto riesgo: 14, 21 y 1 personas), no de instrumento. **`C_agravio` NO es la replicación que `S5 §3.1` anuncia**: el campo `notas_spec.C_agravio_NO_es_la_replicacion_que_la_spec_anuncia` lo explica — las cifras +5.60/+3.72 pp de `D2-d` son el `C2` de `L9`, y lo que aquí se calcula es el `C1`, el que allí ya había caído por guardia. **`PROT2` de 2006 está gateada por `PROT1`**: universo anidado en el desenlace, fuera de todo veredicto. 2004 y 2006 van **sin ponderar**. |
| `data/l2lista-resultados-v1_0.json` | `python3 tools/medidor_l2lista_mps2012.py` (`ACTO MAESTRA38-L2-LISTA`) | JSON: `fuente` (repo, commit, `sha256`), `spec`, `escala`, `ponderador`, `ic95` (qué método por estimando), `P0_universo`, `S10_4_condicion_de_entrada`, `e1_prevalencia_por_lista`, `e2_prevalencia_directa`, `e3_contraste_lista_menos_directa`, `e4_heterogeneidad` (por `mex.wealth`/`mex.urban`/`mex.loyal`), `e5_participacion_verificada_x_directa` | `forense/notas/2026-09-06-MAESTRA38-L2-LISTA-resultados.md` y `milpa/tramite-ola5-propuesta-v0.yaml` (entrada `civico.clientelismo.prevalencia_lista_listcran_mps2012`) | **Es un SUBCONJUNTO, no el panel: `n=1 004` de ~1 555 de la ola 2, y SIN PONDERADOR.** Citar `0.1874` como «la prevalencia de compra del voto en México» afirma algo que este dato no mide. **`e3` excluye 0, así que la fila B-bis NO se disparó** — quien busque en este JSON el caso «la lista no supera a la directa» está leyendo el contrario. **`P0_universo.y_del_Rd_presente` es `false` y eso es carga útil, no ruido:** `man/mexico.Rd` documenta una variable `y` que el `.tab` no tiene, y el conteo de lista es `mex.y.all`; un lector que busque `y` no falla, encuentra otra cosa. Igual con `campo_0`: las filas traen 26 campos contra 25 nombres. **La heterogeneidad NO tiene identificación causal**: sólo el brazo de tratamiento es aleatorio; los estratos `wealth`/`urban`/`loyal` son descriptivos, y el corte de `mex.wealth` es **por mediana, decidido por la caja**, no por la spec. **Ningún tier del canon se mueve**: 0 variables de `R7.3`/`R7.6` en el dataset. Cuando llegue el `.dta` completo de ICPSR 35024 (`FP-314`/`FP-316`), su veredicto **reemplaza** a éste, no promedia. |
| `data/l18-ver-oir-callar-lapop2004-v1_0.json` | `python3 tools/medidor_l18_ver_oir_callar_lapop2004.py --mide --json …` (`ACTO MAESTRA38-L18`) | JSON: `payload`, `payload_gemelo_sav`, `estimador`, `escala_b18_b10a`, `hallazgo_universo_victimas_2004`, `diseno`, `celdas` (14), `contrastes` (7), `veredicto_B_bis` | ídem, entrada `comunicacion.inseguridad.ver_oir_callar_lapop2004` | **Primera medición de `R10.3` en todo el programa** (`ya_medido.py` la daba `NUNCA-MEDIDA`) y, a diferencia de `L5`, **el corazón sí se midió**: `C_completo` tiene numeradores 111 y 60. `NO-DISCRIMINA` **no es `CONTRARIA`** — no refuta la regla `[FUERTE]` — pero el punto va **al revés** del signo esperado (64.91 % de silencio en contexto ALTO contra 67.42 % en BAJO) y no depende del corte. **La cifra que sí es sólida y que se puede citar sola es el nivel**: 172 de 266 víctimas (64.7 %) no denunciaron, con o sin contexto de desconfianza. `escala_b18_b10a` guarda la resolución de la dirección de la escala `B` contra el cuestionario 2004 (`1 = NADA … 7 = MUCHO`), que el `.dta` no trae etiquetada: **quien reuse `b18`/`b10a` de 2004 sin ese campo puede invertir el signo sin notarlo**. Una sola ola, **sin ponderar**, asociación transversal. |
| `tools/censo_lote_lapop.py`, `tools/medidor_l4_clientelar_lapop2019.py`, `tools/medidor_l5_protesta_multiola.py`, `tools/medidor_l18_ver_oir_callar_lapop2004.py` | escritos por este acto; reusan `raiz()`, `sha256()`, `sha_manifiesto()`, `_cod()`, `_celda()`, `_dif()`, `prop_bootstrap()`, `diff_bootstrap()` de `tools/medidor_clientelismo_lapop.py` (`ACTO MAESTRA35-L9`) **sin modificarlo** | `--mide --json <ruta>`; deterministas (seed 42, 10 000 réplicas) | auditoría y reproducción | **`_filas()` de la casa no sirve para un indicador construido por DISYUNCIÓN**: descarta la fila si *cualquiera* de las columnas pedidas falta, así que en `L4` habría borrado a quien tiene `clien1n` faltante y `clien1na = 1` — que **sí** es `EXPUESTO=1` — antes de evaluar el `OR`. `medidor_l4` clasifica fila por fila y arma las tuplas a mano. Y `_filas()` hace `float(df[peso])`: en 2004 y 2006, que **no tienen ponderador**, `medidor_l5`/`medidor_l18` usan una variante local con peso constante 1.0. Los tres medidores reproducen su JSON byte a byte al re-ejecutarse. |

## Cuestionario e ítems de MPS-2012 en español (`ACTO MAESTRA38-L2`, 6/sep/2026)

| archivo | productor | contrato | quién lo lee | advertencia |
|---|---|---|---|---|
| `data/l2-mps2012-cuestionario-v1_0.txt` | `pdftotext` sobre `35024-Questionnaire-spanish.pdf` (raíz `descargas_mx`, `ICPSR_35024/35024-Questionnaire-spanish.pdf`) | texto plano completo del cuestionario ICPSR 35024, 3908 líneas, dos rondas (marzo/julio 2012) | `ACTO MAESTRA38-L2` (cierre de `FP-263`) y cualquier acto sucesor que necesite el wording verbatim de un ítem del cuestionario en español | Es una conversión mecánica de PDF a texto (`pdftotext`, sin OCR): saltos de línea y tablas de códigos pueden no respetar el layout visual original. No es sustituto del PDF para maquetación, sí para grep de wording. |
| `data/l2-mps2012-items-v1_0.tsv` | curado a mano en `ACTO MAESTRA38-L2` a partir del archivo de arriba | TSV: `variable`, `ola`, `item_num_cuestionario`, `pregunta_texto_verbatim`, `fuente_linea_cuestionario`, `nota` — 11 filas, cubre las variables citadas por `FP-263` (`P35A/P35B`, `W2_P35A/W2_P35B`, `P38A`/`P38B`(ola1), `W2_P38A`/`W2_P38B`(ola2), `P39`, `P40`, `P46`) | ídem | No es exhaustivo del cuestionario completo (44+ preguntas por ola): cubre solo las variables que `FP-263`/T9b/ronda-1 necesitaban. `P38B` de ola 1 y `W2_P38B` de ola 2 son preguntas DISTINTAS que comparten nombre corto — ver la fila de colisión que el LEEME del depósito de mesa (Adenda 7-B) ya declaró; este TSV las distingue por columna `ola`. |
## Dos falsadores de caja sobre ENSANUT2024, celdas sacadas a `data/` (`ACTO MAESTRA38-N14`, cierra `MAESTRA38-LOTE-ENSANUT`, 6/sep/2026)

| archivo | productor | contrato | quién lo lee | advertencia |
|---|---|---|---|---|
| `data/l16-atencion-grave-v1_0.tsv` | `python3 tools/medidor_l16_atencion_grave.py` (re-corrido el 6/sep/2026 sobre los `.dta` cacheados en el scratchpad de la sesión que los extrajo; cifras reproducidas idénticas a la nota de resultados) | TSV: por celda (`grave_H0409A_2_3`, `no_grave_H0409A_1_4`) — `n`, `numerador_publico_crudo` (sin ponderar), `proporcion_publico_crudo`, `proporcion_publico_ponderada` (la que decide el veredicto), `ic95_inferior/superior`, `estratos`, `upm`, `universo`, `escala` | `forense/notas/2026-09-06-MAESTRA38-LOTE-ENSANUT-resultados.md` y el tablero (recibo de `MAESTRA38-N14`) | El falsador B-bis (`salud.atencion.grave`, R4.4) es **`NO-DISCRIMINA`**: ambas celdas rondan 52 % de atención pública, IC95 de la diferencia amplio y contiene 0. Rama A (`ENNVIH`+`ENDIREH`) **no corrida** — ponderador `bx`/2002 ambiguo entre tres candidatos, sin codebook de `ENNVIH` en el corpus. |
| `data/l16bis2-atencion-grave-rama-a-v1_0.json` | `ACTO MAESTRA38-L16-BIS-2` (7/sep/2026, CAJA), corrida directa contra `ennvih/ehh02dta_all.zip` + `ennvih/ehh02w_all.zip` (sha256 de `data/manifiesto.yaml` verificados); el pseudocodigo del CHEQUEO DE CONSISTENCIA es el de `prereg-caja-S6-L16` v1.1 §1.3, corrido tal cual | JSON: `veredicto_rama_A`, `disparadores_de_NO_ESTIMABLE` (D1 chequeo de §1.3, D2 desenlace inexistente), `brazos[]` (`bx` y `b3b` POR SEPARADO, nunca agrupados: archivo, variable, etiqueta verbatim, `n`, distribucion de `es09`, folios, ponderador prescrito vs. aplicado, universo declarado, `numerador`/`proporcion_ponderada`/`ic95` todos `null`), `payloads_verificados`, `lo_que_este_acto_no_hizo` | `forense/notas/2026-09-07-MAESTRA38-L16-BIS-2-resultados.md`; `FP-332` | La Rama A de `salud.atencion.grave` (R4.4) es **`NO-ESTIMABLE`** en ambos brazos, por dos causas independientes: el chequeo obligatorio de §1.3 falla sobre el subconjunto real (`fac_3b_px`=5103 queda 25 por debajo de `fac_3a_px`=5128, el orden se invierte y la spec prescribe PARO) y `BUSCA_PUBLICO` es `NO-CONSTRUIBLE-SIN-DIRECTORIO-EXTERNO` (`clave1`/`clave2` son «ID CLINICA COMUNITARIO»/«ID PROVEEDOR SALUD COMUNITARIO», y ningun reactivo de ENNVIH 2002 pregunta a donde acudio — barrido con control positivo). **No se declara ninguna proporcion ponderada ni ningun IC95: no se construyo ninguna celda.** |
| `data/l17-vacunacion-disponible-v1_0.tsv` | `python3 tools/medidor_l17_vacunacion_disponible.py` (mismo re-corrido) | TSV: por celda (Rama B `razon_logistica`; Rama C `d0321j/d0321p/d0508/d05041/d05051`) — `rama`, `n`, `numerador_crudo`, `proporcion`, `ic95_inferior/superior`, `estratos`, `upm`, `universo`, `veredicto`; celdas `NO-ESTIMABLE` (numerador<10) dejan `proporcion`/IC vacíos | ídem | Rama B primaria (`salud.vacunacion.disponible`, R9.2) es **`CORROBORADA`**: 77.78 % de las menciones de no-vacunación son logísticas (suministro/personal), IC95 excluye 50 % por arriba con margen amplio. Rama C es descriptiva y **mixta** (2 de 4 reactivos `CORROBORADA`/`NO-DISCRIMINA` por encima de 50 %, 2 `CONTRARIA` por debajo) y no pondera contra la Rama B. **El codebook invierte la asignación letra/dígito que la spec `S7-L17-spec-v1_0.md` §2 asumía** (letra=razón/dígito=vacuna, no al revés) — defecto abierto en `FP-326`, la spec sellada no se edita. Rama A (`ENNVIH`) **no corrida**, mismo pendiente de ponderador que L16. En ambos archivos, **salud permanece en 2 de 5** (`EXISTE-SATISFACE`): el lote midió falsadores sobre ids ya clasificados, no clasificó ninguno nuevo. |

## Candidatos a ponderador de ENNViH 2002, sacados a `data/` (`ACTO MAESTRA38-C1 · RE-ASIENTO`, 7/sep/2026)

| archivo | productor | contrato | quién lo lee | advertencia |
|---|---|---|---|---|
| `data/ennvih2002-ponderadores-candidatos-v1_0.tsv` | lectura directa de `data/raw/ennvih/ehh02w_all.zip` y `ehh02dta_all.zip` con `pyreadstat` (`.dta`) más `zipfile-deflate64` para los miembros en `compress_type=9`; sin herramienta versionada — `tools/inventario_fd*.py` no alcanzan esta raíz y están fuera del perímetro de `C1` | cabecera de 16 líneas `#` con la procedencia, y 13 filas de dato: `variable`, `etiqueta_verbatim`, `libro`, `archivo_fuente`, `n_filas`, `n_no_nulo`, `n_no_nulo_gt0`, `n_join_bx_proxy_folio_ls`, `n_join_hogar_folio`, `min`, `max`, `media`, `adr357_ambiguo` | dirección, para adjudicar la Rama A de `S6-L16`/`S7-L17` (sucesor: `L16-bis`/`L17-bis`); `forense/notas/2026-09-07-MAESTRA38-C1-cierre.md` §2(h) | **LISTA, NO ADJUDICA** — ninguna medición del motor, ni `p` ni IC. Tres trampas medidas, cada una silenciosa: (1) `ehh02cb_all.zip` va en **deflate64** — `zipfile` de Python aborta limpio, pero `tar.exe` de Windows extrae el miembro con el tamaño exacto y el contenido **todo `00`**; (2) `n_no_nulo` = 35 677 en las once variables de persona porque el archivo guarda `0`, no faltante — **la columna que discrimina es `n_no_nulo_gt0`**; (3) `folio` es cadena con ceros a la izquierda (`'00001000'`) en los pesos y `float64` (`2000.0`) en el microdato — **sin normalizar la llave a entero el join da 0 en las 13 filas**, y una tabla de ceros se lee como "ningún ponderador cubre el libro". Corrige a `ADR-357` en dos puntos: el codebook **sí** está en corpus (`ennvih1_2002_hogar_cb`) y los tres candidatos "ambiguos" comparten **la misma etiqueta verbatim**, además de que existe un cuarto (`fac_s`, LIBRO SALUD). |

## `data/corrida0/demanda-*.tsv` — la demanda derivada de la corrida cero (`ACTO GEN2-E2 · C0-A DEMANDA`, 7/sep/2026)

| archivo | productor | contrato | quién lo lee | advertencia |
|---|---|---|---|---|
| `data/corrida0/demanda-resultados.tsv` | `python3 tools/corrida0.py demanda` (subcomando único del archivo en esta fase; los demás los añade `GEN2-E3`) | cabecera `# DERIVADO — NO EDITAR`, luego 19 columnas: `resultado_id · consumidor · tipo · valor_legacy · escala_legacy · clase_legacy · acto_legacy · fecha_legacy · payload_ids_legacy · sha256_legacy · script_legacy · spec_legacy · spec_sha_legacy · receta_legacy (`OK`/`PARCIAL:<falta>`/`SIN-RECETA`) · depende_de · corrida_natural · estado · vigencia · validacion_independiente`. Una fila por consumidor activo, en el orden (a) conductas con `p:` → (b) coeficientes → (c) `asignados_probabilidad` → (d) celdas del sorteado vigente | `forense/notas/2026-09-07-GEN2-E2-C0A.md`; los subcomandos `preflight`/`run`/`registro` que `GEN2-E3` construirá encima | **Es demanda, no oferta: las 162 filas están todas en `estado=PENDIENTE`, `vigencia=PENDIENTE`, `validacion_independiente=NO-HECHA`, y no hay una sola columna `valor_gen2`.** `valor_legacy` se copia verbatim del registro GEN1 para poder comparar después, y **no autoriza a citar esa cifra como si fuera GEN2**. `receta_legacy` mide lo que el registro DECLARA, no lo que se podría reconstruir leyendo notas: un `SIN-RECETA` no dice que la medición no ocurriera, dice que el archivo que sostiene el valor no la cuenta. Los ids (`RES-0001…`) son posicionales sobre ese orden de recorrido: **cambiar el orden renumera todo**, así que no son llaves estables entre versiones del recorrido — la llave estable es `consumidor`. |
| `data/corrida0/demanda-corridas.tsv` | ídem | cabecera `# DERIVADO — NO EDITAR`, luego 9 columnas: `corrida_id · instrumento · payload_ids · medidor_o_spec_candidato · n_resultados · resultados_ids · entorno_requerido · receta · orden_causal` (1–7 de la escalera causal declarada en el docstring de `tools/corrida0.py`) | ídem | **La agrupación en corridas es una PROPUESTA derivada mecánicamente** de `(orden_causal, instrumento, payload, spec, script)`, no una decisión de mesa: donde el registro no permite decidir, el acto **lista** el caso en vez de agrupar (los 7 casos van en la nota y en la fila `ABIERTA` `FP-339`). `medidor_o_spec_candidato` dice **candidato**: en varias filas es el script que hoy produce el artefacto, no un medidor sellado con hash. `entorno_requerido` se deriva de si hay payload declarado (`CAJA`), de si sólo se relee el repo (`NUBE`), o de ninguna de las dos (`INDECIDIBLE-…`) — un `INDECIDIBLE` es un hueco del registro, no una corrida barata. |
| `tools/corrida0.py` | escrito por este acto | CLI con `demanda` implementado y los otros nueve subcomandos del plan (`spec-check · negativo · preflight · run · verify · registro · status · vigencia · delta`) **declarados y vacíos**: invocarlos sale con código 2 y el rótulo `NO-IMPLEMENTADO` | `GEN2-E3`, que los llena | `demanda` **no mide nada**: sólo lee `milpa/tramite.yaml`, `milpa/procedencia.yaml`, `data/manifiesto.yaml`, el sorteado v1.3 y `forense/prereg-duelo-v2/corridas-{M,R,L}/` + `agregado-v1_3-resultado.json`. Es determinista (dos corridas sobre el mismo SHA dan bytes idénticos) y **PARA sin escribir** si el grafo `depende_de` tiene un ciclo o si una arista retrocede en la escalera causal. |
| `data/corrida0/decisiones.tsv` | **editado a mano por mesa**, no derivado (`ACTO GEN2-T7`, 7/sep/2026) | TSV con encabezado y columnas `objeto · decision · fuente · fecha`; 7 filas hoy, las siete que `FP-339` deja `ABIERTA` en `demanda-corridas.tsv` — `TRA-M-02/03/07` con `decision=M_vivo=__v1_3` y las dos celdas `dinero.ahorro.tiene_ahorros`/`familia.apoyo.recibe_dinero_familiares` (×2 cada una) con `decision=receta_legacy=SIN-RECETA` | `tools/corrida0.py` (`cmd_demanda`, función `_lee_decisiones`) | **No autoriza a `cmd_demanda` a inventar una decisión**: una fila sólo pisa `receta_legacy`/filtra la lista de ambiguas cuando el `objeto` coincide exactamente con el `consumidor` de la fila destino. Añadir una fila aquí sin que exista ya el caso correspondiente en `demanda-corridas.tsv`/`demanda-resultados.tsv` no hace nada — `cmd_demanda` la ignora en silencio. Es artefacto de mesa (firmado por `FP-339`), no generado: no lleva cabecera `# DERIVADO`. |

## `data/corrida0/{corridas,resultados,usos}.tsv` — las tres vistas del registro GEN2 (`ACTO GEN2-E6 · AUTOMATIZA-GEN2-2`, 8/sep/2026)

| archivo | productor | contrato | quién lo lee | advertencia |
|---|---|---|---|---|
| `data/corrida0/corridas.tsv` | `python3 tools/corrida0.py registro` | cabecera `# DERIVADO — NO EDITAR`, luego 22 columnas: `corrida_id · origen · spec_id · estado · generacion · cuenta_gen2 · spec_yaml_sha256 · script_path · script_blob_sha256 · codigo_commit · fecha · n_resultados · resultados_ids · input_ids · input_sha256_efectivos · sello · resultado_replay · contexto_replay · sucesor · entorno_requerido · receta · orden_causal`. Una fila por corrida de **demanda** (`origen=DEMANDA`, las 79 que `demanda-corridas.tsv` pide) y una por carpeta `CALC-*/` sellada (`origen=OFERTA`) | `corrida0 status`, `tools/tablero_programa.py` (bloque GEN2), `tests/check.py` (`T35 · T-REPRO`) | **`resultado_replay`/`contexto_replay` valen `NO-VERIFICADO` salvo que se corra `registro --verifica`**, que sí reejecuta el medidor. El registro no reejecuta microdato por su cuenta: un campo que dijera `REPRODUCE` sin haber corrido nada sería exactamente la evidencia falsa que este aparato existe para no producir. `estado` distingue `SELLADA` de `SUPERADO→<sucesor>` (lo declara `repite_de` de la spec sucesora). |
| `data/corrida0/resultados.tsv` | ídem | cabecera `# DERIVADO — NO EDITAR`, luego 26 columnas: `resultado_id · origen · corrida_id · spec_id · valor · tipo · unidad · estado · generacion · cuenta_gen2 · tolerancia · tolerancia_adopcion · validacion_independiente · validacion_ref · alcance_validacion · rol_evaluacion · origen_numerico · funciones_dependencia · camino_linaje · valor_legacy · delta_legacy · sello · fuente_replay · depende_de · sucesor · n_usos` | ídem | **La unicidad de un `resultado_id` es POR CORRIDA, no global**: un replay declarado (`repite_de`) reproduce los mismos ids a propósito — es lo que significa replicar. El mismo id en dos corridas **sin** esa cadena sí PARA. Los `RESULT-SMOKE-*` entran con `cuenta_gen2 = NO` y no suman a ningún contador GEN2. `validacion_independiente` acredita sólo el resultado exacto que el overlay sucesor enlaza; no cambia por sí sola el rol retenido ni la aptitud para confirmación. |
| `data/corrida0/validaciones-independientes.tsv` | curado por actos sucesores que validan RESULT ya sellados; este acto añade las 21 filas ENVIPE de `GEN2-VALIDACION-R-ENVIPE-CSV` | overlay con llave única `(spec_id, resultado_id)` y campos `validacion_independiente · validacion_ref · evidencia_sha256 · alcance_validacion`; `tools/corrida0.py registro` verifica destino único, estado controlado, existencia y SHA-256 de la evidencia antes de proyectarlo en `resultados.tsv` | `tools/corrida0.py` (`_aplica_validaciones_independientes`), `tests/test_corrida0.py`, `corrida0 status` y la vista común `resultados.tsv` | **VALIDA, NO RESELLA NI ADOPTA:** `PASA` sólo se asigna a los 9 RESULT de punto/`n`/masa que la evidencia reconstruye; 12 RESULT de EE/IC/CV quedan `CONCUERDA-NO-APROBADA`. La proyección falla cerrada si cambia la evidencia o no existe exactamente un destino. No altera specs/recibos congelados, `rol_evaluacion`, `origen_numerico` ni `camino_linaje`; por eso no convierte una corrida de desarrollo en holdout. |
| `data/corrida0/usos.tsv` | ídem | cabecera `# DERIVADO — NO EDITAR`, luego 8 columnas: `resultado_id · consumidor · tipo_uso · activo · reglas_impacto · generacion_leida · corrida0_resultado_id · valor_materializado`. Una fila por consumidor activo | ídem | **`corrida0_resultado_id` vacío significa que el consumidor sigue leyendo la cifra GEN1 materializada** — hoy son las 162 filas, y por eso `dependencias_numericas_legacy_activas == N_resultados_activos` es la lectura correcta y no un error. `valor_materializado` es la cifra tal como vive en el archivo del consumidor: es lo que `T35 · T-REPRO` compara contra el RESULT sellado, con tolerancia **por tipo**. |
| `data/corrida0/envipe-serie-denuncia-v1_0.tsv` | `python3 tools/deriva_serie_envipe_completa.py` (`ACTO GEN2-ENVIPE-SERIE-COMPLETA`, 10/sep/2026) | TSV de 15 filas, años-hecho contiguos 2010–2024; selecciona `p(C1,U1)`, IC95 bootstrap, `n`, unidad, ponderador, reactivo, formato, ruptura y fuente exacta desde RESULT sellados | lectura humana y actos sucesores que pidieron la serie temporal; nota de cierre del acto | **DERIVADO, NO ESTIMA:** falla ante hueco o año duplicado y no interpola. El estimando es la proporción de motivos 01/02/06 entre delitos personales no denunciados con respuesta 01–08, ponderada por `FAC_DEL`; no es tasa bruta de denuncia ni efecto causal. La ola 2011 queda marcada por ruptura nominal/residual (`BP1_21`; 88/98/99). |
| `data/corrida0/envipe-validacion-independiente-v1_0.tsv` · `data/corrida0/envipe-validacion-independiente-exclusiones-v1_0.tsv` · `data/corrida0/envipe-validacion-independiente-incertidumbre-v1_0.tsv` | `python3 tools/valida_envipe_independiente.py` (`ACTO GEN2-ENVIPE-VALIDACION-LECTURA`, 10/sep/2026) | evidencia posterior a ocho CALC sellados: por ola, identidad del ZIP/miembro, numerador y denominador `FAC_DEL`, `n`, punto reconstruido/referencia/delta/veredicto; embudo excluido por etapa/código; comparación de linealización de razón 2019 con el IC analítico sellado | lectura humana, cierre de `NC-0155` y consumidor `GEN2-PUBLICACION-POST693-Y-CIERRES` | **VALIDA, NO RESELLA:** implementación separada sin importar el medidor ni su bootstrap. No edita las specs/recibos históricos con `NO-HECHA`, no crea otra serie canónica, no acredita los otros 39 RESULT de cada CALC y no adopta al motor. |
| `data/corrida0/enif-fintech-correspondencia-v1_0.tsv` | curada a mano antes del primer resultado por `ACTO GEN2-ENIF-FINTECH-SERIE-DESCRIPTIVA` (10/sep/2026), desde cuestionarios y FD ENIF 2018/2021/2024 ya manifestados | 17 campos por ola y familia: reactivo, población/edad, ventana, criterio fintech, canal/códigos, especiales, ponderador, diseño, comparabilidad y decisión | `tests/test_enif_fintech_serie.py`, spec y nota de cierre del acto | **GOBIERNA LA RECODIFICACIÓN, NO ES RESULTADO:** fija 2018 `NO-ESTIMABLE`, cuenta 2021/2024 sin delta por ruptura y crédito comparable con cautela antes de medir. |
| `data/corrida0/enif-fintech-serie-v1_0.tsv` | derivación humana exacta de `CALC-ENIF-FINTECH-0001` sellado más el antecedente 2024 ya publicado; validada por `tests/test_enif_fintech_serie.py` | 17 campos: ola/familia/canal, estado, `n`, masa, p/EE/IC, fuente, comparabilidad, delta y limitación; incluye filas explícitas de ruptura | nota y figura del acto; lectura humana y futuro acto dueño de la propuesta de procedencia | **DERIVADO DESCRIPTIVO:** 2024 no cuenta como medición nueva; no inventa la categoría 2024 no publicada ni calcula delta de cuenta. El canal es del último producto entre tenedores fintech, no del producto fintech exacto. |
| `data/corrida0/enif-fintech-serie-v1_0.svg` | figura SVG determinista construida desde `enif-fintech-serie-v1_0.tsv` por el mismo acto | dos paneles de puntos por canal; IC95 sólo para 2021, triángulos para el antecedente 2024 y ruptura 2018 rotulada | nota de cierre y lectura humana; estructura XML/leyendas verificadas por `tests/test_enif_fintech_serie.py` | **FIGURA, NO FUENTE:** se lee junto con el TSV; el panel de cuenta marca lado a lado sin delta y no fabrica IC para 2024. |

## `data/adq-config.yaml` — configuración del runner de adquisición, sacada del shell (`ACTO ADQ-CRON-V2 · DISPARO-PERSISTENTE-Y-RUNNER-IDEMPOTENTE`, 7/sep/2026)

| archivo | productor | contrato | quién lo lee | advertencia |
|---|---|---|---|---|
| `data/adq-config.yaml` | configuración histórica extraída del shell por ADQ-CRON-V2 y autoridad de ejecutor actualizada por `ACTO GEN2-ADQ-CODEX-PRODUCCION` | YAML: `ejecutor=codex`, límites genéricos `ejecutor_timeout_segundos=1800`/`ejecutor_kill_after_segundos=60`, máximo 5, binario/modelo/sandbox/red/esquema Codex, calendario/zonas, sonda, runbook y sistemas PDN. Los nombres `claude_*` viejos sólo son alias de compatibilidad si falta la clave genérica; no son segunda autoridad. | `tools/adquiere_launcher.sh`, `tools/adquiere_cron.sh` y `tools/adq_doctor.py`, siempre mediante `tools/adq_config.py` | Claude no es dependencia ni fallback: sólo `ejecutor: claude` explícito lo activa. El launcher conserva lock y revisión fijada hasta que `main` la contenga. Cambiar ids PDN sigue siendo decisión de mesa; datos por sistema faltantes abortan en vez de inventarse. |

## `forense/tablero/` — el tablero del programa, cierra el hueco del índice (`ACTO GEN2-TRAMITE-TABLERO-1`, 8/sep/2026)

Hueco confirmado por A.8(1) al abrir este acto: `grep -c "forense/tablero" data/INFRAESTRUCTURA-v1_0.md` daba **0** — el dominio existe desde el 7/sep (`PR #489` lo crea, `PR #571` lo consolida) pero nunca entró a este índice. Esta fila lo cierra.

| archivo | productor | contrato | quién lo lee | advertencia |
|---|---|---|---|---|
| `forense/tablero/TABLERO-PROGRAMA.md` | **VIGENTE.** Dos capas, dos escritores distintos, un solo archivo: (1) el bloque `<!-- TABLERO-DERIVADO:BEGIN -->`…`END` lo escribe `python3 tools/tablero_programa.py --actualiza`, mecánico, sin juicio; (2) todo lo que rodea al bloque (la narrativa, las tablas curadas §1-9) lo escribe a mano la conversación del tablero — un humano o un acto explícito de trámite, nunca el script | prosa markdown; el bloque derivado es el único con forma fija (constantes exactas `TABLERO-DERIVADO:BEGIN`/`END`, `tablero_programa.py:66-67` rebana entre las primeras apariciones de cada una) | `tools/tablero_vista.py` (genera la vista HTML), cualquier acto que necesite el estado vivo del programa, mesa | **El patrón de línea completa, no la subcadena, es el que hay que usar para verificar los marcadores** (`grep -cx`, no `grep -c`): el cuerpo curado puede mencionar `TABLERO-DERIVADO:BEGIN`/`END` en prosa dentro de una tabla (p. ej. describiéndose a sí mismo), y `grep -c` cuenta esa mención como si fuera un segundo marcador. `--actualiza` se niega a escribir si no encuentra exactamente una de cada constante. |
| `forense/tablero/TABLERO-PROGRAMA-v1_1.md`, `forense/tablero/TABLERO-PROGRAMA-v1_0-superado.md` | **HISTORIA — no se editan salvo para anotarlos.** Snapshots congelados de capas curadas anteriores del mismo tablero (v1.1 = snapshot `9cbd8d8`, 2/sep/2026; v1.0 = snapshot `57a365e`), append-only por convención de la casa (`forense/encargos/convencion.md`: un artefacto consumido no se borra). Ninguno de los dos alimenta `tools/tablero_vista.py` ni ningún consumidor programático — son historia recuperable, no una fuente viva | quien audite cómo llegó el tablero a su forma actual | **Toda referencia nueva se alinea al archivo vigente** (`ACTO MAESTRA38-TRAMITE-3`, decisión de mesa "1 solo archivo con un solo nombre"): citar un `-v1_1`/`-superado` como si fuera la fuente de estado actual es el defecto que este acto encontró y corrigió una vez (`TABLERO-PROGRAMA.md` seguía siendo v1.1 seis días después de que Gen 2 abriera, G-D4 del propio tablero). |
| `tools/tablero_vista.py` | genera `forense/tablero/*.html` a partir de `TABLERO-PROGRAMA.md` + la salida `--json` de `tools/tablero_programa.py` (y, en modo Gen 2, del JSON de `corrida0.py status`) — ninguna cifra vive escrita en el script | uso humano (lectura en diez segundos); no lo consume ningún test ni otro script | localiza secciones por título (`marca_por_titulo`), no por número de sección — tolera que el cuerpo curado cambie de forma sin romper la vista, a diferencia de una localización por `## N ·` fija. |

## `data/fuentes-financieras-20/` — CNBV, CONDUSEF, ENCRIGE, Banxico y ENSAFI (`ACTO GEN2-CNBV-CONDUSEF-FUENTES-Y-SERIES` + continuación efectiva, 11/sep/2026)

| archivo | productor | contrato | quién lo lee | advertencia |
|---|---|---|---|---|
| `data/fuentes-financieras-20/cnbv-imor-consumo.csv` | `python3 tools/extrae_fuentes_financieras.py` sobre `gen2_cnbv_040_1a_r16_imor_tipo_cartera` | `fecha · producto_universo · indicador · valor · unidad · fuente · notas`; nueve filas del bloque consumo de `MINFO`, con el faltante original vacío | lectura descriptiva de R1.6 y la nota de cierre del acto | Es **una foto cacheada 202112**, no una serie mensual pública: la macro del XLS requiere una conexión interna de CNBV. IMOR es razón de saldos de cartera en porcentaje, no probabilidad individual de impago. |
| `data/fuentes-financieras-20/condusef-inventario.csv` | el mismo extractor; verifica sha/tamaño y abre los 29 CSV ya manifestados | una fila por payload con identidad, forma, codificación, corte, columnas, unidad y límite interpretativo | auditoría del lote CONDUSEF y futuras selecciones de columnas | El inventario no convierte directorios, sanciones, supervisión o accesos web en reclamaciones; registra explícitamente qué archivo no incluye periodo. |
| `data/fuentes-financieras-20/condusef-acciones-defensa.csv` | el mismo extractor sobre los dos cortes de acciones de defensa | agregado por `periodo · sector · clase · canal_accion`, con conteo, unidades de atención y participación **dentro del mismo canal** | descripción administrativa de N34 | Los canales pueden ser etapas superpuestas. Nunca se suman como expedientes únicos y la participación no es prevalencia entre clientes. |
| `data/fuentes-financieras-20/condusef-reclamaciones-clase.csv` | el mismo extractor; exige por fila `CONDUSEF + REUNE = total_reclamaciones` antes de agregar | `periodo · sistema_registro · sector · clase`, conteos y participación dentro de periodo-sistema | descripción institucional de N34 | Mantiene CONDUSEF y REUNE separados. Los CSV no traen causa ni denominador de clientes; 2024 y 2025 tienen coberturas de instituciones distintas. |
| `data/fuentes-financieras-20/condusef-reclamaciones-producto.csv` | el mismo extractor sobre las dos ediciones BEF de evaluación de producto | agregado por `edicion_archivo · sector · producto`; el periodo queda `NO_INCLUIDO_EN_CSV` | mapa descriptivo de productos reclamados para N34 | No se calcula cambio temporal entre ediciones: los payloads no documentan su corte y hacerlo sería fabricar comparabilidad. La categoría de producto no acredita BNPL, usura ni causa. |
| `data/fuentes-financieras-20/encrige2020-cumplimiento-contratos.csv` | el mismo extractor sobre las tablas publicadas `t1_5`, `t1_9`, `t1_15` y `t1_19` de `conjunto_de_datos_encrige_2020_csv` | seis estimaciones nacionales expandidas; cada fila fija reactivo, actor, universo, denominador, numerador, porcentaje y diseño | lectura del lado empresa-acreedora/contratante de N34 | ENCRIGE observa empresas frente a contrapartes privadas, no consumidores deudores. `t1_15`/`t1_19` excluyen microempresas y `t1_19` admite respuesta múltiple; no se mezclan con `t1_5` ni se suman entre sí. |
| `data/fuentes-financieras-20/banxico-imor-consumo-mensual.csv` | el mismo extractor sobre `gen2_banxico_imor_consumo_producto_mensual_2026t1_html` | 615 filas `fecha×producto`; IMOR mensual 2016-01..2026-03 con numerador, denominador, porcentaje, universo y régimen de definición | lectura descriptiva de R1.6 y continuidad temporal | Publicación oficial equivalente, no R16 CNBV. La ruptura IFRS9 desde 2022-01 impide interpretar el salto como cambio limpio; banca comercial incluye Sofomes ER subsidiarias y excluye CI Banco. |
| `data/fuentes-financieras-20/ensafi2023-atraso-deuda-producto-hogar.csv` | el mismo extractor sobre THOGAR de `ensafi2023_bd_csv_zip`, contrastado con descriptor/cuestionario | cuatro clases de deuda con expuestos, desconocidos, n y masa de numerador/denominador, porcentaje y `FAC_HOG` | prevalencia descriptiva del lado consumidor-deudor para N34 | Clases no excluyentes; la formal agrupa banco/financiera/tienda. No identifica institución, CAT, BNPL ni causalidad. |
| `data/fuentes-financieras-20/ensafi2023-deuda-y-afrontamiento-persona.csv` | el mismo extractor sobre TMODULO, diseño y resultados ENSAFI | nueve estimandos: atraso entre personas con deuda y ocho respuestas ante ingreso insuficiente, con n/masas, `FAC_ELE` y contraste 27.3 | contexto poblacional de N34 | P6_7 no liga atraso a producto ni tiene ventana explícita; P6_10 es respuesta múltiple contextual, no efecto causal. |
| `tools/extrae_fuentes_financieras.py` · `tests/test_fuentes_financieras.py` | extractor determinista y pruebas de fixtures puras; `--verifica` reconstruye las nueve salidas byte a byte en CAJA | resuelve raíces lógicas del manifiesto, verifica payloads, rechaza llaves duplicadas, huecos mensuales, conteos fraccionarios, porcentajes publicados discordantes y denominadores vacíos | reproducción y CI (la prueba unitaria no necesita corpus ni red) | `xlrd` es dependencia explícita porque el original CNBV es XLS/OLE. Las pruebas no fijan totales del árbol; los contrastes de bytes reales ocurren en CAJA. |

## `data/analisis-imor-contexto-temporal/` — derivados descriptivos por régimen (`ACTO GEN2-IMOR-CONTEXTO-TEMPORAL-POR-REGIMEN`, 11/sep/2026)

| archivo | productor | contrato | quién lo lee | advertencia |
|---|---|---|---|---|
| `data/analisis-imor-contexto-temporal/niveles-mensuales.csv` | `python3 data/corrida0/CALC-IMOR-CONTEXTO-0001/materializa.py` desde el CSV Banxico versionado y la spec sellada | 615 filas `fecha×producto`; conserva nivel, unidad, universo, fuente y uno de dos regímenes de definición | lectura descriptiva, figuras, pruebas dirigidas y cierre del acto | Es una reorganización reproducible de la serie publicada, no una medición nueva ni R16 CNBV. |
| `data/analisis-imor-contexto-temporal/cambios-mensuales-e-interanuales.csv` | el mismo materializador | una fila por nivel; cambios mensual e interanual en pp y porcentaje relativo, calculados sólo cuando referencia y observación pertenecen al mismo régimen | lectura descriptiva, figura de cambios, pruebas y control independiente | Los inicios de régimen quedan vacíos por diseño; nunca contiene el salto 2021-12→2022-01 como cambio homogéneo. |
| `data/analisis-imor-contexto-temporal/resumen-producto-regimen.csv` | el mismo materializador | diez filas (cinco productos×dos regímenes) con media, mediana, extremos/fechas, desviación estándar poblacional temporal y diferencias entre extremos | ficha, lectura, control independiente y cierre | La dispersión es temporal (`ddof=0`), no error estándar de encuesta; comparar cifras entre regímenes no elimina la ruptura IFRS9. |
| `data/analisis-imor-contexto-temporal/niveles-por-producto-y-regimen.svg` | el mismo materializador, con `svg.hashsalt` fijo y whitespace normalizado | figura SVG byte-estable, paneles separados para niveles pre-IFRS9 e IFRS9 | lectura humana y pruebas XML/rótulos | Los paneles no autorizan leer el salto de definición como cambio económico limpio. |
| `data/analisis-imor-contexto-temporal/cambios-mensuales-por-producto-y-regimen.svg` | el mismo materializador determinista | figura SVG byte-estable de cambios mensuales en pp, separada por régimen | lectura humana y pruebas XML/rótulos | Sólo muestra diferencias intrarrégimen; no representa porcentajes relativos ni incertidumbre inferencial. |
| `data/analisis-imor-contexto-temporal/ficha-contextual.md` | curada por el acto a partir de la spec y resultados sellados | ficha de unidad, universo, periodo, fuente, ruptura y usos excluidos | cualquier consumidor futuro de R1.6 y auditoría | Uso `DESCRIPTIVO-NO-CALIBRA`: saldo agregado no equivale a riesgo individual, CAT, causalidad o validación predictiva. |
| `data/analisis-imor-contexto-temporal/lectura-resultados.md` | curada por el acto y contrastada contra `resumen-producto-regimen.csv` | síntesis breve de hallazgos, límites y dato faltante para calibración individual | lectura humana y mesa | No adopta R1.6 ni N34, no reabre NC-0163 y no convierte Banxico en R16. |
