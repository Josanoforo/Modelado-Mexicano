...
- **Celdas validadas (métrica rectora, firma de mesa 20/sep/2026).** `219` celdas con predicción emitida antes de ver el dato y error sellado contra R (cruce `162` + persistencia `57`). **No es «N aciertos»: es N celdas con error CONOCIDO.** Tres clases, sin fundir:
  ...
  - *cruce vs R* · `GOB.gobierno_digital.encig2025.edad_x_sexo` · n `8` · champion `C2` · error mediano `0.922` pp (máx `1.806` pp) · brecha `0` años (misma ola) · escala cruda del CALC `PUNTOS-PORCENTUALES` · `data/corrida0/CALC-ENCIG-DUELO-2025-ADJUDICACION-0001/resultados.json`
  - *cruce vs R* · `GOB.gobierno_digital.encig2025.escolaridad_x_sexo` · n `8` · champion `C2` · error mediano `0.868` pp (máx `2.014` pp) · brecha `0` años (misma ola) · escala cruda del CALC `PUNTOS-PORCENTUALES` · `data/corrida0/CALC-ENCIG-DUELO-2025-ADJUDICACION-0001/resultados.json`
...
- **Procedencia.** SHA `8a867a04` · fecha del commit `2026-09-24` · ¿árbol == origin/main? `False`.
...
- **Ramas presentes en origin.** **4 rama(s) presente(s) en origin (política de cero)**:
  - `acto/gen2-contadores-consumo-2-alternativa-punto-fijo-huerfana`: 3 delante / 101 detrás de main · último commit `2026-09-23`
  - `claude/new-session-q4k5jc`: 1 delante / 13 detrás de main · último commit `2026-09-24`
  - `claude/tramite-2026-09-24`: 3 delante / 29 detrás de main · último commit `2026-09-24`
  - `codex/astra5-mapa-dominios-1`: 111 delante / 29 detrás de main · último commit `2026-09-24`
...
- **NC abiertas por razón (token A.14, prefijo exacto).** abiertas `281` · por token: `DECISIÓN-DE-MESA-PENDIENTE` 23 · `DIFERIDO-A` 44 · `FUERA-DE-PERÍMETRO` 54 · `NO-VERIFICABLE-AQUÍ` 16 · `PARO-ENTORNO` 2 · `PARO-PREMISA` 32 · `SUSTITUIDO-POR` 4 · prosa (sin token reconocible) `106`.
- **GEN2 (derivado de `corrida0 status`).** corridas selladas `244` / requeridas `87` · resultados sellados `66493` / activos `211` · pendientes `211` · dependencias numéricas legacy activas `146` · validación independiente `215` · diferencias materiales `0` · NC- abiertas `281` · replays LEGACY-GEN1 sellados `5` (no cuentan). El `0 / N` es la lectura correcta: el aparato se construyó antes que las corridas.
