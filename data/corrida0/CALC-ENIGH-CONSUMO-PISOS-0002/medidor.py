#!/usr/bin/env python3
"""CALC-ENIGH-CONSUMO-PISOS-0002 · pisos de consumo y gasto por segmento, ENIGH 2016-2022.

SUCESOR de CALC-ENIGH-CONSUMO-PISOS-0001 (spec v1.1). Único cambio: en 2016 y 2018 la tabla
`gastoshogar` se lee de la descarga por tabla (`cc1_inegi_enigh_{2016,2018}__enigh{2016,2018}_ns_
gastoshogar_csv`, miembro `gastoshogar.csv`), porque la del ZIP integrado `enigh{2016,2018}_nc_csv`
está truncada en 1 048 575 filas (límite de Excel; hallazgo del sello de 0001). Procedimiento,
lista cerrada, ejes, semilla y réplicas: idénticos a 0001.

ACTO GEN2-CONSUMO-Y-GASTO-PISOS-1 (25/sep/2026, CAJA). Contrato humano:
forense/prereg-caja/CONSUMO-ENIGH-PISOS-spec-v1_1.md, congelado en el COMMIT-1 antes de
...
# gastoshogar completo por tabla para las olas cuyo ZIP integrado lo trae truncado (spec v1.1 §0)
GASTOS_POR_TABLA = {"2016": "cc1_inegi_enigh_2016__enigh2016_ns_gastoshogar_csv",
                    "2018": "cc1_inegi_enigh_2018__enigh2018_ns_gastoshogar_csv"}
...
def _guardia_inputs(inputs):
    esperados = INPUTS_REPO | set(PAYLOADS.values()) | set(GASTOS_POR_TABLA.values())
    if set(inputs) != esperados:
        raise ParoDeGuardia(f"inputs fuera de la lista: {sorted(set(inputs) ^ esperados)}")
    for pid in list(PAYLOADS.values()) + list(GASTOS_POR_TABLA.values()):
...
def lee_ola(ruta, ola, R, ruta_gastos=None):
    ...
    if ruta_gastos is None:
        ...
        gas = R.lee_csv_zip(ruta_gastos, COLS_GAS, miembro=_miembro(ruta_gastos, "gastoshogar.csv"),
                            encoding="utf-8-sig")
    return {"conc": lee("concentradohogar", COLS_CONC), "hog": lee("hogares", COLS_HOG), "gas": gas}
...
def medir(inputs, contrato):
    ...
    por_ola, diags = {}, {}
    for ola in OLAS:
        rg = inputs[GASTOS_POR_TABLA[ola]]["ruta_absoluta"] if ola in GASTOS_POR_TABLA else None
        frames = lee_ola(inputs[PAYLOADS[ola]]["ruta_absoluta"], ola, R, ruta_gastos=rg)
    ...
    out[f"{P}-G-FUENTE-GASTOSHOGAR"] = ("2016, 2018: descarga por tabla cc1_*_gastoshogar_csv; "
                                        "2020, 2022: ZIP integrado enigh*_nc_csv")
    return out
...
def esquema_resultados():
    ...
    f += [_fila(f"{P}-G-BOOTSTRAP-REPLICAS", "entero", "réplicas"),
          ...
          _fila(f"{P}-G-FUENTE-GASTOSHOGAR", "texto", "declaración")]
    return f
