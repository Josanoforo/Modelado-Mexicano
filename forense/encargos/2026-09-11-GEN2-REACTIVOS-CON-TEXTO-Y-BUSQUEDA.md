# ENCARGO 34 · GEN2-REACTIVOS-CON-TEXTO-Y-BUSQUEDA
...
- Estructura: existen `AGENTS.md`, los cuatro extractores/consumidores requeridos, las tablas vigentes, `forense/no-corrido.tsv` y la nota D-14 citada.
- Contenido: al arranque, #725 y #726 ya estaban fusionados; #727–#732 estaban abiertos. #732 conserva sólo la cola/copia del encargo y no implementa este objeto; los demás no duplican el índice ni el buscador.
- Cobertura retroactiva: se preservan las dos tablas históricas y sus posiciones; el índice de contexto es un overlay con `id_origen`. No se modifica cron, motor, CALC, fuentes crudas ni resultados F5.
