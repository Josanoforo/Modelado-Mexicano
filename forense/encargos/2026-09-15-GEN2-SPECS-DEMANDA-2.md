...
| formalizar `payload_id`/`sha256`/`script` de los 7 coeficientes de `CORR-0018` en `milpa/procedencia.yaml:coeficientes_generador_sellados` (`NC-0202`) | `FUERA-DE-PERÍMETRO` — `milpa/` no se toca en este acto. La sonda (`forense/notas/2026-09-15-GEN2-SPECS-DEMANDA-2-sonda-CORR-0018.md`) entrega la tabla completa (6/7 con `sha256` confirmado); `D3` (`ADR-510`, rotular `SIN-PROCEDENCIA-VERIFICABLE`) queda superada por el hallazgo, pero no se ejecuta aquí tampoco — es decisión de mesa cuál de las dos vías tomar | Los 7 coeficientes siguen sin identidad `D-15`/`A.7` formal en `milpa/`, aunque la tabla que la resolvería ya existe | el acto que toque `milpa/` (CAJA o mesa) + `MESA` (decide entre `D3` tal cual o instruir la formalización) |
| verificar las 3 candidatas web de `CORR-0008` (documentación oficial `ENNViH`, `RAND`, `ICPSR`/`openICPSR`) (`NC-0201`) | `NO-VERIFICABLE-AQUÍ`. Bloqueo de red de **esta sesión** confirmado por 3 mecanismos independientes (`curl`, estado del proxy, `WebFetch`) y por control de calibración contra `inegi.org.mx` (también bloqueado hoy) | `CORR-0008` sigue `BLOQUEADA` por `NO-HAY-DISENO-PUBLICO`; `NC-0156` sin cambio de fondo | `CAJA` o una sesión sin este bloqueo de red re-intenta el mismo `curl`/`WebFetch` |
| firma de contador con `OBJETO` para las 6 specs nuevas de esta tanda (`NC-0200`) | `DECISIÓN-DE-MESA-PENDIENTE`. El lanzamiento autoriza **congelar** y no declara `OBJETO` sobre el contador GEN2; `FP-367`/`FP-368` no se satisfacen | Las 6 llevan `cuenta_gen2 = PENDIENTE-DE-MESA`: cuando se sellen, no incrementan el contador GEN2 salvo que la firma llegue | `MESA` o el acto que las selle (vía `decisiones.tsv`) |
| firma de mesa sobre `D1`/`D2`/`D3` (armadas por `ACTO GEN2-SPECS-DEMANDA-1`, `NC-0197`) | `DECISIÓN-DE-MESA-PENDIENTE`, sin cambio — este acto no firma decisiones ajenas, aunque `D3` gane un hallazgo nuevo (`NC-0202` arriba) | `CORR-0001`, `0004`, `0005`, `0006`, `0018`, `0019` siguen `BLOQUEADA` por estas tres firmas | `MESA` |

## CONSUMIDO

PR [#781](https://github.com/Josanoforo/Modelado-Mexicano/pull/781), abierto contra `main` el 15/sep/2026 y **no fusionado por el ejecutor** (el merge es de mesa). Cascada de cierre completa: **ADR-511** en `canon/gobernanza-v1_15.md`, anotación L0 nueva en `canon/estado-programa-v1_13.md` (más una retroactiva para `ADR-510`, que no había recibido la suya), los tres contadores mecánicos reconciliados por `tools/cierre_acto.py --aplica` (`gobernanza 510→511 · L0 510→511 · tabla estado 510→511`), rótulo `GEN2-SPECS-DEMANDA-2` censado en `canon/registro-rotulos.tsv`, `data/corrida0/mapa-demanda-19-corr-v1_0.tsv` actualizado a mano con comando por fila, `NC-0199`…`NC-0202` en `forense/no-corrido.tsv`. `python3 tests/check.py --baseline` **VERDE** (`3 FAIL · 3659 WARN`, nada nuevo frente a `tests/baseline.json`) y `python3 tools/verifica_head_remoto.py` → `PR_HEAD_SINCRONIZADO`.
