...
CALC = "CALC-ENSAFI2023-ESTRATEGIAS-CONJUNTAS-0001-v1_1"
...
def variance(z, strata):
    ...
    return v
def attach(e,strata,df, bounds=(0.,1.)):
    ...
    e.update(se=se,lo=max(bounds[0],e['p']-q*se),hi=min(bounds[1],e['p']+q*se),estado='ESTIMABLE:T95-LINEALIZACION')
    return e
...
def medir(inputs, contrato):
    ...
    mean=ratio(rows,full,lambda x:sum(c(x[i])=='1' for i in ITEMS));attach(mean,strata,df,(0.,8.))
    counts.append({'conteo':'media','n_numerador':mean['n'],'masa_numerador':mean['num'],'masa_denominador':mean['den'],'p':r6(mean['p']),'ee':r6(mean['se']),'ic95_lo':r6(mean['lo']),'ic95_hi':r6(mean['hi']),'estado':mean['estado']})
    for t in (1,2,4):
    ...
    for a,b in combinations(ITEMS,2):
        ...
        aa=ratio(rows,lambda x,a=a:full(x) and c(x[a])=='1',lambda x,b=b:float(c(x[b])=='1'));attach(aa,strata,df)
        bb=ratio(rows,lambda x,b=b:full(x) and c(x[b])=='1',lambda x,a=a:float(c(x[a])=='1'));attach(bb,strata,df)
        n11=base['num']; n10=aa['den']-n11; n01=bb['den']-n11; n00=complete['den']-n11-n10-n01
        pairs.append({'estrategia_a':a,'estrategia_b':b,'n00_masa':n00,'n01_masa':n01,'n10_masa':n10,'n11_masa':n11,'denominador_ambas':complete['den'],'p_ambas':r6(base['p']),'ee_ambas':r6(base['se']),'ic95_lo_ambas':r6(base['lo']),'ic95_hi_ambas':r6(base['hi']),'n_soporte_a':aa['n'],'masa_soporte_a':aa['den'],'p_b_dado_a':r6(aa['p']),'ee_b_dado_a':r6(aa['se']),'ic95_lo_b_dado_a':r6(aa['lo']),'ic95_hi_b_dado_a':r6(aa['hi']),'n_soporte_b':bb['n'],'masa_soporte_b':bb['den'],'p_a_dado_b':r6(bb['p']),'ee_a_dado_b':r6(bb['se']),'ic95_lo_a_dado_b':r6(bb['lo']),'ic95_hi_a_dado_b':r6(bb['hi']),'estado_ambas':base['estado'],'estado_b_dado_a':aa['estado'],'estado_a_dado_b':bb['estado']})
    ...
    singletons=sum(len(us)==1 for us in strata.values())
    diagnostics=[{'n_peso_invalido':bad,'n_diseno_faltante':miss,'n_estratos':len(strata),'n_upm':sum(map(len,strata.values())),'n_singleton':singletons,'gl':df,'regla_singleton':'SIN-APORTE-VARIANZA-NI-GL','fpc':'NO-APLICADA'}]
    files={'cobertura.csv':cov,'conteos.csv':counts,'parejas.csv':pairs,'sensibilidad_parejas.csv':sens,'limites_faltantes.csv':limits,'diagnosticos.csv':diagnostics}
    ...
    ans={'RESULT-ENSAFI-EC2-G-N-MARCO':len(rows),'RESULT-ENSAFI-EC2-G-N-ELEGIBLES':len(eligible),'RESULT-ENSAFI-EC2-G-MASA-ELEGIBLES':r6(mass),'RESULT-ENSAFI-EC2-G-COBERTURA-VECTOR':r6(complete['den']/mass if mass else None),'RESULT-ENSAFI-EC2-G-MEDIA-CONTEO-COMPLETO':r6(mean['p']),'RESULT-ENSAFI-EC2-G-EE-MEDIA-CONTEO':r6(mean['se']),'RESULT-ENSAFI-EC2-G-IC95-LO-MEDIA-CONTEO':r6(mean['lo']),'RESULT-ENSAFI-EC2-G-IC95-HI-MEDIA-CONTEO':r6(mean['hi']),'RESULT-ENSAFI-EC2-G-CONTROL-PARTICION':'COINCIDE' if abs(partition-1)<1e-10 else 'DISCREPA','RESULT-ENSAFI-EC2-G-CONTROL-MEDIA-MARGINALES':'COINCIDE' if abs(mean['p']-sum(marginal))<1e-10 else 'DISCREPA','RESULT-ENSAFI-EC2-G-CONTROL-PADRE':'COINCIDE' if control_parent<1e-10 else 'DISCREPA','RESULT-ENSAFI-EC2-G-ESTRATOS':len(strata),'RESULT-ENSAFI-EC2-G-UPM':sum(map(len,strata.values())),'RESULT-ENSAFI-EC2-G-SINGLETON':singletons,'RESULT-ENSAFI-EC2-G-PESO-INVALIDO':bad,'RESULT-ENSAFI-EC2-G-DISENO-FALTANTE':miss,'RESULT-ENSAFI-EC2-G-GL':df}
    for fn in files:
        key=fn.replace('.csv','').upper().replace('_','-')
        ans[f'RESULT-ENSAFI-EC2-G-SALIDA-{key}']=str((outdir/fn).relative_to(root)); ans[f'RESULT-ENSAFI-EC2-G-SHA256-{key}']=sha(outdir/fn)
    return ans
