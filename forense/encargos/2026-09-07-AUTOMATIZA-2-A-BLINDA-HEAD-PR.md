...
*(archivado verbatim, A.3, por `ACTO AUTOMATIZA-2-A · BLINDA-HEAD-PR`)*
...
**Gap de archivo declarado.** El `DICTAMEN · PLAN AUTOMATIZA-2 · CABLEADO POST-E3` (dirección Fable, sha256 `b6b754903372b609f90ebb93197f1a6b29b6c265d95d015320d3d2d098a84f7a`) no llegó a esta sesión y no se archivó junto a los otros dos documentos de este mismo A.3 — ver la sección `DICTAMEN` arriba y `ADR-375`.
...
**Renumerado `ADR-374` → `ADR-375` tras conflicto real de fusión.** Mientras este PR estaba abierto, `origin/main` avanzó (`e1c3c84` → `8bc1a5d`) con `PR #581` (`ACTO MAESTRA38-L16-BIS`, un `PARO`) fusionado — y ese acto **también** candidateó `ADR-374` contra la misma base `e1c3c84`, tomándolo primero. `git fetch origin main` + `git merge origin/main` produjo conflicto real de contenido en `canon/gobernanza-v1_15.md` y `canon/estado-programa-v1_12.md` (ambos actos editaron la misma cabecera, el mismo ancla `L0` y la misma fila de tabla). Resuelto a mano: se conserva íntegro el `ADR-374` de `MAESTRA38-L16-BIS` (ya fusionado, no se toca), y la entrada/anotación de este acto se renumera a `ADR-375` — regla de la casa, renumera quien fusiona segundo, mismo precedente que `ADR-373` (`AUTOMATIZA-2-E4`, tres actos por delante) más arriba en este mismo documento de gobernanza. `python3 tools/cierre_acto.py --aplica` tras el merge: `APLICADO: gobernanza 374->375` (`L0` ya declaraba `375` tras la resolución manual); segunda corrida → `sin cambios`. `python3 tests/check.py --baseline` re-verificado VERDE tras el merge; ningún archivo fuera del perímetro declarado entró por la fusión (los archivos propios de `MAESTRA38-L16-BIS` — su encargo, su nota, `canon/registro-rotulos.tsv`, `forense/hallazgos.md` — llegan por el merge de `origin/main`, no son commits de este acto).

**Commits:** 0-bis (`a5453a4`, este archivo verbatim) · COMMIT 1 (`5a35a10`, `tools/verifica_head_remoto.py` + test) · COMMIT 2 (`c059c0d`, guard en `/acto` + `/despacha`) · cascada (`a81fe07`, `ADR-374`/`L0`/tabla/`T25`) · `## CONSUMIDO` (`78032b7`) · merge de `origin/main` + renumeración a `ADR-375` (este commit).
